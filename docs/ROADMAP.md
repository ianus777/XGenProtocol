...
│   │   ↳ 🔑 **SPAWNED BY `M-RP-LIVEFEED-REFRESH` Leg A (J-643)** — the live router created a state the panel had never been in: a roster row whose identity was **NEVER LOOKED UP**. Phase-0 `tasks/M_RP_IDENTITY_RESOLUTION.md` **v1.8 ACTIVE**
...
│   │   ├── 🟡 **Leg B the render rules** — ③ filtered from the rendered list (except the DM counterpart, §5a) · 🔒 **runbook `tasks/RUNBOOK_IDENTITY_RESOLUTION_LEG_B.md` v1.1 AUTHORED, NOT LOCKED (J-651) — nobody is implementing it**
...
│   │   │   ↳ trigger: **Joe locks the runbook AND stands Clair up — TWO ACTS, and NEITHER has been performed.** ✅ *An adversarial READ is neither of them, and she has done that (J-651) — the distinction is the whole point of the two acts.* ⚠️ *v6.35–v6.37 read "NEXT-ACTIVE, and it has NO RUNBOOK" — that went false at J-650 when the runbook was authored. Superseded, kept not erased (`D-131`).*
