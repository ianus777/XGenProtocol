...
| 9 | J-110, J-111, J-112, J-118, J-119 | Deployment-level integration tests + NodeRuntime-level validation-asymmetry coverage across the 3b arc. Sub-commit shape: **3b-2-equivalent (J-110, +4 tests)** — Scenarios 5 (unknown-signer first-contact, F-10 identity-arrival hook end-to-end in `phase9_unknown_signer_first_contact.rs`) + 6 (federation-relationship rejection + recovery in `phase9_federation_relationship_rejection.rs`) + Phase 7.5 §5 extended skip-set negative assertion (`StateFederationAdd` + `StateSpaceCreate` + `StateDmSpaceCreate`) + `state.room_create` narrowness regression. **3b-3-pre (J-111, +3 tests)** — harness extension: push-attempt counter on `InProcessNode` (`push_attempts: HashMap<(String,String), u64>`) + tracing-Layer composition (`CounterLayer` + `LogBufferLayer` over `tracing_subscriber::registry()`, per-test installation via `install_harness_subscriber()` to coexist with `tracing-test`'s global-default on a per-thread / process-global dichotomy) + G2 retrofit adding `local_node_id` field to all four federation-push trace events (`federation_push_skipped_origin`/`_sent`/`_dropped_full`/`_dropped_unregistered`). **3b-3 (J-112, +1 test)** — Compound C2 (F-5 anti-transitivity under push queue depth, 100 messages, `phase9_compound_c2_anti_transitivity_at_load.rs`) with five honesty assertions including the load-bearing per-event negative-existence F-5 regression lock (`b.push_attempt_count(event_id, &A.node_id) == 0`). **3b-4 (J-118, +27 tests)** — Scenario 4 NodeRuntime-level validation asymmetry (20 tests across 4 forgery variants × 5 event families in `phase9_validation_asymmetry.rs`) + Compound C5 isolation-under-load (100 shuffled valid+forged events with deterministic seed `0xC5_5EED_0000_0001`) + Compound C7 `continue_from` pagination at boundary (N=999/1000/1001/2000) + Compound C9 F-3 drain-time approximation hazard (accepting approximation pinned within 30 s F-4a window per `runtime.rs:864-865` production doc-comment) + Compound C10 identity-replicate hook serialisation under 3-task concurrent lock contention. 5 new test files (Sc4 + C5 + C9 at `xgen-core/src/node/tests/` new directory; C7 + C10 at `xgen-node/src/tests/` because their production targets `compute_federation_delta_for_space` and `handle_identity_replicate_msg` live xgen-node-side). **3b-5 milestone close (J-119, this commit)** — Federation Event Propagation milestone PLAY → DONE. Pre-3b-arc canonical-record amendments J-113 + J-115 + J-116 + J-117 refined the survey-findings and runbook contract under D-078 production-grounded test enumeration discipline (no new test code; canonical contract corrections only). **D-078 promoted to DECISIONS.md** at J-114 (production reject-path inventory MUST be confirmed against current code BEFORE test enumeration is Joe-locked; sibling-shape to D-077 at protocol-test layer vs D-077's meta layer). Verification rigour at the milestone-bearing 3b-4 commit per Lock 3: 5 isolated (`cargo clean` between each) + 3 workspace runs = 8/8 green; no pre-existing flakes (precedence env-var race, `reconnect_with_existing_tip_small_delta_delivered`) fired across any of the 8 runs. Workspace test count at milestone close: **627**. M6 (new) Node admin write path + XGID Retrofit Pass 1 unblock at this row. |
...
- JOURNAL entries J-082 through J-089, J-110, J-111, J-112, J-118, J-119 — Per-phase shipped-state record. J-088 closes Phase 7; J-089 closes Phase 8 (this section); J-119 closes Phase 9 and the Federation Event Propagation milestone.
