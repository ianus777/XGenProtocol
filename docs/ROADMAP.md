...
│   │   ↳ trigger: **Leg C-bis runbook LOCKED at J-704 — five commits, zero Rust, Clair may write code**: the live measurement moved the prepared page off the message-stream background socket, which is hardcoded aria-hidden and correct for wallpaper but wrong for content, onto a new above WidgetMount socket on stream-panel resolved through resolveMounts. The stream fills its tile and will not collapse, so stream-panel becomes a flex column while MessageStream stays unconditionally mounted and the registry floor proves it. The RMC context menu needs its own design pass and has no runbook
