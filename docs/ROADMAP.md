...
│   ├── ✅ **M-SPACE-ADMISSION** — who may join a Space, and how a leaver comes back · FILED J-741 · Legs A-bis…E-3 and G-0…G-5 · CLOSED J-779 · protocol + node, Space-general
│   │   ↳ Owes: DONE — **WHAT WAS RULED.** D-148: admission is a Space-general owner-settable property, two riders, default open, and the gate is a role predicate never an owner identity · D-154, six clauses on what a Space remembers about someone who left — a rejoin restores PRESENCE and never POSITION; kick is remembered not erased; ban follows kick; the gap stays closed to CONTENT and open to MEMBERSHIP STRUCTURE; rooms are not restored; node-eject follows kick and ban · D-155: a decision belonging to the seat that owns MEANING is asked in the vocabulary of meaning · D-156: a client must not infer node state from the shape of the log. Detail at J-761, J-766, J-769, J-778.
│   │   ↳ Owes: DONE — **WHAT WAS BUILT.** left_at on SpaceMember and four appliers that retain and mark, all fifty readers in one commit · apply_join's three-way gate and the former-member third term of its predicate · 3048 rejoin_not_anchored moved onto the ANSWER path, so the node refuses a rejoin it cannot anchor instead of accepting it and resolving it away · collect_invite_bootstrap admitting a retained departed member who is not banned and serving her only her own membership events · the client anchor selection on state_key_for_event, the same predicate the node judges it by · leave brought inside StateFileLock. Floor across the arc: cargo 1597 to 1667 / 0 / 64 x 57 SUITES, every leg re-driven independently by both seats on forced rebuilds.
│   │   ↳ Owes: KEY — **THE ARC'S OWN MEASUREMENT OF ITSELF: TWENTY-FOUR DEVIATIONS REPORTED BY THE IMPLEMENTING SEAT, ZERO ABSORBED, THIRTEEN OF THEM DEFECTS IN CHAT'S DOCUMENTS** — including one in a rule Joe had already locked, which was routed back as a question with three options measured and NONE applied rather than patched. The seat split paid for itself in specifications, not in code. J-767 through J-779.
│   │   ↳ Owes: STOP — **THE FINDINGS WORTH CARRYING OUT OF THE ARC.** A served batch answers HISTORY questions only, and a type-and-field match over it is a history question whatever the variable is called (D-156) · an inference that is correct today is still a violation if its correctness rests on another subsystem's disclosure policy, because it then fails later with every test still green · a disclosure filter is written from the EVENT's point of view and never the requester's (N-209) · a leg that edits a file invalidates its own citations into that file, so anchor on the SYMBOL (D-152) · a check whose failure mode reads exactly like success is not a check · the same clause can be a defect in one leg and a requirement in the next, and the only way to tell is to open the path.
│   │   ↳ Owes: OPEN and Joe: **M-CLIENT-RESTORE — bringing an account back on a new device.** RULED at J-779: the app tells her in her own words and restores behind ONE deliberate confirmation, never silently, because a launch-time write to node-side identity state is the one thing a person cannot audit. FILED and NOT BUILT on a measured ground — the desktop client has no route to ops::join at all (J-740). Its Phase-0's first question is already visible: re-registration is specified as re-home onto a NEW node (ch3 3.13.8) and is being reused for rebuild-local-state on the SAME node, and whether that is one mechanism or two is a real question.
│   │   ↳ Owes: OPEN and nobody: **FIVE CAVEATS ACCEPTED KNOWINGLY, CARRIED AND NOT DISCHARGED.** D-154 clause 6 — retention makes an ejection a durable federated record while node_eject is itself reversible · D-154 clause 4 — a returning member learns who else was removed and when, third-party information about people who cannot see that she now holds it · self.banned is already a permanent federated list of identities and has been for a long time · the GENERIC_4000 envelope carrying 3048 in reject_code, first sighted J-778, which is D-070's entry and not this arc's · the expired-invite dead end — 1011 is ONE word for every refusal, deliberately, because differentiating it would make the refusal a membership oracle, so a returning member whose invite lapsed is told nothing that helps her and the honest fix belongs to the invite.
│   │   ↳ Owes: OPEN and nobody: **INSTRUMENT NOTES THE ARC PAID FOR.** N-204 heredocs truncate in the implementing harness and the boundary is UNBISECTED · N-206 a launcher can look launched and produce nothing, so launch from a .ps1 file · N-207 a re-drive can return the correct floor over the other seat's binary · N-210 git checkout -- is not a restore primitive for an unstaged working state · N-211 autocrlf defeats a sha256 restore guard, so compare content and not only bytes · N-212 a file-copy restore can hand you a stale binary, so require Compiling before the run · N-213 PowerShell's backtick escape destroys markdown code spans in a double-quoted string · rustfmt is unclean on baseline across 209 files and is therefore not a gate.
│   │   ↳ trigger: DISCHARGED — milestone closed at J-779
│   ├── 🟢 **M-ADMISSION-SOUNDNESS** — driving the ruled admission mechanism as one lived story · FILED J-780 · Phase-0 v1.1 ACTIVE · Legs 0…4 · LEG 0 ✅ · LEG 1 ✅ J-781 · LEG 2 ✅ CLOSED J-782 · NEXT: LEG 3 the verdict document · observation only, no behaviour change
│   │   ↳ Owes: DONE — **LEG 0 CLOSED AT FILE TIME.** Ten situations mapped to drivability against the shipped verb surface; eight reachable, two of them already existing as V-9a/V-9b. S-2 RESTATED after measurement: no role-mutation event exists in the protocol at all, so nobody is ever promoted — but apply_join's no-invite arm hard-codes Role::Member (state.rs:1275), so an admin who leaves and rejoins on her own anchor comes back a plain Member and nothing tells her. S-8 needs no clock harness: invite --valid-for-days 0 stamps an already-spent invite (ops.rs:1187, no lower bound). S-3/S-4 are stageable because desktop.rs:988 resolves the data root by the same precedence as the CLI (D-067).
│   │   ↳ Owes: STOP — **THE FINDING THAT CAME BEFORE ANY SIMULATION: TWO RULED CLAUSES HAVE NO EMITTER ON ANY SHIPPED SURFACE.** membership.kick is parsed, permission-mapped, validated, resolved with its own precedence, state-keyed, applied at state.rs:786 under a D-154 clause-2 comment, disclosed and audited — and every site that constructs one is a test fixture; ClientCommand's 27 variants carry Ban and not Kick, and no node admin op emits one either. state.space_admission is the same shape and worse: no emitter, and create-space carries no admission argument, so every Space ever created is open by DEFAULT_ADMISSION and the live 3047 gate cannot fire outside a unit test. The logic is sound; the surface is absent. J-780.
│   │   ↳ Owes: KEY — **THE ARC'S OWN LESSON RECURRING AT MILESTONE SCALE.** N-194: a check whose failure mode reads exactly like success is not a check. Here — a clause whose only exerciser is a fixture reads exactly like a shipped clause. Every leg gate constructed its own events directly, so no leg ever needed a verb, so no leg ever noticed there was not one, and the milestone closed with every test green and two of six ruled behaviours unreachable.
│   │   ↳ Owes: OPEN — **LEG 2 CANNOT BE A TEST, AND THAT IS RECORDED BEFORE IMPLEMENTATION RATHER THAN DISCOVERED DURING IT.** The GUI attaches to a data dir the harness drove and the node must still be running when it does, so Leg 2's driver is a staging harness that leaves its rig alive, not a cargo test that tears it down at the end. Leg 2's verdict is Joe's or it is absent — the harness sees the wire, never the screen.
│   │   ↳ Owes: DONE — **LEG 1 CLOSED (J-781): FOUR WIRE SITUATIONS DRIVEN OVER REAL BINARIES, AND THE MECHANISM IS INVERTED.** S-8, measured: carol, never invited, was ACCEPTED; bob, invited with a lapsed invite, was REFUSED 3044 quoting his exact missed deadline; bob with a live invite was ACCEPTED. The stranger walked in and the person who was actually invited was turned away. It is not a bug — it is Leg 0's two findings meeting each other: admission is open on every Space that can exist because nothing can set it, so 3047 never fires and carol is never asked for anything, while bob holds a pending_invites entry so the expiry branch is entered and rejects him. The invite is not what opens the door; it is the only thing that can be CHECKED, and therefore the only thing that can FAIL. Holding a spent invite leaves you worse off than having no relationship to the Space at all.
│   │   ↳ Owes: DONE — **S-2: THE DEMOTION IS REAL AND THE SILENCE IS NOW A MEASUREMENT.** role before = admin, role after = member, and none of role/admin/member/demot/privile appears anywhere in the rejoin reply, which is shaped identically to an ordinary member's. D-154 clause 1 working exactly as ruled, and nothing he receives would let him find out. The runbook forbade asserting it: an assert_eq on a ruled behaviour would have reported a contradiction of Joe's own clause as this leg failing, and hidden it. S-7: the ban door speaks plainly, un-banning alone does not put him back, and he returns a plain member with nothing said about the ejection that stays in the record. Floor re-driven by Chat on a forced rebuild: 1667 / 0 / 68 x 58 SUITES — passed +0, ignored +4, suites +1. The passing count did not move; a soundness leg did not become a test leg.
│   │   ↳ Owes: STOP — **THREE DEVIATIONS, NONE ABSORBED, TWO OF THEM DEFECTS IN CHAT'S OWN RUNBOOK.** D-1: S-8's 1011 premise does not hold on this path — 1011 is collect_invite_bootstrap's refusal and ops::join swallows it at the wildcard arm, then submits the join anyway, so 1011 is unreachable by a user attempting to join; J-779's ruling stands and governs the bootstrap door, but the reasoning given for it was about what a returning member hears and a returning member never hears it. D-2: rooms is unreadable by a joined member before AND after leaving, because ops::join never writes client state — the five write_client_state sites are register, create_space, create_room, create_dm_space and leave; a member's own client records his departure and never his arrival, it reaches the GUI through get_spaces, and Leg 2 forbids fixing it because the fix would destroy the observation. D-3, Clair's own and caught by herself: all four READING lines were drafted before the run and one was wrong — a READING written before the run is a prediction wearing an observation's clothes.
│   │   ↳ Owes: OPEN — **THREE FINDINGS AWAIT JOE AT LEG 3 AND ARE DELIBERATELY NOT GATED ON LEG 2:** the open door, every Space that can exist is open and cannot be otherwise; the spent invite, whether a lapsed invite should fall back to the Space's admission rule rather than hard-refuse; and the silent demotion. Chat recommended ruling them with the screens in hand rather than from a JSON transcript, which is the exact substitution this milestone exists to stop making.
│   │   ↳ Owes: DONE — **LEG 2 CLOSED (J-782): THE GAP DID NOT CLOSE, AND BOB'S OWN CLIENT NEVER LISTED THE SPACE AT ALL.** S-3, measured and reproduced independently by Chat on a forced rebuild: bob left, alice said three things while he was away, bob rejoined and read all three alongside the one from before he left. D-154 clause 4 rules the gap closed to content; it is open. The pre-gap message is the discriminator and the situation is worthless without it — absent it, history is empty and history withholds the gap are the same reading. But his roster is byte-identical to the owner's, carol removed and dave present, so clause 4's STRUCTURE half is SATISFIED; what he lacks is the narrative of the change, and that is ops::history projecting MessageText only, a client surface absence rather than a node gap defect. Chat's first reading called the clause inverted end to end and that was wrong by one half — overstating it would have doubled the repair.
│   │   ↳ Owes: DONE — **S-4 AND THE SCREENS.** bob: spaces = [] and rooms failing no known Space with ID at all four points, against alice's control returning the Space and its three rooms at every point, with the node reporting present = true, role = member. He can act in a Space his own client does not know exists — he joined three rooms and sent a message. On the screen: No spaces yet, a roster holding only himself, and nothing to click. V-4 was delivered by a route neither seat had: main.rs resolves the data root as --data-dir then XGEN_DATA_DIR then platform default, and run-client.ps1 never sets that variable, so XGEN_DATA_DIR aims the debug GUI at a staged identity with zero change to the launcher or the client. Both captures read Reconnecting and the control is what disarms it: get_spaces reads LOCAL client state, so alice showing one Space under the identical disconnection proves the connection is not what empties bob's screen. Floor re-driven by Chat: 1667 / 0 / 70 x 58 SUITES — passed +0, SUITES +0, ignored +2, confirmed by name.
│   │   ↳ Owes: STOP — **SIX DEVIATIONS, NONE ABSORBED, AND FIVE OF THE SIX ARE DEFECTS IN CHAT'S OWN RUNBOOK.** V-1 and V-2 contradict each other on ignored, and a gate that can be read two ways is not a gate. The GUI launch line is not reachable as written — the built exe exposes no CDP port, WebView2 136 and later ignores the env-var override which is why cdp-debug.ps1's -Launch was retired, and run-client.ps1 -Debug pins Vite 5173 and CDP 9222, so side by side is permanently unreachable through the shipped launcher and captures are sequential. The staged config points at port 8080 while the staged node runs on 8596, so no GUI launched against a staged dir can connect. cdp-debug.ps1 -OutFile fails silently on a relative path with output identical to success. Clair's two: carol and dave join before carol is banned, since a ban on someone who never joined removes nobody; and the harness stops both residents before handing over because the Tauri shell contends for the same pipe and state file. Also recorded: a process census taken during a run measures the run.
│   │   ↳ Owes: OPEN — **JOE RULED THE GAP THIS SESSION AND IT IS THE MILESTONE'S FIRST FORWARD DECISION: interval-scoped, not marker-scoped — block A retained, block B withheld, block C delivered.** Chat's framing of the option was wrong and Joe's correction fixed it, since Chat had described it as the client opening at the rejoin, which would have killed block A too. Prior art, measured: Discord, Slack public and private, and Teams do not filter the gap at all; Signal and WhatsApp cannot leak it because they hold no server history; Matrix alone made history visibility a protocol primitive, and all four of its modes are marker-scoped rather than interval-scoped, with an open spec issue recording that Matrix cannot express users can only see events if they were joined at the time. Where the A-B-C shape did occur in Matrix it occurred as a federation failure and was filed as a defect. Nobody has built this deliberately on a server that still holds the messages. The repair becomes its own milestone with a Phase-0, never a patch inside an observation leg; its opening questions are the dangling reference, one gap or many, federation, compaction against D-080, and the measured scope asymmetry that bob joins rooms individually and leaves the whole Space in one event.
│   │   ↳ trigger: locked by Joe 2026-08-27 on the Q-1 ruling; Q-2 sequencing only
│   ├── 🟡 **M-ADMISSION-SURFACE** — the verbs that cause a kick and set a Space's admission · FILED J-780 · carries S-5, S-6's missing half and S-10 out of M-ADMISSION-SOUNDNESS
│   │   ↳ Owes: STOP — **NEW PRODUCT, NOT VERIFICATION.** A kick needs its own design and its own ruling about what it looks like to the person kicked; an admission setter needs a ruling about whether it is a create-time flag, a post-create event, or both. Folding either into an observation pass would have turned it into a feature arc, which is why Q-1 ruled B. Both receiving halves are already complete, correct and tested — this milestone is the sending half and nothing else.
│   │   ↳ trigger: Joe schedules. Not gated on M-ADMISSION-SOUNDNESS.
│   ├── 🟡 **M-CLIENT-RESTORE** — bringing an account back on a new device · RULED J-779, node added J-780 · the fresh-install step the whole M-SPACE-ADMISSION arc rested on
│   │   ↳ Owes: STOP — **RULED (J-779): the app tells her in her own words and restores behind ONE deliberate confirmation, never silently**, because a launch-time write to node-side identity state is the one thing a person cannot audit, and this project's thesis is that identity moves deliberately. Filed rather than built on a measured ground: the desktop client has no route to ops::join at all (J-740), and a UI ruling with no surface to land on is a decision locked for a design nobody can see run.
│   │   ↳ Owes: OPEN — **ITS PHASE-0'S FIRST QUESTION, ALREADY VISIBLE.** re-registration is specified as re-home onto a NEW node (ch3 3.13.8) and is being reused for rebuild-local-state on the SAME node. Whether that is one mechanism or two is a real question, not a naming one.
│   │   ↳ trigger: Joe schedules. Blocked in practice until the desktop client gains any route to ops::join.
