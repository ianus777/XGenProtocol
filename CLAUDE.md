...
## 🟢 PLAY — M6 (new): **node-policy C1 SHIPPED (J-197) — store + both verbs + threading + tests**; **next-active = C2 doc-only close** → then M7 `--aicontrol`

**node-policy C1 SHIPPED at J-197 (2026-05-31; code).** Per `tasks/M6_NODE_POLICY_IMPL.md` C1 — no Joe-lock checkpoint (Fork X = no enforcement seam to code-trace). **Shipped:** xgen-core NEW `space/node_policy.rs` (`NodePolicy { auto_moderation: bool, action_threshold: Option<f64> }`, `Default={false,None}`; `NodePolicyStore { policies: HashMap<SpaceXgid, NodePolicy> }`, `FederationPolicyStore` sibling, reuses `crate::federation::registry::RegistryError`; absent==disabled lives in the verb/`Default`, NP-D2) + xgen-node `admin_ops` `node_set_policy` (WRITE, audited; `SPACE_8005` for `action_threshold ∉ [0.0,1.0]` pre-audit pre-write; hosted-here else `SPACE_8001` via a shared `require_hosted_space` helper reusing the `force-eject` pattern) + `node_show_policy` (READ, not audited; stored or `default()` + `is_default`) + `AdminContext` threading (`node_policy_store` + builder/require/`node_policy_store_path()` = `xgen-node_node_policy.json`) + clap `SpaceCommand::{SetNodePolicy, ShowNodePolicy}` + `run_node → start_pipe_server → dispatch_line → dispatch_admin` + 2 pipe arms + help string; **store loaded inside the `#[cfg(windows)]` pipe block** (Fork X, sole consumer = verbs; sibling to the Auth Module registry load). **Four confirm-at-pickup (D-078) all resolved against the live tree, none guessed:** store→`space/` ✅, `RegistryError`→`crate::federation::registry` (pub) ✅, hosted-here→`force-eject` pattern ✅, clap bool→SetTrue flag (`dry_run` precedent) ✅. **Fork X held:** nothing reads the store this arc; prime invariant (empty store + `show` on hosted Space → `{false,None}`, touches nothing) proven by regression. **Verification (Rule 2):** `cargo test --workspace` **841** passed / 0 failed / 1 ignored (+10 vs J-195's 831 = 6 core store + 4 node verb tests; matches runbook ~+8–10); `cargo build --workspace --all-targets` 0/0; clippy `--workspace --lib --tests --all-features -D warnings` clean. No DECISIONS.md change (NP-D# arc-local, D-069). **Next-active: C2 doc-only close** (D-074 atomic) — §6.A4 SHIPPED banners + honest as-built deltas (2-field schema vs the Block-4 "auto-mute thresholds, rate caps"; `SPACE_8005` superseding the guessed `SPACE_8020`; Fork X stored-inert; propagation=none), `tasks/M6_BACKING_AUDIT.md` A4 → SHIPPED + **all M6 deferrals closed**, audit/design → COMPLETED, ROADMAP A4 ✅, PLAY → M7 `--aicontrol`. **Entry point: this PLAY + JOURNAL J-197 per Rule 0, then `tasks/M6_NODE_POLICY_IMPL.md` C2.**
