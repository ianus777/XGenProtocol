...
│   │   └── 🟡 **Leg E records + close**
│   │       ↳ trigger: Leg D lands
│   ├── 🟢 **M-RP-MEMBER-ACT** — the members panel acts: LMC opens the DM, RMC opens the menu · **a COMMAND-SURFACE milestone — `create_dm_space` + `self_open` are built and tested with no Tauri command** · J-680
│   │   ↳ Phase-0 `tasks/M_RP_MEMBER_ACT_PHASE0.md` **v1.1 ACTIVE** · audit `tasks/AUDIT_MEMBERS_PANEL.md` satisfies the `D-071` gate · **9 inherited locks (L-1…L-9), 6 from J-591 and 3 uttered 2026-08-06**
...
│   │   ↳ Legs: **0** Clair's adversarial read → **A** `D-131` annotations → **B** the command surface (cargo) → **C** R7 acts → **D** RMC menu → **E** DM home + `is_dm` (cargo) → **F** close
│   │   ↳ 🔓 **OPEN AND JOE'S (OQ5, no Chat recommendation exists):** the partial first send · **erased members are clickable and a DM to one would go nowhere** (new, surfaced by M-RP-TAIL8's live run)
│   │   ↳ ⚠️ **OQ1–OQ4 ADOPTED AS DELEGATED** (*"let's go by your recomms"*) and **PROVISIONAL pending Leg 0** — P1 · S2 · A3-last · no-early-card
│   │   ↳ trigger: Joe stands Clair up for Leg 0 — **no authority to code**
...
│   │   ↳ 🔒 **`D-142` APPLIED AT THE ROSTER, never *"`D-142` applied"*** — scope **A1+A2** and the **fill/not-found arm** only; the **live-delta arm** untested, **A3 latent**, **A4 (message rows, full 65-char XGID) LIVE and untouched**, feed still `slice(-6)`
│   │   ↳ `Owes:` — **V4's empty-guard sub-case NOT DRIVEN, with its reason** (unreachable by product action) · **`N-168`** the erased row's `line-through` runs through the leading `…` — **SHIP AND FILE** (Joe, DELEGATED, `D-141`)
...
│   │   └── ✅ **Leg F live verify + records** — the milestone's FIRST behaviour verification; **all SEVEN obligations discharged against a real second identity** · **CLOSED J-675** · rig **J1** (headless CLI joiner) + **E1** (registry-row file surgery), both Joe-locked · ①②③④⑤ verified by counters and contrasts, ⑥⑦ delivered as NUMBERS · 🔑 **§2b resolved at a finer grain than it was asked: `data-ai` DOES precede the marker clear in DOM-mutation order, but in ONE flush ⇒ never painted** — the falsification criterion was met and was written at the wrong LAYER · joins proven real at the DAG (10 signed `membership.join`, distinct sender keys) · **seven findings filed, none an obligation** · runbook `tasks/RUNBOOK_IDENTITY_RESOLUTION_LEG_F.md` **v1.7 COMPLETED** · phase-0 **v1.0** · J-653 (J-672, J-673, J-674, J-675)
