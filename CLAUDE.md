...
> Date: May 2026  
> **Status:** ACTIVE  
> **Last updated:** 2026-05-21 (D-074 promoted to a numbered decision — "Milestone-close commits MUST include JOURNAL.md". Candidate principle surfaced 2026-05-20 during J-094 cleanup forensics (Phase 7.5's close shipped without a JOURNAL entry despite cross-doc references naming "J-094"); now a formal entry in `DECISIONS.md` D-074. Forward-binding: every future milestone-close commit's changed-files list MUST include JOURNAL.md. Sibling per-session discipline (CLAUDE.md Rule 4 — "write the journal entry last") binds intra-session ordering; D-074 binds commit-level composition. Together they form the full discipline; either alone is insufficient. ROADMAP.md v1.5 → v1.6 shipped in the same commit: "D-074 candidate" line removed from the Discipline / JOURNAL hygiene cluster (the two JOURNAL Gap retrospectives remain in the cluster as deferred work); Cross-cutting principles section gains "✅ D-074" alongside D-069/D-070/D-071/D-072/D-073. PLAY block content describing the active XGID Retrofit Pass 1 runbook authoring (Pass 1 runbook now at Status: ACTIVE v2.0, all four sub-questions locked, ready for Clair pickup after Federation Event Propagation milestone closes) is unchanged from the previous PLAY entry below; previous J-095 milestone-close header content stands authoritative in the body's DONE-IN-FLIGHT block.)  
> Author: JozefN  
