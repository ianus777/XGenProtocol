...
> Date: May 2026  
> **Status:** ACTIVE  
> **Last updated:** 2026-05-21 (Phase 9 stands down at Commit 3a boundary; bidirectional federation_nodes finding surfaced. Clair shipped Phase 9 Commit 3a as harness + Scenario 1 regression witness with `#[ignore]` annotation. The diagnostic run during Scenario 1 surfaced a real production protocol gap: `stream_federation_delta` emits `state.federation_add` with `content.node_id = peer_b_id` (semantics "A approves B"); both sides ingest the same event and apply `content.node_id` verbatim onto `federation_nodes`. Result: `A.federation_nodes = {B}` correct from A's view, `B.federation_nodes = {B}` incorrect because it should contain A so F-3 lets A's events through. F-3 on B HeldPends every post-bootstrap A→B push event indefinitely; 180s timeout fires; all events discarded. No current code path emits the reciprocal `fed_add(node_id=A)` closing the gap. Existing cold_start tests didn't catch this because they build their bootstrap stream manually; federation_push_integration uses a one-sided harness; Phase 9 Scenario 1 is the first test to exercise the full bidirectional path with real dispatch on both sides. Sibling to the J-093 Phase 7.5 precedent (design-time gap surfaced during dependent work, closes in its own dedicated phase before dependent work resumes). Work surface now back to Chat Claude + Joe for audit phase → design phase → implementation (Clair) → Phase 9 resumes. Phase 9 milestone is doing its job: the 14-entry failure-mode catalogue at survey close named this category of finding as a HIGH-severity expected outcome of bidirectional deployment-level testing. Finding it now — before Pass 1 ships, before M6 goes active, before any downstream code depends on the broken behaviour — is the cheapest possible moment. ROADMAP.md v1.7 → v1.8 in same commit per same-commit discipline. Four candidate fix shapes captured in JOURNAL Commit 3a sub-entry under J-092 (reciprocal a-i on receiver / post-handshake reciprocal hook / `apply_federation_add` records sender too / hybrid); design phase walks them properly with Joe-lock. Previous Last-updated note (drift correction on Pass 1 runbook shipment) stands authoritative in the Past section's Pass 1 runbook authoring close entry.)  
> Author: JozefN  
> Credits: Concept, philosophy, requirements, project direction: Jozef Nižnanský. Technical assistance and implementation support: AI-assisted development tools.  

---

## 🟡 PLAY — Phase 9 PAUSED at Commit 3a boundary; bidirectional federation_nodes audit + design phase next-active for Chat Claude + Joe

**Phase 9 stands down at Commit 3a boundary 2026-05-21.** Clair shipped Phase 9 Commit 3a (harness + Scenario 1 regression witness + JOURNAL sub-entry under J-092) and surfaced a real production protocol gap during the Scenario 1 diagnostic run: `stream_federation_delta` emits `state.federation_add` with `content.node_id = peer_b_id` (semantics "A approves B"); both sides ingest the same event and apply `content.node_id` verbatim onto `federation_nodes`. Result: `A.federation_nodes = {B}` correct, `B.federation_nodes = {B}` incorrect because it should contain A so F-3 lets A's events through. F-3 on B HeldPends every post-bootstrap A→B push event indefinitely; 180s timeout fires; all events discarded. No current code path emits the reciprocal `fed_add(node_id=A)` closing the gap. Existing cold_start tests didn't catch this because they build their bootstrap stream manually with `fed_add(node_id=peer_a_id)` which is not what production produces; federation_push_integration uses a one-sided harness where B is a wire reader without a real NodeRuntime; Phase 9 Scenario 1 is the first test to exercise the full bidirectional path with real dispatch on both sides.

**Commit 3a state on disk:** harness module (`xgen-node/src/tests/phase9_harness.rs`, ~340 lines, exercised end-to-end during diagnostic, works correctly) + Scenario 1 (`xgen-node/src/tests/phase9_two_node_smoke.rs`, authored in full, `#[ignore]`-annotated with inline doc comment naming the finding). Scenario stays on disk as the regression witness: when the protocol fix lands, the `#[ignore]` lifts and the same scenario becomes the activating regression lock. Six files changed total: `xgen-node/Cargo.toml` (`tracing-test = "0.2"` dev-dep), `xgen-node/src/app.rs` (`pub(crate)` on `handle_connection` + `replay_spaces_from_dir`), `xgen-node/src/tests/mod.rs` (two new module declarations), the two test files, `JOURNAL.md` (J-092 umbrella gains Commit 3a sub-entry naming the finding in full). `cargo test --workspace` clean: 571 passing, 0 failing, 1 ignored. Baseline matched exactly; only Scenario 1 added as ignored.

**Sibling shape to J-093 Phase 7.5 precedent.** This is the second instance of the pattern: dependent work (Phase 9 deployment-level testing) surfaces a load-bearing protocol gap that closes in its own dedicated phase before dependent work resumes. Phase 7.5's precedent: J-093 stand-down at Commit 3 → audit → design phase → implementation → Phase 9 resumes. Same shape applies here at one level finer: J-092 Commit 3a stand-down → audit → design phase → implementation → Phase 9 Commit 3b resumes (Scenarios 2 + 3) inside the same Phase 9 milestone scope.

**Phase 9 milestone scope unchanged.** 12 scenarios total (6 baseline + 6 compounds), 4 deferred compounds in `tasks/FEDERATION_STRESS_FOLLOWON.md`. The deferral is not descoped; it is paused until the protocol fix lands. Phase 9 milestone is doing its job: the 14-entry failure-mode catalogue at survey close named this category of finding as a HIGH-severity expected outcome of bidirectional deployment-level testing. Finding it now — before Pass 1 ships, before M6 goes active, before any downstream code depends on the broken behaviour — is the cheapest possible moment.

**Track 1 (Clair): Phase 9 PAUSED.** Phase 9 Commit 3b (Scenarios 2 + 3) stays paused inside the same milestone scope until the protocol fix lands. No active Clair track right now — Pass 1 remains blocked behind Federation Event Propagation milestone closure (which itself is now blocked behind the federation_nodes fix); Phase 9 Commit 3b waits behind the fix. When the fix ships, Phase 9 resumes with Scenario 1's `#[ignore]` lifting to become the activating regression lock, then Commit 3b ships Scenarios 2 + 3.

**Track 2 (Chat Claude + Joe): audit + design phase for the bidirectional federation_nodes question.** Per D-071 audit-precedes-dependent-design discipline (sibling to Phase 9 survey → implementation pattern, and to the J-081 audit → federation design phase pattern). Audit phase produces a canonical document on the bidirectional question; design phase walks four candidate fix shapes captured in JOURNAL Commit 3a sub-entry under J-092: (a) reciprocal a-i on receiver (receiving Node emits its own `fed_add` naming the sender); (b) post-handshake reciprocal hook (dedicated post-handshake step ensures both sides record each other); (c) `apply_federation_add` records sender too (one event documents both directions via sender field of the Event envelope); (d) hybrid combining elements. Each has wire-format implications, persistence implications, edge cases. Design walkthrough picks one with Joe-lock; this PLAY block does NOT pre-decide.

**Audit-phase reading list for Chat Claude pre-walk reconnaissance:** `docs/xgen_federation_propagation_design.md` §6.4 (Federation registry) + §10 (Cold-start bootstrap); `tasks/FEDERATION_PROPAGATION_COMPLETION.md` §3.3 (Phase 7.5 implementation reference); `xgen-core/src/federation/registry.rs` (FederationRegistry, PeerOperationalRecord, FederationRelationship); `xgen-node/src/app.rs::handle_federation_add` (current ingestion path); `xgen-node/src/federation_session.rs::stream_federation_delta` (current emission path); `xgen-core/src/space/state.rs::apply_federation_add` (current SpaceState mutation); JOURNAL J-092 Commit 3a sub-entry (the finding in full). Plus the canonical Pass 1 runbook (`tasks/XGID_RETROFIT_PASS_1_IMPL.md`) since the fix may affect `FederationRegistry` shape that Pass 1 retypes; if so, Pass 1 runbook needs revision before Pass 1 ships.

**Pass 1 runbook potentially affected.** Pass 1 retypes `SpaceState.federation_nodes` and `FederationRegistry` keys. If the bidirectional-fix changes the *shape* of those structures (not just adds fields but restructures them), Pass 1 needs to know before implementation starts. The audit phase will explicitly check this: does the fix change `federation_nodes` semantics or shape in a way that affects Pass 1's retype scope? If yes, Pass 1 runbook gets a sub-question revision. If no, Pass 1 proceeds as-is once Phase 9 closes.

**M6 (new) Node admin write path** remains 🟡 PENDING behind Federation Event Propagation milestone closure (which itself now waits behind the bidirectional federation_nodes fix landing AND Phase 9 Commit 3b completing). M6 Phase 0 design closed in J-085's neighbourhood; ACTIVE flip waits behind the full Federation milestone closure. M6 Block 4 verb-by-verb walks are parallel-eligible per the standing arrangement — Chat Claude + Joe work, doesn't block on Federation completion.

**XGID Retrofit Pass 1 implementation** remains 🟡 PENDING behind Federation Event Propagation milestone closure. Runbook is shipped at Status: ACTIVE v2.0 with all four sub-questions Joe-locked; the audit-phase check on whether the federation_nodes fix affects `FederationRegistry` shape is the only thing that could invalidate any part of the runbook before Clair picks it up.

**Entry point for Chat Claude's next session: audit phase pre-walk reconnaissance.** Read the audit-phase reading list above, then return to Joe for the audit walkthrough. Audit produces a canonical document on the bidirectional federation_nodes question (D-069 pattern); design phase walks the four candidate fix shapes; Joe locks one; Clair implements; Phase 9 resumes with Scenario 1's `#[ignore]` lifting. Expected shape: similar size to Phase 7.5 (multi-session design phase, multiple commits to ship the fix). No urgency — Pass 1 was already waiting on Phase 9, so the audit phase slots in without delaying anything else.

**Entry point for Clair's next session: standby until the protocol fix design phase closes.** Clair has no active Rust implementation track right now. Phase 9 Commit 3a is her last commit; Phase 9 Commit 3b waits behind the protocol fix; Pass 1 implementation waits behind Phase 9 closure. When the audit + design phase closes and Joe locks a fix shape, Clair picks up the implementation against the new locked design. Until then, the work surface is on Chat Claude + Joe's side.
...
*Read `DECISIONS.md` (current range D-000 through D-073) before making any decision that isn't explicitly covered by the spec. If you're unsure whether something needs a DECISIONS.md entry, it does.*
