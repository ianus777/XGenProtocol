...
## 🟢 UI component-library / substrate (RP track) — M-RP2.6 ✅ CLOSED (J-410): `button` retrofit (`ariaLabel` + `pressed`/toggle-mode) + `toggle` `shape` (switch `role`) — additive, skin-free, CDP-verified in BOTH apps
...
**Next-active (UI/RP track):** M-RP2.6 ✅ CLOSED (J-410) — the `button` retrofit (+`ariaLabel`, +`mode` momentary/toggle, +`pressed` $bindable; getter → `{clicks,disabled,pressed}`; `aria-pressed` toggle-mode-only) and `toggle` `shape` (switch → `role="switch"` + reflected `aria-checked`) landed **additive & skin-free**, CDP-verified in both apps — the pressed-latch self-redump finally landed (the event-driven proof the terminal Quit/Shut-Down couldn't, N-028 finding 1); Quit/Shut-Down stay momentary, untouched. **Next: M-RP2.7 — first skin pass** (the N-031 CSS source stack: `xgen-normalize.css` + first `skin.css`; render button text/icon · toggle checkbox/switch · pressed bevel `[aria-pressed]`; reconcile the global `input{}`/`button{}` wrinkle; found the L2 token vocabulary). Needs a **design walk → runbook before code** (open: skin.css home/path, accent gold/blue one-skin-vs-two, normalize wiring). Then `select` (di·A) → display-di `label`/`paragraph`/`image` (N-032) → first composites (`combobox` = textfield + datalist; `textfield-group`). **Entry (Rule 0): this PLAY → JOURNAL J-411 → `ui/docs/xgen-ui-notes.md` (N-029 textfield, N-030 shape families + button retrofit, N-031 CSS source stack, N-032 display-di identities) → `ui/docs/xgen-ui-components.md` (Built-components registry) → `docs/ROADMAP.md` (UI subtree + RP node).** UI sessions additionally read both `ui/docs/` files at open. Not pushed — Joe pushes.

> (Prior PLAY blocks: the AFI/F17 audit-against-code head J-398/J-400 ← doc-opt J-391…J-396 ← J-390 ← … ← M6 / XGID-retrofit — archived to `CLAUDE_HISTORY.md` per D-094, latest at J-411.)  
