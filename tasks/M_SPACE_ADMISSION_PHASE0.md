...
| **A-bis** | ⚠️ **THE TWO EXECUTABLE LEGS, BOTH STILL OWED (J-743).** ① a test that submits a third-party `membership.join` locally against a DM and asserts today's admission, then asserts the new gate refuses it — **the before-assertion is what makes it a regression test rather than a feature test**; ⚠️ **and per F-1 the actor needs NO registration, so the fixture must use a fresh unregistered keypair or it tests a narrower thing than the hole.** ② **a live two-node concurrent-set test for §3.2's divergence** — both Chat and Clair derived it from a measured gate and a measured applier, **and neither ran it** | Leg A |
...
- [ ] ⚠️ **JOE HAS NOT YET RULED §6.1–§6.8 — and two of them changed after the read, so nothing may be locked against the v1.1 text**
