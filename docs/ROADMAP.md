...
│   │   ↳ trigger: **Leg C-bis Phase-0 WRITTEN at J-703 — eight Joe locks, two delegated calls, four legs, zero Rust**: the draft is a render state carried by a sibling store, the prepared page is a Svelte component and not a processed string, truncation is the web's own word-wrap with no ellipsis, and three of the sketch's four controls have no verb behind them and do not ship. Nothing is left open for Joe on the design; the runbook is the remaining deliverable before code. The RMC context menu needs its own design pass and has no runbook
