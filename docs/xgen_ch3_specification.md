...
{
  "protocol_version": "0.1",
  "type": "transport.invite_bootstrap_request",
  "space_id": "xgen://hash/sha256:a3f9b2c1..."
}
```
...
Implementation reference: `xgen-core/src/wire/types.rs` `TransportMessage::InviteBootstrapRequest`; `xgen-node/src/fanout.rs` `collect_invite_bootstrap`. Design: `tasks/M8_5_B_INV_BOOTSTRAP_DESIGN.md` (INV-D1..D6).
...
| `3048` | `rejoin_not_anchored` | A Space-level `membership.join` from a **returning member** — one whose membership record is retained and marked departed (3.7.14.6) — that is concurrent with that identity's own last membership event rather than chained on it (M-SPACE-ADMISSION Leg G-2). Resolution drops such a join, so before this code existed the Node answered `Accepted` and then rebuilt the Space without her: the alternative to this refusal is a reply that lies. Local admission only, on the same footing as `3047` — a join `ReceivedViaFederation` is not re-adjudicated, since a replica trusts the home Node's admission decision. It fires **only** for a returning member; a first-time joiner whose join is concurrent with her own invite is deliberately out of scope, which is what keeps the name exactly true. |
...
These codes live in the existing identity domain (3000–3999, per CLAUDE.md error code convention); 3044/3045 extend the 3040s membership-authority sub-band, and 3047, 3048 and 3049 extend it again. Until M-SPACE-ADMISSION Leg G-2 this paragraph closed with the sentence *"3048 remains reserved for the M-SPACE-ADMISSION causal-anchor invariant and is not yet live"*. That sentence is quoted rather than deleted, because it was true when written and because the reservation is precisely what the row above discharges: `3048` is now live, emitted by the causal-anchor gate it was held for. A reader who remembers the reservation should be able to see where it ended, for the same reason the `3046` paragraph below states its own correction instead of quietly applying it.
...
**Decision record:** D-031a — MLS selected over Megolm. See DECISIONS.md for full rationale.
...
**Cross-referenced decisions:** D-037a (persistent accountable identity), D-046 (state resolution), D-049 (identity replication), D-059, D-060, D-061.
