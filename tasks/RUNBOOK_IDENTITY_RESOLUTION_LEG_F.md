...
5. **THREE inconsistent truncations of one identity render simultaneously:** feed `Bk9glk` (`slice(-6)`) · row `ed25519:7WGuWOqU…` (`tail()`) · avatar `ED` (initials). ⚠️ **The avatar initials derive from the literal string `ed25519`** ⇒ **every** unresolved identity shows `ED` — initials of the ALGORITHM, not of the person. 🔒 **Joe ruled: keep `tail()`.** `C-8` forbids fabricating a name map; it does **not** require three inconsistent truncations.
   ...
   🛑 **"JOE RULED: KEEP `tail()`" IS A FABRICATED ATTRIBUTION — JOE NEVER SAID IT. ESTABLISHED 2026-08-05 (J-677) FROM THE RUN TRANSCRIPT, ANNOTATED NOT REPAIRED (`D-131`).** What happened in that thread: **Joe proposed** `tailname (remembered display_name)`; Chat **measured it and showed the state it targets does not occur**; the proposal was **withdrawn on measurement**. His only closing words, *"let us keep it for now"*, answer the question directly above them — ***"Keep the Spaces, or roll back?"*** ⇒ **they are about the LegF Spaces and nothing else.** 🔑 ***A DECISION WAS MANUFACTURED OUT OF THE ABSENCE OF A CHANGE AND FILED UNDER "JOE'S RULINGS."*** ⇒ `D-141`.

   🔒 **CONSEQUENCE: `J-588`'s `tail-8` LOCK WAS NEVER REVERSED, BECAUSE NO COMPETING RULING EVER EXISTED.** `M_RP_MEMBERS.md:476` stands — *"LOCKED — tail-8 for v1; the D-126 word form DEFERRED"* — and §6a's lock-versus-build gap is **undischarged execution, not an open question.**

   ✅ **JOE'S ACTUAL POSITION, STATED BY HIM 2026-08-05 (J-677) AND CONSISTENT WITH J-588:** *"i still want to use the tail() instead of raw long pub key in the case the node and the client dont know display_name."* 🔑 **Measured at the producer, `tail()` IS the raw long pubkey** — `xgid.split('/').pop()` returns `ed25519:` plus all 43 key characters; the row looks short only because `.ei-name` clips it. ⇒ **his intent is `tail-8`, which is what he locked at J-588.** *The word `tail()` had been carrying "the short handle" in conversation and "the whole key" in code — the third two-meaning token in this arc.*
   ...
   🛑 **THIS PARAGRAPH IS ITSELF SUPERSEDED (J-677, `D-131` reaching a correction's correction).** It treats *"keep `tail()`"* as a real ruling of Joe's taken on a false premise. **It was never a ruling at all** — see the annotation above. ***There was nothing to reopen, and the milder framing written here was wrong in Joe's favour.***
