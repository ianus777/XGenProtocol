...
│   │   ↳ Owes: OPEN and Joe: **LEG G — THE REJOIN ANCHOR — IS SCHEDULED (Joe, 2026-08-24) AND HAS ITS OWN PHASE-0: tasks/M_SPACE_ADMISSION_LEGG_PHASE0.md v1.0 ACTIVE, which is this leg's register.** SIX legs: G-0 Phase-0, G-1 the gate's third term, G-2 the 3048 refusal, G-3 the door, G-4 the client anchor selection, G-5 close. **WIRE SHAPE RULED: ONE MORE KEY on collect_invite_bootstrap's authorization, NOT a new transport verb** — and it was MEASURED, not preferred: TransportMessage is serde tag-only with ZERO serde(other), so an unrecognised transport type is a deserialisation error at connection.rs:454 and the node loop is Err break at app.rs:2048, which means a new verb sent to an older node DROPS HER SESSION instead of being refused — the opposite of what the design section claimed for it
