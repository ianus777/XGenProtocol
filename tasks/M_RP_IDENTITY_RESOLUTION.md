...
**Leg B — the render rules.** ✅ **UNBLOCKED — Leg A landed the ids (J-647).** 🔒 **Runbook `tasks/RUNBOOK_IDENTITY_RESOLUTION_LEG_B.md` v1.2 — ACTIVE + LOCKED (Joe, J-652).**

🛑 **AND IT IS LOCKED AND IDLE. The trigger is *"Joe locks the runbook AND stands Clair up"* — TWO ACTS, and he has performed ONE.** ⇒ **NOBODY IS IMPLEMENTING IT.** ⚠️ ***"Locked" reads like "started" to anyone skimming*** (J-646). ⚠️ *v1.7–v1.8 read "v1.0/v1.1 — AUTHORED, NOT LOCKED"; the first half is discharged, the second is not. Superseded, kept not erased (`D-131`).*
...
**Leg C — the skin.** The dimmed treatment in `ui/assets/skin.css`. ⚠️ **JOE'S FILE.** Chat supplies the hook and measures; **the values are his**. 🔒 **LOCKED SEPARATE FROM LEG B — IT DOES NOT RIDE ALONG (Joe, 2026-08-01, J-650).** 🛑 **CONSEQUENCE, WRITTEN DOWN SO LEG B's CLOSE IS NOT READ AS A FAILURE: LEG B LANDS SILENTLY.** ④'s dimming and E2's mark are both `skin.css` values ⇒ after Leg B the only visible change on screen is that erased non-DM members disappear from the list.
...
✅ **G-A IS CLOSED AND LEG A HAS SHIPPED (J-647).** ⚠️ *v1.0–v1.4 closed this section with "NOTHING HERE IS BUILDABLE UNTIL G-A IS CLOSED — Leg 0 is complete; Leg A needs a runbook"; superseded, kept not erased (`D-131`).* 🛑 **What is still NOT buildable is the part G-B gates: §4's dimming must not SHIP before a refresh trigger exists, or the panel promises a resolution it cannot deliver.** 🟡 **Leg B is next-active.** ⚠️ *v1.4–v1.6 closed this line with "and has no runbook" — **that went false at J-650**, when `tasks/RUNBOOK_IDENTITY_RESOLUTION_LEG_B.md` v1.0 was authored. Superseded, kept not erased (`D-131`).* 🛑 **The runbook is LOCKED (J-652) and Clair has still NOT been stood up — two acts, and ONE has been performed (the J-646 rule: *"locked" reads like "started"*).** ✅ **She HAS read it adversarially at v1.0 and her findings are applied at v1.1 (J-651) — a read is not a standing-up, and the distinction is the whole point of the two acts.** ⚠️ *v1.7–v1.8 read "AUTHORED, NOT LOCKED, and Clair has NOT been stood up — neither has been performed"; the lock landed at J-652. Superseded, kept not erased (`D-131`).* 📌 *Caught by sweeping this document for gates that have fired before locking anything — J-642's own discipline, applied to the file that cites it.*
