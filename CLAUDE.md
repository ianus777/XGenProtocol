...
> Date: May 2026  
> **Status:** ACTIVE  
> **Last updated:** 2026-05-21 (drift correction — PLAY block stale on Chat Claude's active work. The previous header bump (D-074 promotion, earlier today) refreshed only the Last-updated field; the PLAY block body and title still described "authoring `tasks/XGID_RETROFIT_PASS_1_IMPL.md`" as Chat Claude's active track, but that runbook shipped to Status: ACTIVE v2.0 earlier today, with all four sub-questions Joe-locked and walkthrough sections populated. Caught by Clair during her Phase 9 pre-session reconnaissance and surfaced honestly via Joe. This commit corrects the PLAY block to honest state: Chat Claude's next-active slot is open with parallel-eligible work available (M6 Block 4, JOURNAL gap retrospectives, or standby), the Pass 1 runbook is shipped and waiting on Phase 9 closure for Clair handoff, and Pass 1 implementation remains Clair's pickup post-Phase-9. ROADMAP.md v1.6 → v1.7 in same commit per same-commit discipline: line 197 live-frontier line removes runbook authoring from the parallel-eligible list, Present section's "🟢 Retrofit Pass 1 runbook authoring" entry moves to Past as ✅ closed entry ("Pass 1 runbook shipped at v2.0 2026-05-21, all sub-questions locked"), Near future's Pass 1 paragraph slightly refreshed to point at the now-existing runbook location instead of "authored same-day" language. Worked instance of D-074-promoted discipline applied to drift-correction commits: this commit's changed-files list includes both CLAUDE.md and ROADMAP.md, and the lesson the drift teaches (same-commit discipline must touch PLAY block content not just header) is named in the next-available JOURNAL entry when one is written. Header content from previous bump (D-074 promotion summary) stands authoritative in DECISIONS.md and is no longer the freshest news on CLAUDE.md — the drift-correction is.)  
> Author: JozefN  
> Credits: Concept, philosophy, requirements, project direction: Jozef Nižnanský. Technical assistance and implementation support: AI-assisted development tools.  

---

## 🟢 PLAY — Phase 9 active for Clair; Chat Claude's next-active slot open (Pass 1 runbook shipped, awaiting Phase 9 closure)

**XGID Adoption v1 implementation milestone CLOSED 2026-05-20 (J-095).** Clair's two-commit plan shipped per `tasks/XGID_ADOPTION_IMPL.md` (Status: ACTIVE → COMPLETED v1.1 at milestone close): Commit 1 (`c95584a`) ships `xgen-common` XGID type vocabulary + five required wire-format invariance tests; Commit 2 (`24a255b`) retypes `SpaceLocalMetadata.introducer_node_id` from `Option<String>` to `Option<NodeXgid>` as the only production-code retype in v1. Hygiene atom (`904441b`, `chore(workspace): clippy cleanups for new toolchain`) shipped same-session as a separate provenance-distinct sibling commit (flipping workspace clippy gate red → green under Rust 1.95.0; 26 files across all four crates; NOT XGID code). Milestone-close commit (this commit) carries JOURNAL J-095 + Ch4 v1 follow-on pointer (A5 Joe-lock from Phase 2 sweep) + this PLAY block flip + ROADMAP.md state move + `tasks/XGID_ADOPTION_IMPL.md` flip to COMPLETED. Test count **556 → 571** across the milestone (+15: 10 in-module flavour tests + 5 wire-format invariance tests). Workspace clippy clean after every commit.

**XGID Retrofit Pass 1 runbook shipped 2026-05-21.** Chat Claude authored `tasks/XGID_RETROFIT_PASS_1_IMPL.md` from skeleton through v2.0 across a single session, with all four sub-questions Joe-locked: (1) multi-commit sequence (six commits per Pass 1 close); (2) `canonical_event_bytes` module move as isolated head commit of the sequence; (3) explicit `.as_str()` projection at cross-type-boundary call sites with code-comment discipline at non-trivial sites — no `Deref<Target = str>` added on base `Xgid` per v1 precedent; (4) five new invariance tests pinned by name (`event_struct_full_xgid_roundtrip_through_canonical_form`, `space_local_metadata_full_xgid_roundtrip`, `node_state_observability_xgid_roundtrip`, `event_content_struct_xgid_roundtrip`, `legacy_string_json_forward_compat_on_event`), file placement Clair-latitude. Combined v1 + Pass 1 gives 10 invariance tests total. Runbook walkthrough sections (Code scope, Doc scope, Verification gate, Milestone close) populated per the locked sub-questions. Same-commit ROADMAP.md edit shipped: Pass 1 section title renamed "xgen-common core types" → "core data structures" in both the Visual structure tree and the Near future prose, with header note explaining Option-B Joe-lock (Pass 1 spans xgen-common + xgen-core because Phase 2 doc-tree sweep's coordination flag pins Appx C + Appx I to Pass 1 in one commit set). **Pass 1 implementation remains Clair's pickup, blocked behind Federation Event Propagation milestone closure (Phase 9 going DONE).**

**D-074 promoted to a numbered decision 2026-05-21.** Sibling work shipped same-day: "Milestone-close commits MUST include JOURNAL.md" is now a formal entry in `DECISIONS.md` D-074 (the candidate principle was surfaced 2026-05-20 during J-094 cleanup forensics). ROADMAP.md v1.5 → v1.6 propagated the promotion: "D-074 candidate" line removed from the Discipline / JOURNAL hygiene cluster; Cross-cutting principles section gained "✅ D-074" alongside D-069/D-070/D-071/D-072/D-073.

**Track 1 (Clair): Phase 9 — Federation Event Propagation deployment integration tests RESUMED from Commit 3 boundary.** [tasks/FEDERATION_PROPAGATION_PHASE_9.md](tasks/FEDERATION_PROPAGATION_PHASE_9.md) remains Status: ACTIVE v1.0; scope intact. Dependency chain reads "Phase 7.5 ✅ → XGID Adoption design ✅ → XGID Adoption canonical sources ✅ → XGID Adoption Phase 2 doc-tree sweep ✅ → **XGID Adoption implementation ✅** → Phase 9 (NOW ACTIVE)." Integration test code uses XGID types from start. After Phase 9 ships, Federation Event Propagation milestone flips PLAY → DONE and M6 (new) Node admin write path unblocks for implementation per its Pass 3 design.

**Track 2 (Chat Claude): next-active slot OPEN.** The Pass 1 runbook authoring track that previously occupied this slot shipped earlier today (Pass 1 runbook now at Status: ACTIVE v2.0, awaiting Clair pickup post-Phase-9). Chat Claude's parallel-eligible work options:

- **M6 Block 4 verb-by-verb walks.** Pass 3 closed M6 (new) framework decisions; Block 4 walks all seven verb categories (~35 verbs) to confirm names, lock argument schemas, lock result schemas, apply the two-token naming convention. Fills in §6 of the design doc which is currently stubbed per category. Chat Claude + Joe work. Independent of Federation completion — can run in parallel.
- **JOURNAL gap retrospectives.** Two deferred retrospective entries in the Discipline / JOURNAL hygiene cluster: (1) Phase 7.5 implementation milestone retrospective (commits `12cfe5a` + `aa2433f` + `1be7189` + `ecbbf19` + `8859093`, originating incident for D-074); (2) XGID Adoption v1 design + Phase 1 retrospective (commit `a5f3c8b`, the 8-artefact atomic canonical-sources shipment). Both are small honest-provenance fills, not blocking anything.
- **Standby.** Either of the above is welcome but not urgent. Chat Claude can also wait for the Pass 1 handoff signal ("🚦 Pass 1 ready for Clair handoff") which fires when Phase 9 ships and Federation Event Propagation milestone closes.

**Pass 1 handoff signal pending.** When Phase 9 Commit 3+ ships and the Federation Event Propagation milestone flips PLAY → DONE, AND the Pass 1 runbook is still at Status: ACTIVE v2.0 (no invalidating findings surfaced), Chat Claude flags a clear "🚦 Pass 1 ready for Clair handoff" message in the PLAY block update of that closure commit, naming `tasks/XGID_RETROFIT_PASS_1_IMPL.md` as the entry-point file for Clair's first Pass 1 session.

**M6 (new) Node admin write path** remains 🟡 PENDING behind Federation Event Propagation milestone closure (which itself waits for Phase 9 to ship). M6 Phase 0 design closed in J-085's neighbourhood; ACTIVE flip waits behind Phase 9. M6 Block 4 verb-by-verb walks are parallel-eligible per above.

**Entry point for Clair's next session: `tasks/FEDERATION_PROPAGATION_PHASE_9.md`** — Phase 9 Commit 3 is the next active commit, the in-process two-Node deployment scenarios that paused at the cold-start bootstrap dead-lock in J-093 and now have working post-Phase-7.5 bootstrap behaviour to test against. Read J-093 first (the stand-down at Commit 3), then J-094 + J-095 context (XGID adoption shipped between J-093 and now; integration test code uses XGID types from start), then the Phase 9 task file's §3 Commit 3 scope. Q3 escalation criterion from J-091 still applies: any new test exhibiting 127.0.0.1:0 bind race or WS frame-ordering inconsistency under workspace parallelism — STOP and walk back to option (ii).
