...
|---|---|---|---|
| Forgery variants | bad-signature | **{bad_signature, forged_sender_with_resign, mutated_event_id, malformed_prev_events}** | Four variants mapping 1:1 to ExchangeError enum variants at `xgen-core/src/message/exchange.rs:46-87` (SignatureFailure, UnknownSender, EventIdMismatch, DagError); one variant per production reject path. Original 6-variant set (which included `mutated-sender` ambiguous + `future-timestamp` + `past-timestamp`) revised at J-113 — see §2.4.1 below for production-code-verification walkthrough |
...
**Recommendation (J-113 amendment):** 4 forgery variants × 5 event families = **20 NodeRuntime-level forgery tests**. Plus 1 end-to-end stress-complete-shape test with 20 forged + 100 valid mixed. **C5 below is the strong version** — Phase 9 should land C5 in place of an independent Scenario 4 stress-only test. The variant compression from 6 to 4 is recorded explicitly to make future readers aware that timestamp-bound validation (Gap G6 per §4.6 below) is a flagged production gap, NOT a forgotten test case.
...
| `UnknownSender` | Step 11 | "step 11: sender is not a registered Identity" | `forged_sender_with_resign_*` (5 family tests) |
