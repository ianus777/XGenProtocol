...
| **G** | ✅ **CLOSED J-779 — ALL SIX LEGS SHIPPED (`G-0`…`G-5`), AND WITH IT THE MILESTONE.** `cargo` 1641 → **1667 / 0 / 64 × 57 SUITES** across the leg, every number re-driven independently by both seats on forced rebuilds. Register: `tasks/M_SPACE_ADMISSION_LEGG_PHASE0.md` **v1.6 COMPLETED**. 🔒 **SCHEDULED (Joe, 2026-08-24). PHASE-0 LANDED J-773 — `tasks/M_SPACE_ADMISSION_LEGG_PHASE0.md` v1.0 ACTIVE, WHICH IS THIS LEG'S REGISTER.** 🔒 **WIRE SHAPE RULED — ② ONE MORE KEY, NOT A NEW DOOR:** `collect_invite_bootstrap`'s AUTHORIZATION widens to a retained former member; **NO new `transport.*` variant.** 🔑 **THE MEASUREMENT THAT DECIDED IT: `TransportMessage` is `#[serde(tag = "type")]` with ZERO `serde(other)`, so an unrecognised `transport.*` type is a DESERIALISATION ERROR at `connection.rs:454` and the node's client loop is `Err(_) => break` (`app.rs:2048`)** ⇒ ***a new verb sent to an older node DROPS HER SESSION rather than being refused*** — the exact opposite of §15.7's stated fallback. 🛑 **AND THE LEG GREW ONE: the shipped gate implements TWO of §15.4's THREE terms** — `runtime.rs:1620` tests `admission != open && !pending_invites.contains_key`, `left_at` has ZERO non-test occurrences in `runtime.rs`, and `apply_join` (`state.rs:1214`) DOES carry the third arm ⇒ ***the dispatch gate refuses her `3047` before the applier that would admit her is reached, in every DM.*** **SIX LEGS: `G-0` Phase-0 · `G-1` the gate's third term · `G-2` `3048 rejoin_not_anchored` · `G-3` the door · `G-4` the client anchor selection · `G-5` close.** 🔒 **`G-2` BEFORE `G-3`/`G-4`, FORCED: `G-1` alone makes rejoins reachable and therefore makes the accept-then-drop reachable in production for the first time.** **Original row follows.** ~~A node-side `get_rejoin_anchor`, sibling of `collect_invite_bootstrap` … **New wire surface ⇒ Joe's seat ⇒ its own leg**, sequenced after D, **and it may slip without leaving a silent failure behind, because `3048` already made the residue loud**~~ — 🛑 **that last clause is FALSE: `3048` is NOT LIVE (`D-131` annotation on the Leg D row above).** | Legs A + D + E |
...
🛑 **ANNOTATION AT THE SITE (`D-131`, J-773, 2026-08-24) — THIS SECTION IS CORRECT AND THE SHIPPED GATE IS NOT. TWO OF THE THREE TERMS ARE BUILT.** ✅ **MEASURED at `runtime.rs:1620-1621`:** `if space.admission != ADMISSION_OPEN && !space.pending_invites.contains_key(&event.sender)` — **the FORMER-MEMBER term is absent, and `left_at` has ZERO non-test occurrences in `runtime.rs`** (3 hits, all `#[cfg(test)]`). ✅ **`apply_join` (`state.rs:1214`) DOES implement it** — the three-way gate's third arm, *record PRESENT but DEPARTED → fall through*. ⇒ 🔑 ***the dispatch gate refuses her `3047` before the applier that would admit her is ever reached.*** 🛑 **SCOPE: every DM** (both DM constructors pin `invite`, at FOLD time, so pre-Leg-B DMs too) **plus any Space an owner has closed; a Space left at the default `open` is unaffected, which is why the arc's rejoin work looked complete.** 🔑 **CAUSE: §15.4 was written before `left_at` existed; J-763 Deviation ① correctly moved `D-3` to Leg E; Leg E built `left_at` and the applier arm and NOBODY RETURNED TO THE GATE** — ***a deferral is valid only as long as its premise holds.*** 🛑 **AND NO INSTRUMENT COULD SEE IT: thirteen `fn *rejoin*` sites exist and NOT ONE exercises the dispatch gate with a former member** — *the suite is green because the case is unwritten.* ✅ **REPAIR: `Leg G-1`** (`tasks/M_SPACE_ADMISSION_LEGG_PHASE0.md` §2.1).
...
🔒 **SUPERSEDED IN SHAPE, NOT IN PURPOSE (Joe, 2026-08-24; J-773). THE ANCHOR GOAL STANDS; THE NEW-VERB MECHANISM IS REPLACED BY ② — WIDENING `collect_invite_bootstrap`'s AUTHORIZATION.** 🛑 **AND ONE SENTENCE BELOW IS FALSE AS MEASURED: *"An older Node that cannot answer ⇒ the client falls back to exactly today's behaviour."*** ✅ **`TransportMessage` is `#[serde(tag = "type")]` (`wire/types.rs:48`) with ZERO `serde(other)`; an unrecognised `transport.*` type reaches `serde_json::from_value(value)?` at `connection.rs:454` and ERRORS; the node's client loop is `Err(_) => break` (`app.rs:2048`).** ⇒ ***a new verb sent to an older node DROPS THE SESSION — it is not refused, and there is nothing to fall back from.*** 🔑 **The degradation property this section claimed for a new verb is delivered ONLY by ②, where an older node returns `1011` and the client falls back exactly as described.** 📌 **Unknown-variant transport tolerance is FILED as its own node** (the `H2` capability question, J-601) **and must not ride Leg G.** 📌 **The full design is `tasks/M_SPACE_ADMISSION_LEGG_PHASE0.md` v1.0 §3–§5.** *Superseded text follows.* A node-side sibling of `collect_invite_bootstrap` (`xgen-node/src/fanout.rs:572-577`): the rejoiner asks her Node for **her own** last membership event; the join anchors `prev_events=[leave_id]` ⇒ linear `j→lv→rj` ⇒ **frontier of size 1 ⇒ no conflict set ⇒ the fold applies it.** **Authorization is trivial — she asks for her OWN event, no disclosure surface.** ~~An older Node that cannot answer ⇒ the client falls back to exactly today's behaviour.~~ **New wire verb ⇒ Joe's seat ⇒ not smuggled into Leg E.**
