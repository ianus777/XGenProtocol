...
**Uniform assertion shape per test for variant row 2** (supersedes §2.4.1's step 3 for this variant):

```rust
// For 4 families (message_text, membership_join, membership_kick, state_room_create):
match outcome {
    DispatchOutcome::HeldPending(buffer_state) => {
        // Phase 6 F-10 HeldPending shape; missing_identity populated, missing_predecessors empty
        assert_eq!(
            buffer_state.missing_identity.as_deref(),
            Some(attacker_uri.as_str())
        );
        assert!(buffer_state.missing_predecessors.is_empty());
        // Assert event did NOT land in runtime's DAG
        assert_eq!(runtime.spaces[&space_id].event_count, baseline_event_count);
    }
    other => panic!("expected HeldPending, got {other:?}"),
}

// For state_federation_add (asymmetric cell):
match outcome {
    DispatchOutcome::Accepted { .. } => {
        ...
        // This is the security property documented at the table above.
        assert_eq!(runtime.spaces[&space_id].event_count, baseline_event_count + 1);
        // Optional: explicit doc-comment in the test file naming Phase 7 B3 + J-088
        // as the locked design property this test pins.
    }
    other => panic!("expected Accepted (B3 asymmetric cell), got {other:?}"),
}
```

**Source-of-truth pointers.** Code: `xgen-core/src/message/exchange.rs:545+` (validate_event F-4 unified core) + `:626-632` (Phase 6 F-10 unknown-sender HeldPending return) + `:455-509` (Phase 7 B3 federation_add skip block); `xgen-core/src/node/runtime.rs:317` (dispatch_event entry) + `:334-340` (test-only-reachable doc-comment for validate_steps_8_13 path). JOURNAL: J-115 entry for this amendment; J-087 for Phase 6 F-10 origin; J-088 for Phase 7 B3 origin. Adjacent canonical record: `tasks/FEDERATION_PROPAGATION_PHASE_9_COMMIT_3B_4_IMPL.md` v1.1 §4.2 + §4.5 amendments for runbook-side enumeration; `tasks/FEDERATION_PROPAGATION_PHASE_9.md` §3 Commit 6 paragraph for Phase 9-side enumeration.
