...
🛑 **NOT LOCKED. DO NOT IMPLEMENT YET.** Two things are owed before Joe locks it:
1. 🔒 **AN ADVERSARIAL READ BY CLAIR** — recommended in the Phase-0 §5 and repeated here. **E-1 was locked without one and shipped a `§4` command that could not run** (`insertLeaf` is not on `window`). *Attack this document; do not implement it yet.*
2. 🔓 **JOE'S CONSENT FOR THE `V4` SIDE EFFECT** (§5) — that leg writes a named UI state to his disk. Ruled `V-a` under `D-141`, **but a delegation rules the DESIGN, not the SIDE EFFECT.**
...
| `RegionId = string` · `Layout {version, root}` | `types.ts` |
| `PluginDescriptor` already imported as a TYPE in `layout-default.ts` | `layout-default.ts:14` |
| `DEFAULT_LAYOUT` — **8 leaves**, `spaces` a direct child of the ROOT `row` split | `layout-default.ts:105-125` |
| the `dm-spaces` descriptor (`system` · `region` · `regionId: 'dm-spaces'`) | `registry.ts:214-224` |
| the widget's register id | `dm-spaces.svelte:33` → `dm-spaces#region-dm-spaces` |
...
⚠️ **Keep the existing JSDoc block and extend it**; do not delete the N-095/D-115 reasoning already written above the function.
...
✅ **`mountedPlugins` is in scope at all three sites** — it is a component-level `$derived` (`:102`), and boot runs it **after** `installed.hydrate` / `hydrateDisabled` (`:696`/`:700`), so the set is already seeded when the layout resolves.
...
- 🛑 **`__XGEN_LAYOUT__` DOES NOT EXPOSE `insertLeaf`** — the DEV bridge is `current` · `set` · `move` · `fold` · `background` · `setBackground` (`app_client.svelte:392-402`). **E-1's runbook §4 shipped a command that could not run; do not copy it.** `set(l) { layout = l; }` is a bare in-memory reassignment and **does NOT persist**; `move`/`fold` delegate to the shell handlers and **DO** persist.
...
| **V4** | 🛑 **the `:895` path** — a named UI state saved with an 8-leaf tree still yields the home | **JOE'S CONSENT REQUIRED FIRST.** Read the saved state file and show it has **no `dm-spaces` leaf** (the positive control), then Load → home present → **DELETE the state → show exact return to baseline** (`N-123`) |
...
🛑 **V4's control is the point** — *"the home appeared"* proves nothing unless the saved tree is first shown to lack it.
...
- [ ] V4's positive control shown (saved tree lacks the leaf) **and** the named state deleted, baseline shown
...
1. **The `mountedPlugins` read at `:709` sits inside `onMount`.** It is a `$derived` and should read fine there — **but that is reasoned, not driven.** If it reads empty at boot, the guard skips `dm-spaces` and **the home silently never appears**: a failure that looks exactly like *"the feature does not work"* rather than an error. **Check this first, and report it rather than working around it.**
2. **The mounted-set guard may be stricter than the leg needs.** `dm-spaces` is `kind: 'system'` and therefore always in `CLIENT_PLUGINS` and never disableable, so the guard can only ever pass today. **It is defensive for future rows and it is UNEXERCISED** — say so; do not claim it verified.
3. **§3.3 rewrites a function whose JSDoc carries N-095/D-115 reasoning.** Losing that prose is a real cost. **Extend, do not replace.**
...
5. **This runbook has not been read by anyone but its author.** ⚠️ *Every real defect in this arc came from outside the text — Joe's recall or you executing it. Attack it.*
