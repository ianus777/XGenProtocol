...
## 🟢 PLAY — M6 (new): **federation-admin-control 2a sub-arc CLOSED at J-178** (Commits 1–5; `accept`/`reject`/`initiate` SHIPPED, default-off invariant held); **next-active = sub-arc 2b (federation policy) — design phase** (pull `tasks/M6_FEDERATION_POLICY_DESIGN.md` PENDING → ACTIVE, lock FAC-D3/D4: the policy store + enforcement for `set-policy`/`show-policy`); then auth-module-registry → bootstrap-client; then M7 `--aicontrol`
