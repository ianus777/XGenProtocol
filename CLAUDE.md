...
> Date: May 2026  
> **Status:** ACTIVE  
> **Last updated:** 2026-05-19 (J-087 — Federation Event Propagation Phase 6 shipped: F-10 HeldPending generalisation for unknown signer Identity, `ValidationOutcome::HeldPending` struct variant with `missing_predecessors` + `missing_identity` per §3.6.1 Lock B1, per-`PendingBuffer` `waiting_for_identity` secondary index with `NodeRuntime::drain_pending_by_identity` cross-Space fan-out per §3.6.1 Lock A2, `pending_identity_replication` observability counter in `xgen-node_state.json` per §3.6.1 Lock C2, error code `4006 identity_record_timeout` (next-free after 4001-4005 in domain 4000-4999 per Step 6 namespace verification) with predecessor-code-wins sub-rule per §3.6.1 Lock D, legacy `validate_steps_8_13` confirmed test-only-reachable per Step 1 verification (no mirror change to `ExchangeError::HeldPending` needed); 516 tests, +11 from Phase 5's 505.)  
> Author: JozefN  
> Credits: Concept, philosophy, requirements, project direction: Jozef Nižnanský. Technical assistance and implementation support: AI-assisted development tools.  

---

## 🟢 PLAY — Federation Event Propagation implementation milestone (Phases 1–6 SHIPPED; Phase 7 next-active)
...
- **Runbook:** `tasks/FEDERATION_PROPAGATION_COMPLETION.md`. Nine phases — **Phase 1 ✅, Phase 2 ✅, Phase 3 ✅, Phase 4 ✅, Phase 5 ✅, Phase 6 ✅**, Phases 7 through 9 PENDING.
- **Tests:** 468 (handoff) → 476 (Phase 1) → 480 (Phase 2) → 488 (Phase 3) → 491 (Phase 4) → 505 (Phase 5) → **516** (Phase 6 close, +11: 7 unit tests in `xgen-core::dag::pending` covering the F-10 buffer surface — identity-only buffering, dual-dependency wait, reverse-order release, timeout-records-missing-identity — plus 4 integration tests in new `xgen-node/src/tests/heldpending_identity_integration.rs` covering F-10 §13.7 DoD scenarios (a)/(b)/(c)/(d) at the `NodeRuntime` level).
- **Blocks:** M6 (new) still blocked behind this milestone going DONE.
- **Next active phase:** Phase 7 — Federation-relationship verification gate (F-3 second check). Per runbook §3.7. Phase 2's dispatcher pipeline shape reserves a federation-relationship placeholder at step 2; Phase 7 fills it with the real lookup against the federation registry (now load-bearing post-Phase-5). Coordinates with M6 Phase 2 envelope-`event_id` work for the wire-layer rejection signal.
...
**Phase 8 doc-pass items recorded for visibility (NOT closing milestone's burden — corrected in Phase 8).** Cumulative tally — 5 items (3 from Phase 5 + 2 new from Phase 6):
...
**Federation Event Propagation implementation Phases 1-6 shipped (J-082 + J-083 + J-084 + J-085 + J-086 + J-087 across 2026-05-18 and 2026-05-19).** Phase 6 (J-087) generalises HeldPending to handle unknown signer Identity — federation first-contact events buffer pending Identity-record replication rather than reject-then-re-pull. Tests: 468 (handoff) → 476 (Phase 1) → 480 (Phase 2) → 488 (Phase 3) → 491 (Phase 4) → 505 (Phase 5) → **516** (Phase 6, +11: 7 unit tests in `xgen-core::dag::pending` for the F-10 buffer surface; 4 integration tests in new `heldpending_identity_integration.rs` for F-10 §13.7 DoD scenarios). Next active phase: Phase 7 (F-3 federation-relationship verification gate). M6 (new) remains blocked behind this milestone going DONE. Roadmap: M5 ✅ → CLI Audit ✅ → J-080 ✅ → M6 Phase 0 Pass 3 ✅ → Propagation Reliability Audit ✅ → Federation design (Pass 2 + Pass 3) ✅ → **Federation implementation Phases 1-6 ✅, Phase 7 (F-3 federation-relationship gate) next** → M6 (new) → M7 → M8 → M9.
...
- **Federation Event Propagation implementation**: 🟢 PLAY. Nine-phase runbook; **Phases 1-6 ✅ SHIPPED** (J-082 + J-083 + J-084 + J-085 + J-086 + J-087). **Phase 6 ✅ (J-087, 2026-05-19)** — F-10 HeldPending generalisation: `ValidationOutcome::HeldPending` becomes struct variant `{ missing_predecessors, missing_identity }` per §3.6.1 Lock B1; per-`PendingBuffer` `waiting_for_identity` secondary index with `NodeRuntime::drain_pending_by_identity` cross-Space fan-out per Lock A2; `pending_identity_replication` counter exposed in `xgen-node_state.json` per Lock C2; new error code 4006 `identity_record_timeout` (next-free after 4001-4005 in state-resolution domain) with predecessor-code-wins sub-rule per Lock D and verbatim code-comment block at the emit site; legacy `validate_steps_8_13` confirmed test-only-reachable per Step 1 verification (no mirror change). 516 tests at Phase 6 close. Phase 7 (F-3 federation-relationship verification gate) is next-active.
