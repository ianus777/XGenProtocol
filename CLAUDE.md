...
- **Runbook:** `tasks/FEDERATION_PROPAGATION_COMPLETION.md`. Nine phases — **Phase 1 ✅, Phase 2 ✅, Phase 3 ✅, Phase 4 ✅, Phase 5 ✅**, Phases 6 through 9 PENDING.
- **Tests:** 468 (handoff) → 476 (Phase 1) → 480 (Phase 2) → 488 (Phase 3) → 491 (Phase 4) → **505** (Phase 5 close, +14: 10 unit tests in `xgen-core::federation::registry` for the F-1c operational API, 2 unit tests in `xgen-node::reconnect` for backoff ladder constants + step indexing, 2 bilateral integration tests in new `xgen-node/src/tests/reconnect_integration.rs`).
- **Blocks:** M6 (new) still blocked behind this milestone going DONE.
- **Next active phase:** Phase 6 — HeldPending generalisation for unknown signer Identity (F-10). Per runbook §3.6. Extend HeldPending's trigger condition from "unknown predecessor" to "unknown predecessor OR unknown signer Identity OR both." Buffer waits for both dependencies; when all arrive, the event is re-routed through the validation core. Timeout policy unchanged from F-4a (30 s uniform). Four integration tests per F-10 §13.7.
...
**Phase 8 doc-pass items recorded for visibility (NOT Phase 5's burden — corrected in Phase 8).** The Phase 5 survey surfaced three doc-vs-code drift surfaces, all describing SQLite-backed federation storage that doesn't exist (the registry is JSON-backed):

1. `docs/xgen_ch4_implementation.md` §4.11.2 — SQLite `federation_relationships` + `peer_announcements` tables described but never implemented.
2. `CLAUDE.md` Tier-1 file table — lists `xgen-node_federation.db` (SQLite); actual file is `xgen-node_federation.json` (JSON).
3. Runbook §3.5 "Schema decision" paragraph — frames the F-1c storage choice as "extend `peer_announcements` columns vs sibling table" assuming SQLite; the actual choice (Lock A) was between Rust struct extension strategies for a JSON-backed registry.
...
**Federation Event Propagation implementation Phases 1-5 shipped (J-082 + J-083 + J-084 + J-085 + J-086 across 2026-05-18 and 2026-05-19).** Phase 5 (J-086) closes the "zero production callers of `run_initiating` in xgen-node/src/" verdict from audit J-081 §2.2 — the F-1c reconnect scheduler is the first production caller. Tests: 468 (handoff) → 476 (Phase 1) → 480 (Phase 2) → 488 (Phase 3) → 491 (Phase 4) → **505** (Phase 5, +14: 10 unit tests in `xgen-core::federation::registry` for the F-1c operational API, 2 unit tests in `xgen-node::reconnect`, 2 bilateral integration tests in new `reconnect_integration.rs`). Next active phase: Phase 6 (HeldPending generalisation for unknown signer Identity per F-10). M6 (new) remains blocked behind this milestone going DONE. Roadmap: M5 ✅ → CLI Audit ✅ → J-080 ✅ → M6 Phase 0 Pass 3 ✅ → Propagation Reliability Audit ✅ → Federation design (Pass 2 + Pass 3) ✅ → **Federation implementation Phases 1-5 ✅, Phase 6 (F-10 HeldPending generalisation) next** → M6 (new) → M7 → M8 → M9.
...
- **Federation Event Propagation implementation**: 🟢 PLAY. Nine-phase runbook; **Phases 1-5 ✅ SHIPPED** (J-082 + J-083 + J-084 + J-085 + J-086). **Phase 5 ✅ (J-086, 2026-05-19)** — F-1c per-peer operational record inside `FederationRegistry` per §3.5.1 Lock A A3; reconnect scheduler with 60 s tick + 15/30/60/120-min backoff ladder + detached parallel spawn per §3.5.1 Lock B; in-memory-only backoff cursor per the implementation-session lock (α); `run_initiating` gains its first production caller in `xgen-node/src/`; `run_federation_session_post_handshake` extraction shared between receiver-side `handle_federation_incoming` and new initiator-side `attempt_reconnect`; `process_inbound`/`stream_federation_delta`/`handle_identity_msg`/`handle_identity_replicate_msg` made generic over `S: AsyncRead+AsyncWrite+Unpin` to bridge `TcpStream` (server-accept) and `MaybeTlsStream<TcpStream>` (outbound connect). 505 tests at Phase 5 close. Phase 6 (F-10 HeldPending generalisation for unknown signer Identity) is next-active.
