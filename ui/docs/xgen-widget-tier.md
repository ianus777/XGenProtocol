...
- **W-12 (amended v1.3) — a widget has AT MOST ONE SURFACE.** A widget MAY declare exactly one of: **`region`** (a leaf in the D-103 layout descriptor) · **`shelf`** (a compact **face**) · **`window`** (⚠️ **a distinct surface — mechanism unspecified, D-122**: in-DOM **modal area** by default, **separate window** deferred and needing a named reason; **decided in situ, never inherited**). It may declare **none** (**headless** — it computes into a store and is never seen; *you reach its output, not it*). **It may never declare two** — so **a shelf face implies no tile**. W-11 is the *data* seam; W-12 is its *layout* sibling. See `xgen-widget-surfaces-phase0.md` §3.3 and `xgen-region-dock-model.md` (D-103).
...
> **⚠️ MECHANISM WITHDRAWN (2026-07-11, J-502) — the row above is HISTORY, not a plan.** Grounding the protocol proved `temperature-indicator` is **NOT a computation and NOT a dd-socket consumer**. Temperature is an **existing protocol property** — spec **§3.7.13** (Status: complete) + Ch6 **§6.12** + **D-061**: the home Node's **plugin** computes it, the protocol carries it in `meta_atts` (`xgen.room_temperature` / `xgen.member_temperature`), and Ch6 §6.12.1 states the values are **opaque** to the client — *“the client … does not attempt to re-derive them.”* Shipped `xgen-client/src/temperature.rs` already emits a `temperature_update` event and defines the DOM contract (`data-temp-state` + two CSS custom properties). **So the real shape is: a `$common` store + a skin contract, rendered as CONTENT inside other widgets — no `meter`, no W-11 dd-socket, no surface.** The row above was written **before** the widget tier, the dd-socket, the region model and Ch6 §6.12 were consulted — *the first bird, named before any of the conventions it was described in existed* (Joe). **Do NOT inherit `meter`/dd-socket at kickoff; re-derive against the shipped contract.** Gate: live messaging **and** a non-no-op node plugin (`NoOpTemperaturePlugin` returns `None` today, so there is nothing to render). See `docs/xgen-widget-surfaces-phase0.md` §6.2.
