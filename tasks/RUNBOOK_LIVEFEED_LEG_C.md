...
🛑 **v1.4 — THIS RULING'S ORIGINAL JUSTIFICATION WAS FALSE AND IS CORRECTED HERE, NOT DELETED (`D-131`). THE RULING ITSELF STANDS.** v1.0–1.3 said an un-guarded first `READY` costs *"a redundant fill plus **a node round trip**"*. **`get_spaces` is `fn`, NOT `async fn` — a SYNCHRONOUS on-disk read that its own doc comment says never touches `session`** (`desktop.rs:613`; `:612` is the `#[tauri::command]` attribute — **v1.4–1.5 cited `:612`, corrected v1.6 by Clair**). **There is no round trip.**
🛑 **v1.6 — AND THE GROUNDING WAS PARTIAL WHEN IT WAS PRESENTED AS COMPLETE.** `get_spaces`'s doc comment says, in those words, **"no node round-trip, no mutation, no CLI meaning"** — at **`desktop.rs:603`**. Chat's read window for that block started at **`:606`**, three lines late, caught the tail, and **missed the sentence that answers the question outright** — then reported it as *a producer check*. ⇒ ***A PARTIAL READ OF A PRODUCER IS NOT A PRODUCER CHECK, AND A RIGHT CONCLUSION DOES NOT MAKE THE GROUNDING SOUND.*** *Scope your measurement before you believe it.* **Found by Clair while verifying an instruction she had been told was already grounded.** The cost was inferred from the word `invoke` at the call site instead of read at the producer — *verify a claim at its PRODUCER, not at its field name*, and the same species as §4.1's `invoke` and §2's price inversion. ⇒ **TRUE COST of an un-guarded first READY: one local file read, one `setSpaces` reassign, and a cascaded members effect that — with nothing latched at cold start — is a `reset()` on an already-empty book. ① USER-VISIBLE IMPACT: NONE.**
...
🛑 **DEFECT FOUND BY THIS PASS — ✅ DISCHARGED `87307e8` (Clair, comment-only, +2/−1, `svelte-check` 0/34/15 unmoved).** The committed C-b comment carried *"a redundant fill + a node round trip"*, the **false claim corrected in `R-c` at v1.4**. It was true in the runbook when Clair transcribed it, so this was not a Rule 6 miss on her part — **it was Chat's error propagated into code by a faithful implementer, which is exactly how a bad runbook line does its damage.** 🔑 **AND SHE RE-VERIFIED THE GROUNDING ANYWAY, ON A TASK SHE HAD BEEN TOLD WAS ALREADY GROUNDED — WHICH IS HOW `:603` AND `:613` WERE FOUND.** *A kickoff that says "it's only a comment" is the one most likely to be read less carefully; hers was not.*
