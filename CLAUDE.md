...
## 🟢 PLAY — M6 (new): **federation-admin-control 2a — Commit 2 (require_approval flag + pending-request queue store) SHIPPED at J-175**; **next-active = Clair Commit 3 (LOAD-BEARING pause-point in `run_receiving`) per runbook §5, after Joe-lock checkpoint #2** (reject code 2003 + the exact pause condition = approval-state-only, NOT policy); then verbs → close; then 2b (policy) → auth-module-registry → bootstrap-client; then M7 `--aicontrol`
