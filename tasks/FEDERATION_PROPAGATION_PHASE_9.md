...
Also confirm `cargo test --workspace` ships green at the J-108 baseline (599 tests per the milestone-close count; CLAUDE.md J-108 entry quotes the cargo output). If the local working tree shows uncommitted versions of these files or any drift from the `a677244` shipped versions, STOP per CLAUDE.md Rule 3 and report to Joe before starting 3b-2-equivalent.
