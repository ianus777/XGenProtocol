...
│   │   ├── 🟢 **Leg C the skin — SPLIT INTO C-1 / C-2 / C-3 (Joe, J-654, option S3)** · 🔒 **LOCKED SEPARATE FROM LEG B (Joe, J-650)** · runbook `tasks/RUNBOOK_IDENTITY_RESOLUTION_LEG_C.md` **v1.0 AUTHORED, NOT LOCKED — Clair NOT stood up**
...
│   │   │   ↳ 🟡 **C-1 `ui/sampler/src/app_sampler.svelte`** — one `entity-panel#unresolved` cell: resolved **control** + `unasked` + `erased`, pre-selecting the erased row so §5a-i's highlight is CHECKABLE. **The only surface on which either state can render at all** (both need two clients in the product). Catalogue **427 → 434 PREDICTED, measured as a TRANSITION — a mismatch is a FINDING** ↳ trigger: **Joe locks the runbook AND stands Clair up — TWO acts, NEITHER performed**
│   │   │   ↳ 🟡 **C-2 `ui/assets/skin.css`** — **③'s mark ONLY**: strikethrough on `.ei-name` (`--t2` text, `--t4` rule, 1px). **Moves NEITHER floor** — CSS is not type-checked and not a component ↳ trigger: **C-1 lands**
...
│   │   │   ↳ 🛑 **THE FIXTURE USES XGID TAILS, NOT NAMES** — with no book record `toDescriptor` already falls back to `tail()`, so **a real ③ row has NO display name to strike** (§5b). ***A human-named fixture would tune a case the product cannot produce.*** ⚠️ **And the strikethrough makes §6a's `tail-8` gap MORE visible** — it strikes the constant `ed25519:` head the clip preserves
