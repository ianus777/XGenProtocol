...
> Version: 1.1  
> Date: May 2026  
> **Last updated**: 2026-05-22 (v1.1 amendment landed at Step 2 of the post-J-098 design-phase re-walk: §11 added recording the framing gap surfaced at Clair's Commit 3 verification — Q3 asked "is determinism normative?" but did not ask "what semantic property must the canonical order satisfy?"; the Shape A v1 sort fix produces byte-identical wire output across senders but does not produce causally-correct wire output because `build_room_create_event` at `xgen-core/src/space/state.rs:797` constructs `state.room_create` with `prev_events: vec![]` despite its doc-comment claiming `space_id` is the parent's event_id. Path B Joe-locked at J-098 session close: fix the event-DAG at construction (set `prev_events: vec![space_id.to_string()]`); narrow scope (`build_room_create_event` only; sibling event constructors not audited this milestone); Commit 2's sort fix stays useful at the determinism layer beneath the causality layer. Rejected alternatives at re-walk: Path A (EventType-priority sort at topo primitive — wrong layer); Path C (broader sibling-constructor audit — own future phase per D-071 if dependent work surfaces need). Step 2 of the Shape 2 procedure ships this amendment alongside design doc §11 + DECISIONS.md D-076 in-place amendment + JOURNAL J-099 + CLAUDE.md PLAY block flip + ROADMAP.md v1.14 → v1.15 + this HANDOFF's Status flip + new Step 3 HANDOFF, eight files atomic per D-074. Previous v1.0 content stands authoritative as the canonical record of the audit at lock-time; §11 extends without invalidating. Per D-065 + D-069 + D-071 + D-074 + D-076 discipline.) Previous 2026-05-22 update: Status flipped ACTIVE → COMPLETED v1.0 at the design-phase close commit per the bidirectional precedent. The audit doc's role as "input to design-phase deliberation" ends as the design task file `tasks/FEDERATION_TOPOSORT_DESIGN.md` (ACTIVE v1.0) lands the canonical record of the locked design. The three inline locks recorded below (Q1 Shape A v1 + sibling Site 1 fix; Q2 middle + Q2.γ; Q3.ii) are preserved as authoritative historical record of the design-at-lock-time; the design task file carries exposition + rejected-alternative reasoning + discipline-notes + cross-references. D-076 promoted to DECISIONS.md at the same commit as this Status flip, sibling-distinct from D-067 + D-075, pairs with D-070 as the four-decision no-drift-surface discipline family. Implementation runbook `tasks/FEDERATION_TOPOSORT_IMPL.md` is the next-active artefact for Chat Claude + Joe in a fresh session per the split-session discipline (bidirectional precedent: design task file + runbook in separate sessions; different audiences benefit from different headspace). Per D-069 + D-071 + D-074 + D-076 discipline. Previous 2026-05-22 update content below stands authoritative as the audit-phase locks.) Previous 2026-05-22 update: All three audit-phase questions locked at design-phase opening session: Q1 at Shape A v1 + sibling Site 1 fix; Q2 at Q2 middle + Q2.γ (forward-binding to Node-to-Client siblings); Q3 at Q3.ii (canonical wire ordering required). New D-NNN drafted for DECISIONS.md promotion at design-phase close, sibling-distinct from D-067 + D-075, pairs with D-070 as no-drift-surface discipline family. Sections updated: §1.1 + §6 intro + §6.1 + §6.2 + §6.3 + §5.1 + §5.2 + §5.3 + §7.5 + §8. Implementation runbook (`tasks/FEDERATION_TOPOSORT_IMPL.md`, to be authored) sits behind the design task file (`tasks/FEDERATION_TOPOSORT_DESIGN.md`, to be authored). Four atomic commits expected per bidirectional precedent: doc-pass; primitive + sibling fix + unit tests; Phase 9 Scenario 1 lift; milestone close per D-074. Sibling shape to bidirectional audit §6.2 (code-verified-yes pattern — lock at audit-doc lifecycle, not carry into design-phase deliberation).  
> Language: English  
> Author: JozefN  
...
*End of audit document v1.0. See §11 below for the v1.1 amendment landed at Step 2 of the post-J-098 design-phase re-walk. The original v1.0 content above stands authoritative as the canonical record of the audit at lock-time; §11 extends it without invalidating it.*  

---

## 11. Amendment v1.1 (2026-05-22) — Framing gap surfaced at implementation: determinism vs. causal correctness

### 11.1 Why this amendment exists

The original audit (§1 through §10) framed the wire-order non-determinism finding around three structural questions (Q1 tie-break choice; Q2 D-067 audit scope; Q3 wire-format normative question). Q3 walked the question "is wire-order determinism normative?" and locked Q3.ii (canonical wire ordering required). The design phase that followed (`tasks/FEDERATION_TOPOSORT_DESIGN.md`, COMPLETED v1.0 at Commit 1 of `tasks/FEDERATION_TOPOSORT_IMPL.md`) instantiated Q3.ii via Shape A v1 + sibling Site 1 fix (event_id lexicographic sort at the topo primitive + HashMap-feed sort at `compute_federation_delta_for_space:321`).

At Clair's Commit 3 verification (post-J-098 session), the Shape A v1 sort fix did not close Phase 9 Scenario 1. The sort produces byte-identical wire output across senders with identical Space history — which is what D-076 v1's stated contract demands. But byte-identical-across-senders is not by itself sufficient: the chosen canonical order must additionally be **causal-DAG-respecting**, and lexicographic event_id ordering of DAG roots with empty `prev_events` is not.

Concretely: `build_room_create_event` at `xgen-core/src/space/state.rs:797` constructs the event with `prev_events: vec![]` despite the function's own doc-comment claiming `space_id` is "the event_id of the parent state.space_create." The event-DAG layer does not record the protocol-level parent-child relationship; both `state.space_create` and `state.room_create` end up as DAG roots tied at the top of the topological sort. The Shape A v1 sort canonicalises their tie-break order across senders — `state.room_create`'s event_id sorts lexicographically before or after `state.space_create`'s based on the nonce roll — but ~50% of nonce rolls produce a canonical wire order where `state.room_create` precedes `state.space_create`. The receiver's `dispatch_event` Step 1 still rejects `state.room_create` with "space not found"; the bootstrap cascade still produces 2 Accepted / 2 Rejected / 101 HeldPending. Determinism is achieved; correctness is not.

**The framing gap.** Q3 asked "is determinism normative?" but did not ask "what semantic property must the canonical order satisfy?" The unstated assumption — that any deterministic canonical order would also be a causally-correct order — was false for events that the event-DAG layer treats as roots despite having a protocol-level parent-child relationship at a higher layer.

### 11.2 The fix shape (Path B, Joe-locked at J-098 session close)
...
**Commit 2's Shape A v1 sort fix stays useful.** The two fixes operate at different layers:
- Commit 2 sort fix (D-076 v1 instantiation): canonical wire order across senders for events that legitimately tie (e.g., two true DAG roots with no protocol-level parent-child relationship).
- Path B (D-076 v1.1 instantiation): events with a protocol-level parent-child relationship are no longer DAG roots; the topological sort honours causality before tie-break is even consulted.

The two fixes layer cleanly: causality first (the DAG layer), determinism second (the tie-break layer). Neither is sufficient without the other; both are now committed surfaces under the amended D-076 principle.

### 11.3 Narrow-scope honesty note

**Path B scope is narrow by Joe-lock — `build_room_create_event` only.** Sibling event constructors (`state.federation_add`, `membership.*`, `message.*`) are NOT audited in this milestone for similar `prev_events` lies. The audit of those constructors may surface later as their own audit-precedes-dependent-design arc per D-071 if dependent work surfaces a need; for now, the scope is bounded to the single constructor surfaced by Phase 9 Scenario 1's failure.

This is a deliberate scoping choice, not a deferred-audit gap: a fuller audit of every event constructor's `prev_events` shape against its doc-comment claims would be its own substantial subsystem audit phase. The lock-narrow choice keeps the topological-sort milestone closing at honest cost without paving over the broader question.

### 11.4 Rejected alternatives at this re-walk

**Path A — EventType-priority sort at the topo primitive.** Considered: add a hardcoded EventType priority table (`state.space_create` before `state.room_create` before others) inside `topological_sort_events` so the tie-break enforces protocol semantics. Rejected: encodes protocol-level relationships at the wrong layer (the topo sort is a generic causality-respecting primitive; embedding EventType semantics inside it conflates the DAG-correctness layer with the protocol-correctness layer); breaks the principle that causality should be expressed in `prev_events` not in sort-order policy; would need extension every time a new event family with semantic ordering constraints lands.

**Path C — broader re-walk including audit of sibling event constructors.** Considered: open a full audit of every event constructor's `prev_events` shape against its doc-comment claims and protocol-level relationships. Rejected at scope: the topological-sort milestone surfaced one specific constructor; closing that one is the honest next step. A broader audit is its own future phase per D-071 if dependent work surfaces the need. Locking scope narrow here is sibling-shape to how the bidirectional `federation_nodes` milestone closed without auditing every applier for vantage-awareness.

### 11.5 Where this amendment lives in the canonical record

- **This audit doc §11 (here)** — records the framing gap that surfaced at Clair's Commit 3 verification + the Path B locked fix shape + the narrow-scope honesty note + the rejected alternatives at the re-walk.
- **`tasks/FEDERATION_TOPOSORT_DESIGN.md` §11** — records the Q4 framing (causal ordering as load-bearing property) + Q1 supplement (Path B at event-construction layer) + the relationship between Commit 2's already-shipped sort fix and the Path B fix.
- **DECISIONS.md D-076** — amended in place with a new subsection "Amendment (2026-05-22) — causal-DAG-respecting order as second load-bearing property" between the original "Decision" and "Originating incident" subsections. The v1 "Decision" prose stays authoritative as the original lock record.
- **JOURNAL.md J-099** — honest retrospective entry framing the design-phase Q3 framing gap as the origin story of the amended D-076 principle. Sibling-shape to how D-070 / D-075 / D-076 v1 originated from earlier surface moments.

### 11.6 Status at amendment

Audit doc Status stays COMPLETED (the audit's role as input to design-phase deliberation ended at Commit 1 of the implementation runbook; this amendment extends the historical record without re-opening the audit's lifecycle). Version 1.0 → 1.1. Last updated 2026-05-22 with the amendment summary. Original v1.0 §1 through §10 content stands authoritative as the canonical record of the audit at lock-time.

The runbook (`tasks/FEDERATION_TOPOSORT_IMPL.md`) stays at v1.0 until Step 3 of the post-J-098 design-phase re-walk procedure. Clair stays stood down. Phase 9 Scenario 1 stays `#[ignore]`-annotated. The Federation Event Propagation milestone chain (Phase 9 → milestone → M6 (new) + Pass 1) stays paused; this amendment delays milestone closure by one session-arc (Step 3 + Clair's Commit 2-amendment + Commit 3 + Commit 4) — fifth recurrence of "honest longer work over fast shortcuts" within Federation Event Propagation milestone scope.

---

*End of audit document v1.1. Step 3 of the design-phase re-walk (runbook revision authoring) is next-active in a future session per `tasks/HANDOFF_TOPOSORT_RUNBOOK_REVISION.md` (authored as part of Step 2's atomic commit).*  
