...
#### `federation set-policy` — WRITE — **SHIPPED ✅ (federation-admin-control 2b, J-182)**
- **As-built** (`FederationSetPolicyArgs`): `peer_node_id: String`, `mode: String` (`allow | deny`; parsed → `PolicyMode`), `allowed_space: Vec<String>` (repeatable `--allowed-space`; empty → `None` = all shared Spaces; restrictive-only). `rate_limit` **DEFERRED** (FAC-D4).
- **Result** `FederationSetPolicyResult { peer_node_id, mode: String, allowed_spaces: Option<Vec<String>>, set_at: String }`.
...
- **Result** `FederationShowPolicyResult { peer_node_id, mode: String, allowed_spaces: Option<Vec<String>>, is_default: bool }` — `is_default = true` when no policy is stored (the default permit-all shown explicitly, prime invariant).
