...
## 🟢 AFI audit-against-code arc ✅ CLOSED (J-398) — AF ✅ (J-397, Appendix F v1.13) + AI ✅ (J-398, Appendix I v1.6→v1.7). AI sub-pass authored three new fundamentals appendices — M (Trust Assertions), N (Auth-Module/Plugin descriptors), O (`--aicontrol` control plane) — and folded `event_trace` into Appendix G (v1.2). AI-F01–F16 doc-side; F17 (`identity.record` omits `is_ai`/`ai_capabilities`) Joe-routed as a suspected code gap. **Next-active = F17 code fix (Clair seat, Joe-LOCKED J-400) — `tasks/ROUTE_F17_identity_record_ai_fields.md`**; then mockup stock-take → UI clean-table → Streams. (doc-opt phase COMPLETE J-396)

**Doc-opt phase OPENED (J-391, doc-only, no code).** The pre-UI consolidation phase — discharges the documentation bloat the M8→M12 sprint left while the reference/operational docs were maintained reactively. **Chain reconciled:** the J-390 bridge read "next-active = UI" — one step ahead; the true pre-UI chain is **doc-opt → Appendix F/I audit-against-code → mockup stock-take + reconcile-to-as-built → UI (clean-table build) → Streams**. This open flips PLAY + ROADMAP to that chain (atomic, D-074). **DO-1 (the CLAUDE.md split) + D-094 (the archive convention) land in this same commit.** **DO-1:** CLAUDE.md had grown to ~898 KB / 2,769 lines and could no longer be read in one call — a Rule-0 tax every session open. Three zones: the live PLAY head, a ~2,200-line stack of superseded PLAY blocks (J-389 M12-close back through M6 / XGID-retrofit / Phase-9), and the stable briefing tail (MANDATORY Behaviour rules → Build Commands). The superseded stack is lifted **verbatim** → NEW `CLAUDE_HISTORY.md` (ARCHIVED); CLAUDE.md keeps this live head + the stable briefing + a forward pointer. A move, never a rewrite. **D-094 (NEW):** archive-with-forward-pointers for all four canonical records — relocate superseded content to a frozen ARCHIVED sibling, never rewrite history; preserves append-only + no-retroactive-rewrite + D-074 atomicity. **Phase plan:** DO-1 CLAUDE split (this commit) · DO-2 ✅ tasks/ archive (207 + DO-2b 43 → `tasks/archive/`, J-392/J-393; live tasks/ = 3 PENDING) · DO-3 ✅ ROADMAP prose refresh (J-394) · DO-4 ✅ DECISIONS R2G-F03 — 7 dup-ID pairs a/b-suffixed + refs repointed (J-395) · DO-5 JOURNAL.md window (range-archives + recent live window; riskiest, last/deferrable). **Canonical (D-074):** this head + the split; `CLAUDE_HISTORY.md` NEW (ARCHIVED); `DECISIONS.md` D-094 NEW; `docs/ROADMAP.md` v3.79→v3.80 (chain flip — doc-opt inserted ahead of UI at the tree node + the chain arrow); JOURNAL J-391. **DO-2 ✅ (J-392): 207 terminal docs archived. DO-2b ✅ (J-393): 43 of 44 stale-ACTIVE flipped + archived (42 COMPLETED + BATCH_FLAG DEPRECATED); NODE_ADMIN_PASS2 kept live (PENDING — feeds the pending M6 node-admin write-path). Live `tasks/` = 3 (all PENDING). DO-3 ✅ (J-394): ROADMAP prose refreshed — completed M6→M12 + multiparty + Round-2 relocated to Past one-liners; Present = doc-opt; Near/Far cleaned; tree untouched (914→752 lines). Next-active: DO-4.** **DO-4 ✅ (J-395): the 7 duplicate decision IDs split a/b (suffix-not-renumber) + ~30 live refs repointed. Next-active: DO-5 (JOURNAL windowing — last/deferrable).** **Entry (Rule 0): this PLAY → JOURNAL J-393 (→ J-392 → J-391) → `docs/ROADMAP.md` (doc-opt chain) → `DECISIONS.md` D-094 → `CLAUDE_HISTORY.md` (the archived PLAY stack).** Not pushed — Joe pushes. **DO-5 ✅ (J-396): JOURNAL.md windowed.** Live window J-395 … J-376 (20 entries + new J-396); J-375 and older (358) lifted verbatim → NEW `JOURNAL_ARCHIVE.md` (ARCHIVED, D-094) with a forward pointer at the cut; D-094 got a one-line note (no new D-ID). **Documentation-optimization phase COMPLETE.** Next frontier (Rule 0 for the next session): open the Appendix F/I audit-against-code → mockup stock-take + reconcile-to-as-built → UI (clean-table) → Streams. **AFI arc OPENED; AF sub-pass ✅ (J-397):** Appendix F (F.0.4/F.2/F.2.1/F.3) reconciled to as-built CLI — `create-dm-space`/`leave`/node `whoami` added, `members` de-staled, `federate` reframed `N/A — only node concept` (+ ch2 node-to-node note), §F.2.1 node-admin cross-ref to `xgen_node_admin_ops_design.md`. **Next-active: AI sub-pass** (Appendix I ↔ data structures + event catalog). Entry (Rule 0): this PLAY → JOURNAL J-397 → `tasks/HANDOFF_AFI_AUDIT.md`. Not pushed — Joe pushes. **AI sub-pass ✅ (J-398) — AFI arc CLOSED.** Appendix I reconciled to the as-built serializable types + event catalog (v1.6→v1.7): AI-F01–F16 doc-side — thread model (events §I.2 + ThreadState/ThreadStatus §VI.9), SpaceState +jurisdiction/e2e_encryption/threads, RoomState +permission_overrides/mls_commit_tip, PendingInvite §VI.7 (+valid_until), RoomPermission/Effect §VI.8, 8 transport variants + sync_request.limit + auth_ok.node_id, identity.register +re_registration, IdentityRecord +revoked/revoked_at/revocation_reason. Three fundamentals promoted to single-source-of-truth appendices: **M** Trust Assertions, **N** Auth-Module/Plugin framework descriptors, **O** `--aicontrol` control-plane structures; `event_trace` typed enums folded into **Appendix G** v1.2. **F17 Joe-routed** (suspected code gap: the wire `identity.record` omits `is_ai`/`ai_capabilities` that §IV.1 documents + §3.6.10 transparency expects — §IV.1 left intact pending a code call). Reverse cross-refs skipped (B). **Next-active: mockup stock-take + reconcile-to-as-built.** Entry (Rule 0): this PLAY → JOURNAL J-398 → `tasks/HANDOFF_AFI_AUDIT.md` (consolidated AF+AI ledger, COMPLETED). Not pushed — Joe pushes. **Post-AFI close-up (J-399):** F17 routed to Clair — `tasks/ROUTE_F17_identity_record_ai_fields.md` (PENDING; additive code fix recommended — add `is_ai`/`ai_capabilities` to wire `IdentityMessage::Record`; awaits Joe-lock on direction). DO-5 handoff flipped COMPLETED + archived to `tasks/archive/`. **Next-active unchanged: mockup stock-take + reconcile-to-as-built.** **F17 direction LOCKED (J-400) — code fix, Clair seat.** Joe locked the additive code fix: add `is_ai`/`ai_capabilities` to the wire `IdentityMessage::Record`, populated on `identity.get` (serde-skip when false/none); Appendix I §IV.1 already matches the target, so no doc edit. Spec + acceptance criteria: `tasks/ROUTE_F17_identity_record_ai_fields.md` (ACTIVE). **Next-active (Clair): F17 code fix; then Chat resumes mockup stock-take.** New session opens here — Rule 0: this PLAY → JOURNAL J-400 → `tasks/ROUTE_F17_identity_record_ai_fields.md`. Not pushed — Joe pushes.

> (Prior PLAY blocks J-390 ← J-389 ← … ← M6 / XGID-retrofit — archived to `CLAUDE_HISTORY.md` per D-094, J-391.)  
