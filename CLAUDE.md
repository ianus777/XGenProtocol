...
> Date: May 2026  
> **Status:** ACTIVE  
> **Last updated:** 2026-05-20 (J-095 — XGID Adoption v1 implementation milestone CLOSED. Clair's two-commit plan shipped per `tasks/XGID_ADOPTION_IMPL.md` (Status: ACTIVE → COMPLETED v1.1): Commit 1 (`c95584a`) ships `xgen-common` XGID type vocabulary — base `Xgid(String)` newtype + six flavour wrappers (`EventXgid`, `SpaceXgid`, `RoomXgid`, `TrustAssertionXgid`, `NodeXgid`, `IdentityXgid`) each serde-transparent + `Deref<Target = Xgid>` + flavour-specific constructors (hash-anchored `from_canonical_bytes` and principal `from_pubkey`/parse-fallible `pubkey()`) + `XgidLike` trait + `XgidDecodeError`, with all five required wire-format invariance tests pinned by name; new deps `ed25519-dalek = "2"` + `sha2 = "0.10"` + `base64 = "0.21"` + `thiserror = "1"` added to `xgen-common/Cargo.toml`. Commit 2 (`24a255b`) retypes `SpaceLocalMetadata.introducer_node_id` from `Option<String>` to `Option<NodeXgid>` as the only production-code retype in v1; strengthened `serde_roundtrip_with_introducer` test acts as the per-call-site wire-format invariance witness (Appendix J §J.5 invariance 2) including forward-compat from pre-XGID JSON shape. Two same-session sibling atoms shipped alongside: hygiene commit (`904441b`, `chore(workspace): clippy cleanups for new toolchain`) flipping workspace clippy gate red → green under Rust 1.95.0 — 26 files across all four crates, +191/-89 LOC, one behaviour-adjacent fix (`filter_map` → `map_while` on `Lines` in two batch-file readers closing a potential spin-forever loop), rest mechanical or `#[allow]` with per-case rationale (NOT XGID code, honest provenance — `git log -- xgen-common` will show hygiene and XGID as distinct commits); milestone-close commit (this commit) carrying JOURNAL J-095 + Ch4 v1 follow-on pointer (A5 Joe-lock from Phase 2 sweep) + CLAUDE.md PLAY block flip + ROADMAP.md state move + `tasks/XGID_ADOPTION_IMPL.md` flip to COMPLETED. Test count 556 → 571 across the milestone (+15: 10 in-module flavour tests + 5 wire-format invariance tests in `xgen-common/tests/xgid_invariance.rs`); `cargo clippy --workspace --all-targets -- -D warnings` clean after each commit. **Carry-over to Retrofit Pass 1:** hash-anchored convenience constructors (`EventXgid::from_event`, `SpaceXgid::from_space_create`, `RoomXgid::from_room_create`, `TrustAssertionXgid::from_assertion`) deferred from Commit 1 because `canonical_event_bytes` lives in `xgen-core` — Pass 1 picks up the canonical-form module move + the convenience constructors as coordinated work. XGID Adoption v1 milestone now ✅ DONE; XGID Retrofit Pass 1 unblocks in ROADMAP.md Near future. Two parallel tracks now active: Phase 9 (Federation Event Propagation deployment integration tests) resumes for Clair from Commit 3 boundary with integration test code using XGID types from start; Chat Claude's next active work is authoring `tasks/XGID_RETROFIT_PASS_1_IMPL.md` using the classification table at `tasks/XGID_DOC_SWEEP.md` as canonical input.)  
> Author: JozefN  
> Credits: Concept, philosophy, requirements, project direction: Jozef Nižnanský. Technical assistance and implementation support: AI-assisted development tools.  

---

## 🟢 PLAY — XGID Adoption v1 ✅ CLOSED; two parallel tracks now active (Phase 9 resumed for Clair; Retrofit Pass 1 runbook authoring for Chat Claude)
...
**Two parallel tracks open simultaneously after milestone closure:**

**Track 1 (Clair): Phase 9 — Federation Event Propagation deployment integration tests RESUMED from Commit 3 boundary.** [tasks/FEDERATION_PROPAGATION_PHASE_9.md](tasks/FEDERATION_PROPAGATION_PHASE_9.md) remains Status: ACTIVE v1.0; scope intact. Dependency chain now reads "Phase 7.5 ✅ → XGID Adoption design ✅ → XGID Adoption canonical sources ✅ → XGID Adoption Phase 2 doc-tree sweep ✅ → **XGID Adoption implementation ✅** → Phase 9 (NOW ACTIVE)." Integration test code uses XGID types from start — `NodeXgid` for federation peer IDs at construction points, `EventXgid` for event hash anchors, etc. — leveraging the v1 type vocabulary the same way any post-milestone consumer would. After Phase 9 ships, Federation Event Propagation milestone flips PLAY → DONE and M6 (new) Node admin write path unblocks for implementation per its Pass 3 design.

**Track 2 (Chat Claude): author `tasks/XGID_RETROFIT_PASS_1_IMPL.md`.** Pass 1 (`xgen-common` core types) is the first of five staged retrofit passes per the Q3 Shape γ + ASAP discipline (D-072). Canonical input for the runbook author is the classification table at `tasks/XGID_DOC_SWEEP.md` (Status: COMPLETED v1.2) — the table identifies which docs land in Pass 1 (Appendix C + Appendix I) and pins the **coordination flag** that is the load-bearing instruction: Pass 1 code retype + Appendix C + Appendix I MUST land in one coordinated commit set to prevent spec drift at Pass 1 close. The runbook should also incorporate the deferred-convenience-constructors carry-over from XGID Adoption v1 milestone close — move `canonical_event_bytes` from `xgen-core/src/wire/canonical.rs` to `xgen-common/src/canonical.rs` (with `xgen-core` re-exporting to preserve call sites) as part of the Pass 1 commit set, then add the hash-anchored `from_event` / `from_space_create` / `from_room_create` / `from_assertion` convenience constructors on the v1 flavour wrappers as part of the same set (see J-095 "Carry-overs" for the full rationale).

**XGID Retrofit Passes 2–5 stay in ROADMAP.md Near future, picked up in order after each prior pass ships.** Pass 2 has zero doc work (xgen-core consumes types from xgen-common; doc surface covered by Pass 1's Appx C + Appx I retype) — its runbook should explicitly state "doc work is empty, focus is code-only" so the future author doesn't search for missing rows. Pass 4 has zero new doc-table rows but substantial per-section work in two already-pointer-tagged docs (Appendix F ~890 lines + `xgen_aicontrol_implementation.md` ~372 lines) — its runbook should anticipate it is the heaviest doc-work pass.

**M6 (new) Node admin write path** remains 🟡 PENDING behind Federation Event Propagation milestone closure (which itself waits for Phase 9 to ship). M6 Phase 0 design closed in J-085's neighbourhood; ACTIVE flip waits behind Phase 9.

**Entry point for next Chat Claude session: `tasks/XGID_RETROFIT_PASS_1_IMPL.md`** — authoring this runbook is the active next work. Read the classification table at `tasks/XGID_DOC_SWEEP.md` first (canonical input), then `tasks/XGID_ADOPTION_IMPL.md` (the v1 implementation runbook — Pass 1's runbook follows the same shape but for the broader xgen-common code retype), then J-095 in `JOURNAL.md` (the milestone-close entry — its "Carry-overs" section names the convenience-constructor coordination that Pass 1 picks up). After authoring + Joe-lock, Pass 1 implementation is Clair's pickup; no parallel Chat Claude work in scope during implementation.
