...
│   │   ├── 🟡 **Leg A the enforcement mechanism** — ③ grep gate + ④ written rule in `CLAUDE.md`; **moves no floor** 🔑 **FIRST, so every later leg lands UNDER the rule instead of beside it** ↳ trigger: Leg 0's hand-read lands — ✅ **FIRED J-660**
│   │   │   ↳ 🔒 **④ IS NOT "WRITE A NEW RULE" — IT IS *PROMOTE `D-NNN-format-boundary` TO A REAL `D`*, whose own trigger fired at `M6`.** Strictly better than a fresh convention: the rule is **already worked, already worded, already has its instance table**; what it lacks is a home outside `tasks/archive/`
│   │   │   ↳ 🛑 **DoD: the gate must RUN AND FAIL on a planted `String` identifier slot** — *a gate that has never failed is not known to work*
│   │   ├── 🟡 **Leg B the three address-book structs** — `FetchedIdentity` · `SeenRecord` · `FillReport`, the `BTreeMap<String, _>` key, and the `ops.rs:2734`/`:2742` downgrade that **disappears rather than moves**; **moves the cargo floor** 🔒 **THE ONLY LEG ON `M-RP-IDENTITY-RESOLUTION` LEG D's CRITICAL PATH** ↳ trigger: Leg A lands
