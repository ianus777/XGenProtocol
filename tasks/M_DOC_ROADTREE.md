...
**Leg D — `CLAUDE.md`.** 🟢 **PLAY.** **Phase-0: `tasks/M_DOC_ROADTREE_LEGD_PHASE0_BRIEF.md` v1.5 ACTIVE (J-606).** **Runbook: `tasks/RUNBOOK_ROADTREE_LEGD.md` v1.0 ACTIVE (J-612)** — §A the block classification, §B the third input, §C the move procedure, §D the checks V1–V8, §E the hold set. **Surface: `CLAUDE.md`, `CLAUDE_HISTORY.md`.**
...
🛑 **THE MOVE SET IS 50 BLOCKS, NOT 49, AND THE COMPOSITION CHANGED (J-612).** Both earlier lists of self-declared-closed blocks — seven in the session kickoff, six in Phase-0 §6a — were counts carried forward without re-reading the blocks. Reading all 22 gives **4 self-declared closed** (L177 L193 L251 L255) · **4 stale-closed** (L175 L241 L245 L249) · **3 superseded** (L195 L199 L201, disposition 🔓 W1/W2/W3) · **3 live** · **1 live-partially-stale** · **5 stateless and homed** · **2 held for having no record to point at** (L75 → M-RP-SKIN, L235 → M-RP-SELF-SURFACE; neither has a task doc or a ROADMAP node). `tasks/RUNBOOK_ROADTREE_LEGD.md` §A is the authority.
