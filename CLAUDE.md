...
## 🟢 UI component-library / substrate (RP track) — M-RP2.19 ✅ CLOSED (J-430): `select-multiple` — the **fourteenth** `core` component, the **last input-family atomic di**; the **first array value-type** (`bind:value` → `string[]`, the 5th binding shape), empty model `[]` not `null`, getter `{values,count}`; own atomic under the sharpened D-096; **next — `led` + `link`** (di catalogue, locked N-049)
