...
│   │   ↳ trigger: **C-bis-2 SHIPPED at `96a935f` and re-driven at J-706 — R7's click now opens a Discord-style draft**: the dmDraft store carries a stored active plus a text map keyed by counterpart, the click leaves the room latch alone so R7 keeps its group roster and its fill, R5 mounts the fed dm-intro page in the above socket and feeds MessageStream an empty list by props rather than unmounting it, and any room selection closes the draft through a third latch in the shell effect. Two Chat rulings were withdrawn after Clair drove them and reported the contradiction: clearing the latch emptied R7 because members-panel:57 reads roomLatch.effectiveSpaceId as its scope, and R-1 was duplicating a guarantee C-bis-3 already owns. Joe amended dm-intro to ship with content, so the locked sentence is placed and the page is unskinned until he skins it. Runbook is v1.4, task doc v1.6. Next is C-bis-3, the composer draft branch above the early return. The RMC context menu still needs its own design pass and has no runbook
