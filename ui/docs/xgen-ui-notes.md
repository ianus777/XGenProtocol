...
### N-077 — `status`, the self-status dd-atomic; the `entity-avatar` status corner-slot; mounted-but-empty on expiry; the `.status` naming-collision fix

**M-RP5.1a + 5.1b.** Built `status` — a dd-atomic that materializes a **self-set status** (Track A `state.status`, J-461) into a visual, plus the additive `entity-avatar` `status?` corner-slot that composes it. Self-status is personal **expression** (emoji + short line), **NOT** presence/connection state (Track A deferred presence) — it never renders here/away. Source-agnostic: consumes a view-model `{ emoji?, text?, updatedAt?, expiresAt? }` (the shell maps Track A's `StatusRecord`); `core` imports no protocol type.

**Variants = display density (B).** `badge` = emoji glyph (rides an avatar corner) · `line` = emoji + text · `full` = + a relative "updated Nm ago". Root (C): `badge` = `<span class="status" role="img">` (`aria-label = text ?? emoji`); `line`/`full` = plain `<span class="status">`. `title` (E) surfaces the text on an emoji-only badge / the relative time on line·full. Getter G `{ variant, emoji, hasText, expired }`. Emoji is a single grapheme (Track A cap), taken grapheme-safe.

**Mounted-but-empty on expiry (Joe-locked; corrects the runbook's "not rendered").** The root `<span use:envelope>` **always mounts** — so getter G always registers and `expired:true` stays CDP-readable — but renders **no emoji/text content** when expired (lazy `expiresAt < now`) or content-less. A `data-empty` attribute then collapses it (`.status[data-empty] { display:none }`). "Absent" = empty content, **not** absent DOM. Rationale (Joe): matches the envelope-always-registers-while-mounted convention, keeps `expired` testable, gives a **stable registry count** (no cell blinks in/out on expiry), and is the honest "status exists but expired" state. The runbook DoD's "expired → absent (not rendered)" wording is superseded by this — mounted-but-empty, not no-DOM.

**Avatar seam (D, M-RP5.1b).** `entity-avatar` gains a `status?` prop; when passed it renders `<Status variant="badge" id={cid('status')} />` as a bottom-right corner overlay (`.entity-avatar .status` = `position:absolute` on the `position:relative`/`overflow:visible` figure). The child self-registers `<id>__status`; the status atomic owns expiry (no duplication in the avatar). Additive — the avatar getter is unchanged (presence/list/labeled/card + entity-item 0-regression).

**The `.status` naming-collision fix (D-065 — surfaced + fixed, not papered).** `status` is a generic class, and `combobox`/`tag-select` popups already render a bare `<span class="status">` for an option-status label (`.combobox-list .status` / `.tag-select-list .status`, muted `--fs-0`/`--t4`). The new component's envelope type-class is also `status`, so a bare `.status { display:inline-flex; … }` base rule **leaked** `display:inline-flex` (+ gap/align/line-height) into those closed components. Fix: the component **always** emits `data-variant` (badge/line/full) while the ad-hoc popup `.status` never does, so the base rule is scoped **`.status[data-variant]`** — matches only the component, leaves the popup `.status` untouched. No closed-component edits; a synthetic bare `.status` probe reads `display:inline` (leak gone). PROVISIONAL skin note: the isAi spark (`.entity-avatar[data-ai]::after`) also sits bottom-right, so an AI identity that *also* sets a status overlaps there — left for Joe's HMR tuning (self-status keeps the locked bottom-right corner).

**CDP-verified** (sampler DD·atomic panel, 9422, real output — Rule 2). `vite build` clean (153 modules; zero `status.svelte` warnings). Registry **138→146** (+8 = 6 standalone `status#` + `entity-avatar#with-status` + its `status#with-status__status` child); `count===unique===146` → **0 orphans**. Getters exact: `badge-emoji {variant:"badge",emoji:"🎯",hasText:false,expired:false}`; `badge-noemoji {emoji:null,hasText:true}`; **`expired {variant:"badge",emoji:"💤",hasText:true,expired:true}`** (registered while empty); `line`/`line-noemoji`/`full`; `with-status__status {variant:"badge",emoji:"🟢",hasText:true}`; `entity-avatar#with-status` getter **unchanged**. Render: `expired` `SPAN`/`data-empty="true"`/`display:none`/empty text; badge `role=img`/`aria-label="🎯"`/`display:flex`; `noemoji` `title="on vacation"` (tooltip E) with empty glyph; `full` text `"🎧 in a meeting updated 5m ago"`; `line` `"🌙 sleeping"`; corner child `position:absolute` `-3px/-3px` inside the `position:relative` avatar (`childInsideAvatar:true`). 8 `.status*` rules in cascade; collision probe: bare `.status` → `display:inline` (fixed). Screenshots `temp/status-verify.png` + `temp/status-verify-2.png` (badge 🎯; line/full; AN avatar with a 🟢 corner badge).

*UI-implementation record. Second `$core` dd-atomic (after `entity-avatar`); first avatar composite-slot. → registry v0.49 (status BUILT; avatar row +`status?`). Next: `spaces-panel` (M-RP5.2, inherits status via the avatar) → `entity-context-menu` widget (uses `status full`).*

---

### N-078 — `entity-panel`, the last dd-composite (roving-focus listbox); the avatar corner-fix (H); the `entity-item` status-forward amendment ("status once per variant")

**M-RP5.2.** Built `entity-panel` — the **last dd-composite** and the top of the dd-composite tier. It materializes a LIST of address-book entries as a keyboard-navigable, single-select group: a `section` (group chrome) wrapping a `<ul role="listbox">` of `entity-item` rows, owning the roving focus + selection + empty state. `spaces-panel` is a consumer **preset** (a title + a spaces `items` array), not a separate component — entity-generic. Closes the dd-composite tier; widget tier (M-RP5.3/5.4) next.

**Rooting (the "wrap, not beside" lock).** The outermost DOM is the `<section>` (from the Section child, self-registers `__section`); the panel's own identity + getter G ride the **`<ul class="entity-panel" use:envelope role="listbox">` INSIDE the section body** — the panel IS the listbox, Section frames it. (Unlike status-indicator/entity-item, whose root is their own `<div class="type">`.) The `<ul>` always renders so the envelope anchor is stable — holding the option rows, or (empty) a single `role="presentation"` `<li>` with a composed `paragraph` message. Composes real children (matrix multiplies deeply): `section` + `entity-item ×N` (each → `entity-avatar __avatar` + optional `status __status`).

**Focus model (C/D).** Roving tabindex — exactly one row `tabindex=0` (the active row), the rest `-1`; ArrowUp/Down move, Home/End jump, Enter/Space select+activate, click selects+activates. `selected` ($bindable, an entity id) is the persistent choice (→ each row's `aria-selected` + the row's own `[data-selected]` highlight); `activeIndex` is the transient focus position (seeded once at the selected row via a function call — a deliberate one-time capture, not a `$state`-init prop-reference warning). Rows are `<li role="option">` wrapping `entity-item variant="row"`: the li owns the option semantics + keyboard + click; entity-item is the visual. Getter G `{ count, selected, collapsed, hasEmpty }`; `collapsed` is shared with Section (pass-through F). Empty → `emptyText` message + `hasEmpty:true`.

**Avatar corner-fix (H, done first, skin-only).** The `entity-avatar` `isAi` spark `::after` moved bottom-right → **top-right** (`top:-1px`); the status corner-slot stays bottom-right. So an AI identity that also carries a status no longer overlaps — distinct corners. This resolves the PROVISIONAL note surfaced at the M-RP5.1b close.

**`entity-item` status-forward amendment ("status once per variant", D-065-honest wiring).** Phase-0 §1 said panel rows inherit the avatar's status corner-slot "for free", but `entity-item` (built M-RP5.1, before the avatar got `status?`) never forwarded `status` to its avatar child — the wiring the M-RP5.1b slot + corner-fix H were *for*. Added it (Joe-lock, same-commit amendment): a variant shows status **once** — `card` = the inline status LINE (it has room), so its avatar gets **no** corner badge; `row`/`nav`/`inline` (no room for a line) forward `status` to the avatar's **corner badge**. Implemented as `avatarStatus = variant === 'card' ? undefined : status` on the `<EntityAvatar>`. Keeps the M-RP5.1 `card-space` cell **0-regression** (verified: no corner badge, inline line intact). Phase-0's "for free" was aspirational; this forward is the mechanism (runbook Step 2 didn't enumerate it).
...
### N-085 — `Accelerator` + `KeymapRegistry`: the first `$common` value-objects of the frame arc; and the first standing vitest harness (M-RP6.1c)

**What they are.** Two pure, DOM-free objects in `$common` (`ui/common/lib/keymap/`), the third frame prerequisite — **not** visual components (no envelope, no sampler cell, no CDP, no registry delta; component registry stays **299**). `Accelerator` is the one-definition-two-projections value-object (the `Converter<T>` shape): a single binding definition projects to **display** (`toDisplay(platform)` → the menu-item hint) and **dispatch** (`matches(event, platform)` → the keydown predicate), so the two can never drift. `KeymapRegistry` is the pure binding table + `resolve(event) → commandId | null`.

**Modifier model — SHORTCUT abstraction (Joe-locked D2).** `Ctrl` ≡ `Control` is the **platform-accelerator token** (`ctrlKey` on win, `metaKey`/⌘ on mac) — there is deliberately **no** literal-ctrl token; the accelerator key is always the primary. `Cmd`/`Command`/`Meta`/`Super` → literal `meta`; `Alt`/`Option` → `alt`; `Shift` literal. **Platform is a parameter, never stored** (`'win'` default — the only shipped target; keeps the module free of any `navigator` read). Internal canonical form `{accel, meta, alt, shift, key}`. Key normalization at parse: single letters upper-cased (matched case-insensitively later), named-key aliases (`Esc`→`Escape`, `Del`→`Delete`, `Up`→`ArrowUp`, `Space`→`" "`) + `F1`–`F24` → canonical `event.key` form; unknown named keys verbatim. **Exact-modifier match**: extra modifiers held (e.g. Shift on a plain `Ctrl+Q`) suppress the match. `toDisplay('win')` = `+`-joined words (literal meta shown `"Win"`); `toDisplay('mac')` = Apple-HIG symbol order `⌃⌥⇧⌘` no separator (accel→⌘). `toString()` = the platform-neutral canonical string, the registry's dedup key. `matches` is typed against a structural `KeyEventLike` (`Pick` of the 5 read fields) → a real `KeyboardEvent` satisfies it **and** a plain object literal does → DOM-free + trivially unit-testable. Bindings are **Tier-1 trusted code**, so parse **throws** on malformed (fail-fast at author time — the opposite of the lenient Tier-2 processor grammar).

**The D3 split (a Phase-0 §4.4 refinement, flagged not silent — Rule 6).** Phase-0 put the whole keymap registry shell-side. Clair's grounding surfaced a genuine testability win: the **pure table + `resolve`** belong in `$common` (`KeymapRegistry`: `register` throws on a duplicate canonical, `resolve` walks bindings first-match-wins, + `has`/`size`/`list`; both client and node shells will keymap; unit-testable), and **only** the singleton instance + binding population + the one global `keydown` listener stay shell-side (→ 6.1d). Bindings register command **ids** (`"app.exit"`), not handlers — so 6.1d's File→Exit `menu-item` references the *same* id (single source of truth). Frame-phase0 §4.4 refined **in-place** (v1.0→v1.1), arc-local, **no new D** (the J-487/J-490 doc-wording-fix precedent).

**Verify — first standing vitest harness (D4-V1).** These are pure, so verification is **vitest**, not the CDP loop (the `stream/grouping.ts` precedent — which J-482 ran ad-hoc + uncommitted; there was **no standing JS test runner** in the repo). Stood one up here: `vitest` as a devDep on the **sampler** package (it already aliases `$common` + is the test-bed) + `vitest.config.js` + an `npm test` script. Reusable for the rest of the arc (6.1f's descriptor→layout walk is the next pure-unit consumer; `grouping.ts`/`transform.ts`/`clamp.ts` can retro-adopt). **35/35 green** (27 accelerator + 8 registry); Clair ran it in the feat, Chat independently re-ran `npm test` at verify — also **35/35**, `Test Files 2 passed (2)`, exit 0 (Rule 2). `vite build` clean (sampler 772ms, no regression — keymap not yet imported by any cell).
...
### N-087 — `status-bar`: the 30th `core`, a di-composite frame strip; the resize-grip seam; `sb-cell` internal non-registering; `--fs-s1`/`--fs-s2` (M-RP6.1e-A)

**What it is.** The **30th `core`** component and the first step of the M-RP6.1e client-frame consolidation split (J-493) — a **di-composite** display container: the fixed bottom-pane strip. Root `<div class="status-bar">`; side-stacking `sb-cell` groups + `separator`s + an always-visible SE resize-grip. Unlike the 6.1d menu family (frame chrome, no sampler cell), `status-bar` IS a catalogued `core` cell — it's a data-independent display container the sampler can host in isolation; only the grip's *real* Tauri drag graduates to the real client at 6.1e-B. Composes the real `status-indicator` (left cell), which brings its own `led`+`label` — so a `status-bar` cell yields multiple registry entries (the composite-multiply precedent, N-054).

**The resize-grip seam (the interesting bit).** The bar imports **no Tauri and no protocol** (the `link`/`entity-avatar` `core`-stays-app-agnostic rule). Window resize is a real-window effect, so it's exposed as a **seam**: the grip's `onpointerdown` calls `onResizeGrip?.(event)` and the bar does nothing else with it. In the real client (6.1e-B) the shell wires `onResizeGrip → startResizeDragging` (SE corner = width+height). The grip is **pointer-only** (`aria-hidden="true"`, not focusable) — window resize has no keyboard equivalent (an OS concern); the a11y honesty is stated, not faked with a fictional keyboard affordance. The grip glyph is a **pure-CSS SE-corner `clip-path` triangle** in `--t4` (not an `icon`): the grip is a positioned corner affordance, not an inline-with-text glyph, so a CSS triangle keeps it wholly in `skin.css` with zero `icons.ts` churn and no `Icon` import inside `status-bar` (noted for the icon-adoption backlog — a lighter, cleaner fit here).

**`sb-cell` = internal, non-registering (the N-064 pattern again).** `sb-cell` is a `<span class="sb-cell" data-side=left|right>` flex group whose *entire* contribution is CSS positioning — no state, no value. It has its own file (readability + the "hosts any display component" `children`-snippet contract) but **no `use:envelope`**, so it does **not** register a debug getter: a registry entry for a value-less layout wrapper would be pure ordinal noise. This is the **N-064 used-internally-without-registration** pattern (the `chip` `register` opt-out precedent, and the `sb-grip` shares it — a pointer-only affordance, not a state-bearing component). The consequence: the CDP delta counts the composite roots + composed `status-indicator` subtrees + the demo separator/label, but **not** `sb-cell`/`sb-grip`.

**Getter G** `{ leftCount, rightCount, hasGrip }` — `leftCount = secondaryText ? 2 : 1`, `rightCount = grip ? 1 : 0`, `hasGrip = grip`. Two honest-config props: a `grip` boolean (default `true`) so `hasGrip` is prop-driven not a constant lie (and a non-resizable host — e.g. the node app — can drop it), and a `secondaryText?` that renders a vertical `separator` + a `label` in the left cell (the second-left-cell demo that exercises the vertical separator). Colours/captions live on the child entries — no duplication in G.

**Font tokens (additive).** `--fs-s1: 9px` / `--fs-s2: 8px` added **below** `--fs-0: 10px` in `skin.css` (`s` = small); the status-bar caption defaults to `--fs-s1`. General L2 tokens (other dense-UI spots want sub-10 too), not status-bar-only.

**CDP-verified (sampler 9422, both accents — Rule 2).** Registry `ids().length` **299→309 (+10)**, `count===unique===domCount` (309/309/309), **0 orphans both directions**. The exact +10 = `status-bar#default`/`#secondary` + `status-indicator#{default,secondary}__status-indicator` (each + its `__led`/`__label`) + `separator#secondary__sep` + `label#secondary__secondary`; `sb-cell` + grip absent from the registry (the non-registering proof). Getter G exact (`#default {1,1,true}`, `#secondary {2,1,true}`). Structure: root `DIV.status-bar`, 4 `sb-cell` SPANs `data-side` left/right/left/right, grip `SPAN.sb-grip` `aria-hidden="true"` inside `.sb-cell[data-side=right]`, clip-path `polygon(100% 0px, 100% 100%, 0px 100%)`. Grip seam: a bubbling `pointerdown` reached the grip (capture probe fired) — Svelte 5 delegates `pointerdown` at root, so the bubbling dispatch invoked `onResizeGrip` clean (`dispatchThrew:null`); source confirms `onpointerdown={(e) => onResizeGrip?.(e)}`, imports envelope/SbCell/StatusIndicator/Separator/Label only (no Tauri). Tokens `--fs-s1`=9px / `--fs-s2`=8px; caption computed `font-size`=9px. Separator `role="separator"` / `aria-orientation="vertical"`. Skin `.status-bar` / `.status-bar .label` / `.sb-cell` / `.sb-cell[data-side="right"]` / `.sb-grip` in cascade. Accent-neutral: grip bg `--t4 rgb(88,92,100)`, separator border `--s5 rgb(52,59,71)`, led ON `rgb(34,197,94)` — all unchanged under an injected `--accent2` swap (`neutralHeld:true`); this composition has **no accent-carrier** (the status-indicator has no `link`), so both accents render identically. Eye-check geometry-covered (grip in right cell + SE clip-path + 9px caption + vertical separator); harness screenshot flaky (J-489/J-490 precedent), not run. Feat `afcfaff` (Clair, code-only, 4 files), `vite build` clean 166 modules.

*New `core` di-composite; no wire/prop/protocol change → no `DECISIONS.md` touch. Registry 299→309 (+10). → components registry v0.65. The grip's real `startResizeDragging` wiring + the `.state-indicator`→`status-indicator` migration + window-config flips are 6.1e-B (real client 9222, no sampler). Next: M-RP6.1e-B real-client frame consolidation.*

---

### N-088 — the real-client frame lands: `#app` had no height (a latent shell bug), the menu-bar can't host a drag strip, and the native-title-bar pivot (M-RP6.1e-B)

**What the milestone was.** No component built — the **assembly** of the shipped `core` frame components into the real client shell (9222; the sampler structurally cannot host shell/window effects, D-097). Status-bar mounted bottom; the hand-rolled `.state-indicator` migrated onto `status-indicator`; the SE grip wired to Tauri `startResizeDragging`; centre-only scroll; centre logo + redundant Quit removed; window made resizable. Five files, none of them `core` (D10 fence held). **Sampler catalogue registry unchanged 309**; the client's own registry measured at **7** (`menu-bar`, `menu`, `status-bar`, `status-indicator`, its `led` + `label`, and the centre `paragraph` placeholder) — `count===unique===domCount`, 0 orphans both directions, `button#quit` gone.

**The find — `#app` had no height, and every previous window masked it.** The status-bar rendered *correctly* (last child of `.app-frame`, `flex: 0 0 auto`) yet floated mid-window. Root cause was three levels up: the Svelte mount target `<div id="app">` has **no height rule anywhere** — not in `main.js`, not in `index.html`, not in any CSS. So `.app-frame { height: 100% }` resolved against an auto-height parent and **collapsed to content height** (100px inside a 480px window). `html, body { height: 100% }` was set; the mount div between them was not. The old 420×260 window with a content-sized centre pane made this **invisible for the project's entire life** — a bottom-pinned chrome pane is the first thing that ever needed the frame to *fill*. Fix: `html, body, #app { height: 100% }`. **The general lesson:** a `height:100%` chain is only as good as its weakest link, and a *framework mount div* is the link nobody writes CSS for. Any shell that later mounts a BorderPane (the node app, M-RP7's dock engine) inherits this or reproduces the bug.

**The dead end that still teaches — `data-tauri-drag-region` cannot ride the menu-bar.** The runbook assumed the `menu-bar` would sit at intrinsic width, leaving a bare strip to its right that a shell wrapper could carry the drag attribute on. It does not: the `core` skin sets `.menu-bar { width: 100% }` — it is **designed** as a full-width bar with its own background + border — so it filled the strip and the drag target was always `.menu-bar`. And Tauri drags **only when the event target itself carries the attribute**, never an ancestor. The fix (shrink the bar to `width:auto` from `app.css`, move the bar treatment onto the wrapper) worked without touching `core`/skin, and was then **reverted by the pivot below**. **The lesson survives the revert:** a custom title-bar must own its **own** drag-region root; it must not borrow the menu-bar's strip.

**The pivot — native decorations as a temporary development affordance (NOT a reversal).** J-493 §10.3 locked the window frameless with the SE grip as the sole resize path. Joe flipped it mid-session to `decorations: true`: a frameless window during development has no window controls and no way to move it. The native title bar supplies move + min/max/close + native edge-resize, so the drag region was reverted entirely (`allow-start-dragging` dropped) and the grip demoted to a **supplementary** affordance. The honest correction that came out of the same conversation: a *customised native* caption cannot deliver either thing actually wanted — extra buttons are impossible, and only colours are themeable (Win11 DWM attributes, silently ignored on Win10). **The Discord-shaped endpoint IS a custom title bar**, so it is filed as **M-RP8** (after the widget grid is live on both apps) rather than approximated. **Joe's rule, locked: no native elements within the window's main pane** — the native chrome is the OS title bar and nothing else.

**Why the grip stays.** Native edges now resize, so the grip is redundant *today*. It is kept anyway (`grip=true` + `core:window:allow-start-resize-dragging` retained) — it is the component default, it is a conventional SE-corner affordance, and keeping the seam **wired and proven the whole way through** is exactly why M-RP8 is cheap: the resize path never has to be rebuilt when the frame goes frameless again. A seam that stays live cannot rot.

**Verify notes (real client 9222, decorated).** Grip proven by **Joe's physical gesture** — an OS drag loop cannot be synthesised over CDP, so the honest proof is a human hand plus the measured consequence (window 720×480 → 743×470, error trap empty, **zero permission denials** → the capability grant is real, not silently failing). Centre-only scroll proven by **CDP height-injection** into the placeholder (centre `scrollTop` 0→500, document `scrollTop` 0, `docScrollable:false`, menu-bar `.top` and status-bar `.bottom` constant) and restored — no shipped filler content, nothing faked. **Method finding:** Svelte 5 flips state **synchronously** but tears popup DOM/registry down on the **effect flush**, so a same-tick CDP read after a click or an Esc sees the state change *without* the mount/unmount — **read after settle**, or a menu will look like it registers nothing on open and orphans on close (it does neither). **DPI wrinkle, logged not fixed:** at DPR 1.25 the configured 900×600 lands as **physical** px → 720×480 CSS on screen; the 900×600 **logical** intent is not literally met at scaled DPI.

*No component built, no wire/prop/protocol change → no `DECISIONS.md` touch, no components-registry touch (sampler stays 309). Feat `8f401bd` (Clair, code-only, 5 files), `vite build` clean 138 modules. frame-phase0 §10.3 revised in place (v1.4→v1.5). Filed: **M-RP8** `title-bar` + frameless restore; **M-RP6.1e-B1** no-select chrome (global `user-select:none` in skin L2, opt-in `text` on editable natives + `.paragraph` — the message body must stay copyable). Next: **M-RP6.1e-C** `dialog` core + Help→About (which now needs a real Tauri `get_about_info` read verb — build metadata and filesystem paths are invisible to the frontend).*

---

### N-089 — `dialog`: the 31st `core`; the native element does the whole W-2 machine's job; `:modal` is the only honest proof; and a `$bindable open` lies unless the element writes back (M-RP6.1e-C1)

**What it is.** The **31st `core`** component — a **di composite** modal container, flagged as a gap since J-432 and built now as the reusable primitive the Help→About assembly (6.1e-C3) consumes. Root `<dialog class="dialog">`; header `title` → `children` body slot → footer with a single composed `button` (`…__close`, default label **"Close"**, not "OK"). First step of the three-way M-RP6.1e-C split (C1 `dialog` core → C2 `get_about_info` Tauri read verb → C3 Help→About assembly). Component only — no Rust, no client shell.

**The headline: the native element replaces the machine.** Every other popup in this library (`combobox`, `tag-select`, `color-picker`, `entity-context-menu`, `menu`) hand-rolls a W-2 owned-popup behaviour machine — **not because we wanted to, but because no native element gave it to them.** `<dialog>` + `showModal()` supplies **top-layer stacking, `::backdrop`, focus trap, background-inert, and Esc-dismiss natively**. So `dialog` builds **no machine at all**. That is the whole design, and it is why this component is small. The lesson generalises: *check whether the platform already owns the behaviour before writing a state machine for it.*

**The trap: `showModal()` reflects the `open` attribute itself — so the attribute proves nothing.** There are two ways to open a `<dialog>`: `showModal()` (top layer, backdrop, focus trap) and the `open` **attribute** (non-modal — none of those things). The failure mode is that the attribute path **looks like it works**: it renders, it's visible, and `el.open` reports `true`. Measured here: while correctly modal, `openAttr` is **also** `true`, because `showModal()` sets the attribute as a *side effect*. **The only discriminator is `el.matches(':modal')`** — verified `isModal: true`. Had we asserted on the attribute (the obvious thing to assert on), we would have written a green verify for a silently-downgraded dialog. **This is the leg that must never be dropped from a dialog's DoD.** (`:modal` is supported in this WebView2 — grounded, not assumed.)

**The load-bearing axis: a `$bindable open` LIES unless the element writes back.** Native `<dialog>` **owns its own open state**: `showModal()`/`close()` are imperative, and **Esc fires `cancel` → `close` without ever consulting the prop.** So the naive binding desynchronises the instant the user presses Esc — the element is closed, the prop still says `true`, and the *next* `open = true` is a **no-op because the prop never changed**: the dialog can never be reopened. Three rules close it: (1) a **guarded** `$effect` prop→element (`if (el.open !== open)`, or the effect and the listener ping-pong); (2) a **`close`-event listener writing `open = false` back into the binding** — this is not polish, it is what keeps the bindable honest; (3) `onClose?` fires from the same listener. **Getter G reads `open` from `el.open` (the DOM), not from the prop** — G's job is to report what *rendered*, not what we *intended*; if prop and element ever desynchronise, G must **expose** that, not paper over it (Rule 1 in component form).

**Children stay mounted when closed — the registry does NOT flip.** A closed native `<dialog>` is `display:none`, **not unmounted**. So composed children register on mount and **stay registered in both states**: measured **313 open, 313 closed**. This is the **exact opposite of `menu-item`** (N-086), which mounts on popup-open and unregisters on close. Anyone verifying a dialog and hunting an open/close registry delta will hunt forever — there isn't one; `open` is what flips.

**`::backdrop` does not inherit custom properties reliably.** Clair grounded rather than guessed (Rule 6) and used a **literal** `rgba(0,0,0,0.55)` scrim instead of a `var(--…)` token. A modal dim is accent-neutral chrome, so nothing is lost — but the general point stands: `::backdrop` is not a normal descendant and the custom-property cascade into it is engine-uncertain. If a `var()` renders as nothing there, this is why.

**CDP-verified (sampler 9422, both accents — Rule 2).** Registry **309→313 (+4)** = `dialog#default` + `dialog#labelled` + their `button#…__close`; `count===unique===domCount` (313/313/313), **0 orphans both directions**. Getter G exact (`{title:"Confirm action", open:false}` / `{title:"About XGen", open:false}`). Open → `elOpen:true`, **`isModal:true`**. Focus lands **inside** the dialog on open (`activeTag: BUTTON`, native autofocus onto Close). Skin `.dialog` → `rgb(28,31,36)` = `--s2`, radius `10px` = `--rad2`; 5 `.dialog*` rules in cascade. Esc honesty: Clair drove a **trusted** `Input.dispatchKeyEvent` Escape (`elOpen:false` + `getterOpen:false` + `onClose` fired); Chat re-drove the same code path **element-first** (`el.close()` → getter `open:false` → **re-click the trigger → reopens, `isModal:true`**) — reopening is *impossible* if the prop had lied `true`, so the write-back is proven from both ends. Accent-neutral surface; sole carrier = the inherited `button` focus-ring (gold `#c28840` ↔ blue `#3a7ab0`) — the N-087 "no accent-carrier in this composition" pattern, stated rather than invented. Feat `4f35d87` (Clair, code-only, 3 files), `vite build` clean **167 modules** (166→167).

**Runbook deviation, flagged not absorbed (Rule 6).** The runbook specified `title` as a required `string`; Clair authored `title?: string` defaulting to `''` with a conditional header, matching the **`section` precedent**. Accepted as the shipped contract: strictly more robust (a titleless dialog degrades to a bare surface), never reached by About (always titled), and G still reports `title` verbatim. Recorded here rather than retrofitted into the runbook.
...
> ### ⚠️ SUPERSEDED (M-RP7.4b, 2026-07-15) — TWO CORRECTIONS TO THIS NOTE
...
### N-136 — an `$effect` that persists via a read-modify-write store method must `untrack` the write, or it becomes its own dependency (M-RP-SETTINGS Leg C, J-540)

Leg C's shell binds the backdrop value into `region-shell`'s `backgroundLive` and **persists** it. The first cut used an `$effect` that called `setSessionBackdrop(backdrop.pattern)` — and `setSessionBackdrop` does a read-modify-write of the session bag (`_store.session = {...session, backdrop}`, the N-107 per-key merge). **The READ of `_store.session` executed inside the effect → `_store.session` became a dependency of the effect → the WRITE self-invalidated it → `effect_update_depth_exceeded`.** Svelte halts the render scheduler: `$state` keeps mutating (getters advance, the toggle animates natively) but **the DOM freezes** — the symptom was a *dead-button UI* (Back / Close unresponsive), **not a crash**, so it slipped every static gate (build / test / typecheck cannot see a scheduler halt). **Only a live drive surfaced it** (Joe caught it on screen first). Fix = `untrack(() => setSessionBackdrop(...))` so `backdrop.pattern` is the effect's SOLE dependency.

**Why the other persist axes were safe, and the rule that generalises.** `installed` / `disabled` / `locked` / `layout` all persist through the SAME read-modify-write `setSessionX` merge — but they are called from **event handlers** (a toggle click, an install action), never from an effect, so the read never becomes a tracked dependency. Backdrop was the **first persist wired through an effect** (the shell mirrors a reactive `$common` store, not a discrete user action), which is exactly what exposed the trap. **Rule: an `$effect` whose body persists via a store method that READS-then-WRITES the same state must wrap the write in `untrack` (or read the to-be-written key outside the effect), or the write re-triggers the effect forever.** Sibling of the Rule-5 / N-099 family: *a failure that only a live drive can see, because the static artefacts all pass.* Re-verified on 9222 (J-540): the two-way toggle flips the painted plate and both getters, and the persist read returns cleanly through the flip — no depth-exceeded, DOM responsive.

### N-137 — a session state machine that RETURNS a terminal outcome has not EMITTED it; returning and emitting are two acts, and the CALLER owns the terminal state (M-RP6.6 Leg A, J-543)
