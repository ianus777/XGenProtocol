...
⚠️ **CORRECTED AT THE SITE (J-742): *origin-independently* IS THE WRONG WORD, AND THE REAL MECHANISM IS SOUNDER.** The existing admission-gate precedent is explicitly **origin-SCOPED** — the invite-expiry gate runs if origin == EventOrigin::LocallySubmitted (`runtime.rs:1580`), under INV-EXP's rule that *a peer trusts the home node's already-made admission decision and does not re-adjudicate* (:1567-1572). ⇒ **it is not ONE rule holding on both channels; it is TWO rules, one per channel, and the new one fills the gap the old one leaves** — local submissions take the new admission gate, federation submissions take F-3's relationship check. **The conclusion survives; the word does not.** See 	asks/M_SPACE_ADMISSION_PHASE0.md §3.3.
...
🔑 **THE DISCRIMINATOR, AND IT DECIDES RATHER THAN BALANCES: ASK WHY THE SET-ONCE PRECEDENTS ARE SET-ONCE.** `jurisdiction` says *"a silent post-hoc change is the integrity hazard to avoid"*; `e2e_encryption` says *"a Space cannot be retroactively encrypted or decrypted"*. **Both re-label data that already exists — they falsify the past.** ⚠️ **Admission has neither property: it is evaluated ONCE, at a join, and never re-evaluated.** Opening a Space admits nobody retroactively; closing one ejects nobody. ⇒ ***there is no past for a change to falsify, so the set-once argument does not transfer.*** **The right sibling is `member_temperature_visibility`** — a forward-looking behaviour rule, owner-settable, whose event type and applier arm already exist. ⚠️ **CORRECTED AT THE SITE (J-742): the RESOLUTION arm does NOT exist.** `StateSpaceTemperatureVisibility` and `StateSpacePacing` appear **zero** times in `resolution/state_key.rs` and fall to `_ => None`; the one resolution-layer hit is a `cfg(test)` fixture (`algorithm.rs:435`) ⇒ **the sibling is a TWO-part precedent, not a three-part one.** The naming survives — it is chosen on the set-once-vs-forward-looking axis, where it is correct — but 🛑 **its APPLIER is `state.rs:753`, one of the two `owner_id`-equality sites RIDER 2 forbids**, so *a precedent named for the right reason can still be the wrong thing to copy*. See `tasks/M_SPACE_ADMISSION_PHASE0.md` §3.1–§3.2.
