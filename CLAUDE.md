...
## 🟢 UI component-library / substrate (RP track) — M-RP2.17 ✅ CLOSED (J-426): `color` — the **twelfth** `core` component, the native swatch atomic; a singleton that **stands alone** (the `range` case, not a fold; applies D-096, N-047); the native picker is OS-painted → a themed `color-picker` composite logged; **next — `file`**
