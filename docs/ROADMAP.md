...
│   │   │   ↳ 🛑 **THE FIXTURE USES XGID TAILS, NOT NAMES — AND AT REAL LENGTH (44 chars), WHICH IS LOAD-BEARING** — with no book record `toDescriptor` already falls back to `tail()`, so **a real ③ row has NO display name to strike** (§5b). ***A human-named fixture would tune a case the product cannot produce.*** ✅ **Measured: 52-char name → `scrollWidth 391` vs `clientWidth 250` ⇒ CLIPPED** — ⚠️ *the first fixture used 16-char tails, which do NOT clip at 300px (Clair F5, then measured rather than left a likelihood; `D-131`).* ⚠️ **And the strikethrough makes §6a's `tail-8` gap MORE visible** — it strikes the constant `ed25519:` head the clip preserves
