...
│   ├── 🟡 **M-SPACE-ADMISSION** — who may join a Space, and how a leaver comes back · FILED at J-741 on Joe's structural ruling (*the rejoin mechanism must be space general*) · protocol + node, Space-general, NOT a UI leg · **PHASE-0 LANDED J-742**, tasks/M_SPACE_ADMISSION_PHASE0.md **v3.0 ACTIVE** · **THE REJOIN ARC IS COMPLETE (J-772); LEG G, the rejoin anchor verb, is the only leg left and is Joe-locked**
│   │   ↳ Owes: DONE — **THE REJOIN ARC IS COMPLETE (J-742 through J-772). Legs A-bis, A, B, C, E-0, D, E-1, E-2 and E-3 all shipped.** REDUCED ON COMPLETION per Joe's J-715 mechanism — this node carried **97 Owes lines totalling 99,929 characters** at close, and the detail now lives in the JOURNAL and in tasks/M_SPACE_ADMISSION_PHASE0.md v3.0, which is the arc's own register. **Floor 1623 to 1641 / 0 / 62 x 56 SUITES across the arc, every delta MEASURED with --skip, never arithmetic**
│   │   ↳ Owes: DONE — **WHAT WAS RULED.** D-148: admission is owner-settable, two riders, default open. **D-154: what a Space remembers about someone who left, and what rejoining gives back — SIX clauses.** A rejoin restores PRESENCE, never POSITION: ① back as of now with role RE-DERIVED · ② kick remembered · ③ ban follows kick · ④ **the gap stays closed** · ⑤ rooms NOT restored · ⑥ **node-eject follows kick and ban (Joe, J-766 — it also BANS, so it reaches apply_ban's end state by a different authority)**. **Clause ④ CLARIFIED at J-769: the gap is closed to CONTENT and OPEN TO MEMBERSHIP STRUCTURE — recorded as a NARROWING, not an application**
│   │   ↳ Owes: DONE — **WHAT WAS BUILT.** left_at on SpaceMember; four appliers retain and mark; apply_join's three-way gate; both accessors present-tense; **all fifty readers in ONE commit** (E-1, J-767). **Presence intervals DERIVED from the log at delivery time and applied at all three history doors** (E-2, J-770) — the boundary ordered by core's topological_sort so it agrees with left_at BY CONSTRUCTION, while delivery keeps its own order. **The departure HISTORY is not stored on the record: an absence list would re-mint the former_members GDPR shape section 6.5 refused**
│   │   ↳ Owes: DONE — KEY — **THE ARC's OWN MEASUREMENT OF ITSELF: SEVEN specification defects across E-1, E-2 and E-3, every one found by the implementing seat and NONE absorbed.** Including a gate that could only pass by doing what its own runbook forbids, a runbook sentence whose prescribed order PRODUCED the defect its own test forbade, close conditions that omitted the room_id-empty requirement, and an instruction to cite a J-number that records ZERO PRODUCT CODE. **Rule 6 working, and a standing measurement of how much a Phase-0 written from grounding still gets wrong at the point of contact**
│   │   ↳ Owes: DONE — STOP — **THE FINDINGS WORTH CARRYING OUT OF THE ARC.** **A semantic change can leave an assertion TRUE and simultaneously empty it of meaning** — a green convergence witness that had silently become a tautology, invisible to compiler and red run alike. **Where a change PRESERVES an assertion, no automated instrument can enumerate the affected tests: name READING as the closure mechanism.** **A Space CREATOR emits no membership.join**, so open-at-index-0 is required for an owner to receive anything at all. **N-202: WriteAllLines flips a whole file's line endings and the CR equals LF guard cannot see it.** **A detached run's notification exit code is the LAUNCHER's, not cargo's; only the sentinel separates them.** **N-203: asserting that a mutation CHANGED something is not asserting it changed into what was intended**
│   │   ↳ Owes: OPEN and Joe: **LEG G — THE REJOIN ANCHOR — IS SCHEDULED (Joe, 2026-08-24) AND HAS ITS OWN PHASE-0: tasks/M_SPACE_ADMISSION_LEGG_PHASE0.md v1.0 ACTIVE, which is this leg's register.** SIX legs: G-0 Phase-0, G-1 the gate's third term, G-2 the 3048 refusal, G-3 the door, G-4 the client anchor selection, G-5 close. **WIRE SHAPE RULED: ONE MORE KEY on collect_invite_bootstrap's authorization, NOT a new transport verb** — and it was MEASURED, not preferred: TransportMessage is serde tag-only with ZERO serde(other), so an unrecognised transport type is a deserialisation error at connection.rs:454 and the node loop is Err break at app.rs:2048, which means a new verb sent to an older node DROPS HER SESSION instead of being refused — the opposite of what the design section claimed for it **G-1 RUNBOOK AUTHORED J-774: tasks/RUNBOOK_SPACE_ADMISSION_LEG_G1.md v1.0 PENDING, Joe to lock.** Two grounded facts shrank it to about three production lines: the banned pre-check ALREADY runs at runtime.rs:1523, ABOVE the LocallySubmitted block at :1580, so the new term needs NO ban clause and a second one would be D-067 target; and apply_kick marks without banning, so D-154 clauses two and three fall out of the checks already present. **The absent ban clause is asserted by a CONTROL, not by a code path — a control is how you write down a dependency you chose not to re-implement.**
│   │   ↳ Owes: DONE — **LEG G-1 SHIPPED J-775: the gate third term is live and a departed member can come back.** cargo 1641 to **1644 / 0 / 62 x 56 SUITES**, re-driven INDEPENDENTLY BY BOTH SEATS, delta measured three ways and never arithmetic. **THREE DEVIATIONS, ALL CORRECT, TWO OF THEM DEFECTS IN THE RUNBOOK ITSELF:** the comment point requiring the source to say a present member reaches the applier was FALSE ON BOTH PATHS and was refused before it reached the source, and a named control lived outside the leg may-change list so it could not fail. — **a control that cannot fail is not a control, and a document refuted by its own later paragraph has now produced TWO defects in this arc, neither of which fell to a re-read**
│   │   ↳ Owes: DONE — **LEG G-2 SHIPPED J-776: the node refuses a rejoin it cannot anchor, and 3048 rejoin_not_anchored is LIVE.** cargo 1644 to **1648 / 0 / 62 x 56 SUITES**, re-driven independently by both seats, skip run returning exactly 1644 and libtest own filtered out equal to 4. **THE LEG INVENTED NO PREDICATE — ingest_event already computed this boolean at the SR-D1 conflict gate and then rebuilt the Space without her, having already replied; G-2 moved the existing predicate onto the ANSWER PATH.** **And the reverted reply is worse than the design predicted: the revert run observed Accepted carrying new_joiner set to the sender while is_member was false — not merely a yes but a POSITIVE IDENTITY ASSERTION about a person the fold had already dropped.** **The ch3 row rode the same change on section 3.6.10.10 own allocation rule, correcting this arc assignment of it to G-5 — C-8 shape again.** **LIMIT, NOT SOFTENED: nothing ran against a live node, a wire or a second identity; 3048 has never been observed on a wire, and that bound is G-4**
│   │   ↳ Owes: DONE — **LEG G-3 SHIPPED J-777: the door now admits a retained departed member who is not banned, and serves her ONLY HER OWN membership events.** cargo 1648 to **1654 / 0 / 62 x 56 SUITES**, re-driven independently by both seats on FORCED REBUILDS, skip run returning exactly 1648 and libtest filtered out equal to 6. **THE FINDING: one line was doing two jobs — banned has ZERO occurrences in the fanout module, the transport arm defers all authorization to that function, and the dispatch pre-check guards event submission rather than transport requests, so a banned identity was excluded ONLY as a side effect of holding no invite.** **The ban term is therefore REQUIRED here, the exact inverse of G-1 where it would have been a second source of truth — the same clause is a defect in one leg and a requirement in the other, and the only way to tell which is to open the path.** **Section 3 ruled option two: a requester admitted by the gate but not yet rejoined is not yet returning, so serving her the full chain would have widened a disclosure ruling as a side effect of a mechanism change**
│   │   ↳ Owes: OPEN and nobody: **N-209 — A DISCLOSURE FILTER MUST BE WRITTEN FROM THE EVENT POINT OF VIEW, NOT THE REQUESTER.** The runbook said measure the field per type without saying why, because the reason had not been seen when it was written. **A kick SHE issued carries HER as sender and a THIRD PARTY as target, so a naive sender-or-target union would have leaked exactly what the ruling withholds — the ruling defeated by one plausible-looking line.** **The union is not wrong because it is imprecise. It is wrong because it asks the requester question, does this event mention her, where the ruling asks the event question, whose removal does this event disclose. The two agree on every event except the one that matters.** All five actor-on-subject types verified at their appliers, and no room_id condition deliberately, because this is a disclosure test and not a boundary
│   │   ↳ Owes: DONE — **RULED (Joe, 2026-08-26): THE PROSE AND THE SKETCH IN THE G-3 RUNBOOK WERE NOT THE SAME PREDICATE, AND THE SKETCH IS CORRECT.** A former member holding an EXPIRED invite is refused and must obtain a fresh invite or stay out. **Chat had recommended the true OR and was WRONG; recorded as wrong rather than dropped.** **Two facts refute it. First, the invite is the CARRIER OF THE ROLE and not merely permission to enter: the join applier derives role and inviter from the pending invite, absent meaning plain Member, so under the OR an expired Moderator re-invite would fall through and admit her a plain Member with the grant silently dropped. Second and decisive, the invite-expiry gate at submission is keyed on the presence of a pending invite and is NOT conditioned on the rejoin flag, so she is refused there whatever the door served her — the OR would have opened a door onto a locked gate.** **Reachability had only ONE half measured: that pending invites survive a departure was proven, but that an invite can be ISSUED to a departed member was assumed until the invite applier was opened — a necessary condition proved and a sufficient one assumed.** **What survives is a STRING and not a predicate: the door refusal is indistinguishable from a stranger, so the rule is defensible and the experience is not. Filed for G-5, with two comment-only source riders that are not the records seat to write**
│   │   ↳ Owes: OPEN and nobody: **N-206 and N-207 — TWO INSTRUMENTS, ONE CAUSE, AND THE SILENT ONE WORE THE RIGHT ANSWER.** N-206, LOUD: an inner ampersand in the background wrapper made it exit instantly, the notification reported exit code zero which was the LAUNCHER code, the log had no sentinel and zero test result lines, and cargo was still running. N-207, SILENT: a re-drive returned the CORRECT floor with sentinel zero, a final result line and zero failures, **and Compiling xgen-core zero — it was measuring the other seat binary, not its own tree.** **A wrong-looking number invites a second look; a right-looking number over someone else artefact invites nothing at all.** **This is why N-199 guards on the Compiling line and not on the result line: the result line cannot distinguish my source passed from your binary passed, and the re-drive rule has a failure mode where the re-drive silently re-reads the report.** **N-208: Python cannot open Git Bash tmp paths, and a try-except there would have produced a clean-looking zero**
│   │   ↳ Owes: OPEN and nobody: **N-204 — HEREDOCS TRUNCATE IN THE IMPLEMENTING HARNESS, AND THE NOTE SAYS WHICH HALF IS MEASURED.** 71 lines wrote correctly; about 170 and about 430 lines both failed at PARSE time, so nothing partial applied. **The boundary lies between 71 and about 164 and was NOT bisected — chunk at about seventy is a WORKING RULE, NOT a measured threshold, and it does not enter the record as one.** The dangerous sibling, a cut landing outside a quoted region that still parses and writes a half-file looking complete, is N-203 family and is INFERRED FROM THE MECHANISM, never observed. **In both observed cases the guards never ran, so they proved nothing either way — which is the entire reason their assertions are worth anything**
│   │   ↳ Owes: OPEN and Joe: STOP — **THE SHIPPED GATE IMPLEMENTS TWO OF ITS PREDICATE'S THREE TERMS, WHICH IS WHY THE LEG GREW A SECOND ONE.** runtime.rs:1620 tests admission-not-open AND no-pending-invite; **the FORMER-MEMBER term is absent and left_at has ZERO non-test occurrences in runtime.rs**, while apply_join at state.rs:1214 DOES carry the third arm, so the dispatch gate refuses her 3047 before the applier that would admit her is ever reached — **in EVERY DM, since both DM constructors pin invite at fold time.** **Cause: the design section predates left_at, J-763 correctly deferred that item to Leg E, Leg E built it, and nobody returned to the gate.** **Thirteen rejoin test functions exist and NOT ONE exercises the dispatch gate with a former member — the suite is green because the case is unwritten**
│   │   ↳ Owes: OPEN and nobody: **THREE CAVEATS ACCEPTED KNOWINGLY, CARRIED NOT DISCHARGED, and none of them Leg G's.** ① **D-154 clause ⑥: retention makes an ejection a durable federated record while node_eject is REVERSIBLE** — a reversed ejection still leaves the record saying it happened. ② **D-154 clause ④-as-clarified: a returning member learns who else was removed and when**, about people who never consented to her return and cannot see that she holds it. ③ **self.banned is already a permanent federated list of identities and that look is still owed** — filed at D-154, older than this arc and not created by it. NOTE — one small one from E-2: **a close event naming her BEFORE her first join closes an interval the fold never closed; errs restrictive**
│   │   ↳ Owes: DONE — **LEG G-4 SHIPPED J-778: SHE COMES BACK FROM A FRESH INSTALL, AND THE ARC'S STANDING LIMIT IS CLOSED.** cargo 1654 to **1665 / 0 / 64 x 57 SUITES**, re-driven independently by both seats on FORCED REBUILDS, skip run returning 229 against 232 on the client lib and libtest filtered out equal to 3. **THE FIRST LEG OF THE ARC THAT RAN AGAINST A LIVE NODE, A WIRE AND A SECOND IDENTITY: xgen-mptest spawns the real binaries as OS processes over aicontrol, and both scenarios are GREEN — a departed member rejoining from a cleared anchor, and the same identity rejoining from a clean data directory. 3048 was RECEIVED BY A CLIENT for the first time; it had existed in the node for two legs and nothing had ever seen it.** SUITES 56 to 57 and ignored 62 to 64 are this leg deliberately: a new tests file is a new binary carrying two ignore-gated scenarios and zero passing tests
│   │   ↳ trigger: SCHEDULED 2026-08-24 — Leg G-0 Phase-0 landed J-773
