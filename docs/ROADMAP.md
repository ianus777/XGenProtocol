...
│   │   ↳ 🔑 **THE FINDING — PASS 4's CLASSIFICATION IS BINARY AND THE CODEBASE HAS THREE CATEGORIES.** `admin_ops.rs` holds **63 of the 130** and they are **CLI-arg and admin-pipe result structs**, with `admin_ops.rs:977` already carrying a documented projection (*"Project a wire-format Identity URI to the typed key at the registry boundary"*). 🛑 **The EXACT INVERSE of the case that minted `D-136`, where the downgrade *"reads as a deliberate seam and is not one"*.** ⇒ ***`admin_ops.rs` is not a file that broke the rule; it is a file the rule does not reach*** — and **that, not carelessness, is the better explanation for a two-month silent regression**
...
│   │   ├── 🟢 **Leg 0 Phase-0** — the sweep, the three rulings, **plus the hand-verified classification of all 88** 🛑 **the classification is a DELIVERABLE of this leg, not a preamble to the next** ↳ trigger: fired — the hand-read is outstanding
│   │   ├── 🟡 **Leg A the enforcement mechanism** — ③ grep gate + ④ written rule in `CLAUDE.md`; **moves no floor** 🔑 **FIRST, so every later leg lands UNDER the rule instead of beside it** ↳ trigger: Leg 0's hand-read lands
...
│   │   ├── 🟡 **Leg C the remainder** — sized by Leg 0's hand-read 🔓 **MAY SPLIT INTO ITS OWN MILESTONE RATHER THAN BLOCKING; the split and its name are Joe's (`D-123`), triggered by the hand-read and NOT guessed at now** ↳ trigger: Leg B lands + Joe rules the split
