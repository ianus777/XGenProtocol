...
## 🟢 UI component-library / substrate (RP track) — M-RP2.16 ✅ CLOSED (J-425): `date` — the **eleventh** `core` component; the date-input family (`date`/`time`/`datetime-local`/`month`/`week`) **folds** into one atomic (the `textfield` fold again, not `range`; applies D-096, N-046), built/tuned/verified in the **sampler** (D-097); the **sampler-DoD is now standing** (→ D-097 note); **next — `color`**
