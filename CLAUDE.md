...
## 🟢 PLAY — M6 (new): **federation-admin-control 2b (policy) — design FAC-D3/D4 LOCKED + checkpoint #1 LOCKED at J-179; runbook ACTIVE v1.1**; **next-active = Clair Commit 1** (the `FederationPolicy` sibling store + `PolicyMode` types — checkpoint #1 closed, proceed directly); then Commit 2 (enforcement — pure `policy_permits` at both sites, checkpoint #2 = the code-traced inbound site) → Commit 3 (`set-policy`/`show-policy` verbs) → Commit 4 (close); then auth-module-registry → bootstrap-client; then M7 `--aicontrol`
