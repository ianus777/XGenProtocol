...
│   ├── 🟡 **M-SPACE-ADMISSION** — who may join a Space, and how a leaver comes back · FILED at J-741 on Joe's structural ruling (*the rejoin mechanism must be space general*) · protocol + node, Space-general, NOT a UI leg · **PHASE-0 LANDED J-742**, tasks/M_SPACE_ADMISSION_PHASE0.md **v2.8 ACTIVE** · **THE REJOIN ARC IS COMPLETE (J-772); LEG G, the rejoin anchor verb, is the only leg left and is Joe-locked**
│   │   ↳ Owes: DONE — **THE REJOIN ARC IS COMPLETE (J-742 through J-772). Legs A-bis, A, B, C, E-0, D, E-1, E-2 and E-3 all shipped.** REDUCED ON COMPLETION per Joe's J-715 mechanism — this node carried **97 Owes lines totalling 99,929 characters** at close, and the detail now lives in the JOURNAL and in tasks/M_SPACE_ADMISSION_PHASE0.md v2.8, which is the arc's own register. **Floor 1623 to 1641 / 0 / 62 x 56 SUITES across the arc, every delta MEASURED with --skip, never arithmetic**
...
│   │   ↳ Owes: OPEN and Joe: **LEG G — THE REJOIN ANCHOR VERB — IS THE ONLY LEG LEFT.** A node-side get_rejoin_anchor, sibling of collect_invite_bootstrap: the rejoiner asks the Node for her own last membership event and anchors on it, so the causal anchor stops depending on client-local memory. **New wire verb, so Joe-lock.** NOTE — **clause ④-as-clarified strengthened the case for it: a returning member now receives the intervening membership chain, which is what she needs to compute an anchor at all** — the mechanical consequence named when Joe ruled (b) at J-769
