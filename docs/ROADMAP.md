...
> Version: 1.0  
> Date: May 2026  
> **Last updated**: 2026-05-18 (J-083 — Federation Event Propagation implementation Phase 2 SHIPPED: F-4 `process_inbound` validation pipeline unification; HeldPending uniform across all event families; three-path asymmetry closed; audit's HIGH-severity precondition for Phase 4 federation push met. 480 tests. Present summary updated; Past gains a Phase-2 close entry under Federation Event Propagation implementation.)  
> Language: English  
> Author: JozefN  
...
🟢 **Federation Event Propagation implementation — Phase 1 ✅ + Phase 2 ✅ SHIPPED; Phase 3 next-active.** The nine-phase runbook at `tasks/FEDERATION_PROPAGATION_COMPLETION.md` (Status: ACTIVE, v1.0) drives this milestone. **Phase 1** closed in J-082 on 2026-05-18 (F-6 `transport.sync_complete` wire shape + F-7 response-size pagination + four-call-site migration; cross-Space SyncComplete Clair-locked as whole-batch; `[sync]` config on both binaries). **Phase 2** closed in J-083 on 2026-05-18 (F-4 `process_inbound` validation pipeline unification: `ValidationOutcome` + `validate_event` core in `xgen-core/src/message/exchange.rs`; `DispatchOutcome` + `NodeRuntime::dispatch_event` orchestrator in `xgen-core/src/node/runtime.rs`; pre-F-4 three-path asymmetry collapsed to one `dispatch_event` call in `process_inbound`; HeldPending now applies uniformly to messages, membership.join, and other state events — closing audit Scenario-A; pre-F-4 `accept_message` preserved for smoke.rs backward-compat). The audit's HIGH-severity precondition for Phase 4 federation push is now met — pushing federation events into the pre-Phase-2 `process_inbound` would have landed forged-signature acceptance of membership.join + state events. 480 tests passing (+4 over Phase 1's 476: three Scenario-A regression tests covering Path A/B/C + one forged-signature regression on Path B). **Next: Phase 3 — federation handshake reshape to tip exchange (F-1a).** Today's `handle_federation_incoming` runs a one-time history dump and closes the connection. Phase 3 replaces that with: peer sends current tip per shared Space → home Node responds with delta via Phase 1's `collect_sync_history` + pagination → session **stays open** as the persistent F-2 push channel for Phase 4. Wire shape latitude per design doc §4.4. Remaining work after Phase 3: Phase 4 (federation event push, F-1 + F-1b + F-5) → Phase 5 (per-peer record + reconnect scheduling, F-1c) → Phase 6 (HeldPending generalisation for unknown signer Identity, F-10) → Phase 7 (federation-relationship verification gate, F-3 second check) → Phase 8 (documentation pass) → Phase 9 (integration tests for full federation push path). Blocks M6 (new) going ACTIVE.
