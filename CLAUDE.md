...
## 🟢 UI component-library / substrate (RP track) — M-RP2.13 ✅ CLOSED (J-418): `textarea` — the eighth `core` component, a stand-alone atomic `<textarea>` (multi-line free-text, string `bind:value`); the next atomic di per N-038, the edit-side counterpart to `paragraph` — CDP-verified BOTH apps
