...
> Date: May 2026  
> **Status:** ACTIVE  
> **Last updated:** 2026-05-21 (Bidirectional `federation_nodes` implementation milestone CLOSED in four-commit sequence per JOURNAL J-096 — Commit 1 doc-pass (`e975162`), Commit 2 origin-aware applier + plumbing + six unit tests (`a730eda`), Commit 2.5 sibling vantage-aware drain hook (`cbceb41`, in-flight gap closure), Commit 3 Phase 9 Scenario 1 resurrection (`f051039`), Commit 4 milestone close (this commit). Test count 571 → 577 across the milestone (+6 unit tests in Commit 2; Scenario 1 originally lifted in Commit 3 then re-stood-down in this commit on a separate topological-sort finding, net wash on ignored count). Bidirectional fix is verified-correct — six unit tests in `xgen-core/src/space/state.rs::mod tests` (notably `apply_federation_add_two_vantages_mirror`) remain green and stand as the live D-075 regression lock. SEPARATE pre-existing finding surfaced during Commit 4 verification, NOT implicated by the bidirectional fix: wire-order non-determinism in `topological_sort_events` at `xgen-node/src/fanout.rs:193` fed by non-deterministic `HashMap.values()` iteration in `compute_federation_delta_for_space` at the same file ~:321. Both `state.space_create` and `state.room_create` are DAG roots with empty `prev_events`; they tie at the top of the topo sort and their wire order tracks HashMap iteration. Evidence: 105 `dispatch_event` calls on B in both pass and fail runs (divergence is upstream wire order); in a failing run `room_create` reaches B's dispatcher before `space_create` and is rejected with "space not found", cascading through the bootstrap chain. Per Joe-direction this opens as its own phase per D-071, sibling-shape to the bidirectional `federation_nodes` audit → design → impl arc just closed. Phase 9 Scenario 1 re-stood-down with updated stand-down comment naming the topological-sort finding and forward-referencing placeholder `tasks/FEDERATION_TOPOSORT_AUDIT.md` (to be authored by Chat Claude as the new active phase). Phase 9 Commit 3b stays paused inside milestone scope. ROADMAP.md v1.10 → v1.11 in same commit per same-commit discipline; `tasks/FEDERATION_BIDIRECTIONAL_NODES_IMPL.md` Status flipped ACTIVE → COMPLETED v1.1. Per D-069 canonical-document + D-071 audit-precedes-dependent-design + D-074 milestone-close-includes-JOURNAL + D-075 + "honest longer work over fast shortcut" discipline. M6 (new) blocking chain extended by one more node, unchanged in shape. Pass 1 of XGID Retrofit stays at Status: ACTIVE v2.0 unchanged.)  
> Author: JozefN  
> Credits: Concept, philosophy, requirements, project direction: Jozef Nižnanský. Technical assistance and implementation support: AI-assisted development tools.  

---

## 🟢 PLAY — Topological-sort wire-order non-determinism phase OPENS 2026-05-21; Chat Claude + Joe author audit doc (Phase 9 PAUSED at Commit 3a boundary, stand-down updated)

**New finding surfaced during Bidirectional `federation_nodes` Commit 4 verification 2026-05-21.** Per JOURNAL J-096: wire-order non-determinism in `topological_sort_events` at `xgen-node/src/fanout.rs:193` (Kahn-like algorithm that preserves input order when tie-breaking ready, in-degree-zero events) fed by non-deterministic `HashMap.values()` iteration in `compute_federation_delta_for_space` at `xgen-node/src/fanout.rs:321`. Both `state.space_create` and `state.room_create` are DAG roots with empty `prev_events`; they tie at the top of the topo sort and their wire order tracks HashMap iteration. Sibling function `topological_sort` in `xgen-core/src/node/runtime.rs:859-912` (used for in-process ordering, separate code path) uses Kahn's algorithm with explicit `queue_vec.sort()` for stable tie-breaking; the xgen-node-side delta function does not.

**Evidence from instrumented diagnostic.** 105 `dispatch_event` calls on B in **both** pass and fail runs (the bidirectional fix's drain hook is exercised correctly in both; divergence is entirely upstream of the dispatcher, in delta serialisation order). In a failing run, `room_create` reaches B's dispatcher at timestamp 35.246931Z **before** `space_create` at 35.247021Z; B's `dispatch_event` Step 1 rejects `room_create` with "space not found"; cascade through the bootstrap chain produces 2 Accepted / 2 Rejected / 101 HeldPending against the passing-run baseline of 102 Accepted / 3 HeldPending. Pass rate ~50% in isolation; raising the per-event wait budget from 120s to 300s did not improve it (3 of 5 runs hit the new 302s ceiling exactly), confirming events truly never drain rather than arriving slowly.

**Bidirectional fix verified NOT implicated.** Six unit tests in `xgen-core/src/space/state.rs::mod tests` (notably `apply_federation_add_two_vantages_mirror`) remain green and stand as the live D-075 regression lock. The bidirectional milestone closed cleanly in four commits per JOURNAL J-096 (see DONE-IN-FLIGHT block below). The new finding is a separate pre-existing bug that the bidirectional milestone's verification surface exposed but did not cause.

**Joe-direction at finding close: option (c) — new audit/design/impl phase per D-071, sibling-in-shape to the bidirectional `federation_nodes` arc just closed.** Same four-step pattern: (1) audit task file (canonical record of finding with code-grounded mechanism at file:line granularity, candidate fix shapes with cost/benefit and Pass-1 impact assessment, Phase-9 + milestone implications, discipline notes); (2) design walkthrough with Joe-locks on fix shape; (3) Clair-facing implementation runbook with sequenced atomic commits; (4) milestone close lifting Phase 9 Scenario 1's `#[ignore]` and closing this phase.

**Phase 9 Scenario 1 re-stood-down 2026-05-21.** Updated stand-down comment in `xgen-node/src/tests/phase9_two_node_smoke.rs` names the topological-sort finding with verbatim evidence (timestamps + outcome tally), forward-references placeholder `tasks/FEDERATION_TOPOSORT_AUDIT.md` (to be authored by Chat Claude — placeholder pattern acceptable per Commit 3a precedent). Phase 9 Commit 3b (Scenarios 2 + 3 + compound scenarios C2/C3/C5/C7/C9/C10) stays paused inside the same milestone scope until the topological-sort fix lands and Scenario 1 lifts a second time. Milestone scope unchanged.

**Track 1 (Chat Claude): author `tasks/FEDERATION_TOPOSORT_AUDIT.md` from JOURNAL J-096 evidence.** Same shape as `tasks/FEDERATION_BIDIRECTIONAL_NODES_AUDIT.md`: ten sections, code-grounded mechanism at file:line granularity, candidate fix shapes with cost/benefit + Pass-1 impact assessment, Phase-9 + milestone implications, discipline notes. Candidate fix shapes likely include: stable tie-break by `event_id` inside `topological_sort_events`; pre-sort the input vector by `event_id` before topo sort; or move from HashMap to a sort-stable container at the `EventStore` level — each with cost/benefit. The candidate that matches the sibling `xgen-core/src/node/runtime.rs::topological_sort` (Kahn + `queue_vec.sort()`) is the precedent-aligned shape; alternative shapes are legitimate but should be argued explicitly.
...
**M6 (new) Node admin write path** remains 🟡 PENDING behind Federation Event Propagation milestone closure (now extended by the topological-sort phase). M6 Block 4 verb-by-verb walks remain parallel-eligible per the standing arrangement.

**Entry point for Chat Claude's next session: author `tasks/FEDERATION_TOPOSORT_AUDIT.md`.** JOURNAL J-096 is the canonical evidence source. The bidirectional audit at `tasks/FEDERATION_BIDIRECTIONAL_NODES_AUDIT.md` is the structural template (ten sections; same scope-of-investigation; same disciplined cost/benefit framing).

**Entry point for Clair's next session: standby until the topological-sort implementation runbook ships.** Bidirectional fix is closed; the new audit + design phases produce the next Clair-facing runbook. Same shape as the bidirectional arc.
