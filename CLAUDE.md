...
## 🟢 UI component-library / substrate (RP track) — M-RP2.14 ✅ CLOSED (J-419): `number` — the ninth `core` component, a stand-alone atomic `<input type="number">` (numeric free-entry, numeric `bind:value`); same `<input>` root as `textfield` but a distinct component on the value-type discriminator (D-096 held); first non-string/non-boolean registry value — CDP-verified BOTH apps
