...
| | ships | why |
|---|---|---|
| **C-1** `ui/sampler/src/app_sampler.svelte` | **now** | one `entity-panel#unresolved` cell — resolved control + `unasked` + `erased`. **The only surface on which either state can be made to render at all** (both need two clients in the client). Catalogue **427 → 434 PREDICTED**, measured as a transition |
