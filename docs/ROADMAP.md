...
> Version: 1.0  
> Date: May 2026  
> **Last updated**: 2026-05-18 (D-070 promoted to DECISIONS.md — "Two events of equal importance, opposite direction" named protocol principle, with corrected post-audit framing requiring BOTH existence (acceptance + rejection signals) AND envelope-level `event_id` correlation. Moved from Near future to Past; Cross-cutting entry flipped to ✅ settled.)  
> Language: English  
> Author: JozefN  
...
*(none currently active — Federation Event Propagation implementation queued; runbook ready at `tasks/FEDERATION_PROPAGATION_COMPLETION.md`)*

The design phase closed 2026-05-18 with Pass 3. The canonical design doc (`docs/xgen_federation_propagation_design.md` v1.0, ACTIVE) and the implementation runbook (`tasks/FEDERATION_PROPAGATION_COMPLETION.md` v1.0, ACTIVE) are the handoff artefacts. Implementation hasn't started yet — Clair's Phase 1 commit (`sync_complete` + pagination wire shape + four-call-site migration) is what flips Federation Event Propagation implementation from 🟡 PENDING in Near future to 🟢 PLAY here. Until then, this section is empty by design: PLAY means "active work right now," and a runbook-ready milestone is PENDING, not PLAY.
