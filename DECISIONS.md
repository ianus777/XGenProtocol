...
> **Status:** ACTIVE  
> **Last updated:** 2026-05-24 (D-078 promoted — Production-grounded test enumeration. At every test enumeration (named list of test cases that will become a regression lock), the production reject-path inventory MUST be confirmed against current code BEFORE the enumeration is Joe-locked, not retroactively after implementation surfaces drift. Three-instance threshold met (J-099 audit-doc §11 + J-109 survey §2.6 + J-113 survey §2.4 — three independent canonical-document-staleness-at-dependent-milestone-implementation-time recurrences); pattern durable. Sibling-shape to D-077 (bidirectional sustainability at silent-discard sites): D-077 is the meta-layer principle; D-078 is its protocol-test-layer sibling — same "ask the question before locking" discipline at different scopes. Sibling-shape to D-076 v1 → v1.1 promotion at J-097 design close for the family's promotion-from-recurrence-threshold pattern. Application surface: Joe-lock checkpoint #4 of `tasks/FEDERATION_PROPAGATION_PHASE_9_COMMIT_3B_4_IMPL.md` (this commit's runbook). Each of the five Commit 3b-4 test family enumerations (Scenario 4 + C5 + C7 + C9 + C10) gets the same prospective production-code verification Scenario 4 retroactively got at J-113 — the J-113 pattern caught proactively rather than retroactively. Promotion path B.i locked at runbook-authoring session J-114: D-078 promoted in the runbook atom rather than in a separate pre-runbook atom; sibling-shape to D-076 promotion at J-097 design close (six-file atomic). Shipped same-commit as `tasks/FEDERATION_PROPAGATION_PHASE_9_COMMIT_3B_4_IMPL.md` NEW ACTIVE v1.0 + JOURNAL J-114 + CLAUDE.md PLAY block flip + ROADMAP.md v1.28 → v1.29 per D-074 same-commit discipline (fourteenth instance). Previous D-077 content stands authoritative — see body below.) Previous 2026-05-23 update: 2026-05-23 (D-077 promoted — Bidirectional sustainability discipline. At every silent-discard, conditional-mutation, or fallible-operation-with-discard pattern, the sustainability question MUST be asked in both directions: forward-drift (what future callers could bypass this) AND backward-coherence (what current callers depend on this as a feature). Both answered simultaneously before closing any single silent in isolation. Originated at the persistence-amendment sub-amendment milestone re-walk Track 1 (J-107) after the J-105 design phase asked the forward-drift question only at Q1; cross-milestone Phase 7 B3 amendment dependency on the `graph.add_event` silent-discard surfaced at Clair's Commit 2 implementation; Y-lock reverted Q1 from (a).iii.β to (a).iii.α; the bidirectional discipline named at this re-walk is the principle the forward-only frame missed. Sibling-shape to D-076 v1 → v1.1 in-place amendment pattern: principle stated → implementation surfaced gap → amendment makes the missing property explicit. Application surface-driven per D-071, NOT pre-applied retroactively. Candidate D-NNN "ingest path invariant encoding" (flagged at J-105 design doc §8, expanded scope at this re-walk to cover all five `ingest_event` silents + three drain helpers + M6 reject paths + B3 apply_event dependency) is the first scheduled future-walk under D-077's discipline. Shipped same-commit as design doc §3 + §8 amendments + runbook §4 + new §7.8 + JOURNAL J-107 + CLAUDE.md header + ROADMAP.md v1.21 → v1.22 + HANDOFF Status flip per D-074 same-commit discipline (tenth instance). Track-1-while-Clair-active first project instance; allowed because Track 1 here is record-of-already-locked-decision, not decision-input — directional asymmetry from topo-sort J-099/J-100 Track-1-as-decision-input precedent. Previous D-076 v1.1 amendment content stands authoritative — see body below.)  
> _2026-05-22 (D-076 amended in place at Step 2 of the post-J-098 design-phase re-walk — new "Amendment (2026-05-22) — causal-DAG-respecting order as second load-bearing property" subsection inserted between the original "Decision" subsection (v1 lock record, stays authoritative) and the existing "Originating incident" subsection. The amendment names the second load-bearing property of D-076's contract: byte-identical-across-senders determinism alone is not sufficient; the canonical wire order must additionally be causal-DAG-respecting. Splitting into D-076 + D-077 was considered and rejected at J-098 session close: the two properties cannot vary independently (a causal-but-non-deterministic wire format is broken; a deterministic-but-non-causal wire format is broken — exactly what surfaced as the gap), and the no-drift-surface discipline family's one-per-layer shape would have been broken by adding a second wire-format-layer decision. The amendment also names Path B (fix `build_room_create_event` at `xgen-core/src/space/state.rs:797` to set `prev_events: vec![space_id.to_string()]`) as the locked instantiation alongside Commit 2's already-shipped Shape A v1 sort fix; the two fixes layer cleanly (causality first, determinism second; neither sufficient alone). Path B scope locked narrow to `build_room_create_event` only per J-098 session close; sibling event constructors not audited this milestone, may surface later as own audit-precedes-dependent-design arc per D-071. Step 2 of the Shape 2 procedure ships this amendment alongside audit doc §11 + design doc §11 + JOURNAL J-099 + CLAUDE.md Rule 0 addition + CLAUDE.md PLAY block flip + ROADMAP.md v1.14 → v1.15 + Step-2 HANDOFF Status flip + new Step-3 HANDOFF, eight files atomic per D-074. Previous D-076 v1 note stands authoritative as the original lock record; this amendment extends the principle's stated content without invalidating it.)  
> _2026-05-22 (D-076 added — Wire-order determinism is a sender-side normative property for Node-to-Node federation. Two senders with identical Space history MUST produce byte-identical federation deltas modulo signature-bearing fields. Wire ordering is part of the protocol's contract, not implementation latitude. Forward-bound by Q2.γ to Node-to-Client sender output where analogous. Sibling-distinct from D-067 (code-organisation layer) and D-075 (event-model layer); pairs with D-070 (transport-layer correlation pair) as the four-decision no-drift-surface discipline family. Locked at the topological-sort design-phase close 2026-05-22, instantiating Q3.ii + Q2 middle + Q2.γ + Q1 Shape A v1 + sibling Site 1 fix. Shipped same-commit as the design task file at `tasks/FEDERATION_TOPOSORT_DESIGN.md` (ACTIVE v1.0) + audit doc Status flip ACTIVE → COMPLETED v1.0 + cross-doc updates per D-074 same-commit discipline. Implementation runbook authoring is the next-active step. Previous D-075 note (state.federation_add as one party's act) stands authoritative in the body below.)_  
