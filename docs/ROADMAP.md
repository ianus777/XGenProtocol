...
│   │   ↳ trigger: **C-bis-1 SHIPPED at `e0d4d9a` and re-driven at J-705 — every one of Clair's numbers held**: stream-panel is a flex column with an empty above socket, MessageStream stays unconditionally mounted and the quiescent registry is unmoved at 164. All three defects the re-drive found were in the runbook Chat wrote, not in the work: layout-default.ts listed on a leg that cannot touch it, 174 stated as a registry floor when the selected-room count is Space-dependent, and a flat class selector that reads the first of eight tile bodies. Runbook is v1.1. Next is C-bis-2, the dmDraft store and the click. The RMC context menu needs its own design pass and has no runbook
