...
> Date: May 2026  
> **Status:** ACTIVE  
> **Last updated:** 2026-05-20 (Phase 7.5 design-phase walkthrough closed. All four framework decisions P7.5-A through P7.5-D `[JOE-LOCK: locked 2026-05-19]` at `tasks/FEDERATION_PROPAGATION_PHASE_7_5_DESIGN.md` (now ACTIVE v1.0). ROADMAP.md updated with XGID concept added to Near future, sequenced between Phase 7.5 closure and Phase 9 Commit 3 resumption. Remaining design-phase DoD items: canonical design doc §6.4.1 + §15 row, this PLAY block update, implementation runbook authoring, ROADMAP refresh, design task file flip to COMPLETED. Implementation runbook is Clair's pickup after this session. Phase 9 status PAUSED unchanged at Commit 3 boundary; J-092 Commits 1+2 stay shipped. Test count unchanged at 519.)  
> Author: JozefN  
> Credits: Concept, philosophy, requirements, project direction: Jozef Nižnanský. Technical assistance and implementation support: AI-assisted development tools.  

---

## 🟢 PLAY — Phase 7.5 Federation Cold-Start Bootstrap implementation runbook authoring (Phase 9 paused at Commit 3 boundary)

**Phase 7.5 design phase CLOSED 2026-05-19 (J-093 walkthrough).** All four framework decisions `[JOE-LOCK: locked 2026-05-19]` at [tasks/FEDERATION_PROPAGATION_PHASE_7_5_DESIGN.md](tasks/FEDERATION_PROPAGATION_PHASE_7_5_DESIGN.md) (Status: ACTIVE, v1.0). Failure-mode catalogue M5 ("F-3 brand-new federation bootstrap dead-locked") closes structurally once Phase 7.5 implementation ships.

**Four locks (one-liner form for orientation; full text at design task file §5–§8):**
- **P7.5-A** — Narrow F-3 + F-4-step-1 skip rule for `state.space_create` and `state.dm_space_create` EventTypes (sibling to Phase 7 B1's skip for `state.federation_add`). Authority preserved via signature verification + F-10 HeldPending for unknown-signer.
- **P7.5-B** — Third HeldPending trigger condition: "missing federation relationship for (peer, space)". Resolves on `state.federation_add` arrival hook via new `drain_pending_by_federation_relationship`. New error code `4007 federation_relationship_timeout`. Combines with F-10 (Identity) + F-4a (predecessor) triggers via existing struct-variant Option fields. Held-not-bypassed posture.
- **P7.5-C** — Per-trigger timeout: predecessor + Identity remain at 30s; federation-relationship defaults to 120s with new `[sync].federation_relationship_timeout_seconds` config field. Brings forward F-10a's anticipated v2 evolution path.
- **P7.5-D** — Observability: new `pending_federation_relationship: usize` counter in `xgen-node_state.json` (sibling to Phase 6's `pending_identity_replication`). Plus the design-phase finding that introducer-peer attribution requires a new `SpaceLocalMetadata` structure in `xgen-common` with `introducer_node_id` field (name locked through any future XGID-typing pass — field name carries the role, type carries the contract).

**Entry point for next session: implementation runbook authoring.** New task file `tasks/FEDERATION_PROPAGATION_PHASE_7_5_IMPL.md` (Status: ACTIVE, v1.0) per the five-commit sequence sketched in design task file §9.1: doc-pass commit, F-4 step 1 + F-3 skip implementation, HeldPending third trigger implementation (data structure extension + arrival hook + drain helper + config field + error code 4007 + observability counter + new `SpaceLocalMetadata` structure), NodeRuntime-level integration tests, Phase 7.5 close commit. Then Clair picks up implementation.

**Sequence after runbook authored:** Clair implements (5 atomic commits) → Phase 7.5 ships and milestone-internal status flips to closed → **XGID concept work** (Near future first-in-queue per ROADMAP.md; do NOT implement XGID during Phase 7.5 implementation) → Phase 9 resumes from Commit 3 with Scenario 1 working as originally designed → Phase 9 ships → milestone flips PLAY → DONE → M6 (new) unblocks.

**Phase 9 status: PAUSED at Commit 3 boundary, unchanged.** [tasks/FEDERATION_PROPAGATION_PHASE_9.md](tasks/FEDERATION_PROPAGATION_PHASE_9.md) Status: ACTIVE v1.0; scope intact, only dependency order changes. J-092's Commits 1 + 2 (observability preconditions G1+G2+G3 + `#[serial_test::serial]` flake fixes) **stay shipped** — pure infrastructure, neither depends on the broken cold-start path.

**Why D-071 ("subsystem audits precede dependent milestones") still won.** The Phase 9 survey (J-090) catalogued M5 explicitly; the design phase locked it as a known gap; Phase 9 Commit 3's harness setup walked into it exactly where predicted. The discipline extends one level finer: "design gaps surface during dependent work and close before the dependent work proceeds." Phase 7.5 is the formal recognition of that finer-grained pattern.
