...
> Date: May 2026  
> **Status:** ACTIVE  
> **Last updated:** 2026-05-21 (Bidirectional `federation_nodes` design phase CLOSED same-day as the audit doc shipped, with Joe-locks on Q1 (Reading (i) — `state.federation_add` is one party's act, `federation_nodes` is a vantage-aware derived projection), Shape A (origin-aware applier, wire format unchanged), and sub-option A.1 (re-derive on load — native fit confirmed against verified `SpaceState` non-persistence model). Design task file shipped at `tasks/FEDERATION_BIDIRECTIONAL_NODES_DESIGN.md` Status: ACTIVE v1.0 ~30 KB. Implementation runbook shipped at `tasks/FEDERATION_BIDIRECTIONAL_NODES_IMPL.md` Status: ACTIVE v1.0 ~24 KB, four-commit sequence for Clair: (1) doc-pass + audit + design Status flips; (2) origin-aware applier + apply-pipeline plumbing + unit tests; (3) Phase 9 Scenario 1 `#[ignore]` lift (scenario becomes activating regression lock); (4) milestone close (CLAUDE.md + ROADMAP.md + JOURNAL.md per D-074). D-075 promoted to DECISIONS.md (`state.federation_add` is one party's act; `federation_nodes` is a vantage-aware derived projection — sibling-distinct from D-070's transport-layer "two events" principle; the disciplines operate at different layers). Verification check performed at design close against `xgen-core/src/node/runtime.rs`: `SpaceState` is not persisted anywhere; `NodeRuntime::new()` initialises `spaces: HashMap::new()`; rebuilt via `apply_event` on cold start. A.1 (re-derive on load) is the existing model, not a deviation. Fix scope: applier change (`apply_federation_add` gains `my_node_id: &str` parameter at `xgen-core/src/space/state.rs:351-363` with verbatim code-comment block citing design §3.3), apply-pipeline plumbing (`SpaceState::apply_event` threads `my_node_id`), two call-site updates in `NodeRuntime::ingest_event` (`xgen-core/src/node/runtime.rs:189` + `:197`), unit tests covering both vantage branches + mirror property + DM constraint, Phase 9 Scenario 1 resurrection, doc-pass at canonical design doc §6.4.2 + §15 row. Wire-format-neutral. Pass-1-neutral. Per D-069 canonical-document + D-071 audit-precedes-dependent-design + D-074 milestone-close-includes-JOURNAL + "honest longer work over fast shortcut" discipline. ROADMAP.md v1.9 → v1.10 in same commit per same-commit discipline. Audit doc Status flip ACTIVE → COMPLETED queued for Commit 1 of the implementation runbook (it flips when the design phase formally closes by the runbook's Commit 1 doc-pass shipping). Phase 9 stays PAUSED at Commit 3a boundary; resumes from Commit 3b after the bidirectional fix lands. M6 (new) blocking chain extended by one node but unchanged in shape. Pass 1 of XGID Retrofit stays at Status: ACTIVE v2.0 unchanged.)  
> Author: JozefN  
> Credits: Concept, philosophy, requirements, project direction: Jozef Nižnanský. Technical assistance and implementation support: AI-assisted development tools.  

---

## 🟢 PLAY — Bidirectional `federation_nodes` fix DESIGN + IMPL RUNBOOK shipped 2026-05-21; Clair's pickup as four-commit sequence (Phase 9 PAUSED at Commit 3a)

**Design phase closed 2026-05-21.** `tasks/FEDERATION_BIDIRECTIONAL_NODES_DESIGN.md` (Status: ACTIVE v1.0) ships three Joe-locks: **Q1 Reading (i)** — `state.federation_add` is one event recording one party's act ("A declares A federates with B for Space S"); `federation_nodes` is a derived projection with vantage-aware applier logic. **Shape A** — origin-aware applier; wire format unchanged; `apply_federation_add` gains `my_node_id` parameter and derives the OTHER party from this Node's view (event.sender if I am content.node_id, else content.node_id). **Sub-option A.1** — re-derive on load; native fit confirmed against verified `SpaceState` non-persistence model. Reading (ii) (two events per relationship) considered and rejected on precedent grounds (would have been first event family in registry requiring two signed assertions for semantic completeness); Shape D (extend wire format) considered and rejected (D.1 is worst-of-both-worlds duplication; D.2 secretly re-introduces Reading (ii) thinking through schema design and pays Pass-1 cost). Q2 (signature carries enough info to derive receiver's value) code-verified yes in audit §6.2; no further walkthrough needed.

**Implementation runbook shipped 2026-05-21.** `tasks/FEDERATION_BIDIRECTIONAL_NODES_IMPL.md` (Status: ACTIVE v1.0) is Clair's pickup. Four atomic commits: (1) **doc-pass** — canonical design doc §6.4.2 sibling subsection added (sibling to Phase 7.5's §6.4.1), §15 Implementation Complete row added with `[PENDING]` journal placeholder; design task file Status ACTIVE → COMPLETED; audit doc Status ACTIVE → COMPLETED; test count 571 → 571 (no code). (2) **origin-aware applier + plumbing** — `apply_federation_add` in `xgen-core/src/space/state.rs:351-363` signature change with verbatim code-comment block citing design §3.3; `SpaceState::apply_event` threads `my_node_id: &str`; both `NodeRuntime::ingest_event` call sites updated (`xgen-core/src/node/runtime.rs:189` + `:197` pass `&self.node_id`); other call sites surfaced during compile updated; six suggested unit tests covering both vantage branches + mirror property (the regression lock at unit level) + DM constraint + missing field. (3) **Phase 9 Scenario 1 resurrection** — `#[ignore]` removed from `xgen-node/src/tests/phase9_two_node_smoke.rs`; scenario passes in isolation and as part of full workspace; becomes activating regression lock at integration level. (4) **milestone close** — CLAUDE.md PLAY block flip (to Phase 9 Commit 3b next-active), ROADMAP.md Past entry + Present + Visual structure tree updates, design doc §15 journal-number placeholder updated, JOURNAL.md entry written with actual test counts per D-074.

**D-075 promoted to DECISIONS.md.** `state.federation_add` is one party's act; `federation_nodes` is a vantage-aware derived projection. Distinct-but-sibling from D-070's transport-layer "two events" principle (D-070 is acceptance vs. rejection signal pair on `TransportMessage`; D-075 is event-model semantics at the protocol-event layer). The disciplines operate at different layers and address different questions. D-075 also commits the protocol's general approach: every relationship-shaped event records one party's act; the resulting data object is a derived projection; vantage-aware appliers are the legitimate pattern when the event has a sender-vs-other-party asymmetry. Future events that genuinely cannot be modelled this way require an explicit future decision to depart from D-075, not a silent precedent break.

**Wire-format-neutral. Pass-1-neutral.** Per audit §7.5 summary table: Shape A is Pass-1-neutral. `tasks/XGID_RETROFIT_PASS_1_IMPL.md` stays at Status: ACTIVE v2.0 unchanged. The applier-signature change adds a parameter typed as `&str` at v1, which Pass 1 may want to retype to `&NodeXgid` in the appropriate Pass — but that's normal scope for whichever Retrofit Pass touches the apply pipeline, not a coordination obligation on this phase.

**Phase 9 still PAUSED at Commit 3a boundary.** Scenario 1 stays `#[ignore]`-annotated on disk as the regression witness until Commit 3 of the implementation runbook lifts the annotation. Phase 9 Commit 3b (Scenarios 2 + 3 + compound scenarios C2/C3/C5/C7/C9/C10) stays paused inside the same milestone scope until the bidirectional fix lands. Milestone scope unchanged.

**Track 1 (Clair): bidirectional `federation_nodes` implementation runbook is next-active.** Entry point for next Clair session is `tasks/FEDERATION_BIDIRECTIONAL_NODES_IMPL.md`. The runbook is self-contained; reading order specified in §1. Design task file is referenced for "what" and "why"; runbook is authoritative for "how" and "in what order." Latitude reminder applies per §1: implementation-internal decisions (parameter naming, threading shape, internal function signatures, test helper shapes) are Clair's latitude; wire-format-visible decisions require Joe-lock.

**Track 2 (Chat Claude + Joe): standby.** Design phase work for the bidirectional fix is closed. Next chat-side work is whatever surfaces during Clair's implementation (B3-shape question may or may not arise; if it does, the precedent at `tasks/FEDERATION_PROPAGATION_PHASE_7_B3_AMENDMENT.md` is the template). M6 (new) Block 4 verb-by-verb walks remain parallel-eligible per the standing arrangement; the two JOURNAL gap retrospectives remain small follow-ups.

**M6 (new) Node admin write path** remains 🟡 PENDING behind Federation Event Propagation milestone closure (which itself waits behind the bidirectional fix landing AND Phase 9 Commit 3b completing). M6 Block 4 verb-by-verb walks remain parallel-eligible.

**Entry point for Chat Claude's next session: standby.** Design phase work is closed; the bidirectional fix is now Clair's pickup. If Chat Claude session opens before Clair ships, the appropriate move is to wait — design walkthrough is done; no further chat-side authoring is needed until the runbook completes (or until something surfaces during implementation that needs a Joe-lock or design clarification).

**Entry point for Clair's next session: `tasks/FEDERATION_BIDIRECTIONAL_NODES_IMPL.md`.** Read the runbook (§1 §2 first, then per-commit work). Design task file at `tasks/FEDERATION_BIDIRECTIONAL_NODES_DESIGN.md` is authoritative on the three locks (read §3 before touching code). Audit doc at `tasks/FEDERATION_BIDIRECTIONAL_NODES_AUDIT.md` §3 grounds the mechanism in code at file:line granularity. Four atomic commits per the runbook; each ships with verification. D-074 discipline: milestone-close commit (Commit 4) MUST include JOURNAL.md.
