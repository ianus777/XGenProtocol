...
│   │   ├── 🟡 **Leg C reconnect rule** ↳ trigger: **fired J-658 — Joe ruled §5's R1** · **none outstanding; the leg is openable and needs a runbook** — ⚠️ **a DIFFERENT §5 from the runbook's, which is closed; the two must not be collapsed**. ⚠️ *v6.34–v6.45 read `↳ trigger: §5 of the MILESTONE ruled (Joe)`; **the trigger has FIRED and a trigger that has fired is a defect**, so it moves in the same commit. Superseded, kept not erased (`D-131`)*
│   │   │   ↳ 🔒 **ONLY THE R1 HALF WAS RULED — §5's R4 (the STREAM's sync-from-cursor replay) IS STILL 🔓 OPEN AND IS IN NO LEG.** Deliberately not bundled: **panels need current STATE ⇒ re-fill; the stream needs the missed EVENTS ⇒ replay** (§5a). ⚠️ **Nothing in Leg C may assume R4**, and its honest cost is named — **the GUI client persists no sync cursor**, so `since=""` means full replay
│   │   │   ↳ 🛑 **AND THIS LEG SERVES TWO MILESTONES — IT *IS* `M-RP-IDENTITY-RESOLUTION`'s LEG E.** Both were filed as *an `$effect` on `selfState.connection`* in **the same file**. 🔒 **THIS MILESTONE OWNS THE BUILD** (§5 is its decision); **that Leg E consumes and verifies.** ⚠️ *Two seats writing one `$effect` from two runbooks is the one-writer-per-file-per-atom breach — named here rather than discovered at implementation*
...
│   │   ↳ 🔒 **§4 ④ RENDERS DIMMED, ITS OWN SELECTOR** · 🔒 **§5 ③ IS HIDDEN** · 🔒 **§5 COUNT = C1, count the roster and accept the mismatch** — all three Joe, 2026-08-01. ⚠️ **`ui/assets/skin.css` values are HIS; the milestone adds only the hook** — 🔒 **NARROWED AT J-654 FOR LEG C ONLY:** Joe delegated **the two Leg C selectors** to Chat as a carve-out (*"those are small elements that are not worthy obvious workflow"*) while **the file and the standing rule remain his.** ***What moved is two selectors, not the file.*** Kept not erased (`D-131`)
...
│   │   │   ↳ ⚠️ **B-3 LEG B MUST WIRE `outcome.fill` INTO THE STORE FIRST — `app_client.svelte:183` DISCARDS it**, so Leg A's ids reach the webview and stop. `setResult`'s signature changes and the fill becomes a second concern in the store
...
│   │   │   ↳ 🛑 **THE +8 IS THE FINDING: `2 + 2N`, NOT `1 + 2N`.** `entity-panel.svelte:168` wraps a `<Section>` and `section.svelte:69` **self-registers** ⇒ the panel costs TWO (`entity-panel#unresolved` **and** `section#unresolved__section`). ⚠️ *v6.42 carried **434 PREDICTED**; the runbook bound the leg to treat a mismatch as a FINDING and it was one — **Chat's grounding extrapolated a comment about `entity-item` CELLS onto `entity-panel`.** Superseded, kept not erased (`D-131`)*
...
│   │   │   ↳ 🔒 **THE VALUES ARE DELEGATED TO CHAT FOR THIS LEG ONLY (Joe, J-654) — `ui/assets/skin.css` REMAINS HIS AS A STANDING RULE.** *"normally i have skin.css, this rule still stays, especially when we build complex components. but those are small elements that are not worthy obvious workflow."* **He re-tunes any of it in Notepad++ at any time, no milestone, no runbook.** ⚠️ *Chat first stated this as owning "the Leg C default values" and proposed annotating five documents — **wider than what was given**; superseded before anything was written (`D-131`).* 🔑 ***A delegation accepted more broadly than it was given is how a seat quietly moves***
│   │   │   ↳ 🛑 **THE FIXTURE USES XGID TAILS, NOT NAMES — AND AT REAL LENGTH (43 chars), WHICH IS LOAD-BEARING** — with no book record `toDescriptor` already falls back to `tail()`, so **a real ③ row has NO display name to strike** (§5b). ***A human-named fixture would tune a case the product cannot produce.*** 🔑 **43, NOT 44, AND IT IS SOURCED:** an Ed25519 pubkey is **32 raw bytes** (`handshake.rs:405` encodes `vk.as_bytes()`) through **base64url NO padding** (`crypto/encoding.rs`, spec 3.1.9) ⇒ 32 B → **43** chars. ✅ **Measured at 43: `textLen 51`, `scrollWidth 383` vs `clientWidth 250` ⇒ CLIPPED.** ⚠️ *v6.43 read "44 chars → scrollWidth 391" — **a hardened hedge that was one char high**; the first fixture used 16-char tails, which do NOT clip at all. Clair, second read, N3; both superseded, kept not erased (`D-131`).* ⚠️ **And the strikethrough makes §6a's `tail-8` gap MORE visible** — it strikes the constant `ed25519:` head the clip preserves
...
│   │   └── 🟡 **Leg F live verify + records** — two clients, a real join, a real `not_found` ⚠️ **a store driven by hand is a probe that cannot fail** · 🔒 **NOW CARRIES THREE OBLIGATIONS MOVED OUT OF LEG B (Joe, J-653): a real join producing `data-unresolved="unasked"` · a real `not_found` producing the ③ filter AND §5a's E2 exception · a populated roster giving `erasedHidden` something to count** 🛑 **Leg F is the FIRST behaviour verification of this milestone — Legs A and B are compile- and type-verified only** ↳ trigger: Legs A–E land
│   ├── 🟢 **M-RP-XGID-SLOT-RETYPE** — the identifier slots that regressed to `String` after the retrofit arc closed · J-645 (J-658, J-659)
│   │   ↳ 🟢 **PHASE-0 WRITTEN J-659 — `tasks/M_RP_XGID_SLOT_RETYPE.md` v1.0 ACTIVE.** ⚠️ *v6.46 read "Phase-0 not written"; superseded, kept not erased (`D-131`)*
...
│   │   ↳ ⚠️ *SUPERSEDED BY §2b ABOVE — v6.47's finding line, kept not erased (`D-131`). The OBSERVATION is right and the CONCLUSION is wrong:* `admin_ops.rs` holds **63 of the 130** and they are **CLI-arg and admin-pipe result structs**, with `admin_ops.rs:977` already carrying a documented projection (*"Project a wire-format Identity URI to the typed key at the registry boundary"*). 🛑 **The EXACT INVERSE of the case that minted `D-136`, where the downgrade *"reads as a deliberate seam and is not one"*.** ⚠️ *⇒ "`admin_ops.rs` is not a file that broke the rule; it is a file the rule does not reach" — **the second half is FALSE: Pass 3 §4.3 and Pass 4 §4.3 α reach it by name.***
...
│   │   ↳ trigger: **none outstanding — Phase-0 v1.0 ACTIVE, Leg 0 in progress (the hand-read).** ⚠️ *v6.46 read `↳ trigger: fired J-658 — §7 ruled, so Leg D is live work · none outstanding; NEXT to open. Phase-0 not written`; **the Phase-0 was written at J-659**. Superseded, kept not erased (`D-131`)*
│   │   ↳ 🔒 **THE FORK IS TAKEN, NOT LEFT OPEN — IT LANDS FIRST, STANDALONE (Chat under `D-123`, J-658; Joe reverses on one line).** ⚠️ *v6.32–v6.45 offered "**before Leg D**, or Leg D absorbs it"; superseded, kept not erased (`D-131`).* Three reasons, the third decisive: ① **`D-071`** — subsystem audits precede dependent milestones · ② Leg D's new command **carries an `identity_id` slot**, so retyping afterwards means touching it **twice** · ③ 🔑 **it is a FILED MILESTONE WITH ITS OWN ID, and folding it into a leg makes it a RIDER** — *"its own milestone, never a rider"* is a standing refusal here (`mergeClasses` · `M-RP-ROVING` · the `dialog` footer slot). ⚠️ **Both move cargo**, so absorbing it also makes the delta unattributable **within** the floor
