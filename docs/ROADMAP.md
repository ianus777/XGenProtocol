...
│   │   └── 🟡 **Leg E records + close**
│   │       ↳ trigger: Leg D lands
│   ├── 🟢 **M-RP-SELECT-ORIENT** — the panels keep saying where you are: R1 and R2 highlight from the latch, not the bus · Phase-0 `tasks/M_RP_SELECT_ORIENT_PHASE0.md` v1.2 · runbook `tasks/RUNBOOK_SELECT_ORIENT.md` v1.3 ACTIVE · `DECISIONS.md` D-146 + D-147 · supersedes `M-RP6.2` D4 opt-1 → opt-2 · J-696 (J-692, J-693, J-694, J-695)
...
│   │   ↳ trigger: **RUNBOOK LOCKED v1.3 ACTIVE (Joe, all fifteen points L-1–L-15)** — Clair implements C-1→C-4, then Chat re-drives every gate
