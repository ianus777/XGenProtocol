...
│   │   ↳ trigger: **C-bis-3 + C-bis-4 SHIPPED (`8601e677`, `37c09d7`) and re-driven at J-707/J-708 — the draft is now a REAL DM**: the composer routes its text through dmDraft so a private draft and a room never share one buffer, the draft branch sits above the early return, and the send sequence runs create then a spaces refresh then latch, send and clear. The refresh is a v1.6 amendment to section 5.2, which as written resolved the latch to null and landed the user on Select a room with the message nowhere. Both live gates are driven: the failure path keeps the draft and its text and writes nothing, and the happy path minted one real DM with Joe consenting per gate. Joe then found two defects in a screenshot that the store reads had called green, and they are now C-bis-6, where R1 unselects while you are in a DM, and C-bis-7, where R7 reads the counterpart from the Space record so a DM shows self and counterpart. Next is C-bis-5, the verification leg that also owes the OWED-4 measurement shown to Joe. The RMC context menu still needs its own design pass and has no runbook
