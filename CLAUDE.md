...
## 🟢 PLAY — M6 (new): protocol-audit-log arc — **Commit 2 (reader `space audit-events`) SHIPPED at J-167**; writer Commit 1 SHIPPED at J-166; **next-active = Clair Commit 3 (rebuild `space audit-rebuild`, PAL-D3) per runbook §6 + Joe-lock checkpoint #3; then Commit 4 close; then arcs 2–4 (federation-admin-control next), then M7 `--aicontrol`**
...
**Entry point for next session:** read this PLAY block + JOURNAL J-167 first per Rule 0. Commits 1 (writer) + 2 (reader) are SHIPPED; next is **Clair Commit 3 (rebuild `space audit-rebuild`, PAL-D3)** at `tasks/M6_PROTOCOL_AUDIT_LOG_IMPL.md` §6 — replay each in-scope Space's persisted events, append entries for the 11 types whose `event_id` isn't already logged (dedup → idempotent), closing PAL-D2 gaps + cold-start backfill; operator-invoked only. **Joe-lock checkpoint #3** fires there (scope one/all · dedup approach · no startup-reconcile in v1). Read `xgen-node/src/protocol_audit.rs` (writer + `read_all_entries`) + `app.rs::replay_spaces_from_dir`/`persist_event` first. Then Commit 4 close → flips runbook COMPLETED + PLAY → federation-admin-control.
