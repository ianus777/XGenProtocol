...
> Version: 1.0  
> Date: May 2026  
> **Last updated**: 2026-05-19 (XGID concept added to Near future as first item in queue during Phase 7.5 §5.3 design walkthrough. Sequenced AFTER Phase 7.5 closes (so 7.5 design phase is not interrupted) and BEFORE Phase 9 Commit 3 resumes (so Phase 9 test code, trace events, and observability fields use XGID from the start rather than being retrofitted). Parent-type surface scoped to `Primitive` + `SignedPrimitive` in `xgen-common` plus the two Ed25519-pubkey ID surfaces — ~3 touch points to type the whole protocol. No code changes; 519 tests unchanged.)  
> Language: English  
> Author: JozefN  
