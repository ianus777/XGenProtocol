...
│   │   ↳ trigger: **J-692 RULED by Joe 2026-08-08 (J-702) — option B, the draft view**: a click on a member with no existing DM opens a Discord-like draft (empty stream + prepared page, live composer, DM Space materialised on first send), by LMC not the context menu. Leg C-bis is unblocked and needs its Phase-0 before any code; the dead control stands until that leg ships. The RMC context menu needs its own design pass and has no runbook
