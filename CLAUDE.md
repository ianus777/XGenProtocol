...
## 🟢 Appendix F/I audit-against-code OPENED — AF sub-pass ✅ (J-397): Appendix F reconciled to the as-built CLI; `federate` reframed `N/A — only node concept` (+ ch2 note); §F.2.1 node-admin cross-ref. Next-active = AI sub-pass (Appendix I ↔ data structures + event catalog). (doc-opt phase COMPLETE J-396)
