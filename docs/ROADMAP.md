...
│   ├── 🟡 **M-SPACE-ADMISSION** — who may join a Space, and how a leaver comes back · FILED at J-741 on Joe's structural ruling (*the rejoin mechanism must be space general*) · protocol + node, Space-general, NOT a UI leg · no Phase-0
...
│   │   ↳ Owes: KEY — **ADMISSION BELONGS ON THE SPACE AND THERE IS ALREADY A SLOT: auth_tier IS an admission contract on SpaceState** (a tier floor checked at join), so this adds a second dimension to an object that already carries one rather than inventing a place · **no admission field exists today** — join_policy, open_join, discoverable and is_public all return 0, and space_visibility's 3 hits are TEST FUNCTION NAMES for member_temperature_visibility, checked rather than assumed · **it closes the origin-scoped third-party hole as a side-effect**, since an invite-required Space refuses a third party by the admission rule itself rather than by F-3, which runs only on the federation channel
│   │   ↳ Owes: LOCKED by Joe 2026-08-16 — **OWNER-SETTABLE, not set-once.** KEY — **the discriminator is why the set-once precedents are set-once**: jurisdiction and e2e_encryption both RE-LABEL DATA THAT ALREADY EXISTS, so they falsify the past; **admission is evaluated ONCE at a join and never re-evaluated**, so opening admits nobody retroactively and closing ejects nobody — there is no past for a change to falsify and the set-once argument does not transfer. The right sibling is member_temperature_visibility, whose event type, applier arm and resolution arm already exist · STOP: **set-once would mean a Space under abuse could never be locked** · **RIDER 1 — the DM PINS the value (invite-required, not settable there)**, or the stranger who owns a DM could flip their own DM open · **RIDER 2 — gate on the ROLE PREDICATE, never on owner_id equality**, which would add a THIRD site to a split-authority problem that currently has exactly two (state.rs:733 and :753) · a new event type is three hand edits, not one line (enum, as_str, from_str) · **default MUST stay open** or J-275's model breaks for every existing Space
│   │   ↳ Owes: OPEN and Joe: **the D's number and wording** · **whether the ratchet variant is wanted** (open to invite permitted, invite to open refused — filed as a partition member, NOT recommended) · WARNING: **F-2 is a SOURCE TRACE and not a measurement** — whether xgen-node routes client submissions as LocallySubmitted is UNVERIFIED, and until that leg closes the third-party-join hole must not be recorded as a measured security finding · **it inherits the read-path cost of M-INTRO-POLICY Q11c**: a leaver needs the other party's consent to see the record again, so cutting an abusive stranger means the abuser controls access to the evidence
│   │   ↳ trigger: none — filed, not scheduled
