...
- **🔒 ANNOUNCE THE RUN AND ASK FOR HANDS OFF — NEW AT J-653.** When Chat launches an app in order to MEASURE, it **says so before launching and says when it is done**, and the window is **not to be touched in between**. The registry breathes with **quiescence (N-105) · store contents (N-108) · selection (N-112) · saved-state count (N-115)** — so a click, an open menu, or a room switch **during** a run changes the number being measured, and **the reading looks perfectly ordinary afterwards**. ⚠️ **N-123 is this rule's mirror and it already bit Joe once:** an instrument left an inline override behind and *he* reported it as a bug in his own CSS. ***An instrument that can be disturbed without saying so lies quietly in both directions — it can mislead the measurer, and it can mislead the person whose desktop it is running on.*** 📌 **Also applies to the launch itself: two Tauri windows appearing unannounced on someone's screen is the tool taking custody without asking (`D-132`).**
