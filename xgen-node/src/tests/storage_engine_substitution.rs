...
fn engine_backed_space_survives_restart_via_engine_replay() {
    ...
    let space_id_str: String;
    {
        ...
        let space_ev = sign_event(
            build_space_create_event(&alice, "s", None, 1, "node", None, false),
            &alice,
        );
...
fn two_spaces_are_isolated_in_separate_db_files() {
    ...
    let s1 = sign_event(build_space_create_event(&alice, "s1", None, 1, "node", None, false), &alice);
    let s2 = sign_event(build_space_create_event(&alice, "s2", None, 1, "node", None, false), &alice);
    rt.ingest_event(s1);
    rt.ingest_event(s2);
