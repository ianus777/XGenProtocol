...
## 🟢 PLAY — M6 (new): **federation-admin-control 2a — Commit 1 (FederationState model + FAC-D2 migration) SHIPPED at J-174**; **next-active = Clair Commit 2 (require_approval flag + pending-request queue store) per runbook §4**; then pause-point → verbs → close; then 2b (policy) → auth-module-registry → bootstrap-client; then M7 `--aicontrol`
