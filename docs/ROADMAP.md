...
> Version: 1.7  
> Date: May 2026  
> **Last updated**: 2026-05-21 (drift correction — Present section + live-frontier list. The XGID Retrofit Pass 1 runbook authoring track shipped to Status: ACTIVE v2.0 earlier today (all four sub-questions Joe-locked; walkthrough sections populated) but the Present section's "🟢 Pass 1 runbook authoring" entry and the "How to use this view" live-frontier line both still described it as actively playing. Drift caught by Clair during her Phase 9 pre-session reconnaissance; surfaced via Joe to Chat Claude. Three edits in this update: (1) Present-section entry for Pass 1 runbook authoring removed (the work is shipped, not currently playing); (2) new Past-section entry recording the runbook authoring close ("Pass 1 runbook shipped 2026-05-21—all sub-questions locked, walkthrough populated—awaiting Phase 9 closure for Clair handoff"); (3) "How to use this view" live-frontier sentence updated: parallel-eligible items now read "Phase 9 Commit 3 onwards (Clair); M6 (new) Block 4 verb-by-verb walks (Chat Claude + Joe); the two JOURNAL gap retrospectives (Chat Claude small follow-ups)" — removing the runbook authoring item which is no longer live. Header version bumped 1.6 → 1.7. CLAUDE.md gets the sibling drift correction in the same commit per same-commit discipline (PLAY block content rewritten to reflect Chat Claude's next-active slot is open with parallel-eligible work options). Worked instance of the drift-class lesson the D-074 promotion principle hints at but doesn't directly name: same-commit discipline must touch PLAY block content not just header text — when a previous header bump touches only the Last-updated field and the body content describing active work is now stale, the body content has silently drifted. The pattern is named here for the next-available JOURNAL entry to capture. Previous v1.6 D-074 promotion content stands authoritative — see Cross-cutting principles tree + Discipline / JOURNAL hygiene cluster.)  
> Language: English  
> Author: JozefN  
...
- **What's the live frontier?** Three items are parallel-eligible and ready to pick up: Phase 9 Commit 3 onwards (Clair); M6 (new) Block 4 verb-by-verb walks (Chat Claude + Joe); the two JOURNAL gap retrospectives (Chat Claude small follow-ups). The Pass 1 runbook authoring slot previously shown here shipped 2026-05-21 (runbook now at Status: ACTIVE v2.0); Pass 1 implementation remains Clair's pickup post-Phase-9 closure. Everything else is either done, waiting on one of those, or further out.
