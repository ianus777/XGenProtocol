...
> Version: 1.0  
> Date: May 2026  
> **Last updated**: 2026-05-19 (J-091 — Federation Event Propagation Phase 9 survey findings LOCKED across all four §8 open questions: Q1 → 12 scenarios (6 baseline + 6 compounds C2/C3/C5/C7/C9/C10); Q2 → defer G4 audit log to M6; Q3 → `#[serial_test::serial]` first, escalate on Phase 9 signal only; Q4 → multi-commit Phase 9 (~7 commits). Phase 9 implementation task file `tasks/FEDERATION_PROPAGATION_PHASE_9.md` authored (ACTIVE v1.0) — 7-commit sequence locked: observability preconditions (G1+G2+G3) → flake fixes → baseline deployment scenarios 1-3 → baseline scenarios 5+6+B1 → compound deployment C2+C3 → compound NodeRuntime (4, C5, C7, C9, C10) → milestone close. Federation-stress follow-on stub `tasks/FEDERATION_STRESS_FOLLOWON.md` created (PENDING) — deferred compounds C1/C4/C6/C8 + clock-injection seam. Findings doc flipped Status PENDING → COMPLETED v1.1. No code changes; 519 tests unchanged. Present reframed to LOCKED + awaiting-Clair-pickup; Phase 9 survey entry added to Past; federation-stress follow-on added to Far future.)  
> Language: English  
> Author: JozefN  
...
🟢 **Federation Event Propagation implementation — Phases 1-8 ✅ SHIPPED; Phase 9 LOCKED, implementation task file ACTIVE, awaiting Clair pickup.** The nine-phase runbook at `tasks/FEDERATION_PROPAGATION_COMPLETION.md` (Status: ACTIVE, v1.0) drives this milestone. Phases 1-7 closed in J-082..J-088; Phase 8 documentation pass closed in J-089 (519 tests at Phase 8 close). **Phase 9 LOCKED in J-091 (2026-05-19).** Survey findings (`tasks/FEDERATION_PROPAGATION_PHASE_9_SURVEY_FINDINGS.md` COMPLETED v1.1) Joe-locked across all four §8 open questions; Phase 9 implementation task file authored at `tasks/FEDERATION_PROPAGATION_PHASE_9.md` (Status: ACTIVE, v1.0); federation-stress follow-on stub created at `tasks/FEDERATION_STRESS_FOLLOWON.md` (PENDING). Scope locked at **12 scenarios** — 6 baseline (push smoke, anti-transitivity, drop-and-recover, validation-asymmetry regression, unknown-signer first-contact, federation-relationship rejection) + 6 compounds (C2 anti-transitivity at queue depth, C3 F-3 during F-1a recovery, C5 validation asymmetry under load, C7 pagination at boundary, C9 F-3 drain-time hazard, C10 identity-replicate hook under contention). Four compounds (C1, C4, C6, C8) deferred to federation-stress follow-on, blocked on clock-injection seam. **Sequence: 7 atomic commits** — (1) observability preconditions G1+G2+G3 (state.json peers + stable F-1/F-3 trace events + fan-out trace event), (2) flake fixes `#[serial_test::serial]` (Q3 locked option (c)), (3) baseline deployment scenarios 1-3, (4) baseline 5+6+B1 binary-level, (5) compound deployment C2+C3, (6) compound NodeRuntime (Scenario 4, C5, C7, C9, C10), (7) milestone close flipping CLAUDE.md + ROADMAP.md + design doc §15 to ✅ DONE. Q2 locked: G4 audit log for F-3 reject + G5 drop-peer affordance defer to M6 (option c). Escalation rule in task file §6: if any new Phase 9 integration test exhibits 127.0.0.1:0 bind race or WS frame-ordering inconsistency under workspace parallelism, STOP and walk back to option (ii) on Q3 (investigate underlying race). **Entry point: `tasks/FEDERATION_PROPAGATION_PHASE_9.md` Commit 1 (observability preconditions).** After Phase 9 ships, milestone flips PLAY → DONE and M6 (new) unblocks.
