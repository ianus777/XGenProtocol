...
| **E** | **THE REJOIN STORY** — §6.5's `left_at` model, §8's convergence questions, and §8 item 3's surviving anchor note. 🔒 **NOW GOVERNED BY `D-154`'s five clauses**, and carrying `E-0`'s six open findings (`C-3` mechanical · `C-4` · `C-5` · `C-6` · `C-7` · `F-E`), **all Chat's** | Legs A + D + **E-0** |
| **G** | **THE REJOIN ANCHOR** — `get_rejoin_anchor`. ⚠️ **`D-154`③ MAKES THIS LOAD-BEARING, NOT A FORMALITY:** *the gap stays closed* requires a per-member history boundary, and **the gap must be MARKED — a silent jump reads as *nothing was said*, which is the direction `D-065` governs. The marker's wording and form are Joe's** | Leg E |
