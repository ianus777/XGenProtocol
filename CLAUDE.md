...
## 🟢 UI component-library / substrate (RP track) — M-RP2.5 ✅ CLOSED (J-407): third `core` component (`textfield`) built + live CDP-verified in BOTH apps — string bind-in path, three binding shapes complete
...
**Next-active (UI/RP track):** M-RP2.5 ✅ — next move is Joe's call: (a) `select` (di · A, single-select — closes the core di binding shapes), (b) the `textfield-group` clear/copy composite (first N-022 composite; reuses `textfield` + `icon-button`), (c) the `use:processor` `common` action (shared with `<textarea>`), or (d) stand up the sampler (Phase-A). **Entry (Rule 0): this PLAY → JOURNAL J-407 → `ui/docs/xgen-ui-notes.md` (N-027 substrate proof, N-028 button + sampler, N-029 textfield) → `ui/docs/xgen-ui-components.md` (Built-components registry) → `docs/ROADMAP.md` (UI subtree + RP node).** UI sessions additionally read both `ui/docs/` files at open. Not pushed — Joe pushes.

> (Prior PLAY blocks: the AFI/F17 audit-against-code head J-398/J-400 ← doc-opt J-391…J-396 ← J-390 ← … ← M6 / XGID-retrofit — archived to `CLAUDE_HISTORY.md` per D-094, latest at J-407.)  
