...
> Date: May 2026  
> **Status:** ACTIVE  
> **Last updated:** 2026-05-20 (XGID Adoption v1 design walkthrough closed; Phase 1 canonical sources shipped — DECISIONS.md D-072 (XGID Adoption v1) + D-073 (field-name-vs-type discipline), `docs/xgen_appendix_j_en.md` (canonical expository document), Ch3 §3.0 (terse normative section), `tasks/XGID_ADOPTION_IMPL.md` (Clair-facing implementation runbook), normative pointers in `docs/xgen_aicontrol_implementation.md` + `docs/xgen_appendix_f_en.md`, `tasks/XGID_ADOPTION_DESIGN.md` (retrospective design record, COMPLETED). Phase 2 doc-tree sweep is next-active for Chat Claude; Clair's implementation runbook is ready for independent pickup. Phase 9 stays PAUSED at Commit 3 boundary; XGID Adoption v1 milestone closes when Clair's two-commit implementation ships, and Phase 9 resumes after.)  
> Author: JozefN  
> Credits: Concept, philosophy, requirements, project direction: Jozef Nižnanský. Technical assistance and implementation support: AI-assisted development tools.  

---

## 🟢 PLAY — XGID Adoption v1 Phase 2 doc-tree sweep (Phase 1 canonical sources ✅ shipped; Phase 9 PAUSED behind XGID implementation)

**XGID Adoption v1 design walkthrough CLOSED 2026-05-20** across two same-day sessions (Q1–Q3 + Q4 partial in session A; Q4 final framing + Q5 + Q6 + Phase 1 canonical sources drafting in session B). Six locked decisions Q1–Q6 captured in retrospective design record at `tasks/XGID_ADOPTION_DESIGN.md` (Status: COMPLETED, v1.0). Mid-walkthrough restructure replaced the original five-upfront-deliverable shape with canonical-sources-first / doc-tree-sweep-second to avoid the piecemeal-doc-discovery problem (D-065 in action; "honest longer work over fast shortcuts" sibling principle).

**Phase 1 canonical sources commit SHIPPED 2026-05-20** — eight artefacts in one atomic commit per session-opener lock:

1. `DECISIONS.md` — **D-072** (XGID Adoption v1) and **D-073** (field-name-vs-type discipline).
2. `docs/xgen_appendix_j_en.md` — canonical expository document (twelve sections §J.1–§J.12: taxonomy, construction, immutability, wire-format invariance with both wire crossings named, type representation, sub-axes, scope boundaries, two worked rejected-proposal examples in §J.9 + reserved third slot, immutability worked example in §J.10, adoption discipline, cross-references).
3. `docs/xgen_ch3_specification.md` §3.0 — terse normative section (six subsections: Definition, Immutability, Wire-format invariance, Field-name-vs-type discipline, Scope boundaries, Adoption discipline). Inserted as new §3.0 before §3.1 Wire Format so the foundational identifier vocabulary precedes every section that references it.
4. `tasks/XGID_ADOPTION_IMPL.md` — Clair-facing implementation runbook (two-commit plan: Commit 1 ships `xgen-common` types + invariance test suite; Commit 2 retypes Phase 7.5's `introducer_node_id` from `Option<String>` to `Option<NodeXgid>` as the only production-code retype in v1). Five required invariance test names pinned: `xgid_serializes_as_plain_string`, `xgid_deserializes_from_plain_string`, `flavour_wrapper_is_serde_transparent`, `event_xgid_roundtrip_through_event_canonical_form`, `node_xgid_roundtrip_through_handshake_message`.
5. `docs/xgen_aicontrol_implementation.md` — one-line normative pointer added near document head ("XGID discipline applies; full annotation pending Retrofit Pass 4").
6. `docs/xgen_appendix_f_en.md` — one-line normative pointer added near document head (same shape, with Pass 3 + Pass 4 both named because Appendix F crosses Node-side and Client-side surfaces).
7. `tasks/XGID_ADOPTION_DESIGN.md` — retrospective design walkthrough record. Created at Phase 1 drafting time with explicit provenance disclosure (Status: COMPLETED from creation, honest about its retrospective shape per D-065).
8. **This CLAUDE.md PLAY block refresh** — reflects walkthrough closure and Phase 1 shipment.

**Next-active work for Chat Claude: Phase 2 doc-tree sweep.** A separate task file (proposed: `tasks/XGID_DOC_SWEEP.md`) walks every main doc in `docs/` and produces a classification table — which docs update at v1, which update with a specific Retrofit Pass (1–5), which need no update. **Deliverable is the classification table, not the edits themselves.** The classification axis includes the Scope-A-vs-Scope-B question deferred at Phase 1: do all AI-control-surface docs (including Ch6 §6.15) receive normative pointers (Scope A), or do spec docs inherit invariance through Ch3 §3.0 normatively without needing pointers (Scope B)? Phase 2 resolves this uniformly across the document tree, not piecemeal.

**Parallel workstream available for Clair: XGID Adoption v1 implementation runbook ready for pickup.** Clair's two-commit plan in `tasks/XGID_ADOPTION_IMPL.md` does not depend on Phase 2 sweep completing — the canonical sources Clair builds against are already shipped (DECISIONS.md, Appendix J, Ch3 §3.0). Commit 1 (`xgen-common` types + invariance tests) can land at any time; Commit 2 (Phase 7.5 `introducer_node_id` retype) follows Commit 1's merge. Honest-broadening warning pinned in the runbook: `introducer_node_id` is the ONLY production-code retype in v1; other String-typed XGID fields belong to Retrofit Passes 1–5, not to Commit 2.

**Sequence after XGID Adoption v1 milestone closure:** Phase 9 (Federation Event Propagation deployment integration tests) resumes from Commit 3 boundary with the integration test code using XGID types from the start. Phase 9 ✅ → Federation Event Propagation milestone flips PLAY → DONE → M6 (new) unblocks. Retrofit Passes 1–5 land in ROADMAP.md Near future, picked up as the next available slot after v1 implementation ships.

**Phase 9 status: PAUSED at Commit 3 boundary, XGID implementation now the active blocker.** [tasks/FEDERATION_PROPAGATION_PHASE_9.md](tasks/FEDERATION_PROPAGATION_PHASE_9.md) remains Status: ACTIVE v1.0; scope intact. The dependency chain that previously read "Phase 7.5 ✅ → XGID Adoption design → implementation → Phase 9" now reads "Phase 7.5 ✅ → XGID Adoption design ✅ → XGID Adoption canonical sources ✅ → XGID Adoption implementation (Clair's two commits) → Phase 9."

**Entry point for next Chat Claude session: Phase 2 doc-tree sweep.** Open a new task file at `tasks/XGID_DOC_SWEEP.md` (Status: ACTIVE) walking every main doc in `docs/`. Classification axis is Scope-A vs Scope-B per the flag recorded in `tasks/XGID_ADOPTION_DESIGN.md` §"Findings deferred to Phase 2 sweep." Deliverable is the classification table; per-doc edits land afterwards under the appropriate Retrofit Pass once classified.

**Entry point for Clair's next session: `tasks/XGID_ADOPTION_IMPL.md`.** Commit 1 first (clean `cargo test -p xgen-common`, clean `cargo clippy --workspace --all-targets -- -D warnings`, all five required invariance tests pass). Commit 2 follows Commit 1's merge.
