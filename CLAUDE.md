...
## 🟢 PLAY — M6 (new): A4 force-eject Option B SHIPPED (J-160, live fan-out + federation push); M6 admin write-path COMPLETE (16 verbs, all live-propagating); **next-active = Clair implements protocol-audit-log Commit 1 (writer side — inside `persist_event`) per runbook §4 (design COMPLETED, runbook ACTIVE, J-165); then reader → rebuild → close; then arcs 2–4, then M7 `--aicontrol`**
...
**Entry point for next session:** read this PLAY block + JOURNAL J-165 first per Rule 0, then `tasks/M6_PROTOCOL_AUDIT_LOG_IMPL.md` v1.0 (the runbook — Clair starts at Commit 1 §4 after checkpoint #1) + `tasks/M6_PROTOCOL_AUDIT_LOG_DESIGN.md` v1.1 + spec §3.11.8.
