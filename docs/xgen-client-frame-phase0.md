...
- **M-RP6.1e** — **client frame consolidation, split A/B/C (J-493).** See §10.
  ...
  - **M-RP6.1e-C** — `dialog`/`modal` `core` + **Help→About**. **Split three ways (Joe-locked, J-496)** — the milestone is `dialog` **+ real Rust**, so it does not close in one commit pair:
    ...
    - **M-RP6.1e-C2** ✅ **DONE (J-497)** — **`get_about_info`** (real Rust, **no UI**). Shipped as `xgen-common::about` (**`AboutInfo`** shared env block + typed **`ClientAboutInfo`**; paths + app facts **passed in**, never derived; **`xgen-common` gained no `tauri` dep**) + **one** new `build_info` field (`RUSTC_VERSION`, from the **existing** `xgen-common/build.rs` — the node inherits it free) + a thin `desktop.rs` command. **Three of Chat's own design assumptions were WRONG and were caught by grounding the code first:** (1) `build_info` **already existed** and already emitted `BUILD_TIMESTAMP`+`GIT_HASH` — a client `build.rs` would have been a **second build-metadata surface** (the D-067 drift this project kills); (2) `package.json` declares a caret **range** (`"^5"`), not a version — the resolved `5.55.5` is in the **committed lockfile**; (3) app-defined Tauri commands need **NO capability grant** (J-495's lesson was `core:window`-specific — the inverse trap). Verified **real client 9222**: full JSON returned, every field checked externally (`commit` = `git rev-parse --short HEAD` exact, `tauri` 2.11.1 = `Cargo.lock`, `svelte` 5.55.5 = lockfile, all paths exist), `errCount:0` (no grant needed — proven, not argued), client registry **7**, sampler catalogue **313** unchanged (grounded **by scope**: 7 files, zero `ui/**`), workspace tests **1507/0/62**. **The `version` trap held:** `build_info::VERSION` is *xgen-common's* — both are `0.10.3` today, so the wrong wiring would **look** right; proof came from the **source line**, not the value. Feat `50b5640`.
    - **M-RP6.1e-C3** ✅ **DONE (J-498)** — **Help→About assembly**: the Help menu (`help.about`, **no accelerator**) + the C1 `dialog` fed by C2's `get_about_info` **on mount** + the logo. Shell-local `ui/client/src/about-dialog.svelte`; values render as `label` (registry-visible), keys as `<dt>`; **Built + commit in one row** (`date · shortSHA`); "**Developed by Alchemy Dump**" (no personal name); website link → **OS browser** via `tauri-plugin-opener` (Cargo · package.json · capability · `desktop.rs` — a **plugin** command **does** need a grant: the third distinct answer in three milestones, per J-497's rule that the permission model is grounded **per command class**). C2's `name`/`link` literals **CONFIRMED KEEP** (Joe). Verified real client 9222: registry **7→22** (`count===unique`, measured), `:modal` true, all 10 fields real (the rendered SHA **is** HEAD — `build_info` proven live), Close → re-open, and the **first real File↔Help roving**. **+N-090** (every skinnable setting → `skin.css`; `app.css` = frame skeleton only) · **+N-091** (`.dialog{margin:auto}` — a **C1 gap**: every modal was pinned top-left; C1's legs never tested *position*) · **+N-092** (the client debug bridge is **state-only** → the orphan leg is sampler-only; the window config is **physical px**). Window default moved **900×600 → 1240×1080** (both configs — the J-495 twin-config rule held; see §10.4). **→ M-RP6.1e-C CLOSED. Next-active = M-RP6.1f.**
- **M-RP6.1f** ✅ **DONE (J-499)** — **centre region-shell scaffold + selection bus.** Renderer **A** (`region-shell`, the **32nd `core`**) walks the D-103 `Layout` descriptor; `leaf` + `split` only — a **`tabs` node is dropped with a DEV warn** (renderer B owns tabs; an unfed branch is an unverified branch). Leaves resolve against a **prop-injected `widgets: Record<widgetId, Component>` registry with unknown-id drop** — **the shape already shipped in `message.svelte`, reused, not reinvented** (N-093). Internal `region-node` recursion registers nothing (N-064). **No Rust (D2):** `get_layout` would duplicate the descriptor type in Rust (the D-067 drift surface) — the seam is a frontend `async loadLayout()` whose *body* becomes `invoke('get_layout')` at M-RP7.3, with Rust persisting an **opaque blob**. **Selection bus = a `$common` store (D3), forced not preferred:** R8 and `entity-context-menu` are widgets in `ui/common/…/widgets/` and **W-3 forbids a `common` widget importing a shell dep**, so a shell-local bus is structurally unconsumable; shape unchanged `{regionId, entity} | null`. 8 placeholder leaves (R1–R8 → one `section` each). Verified **real client 9222**: registry **22→30** (`count===unique`, enumerated), G `{leafCount:8, depth:3}`, **geometry green** (fills, no doc scroll, ratios `[1,2,7,2]` exact), drops + bus + accent-neutral; `npm test` **41 passed**, `vite build` **150 modules**. **+N-093/N-094/N-095**; **§10.3 D5 SUPERSEDED — see below.** Feat `899ff6e`.
- **M-RP6.1g** ✅ **DONE (J-500)** — **R3 Self/connection: the first real system widget + the selection bus's first writer.** **⚠️ THIS BULLET'S ORIGINAL WORDING WAS WRONG IN THREE PLACES** — grounding the source before writing the runbook proved it, and the corrections are promoted here so the old text is never read as current:
  1. **“scoped `app.emit('self-state', …)` push” — NO NEW EMIT WAS BUILT.** `desktop.rs::emit_state()` **already** fires `app.emit("xgen-client-state-changed", …)` on every lifecycle transition, and `app_client.svelte` **already** listens. A second channel would have been a **second surface** — the exact **D-067 drift** this project exists to eliminate.
  2. **“`get_self_state` read verb” — it is a THIN WRAPPER, not a new projection.** `ops::whoami` already existed (sync, no network), and `session::ClientIdentity::load()` derives the identity XGID **from the keypair alone** — no registration, no node. The command composes those two (the `get_about_info` shape). **`NodeSelfState` deliberately NOT declared** (the J-497 `NodeAboutInfo` precedent).
  3. **“stop the node → the led flips via the push” — NOT RUNNABLE.** `run_startup` does **one** 2-second `connect_async`, **drops the stream**, emits `Ready`, and never touches the socket again. **There is no resident.** → **6.1g closes only the *read shape* of F-1;** the live half is **M-RP6.6** (below).

  **Shipped:** a thin `get_self_state` · a `$common` `self-state` store — **the `STATE_COLOURS`/`PULSING_STATES` relocation was FORCED, not stylistic**: `region-node` mounts a leaf with **`regionId` and nothing else**, so with **W-3** a region widget can be fed **only** by a store; the `status-bar` and the widget now read the **same** map (*one channel, two views*, **N-096**) · **`self-panel`**, the first real **system widget**, composing shipped `entity-item`(card) + `status-indicator` **unmodified** · `widgetRegistry.self` swapped — **renderer A's first real leaf**, a clean **swap in place** (`id = region-${regionId}`). **Honest by construction (D-065/W-8):** the Tauri shell has **never registered**, so `registered:false` + the **real keypair-derived XGID** + an explicit *not registered* line **is** the correct render; the Track-A `status` slot ships **absent, not faked**. **Verified real client 9222, Chat re-drove every leg (Rule 5):** registry **30→36** (`count===unique`, enumerated) · **the same verb, two grounded outcomes** (`registered:false` ↔ `registered:true`, **identical XGID**, driven by moving the state file aside in the **same process** — `whoami` re-reads per invoke) · both leds `READY`/`DISCONNECTED` **from one store** · the bus's first writer · geometry green · the relocation **proven pure** (both leds `rgb(45,122,58)`) · churn **36→29→36 exactly** · `cargo test` **1507/0/62** · `npm test` **41** · `vite build` **156**. **⚠️ +N-097 — the verify pass found an unfed branch, and it was OURS:** the panel **wrote** the bus and never **read it back**, so `entity-item.selected` was a **constant false**, stranding the **already-shipped** `.entity-item[data-selected]` skin rule (M-RP5.1) that no client code could reach. Fixed by deriving `selected` from the bus — **the writer is now also the first reader** — and re-verified on the **painted pixel** (the gold `--accent` bar), not merely the attribute. **+N-096/N-097/N-098**; **no new D**. Feat `84b482a`.
- **M-RP6.1h** ✅ **DONE (J-501)** — **R8 inspector: the selection bus's first CROSS-REGION reader. The read loop closes end-to-end.** `inspector-panel` (`ui/common/…/widgets/`) — the **2nd system widget** (`kind: system`, W-13; **4th widget** overall), leaf-id `region-inspector` (N-096) → a clean **swap in place**, one `widgetRegistry` entry. **3 files, +143. No Rust.**

  **🔑 It renders the DESCRIPTOR and nothing more — and the refusal is the design.** `EntityDescriptor` is `{kind, name?, id, flags?, image?}`: five fields, three optional, `image` **reserved-unfed**. A `get_entity_info`-shaped verb was **rejected outright** — the only selectable entity today is *self*, whose fields already come from `get_self_state`, so a second Rust projection would be a **second surface (D-067 drift) delivering ZERO new information**. **Do not invent fields to make a panel look substantial.** Rows reuse the About `<dl>` (keys `<dt>`, **values `Label`**): **Kind · Name · ID · Source** (+ **Flags**, conditional). Empty state = the `entity-panel` composed-`Paragraph` pattern; root always mounted, which is what makes *clear → exact return to baseline* the honest orphan proxy (N-092a). G `{hasSelection, regionId, kind, rowCount}` — XGID/name **not republished** (N-060); `rowCount` is render-truth.

  **🔑 The N-097 trap was DESIGNED AROUND, not walked into.** The header composes **`entity-avatar`, deliberately NOT `entity-item`** — `entity-item` carries a `selected` prop with a live `[data-selected]` skin rule, and in R8 "selected" is **trivially always true**: exactly the constant-flag-feeding-a-shipped-affordance shape that stranded that rule at 6.1g. *One milestone after learning the lesson, the design routed around it.*

  **Verified real client 9222, Chat re-drove every leg (Rule 5):** registry **36→38** (`count===unique`, enumerated) · **V3 the loop, on the PAINTED TEXT** — click R3 → the rendered `<dd>` reads `identity` / `Joe` / the real XGID / `self`, matched **against the bus payload**; R3's gold `[data-selected]` bar still paints · **V4, THE STRONGER PROOF: R8 reads the BUS, not R3** — driving `__XGEN_SEL__` directly with a **space** and a **room** (regions with **no writer at all**) renders them anyway (shape circle/square/hexagon, `Source` flips), and **exercises the conditional flags row both ways** · churn **38→42→43→38 exactly**, zero leaks · geometry green (`docNoScroll`; members/inspector **409/409** = `[1,1]`; leaf self-scrolls, document does not) · **7** `.inspector-*` rules in cascade, **zero `<style>`**, **accent-neutral** · `vite build` **158** · `npm test` **41** · `cargo test` **1507/0/62 — identical to baseline, which PROVES the no-Rust claim.**

  **⚠️ +N-099 — Chat produced a PHANTOM GREEN in this verify and threw it away.** The first accent leg set the bus and read the DOM in the **same eval**; Svelte renders on the **effect tick**, so both reads were `null` and the check reported **`accentNeutral: true` by comparing `null === null`**. **A leg that cannot see its subject has not passed it.** Re-driven across two evals with a `readable` assertion **first**. *(Also: the runbook's V4 literal was **wrong** — a **DM space is a CIRCLE**, not a square, `entity-avatar.svelte:56-62`. Chat's error, Clair caught it, grounded against source.)* Feat `c4346bf`.
- **M-RP6.1i** ✅ **DONE (J-508)** — **`shelf` (33rd `core`) + `shelf-face` (34th)**: the ordered `role="toolbar"` command strip that frames the widget grid (top = user favourites · bottom = system), **frame chrome OUTSIDE the `Layout` descriptor** (S-1 / D-107 — *the controls that govern the grid must not be dockable by the grid*). Roving tabindex + `onCommand` seam (**byte-for-byte the `menu-bar` seam** — a shelf button and a menu item become **one command, two triggers**). **The face owns its own `<button>`** (the `menu-trigger` precedent — the `button` core renders `{label}` **text only**: no child slot, no `tabindex`). **`aria-disabled`, NOT native `disabled`** (grounded in `menu-item`) — a natively-disabled button **is not focusable**, and at 6.1j the *whole bottom shelf* is disabled, so native `disabled` would leave the strip **invisible to the keyboard**. **No badge** (nothing produces a count; unread counts have **no protocol mechanism**, J-503 — a skinned affordance nothing can feed is the N-097 scar). **Empty = mounted-but-collapsed** (`[data-empty]`, the `status.svelte` precedent, N-053). **3 new glyphs** (Material Icons **Apache-2.0**, `d` taken **from the real SVGs**, colour-free geometry, provenance recorded — D-108/D-110). Sampler **313→328** (+15), 0 orphans both directions. **Filed, not built: M-RP-ROVING** — `shelf` makes roving-tabindex the **4th** independent implementation (`entity-panel`/`menu-bar`/`menu`/`shelf`), i.e. **D-069's four-recurrence bar, met**; extraction touches three closed, CDP-verified components and is **its own milestone**. → **N-104**. Feat `c8010e2`.
- **M-RP6.1j** ✅ **DONE (J-509)** — **both shelves mounted in the real client frame. A MOUNT, not a build: 2 files, +30, no `core`, no `skin.css`, no Rust.** Top strip **empty → collapses to height 0** (registered anyway — N-053 — so pinning is a **one-line population** later). Bottom strip: three faces, **all `disabled`** — **the countdown (W-8, honest phase-limit, the `self-panel` posture)**. **🔒 `commandTable` deliberately NOT touched**: `widget.manager` / `layout.save` / `layout.load` **do not exist yet** — ***a registered command id that resolves to nothing is a worse lie than a disabled button***; but the **`onCommand` seam IS wired live**, so **6.1k and 6.1l are each one table entry + one `disabled` flip.** Frame-column pin (`.app-frame > .shelf { flex: 0 0 auto; }`) goes in **`app.css`**, following the shipped `.menu-bar` / `.status-bar` lines. **Verified real client 9222 (Chat re-drove every leg):** registry **38→46 (+8)**, enumerated · **geometry — the milestone's real risk** (two new fixed rows entered the flex column, where **N-088** and **J-499** both bite): `docNoScroll` **true**, the grid still **fills**, split ratios **`[1,2,7,2]` EXACT at a different window width than any prior measurement** — *the stronger proof* · the collapsed strip paints **no hairline** (`borderBottomWidth: 0px`, **computed style, not the attribute** — N-097) · the three faces are `aria-disabled` **and keyboard-reachable** (`nativeDisabled:false`) · the menu-bar was **not shadowed** by two new toolbars · `cargo test` **1507/0/62 identical to baseline** — *which proves the no-Rust claim rather than asserting it*. → **N-105**. Feat `96a5a60`.
- **M-RP6.6** 🆕 — **client resident — the REAL F-1 close (filed at J-500).** ***The frame arc's next Rust item.*** A sustained WS to the home node + a reconnect loop + a **live** lifecycle, so the connection state changes **while the app runs**. Today it does not: the startup path connects once, **discards the socket**, and the client's connection state is **relaunch-scoped**. **This is a Rust/protocol arc, not a UI one — and no part of it may be smuggled into a UI milestone.** Until it lands, the honest proof of any connection UI is *node up → launch → `READY`; node down → relaunch → `DISCONNECTED`* — which is exactly what 6.1g proved.
...
- **Default + min size** — **1240×1080** (was 900×600; changed at C3, J-498), **min 640×400**. **Both `tauri.conf.json` AND `cdp.dev.conf.json` carry the window block** — the `--config` overlay replaces `windows[]` **wholesale**, so every window-config edit touches both files (J-495). *Known wrinkle, now measured twice and OPENED as a real question (N-092b): the config is applied in **physical** px — at DPR 1.25, 1240×1080 renders as **993×865 CSS** (and the old 900×600 rendered as 720×480). The **logical-vs-physical** unit decision is deferred to **M-RP-WINSTATE** ⏸️ (window size/position memory, gated on the widget grid), which must clamp restored geometry to the monitor work area anyway. **Do not tune 1240×1080** — it is a first-launch default that remembered geometry will override.*
...
- **⚠️ Scroll — D5 SUPERSEDED at M-RP6.1f (J-499).** See §10.5.
...
## 10.5 — ⚠️ The centre-only-scroll lock (D5) is SUPERSEDED (M-RP6.1f, J-499)

**J-495 locked:** *“the centre is the ONLY scroller”* — `.app-center { overflow-y: auto; padding: 12px 16px; }`. That was **correct for a single placeholder paragraph** and **wrong for a dock layout**.

**A widget grid must FILL its box and never scroll as a unit; each LEAF owns its own scroll.** That is what a docked panel *is* — scrolling the whole grid would drag the room list, the composer and the member list off-screen **together**. So at M-RP6.1f:

```
.app-center  { flex: 1; min-height: 0; overflow: hidden; padding: 0; display: flex; }   /* app.css  — frame skeleton */
.region-leaf { overflow: auto; min-height: 0; min-width: 0; }                            /* skin.css — N-090 */
```

- `overflow-y: auto` → **`overflow: hidden`** — the grid never scrolls.
- `padding: 12px 16px` → **`0`** — the grid meets the frame; inner gaps are the **skin's** job (N-090).
- **`min-height: 0` must ride EVERY nested flex level**, or the classic flexbox blowout puts the scrollbar straight back onto the **document**. This is a **required verify leg**, not a detail — measured green at J-499 (`documentElement.scrollHeight === clientHeight`).

**What is UNCHANGED — and this is the part that matters.** The **frame chrome still never scrolls.** The menu-bar and status-bar remain fixed panes **outside** the `Layout` descriptor. The supersession is **scoped strictly to the interior of the centre**; the BorderPane guarantee (§2 — *File→Exit can never be docked away*) is untouched.

**⚠️ Reading note.** §10.3's *“centre-only scroll”* wording and the 6.1e-B entries in §10.2 / §10.4 are the **historical record of M-RP6.1e-B** and were **correct as of then**. On the scroll question they are **superseded by this section** — do not read them as current.
