...
## 🟢 UI component-library / substrate (RP track) — M-RP2.21 ✅ CLOSED (J-432): `link` — the **sixteenth** `core` component, the **first navigation-kind di** (atomic `<a href>`); commits `<a>`-vs-`<button>`; synthesised `disabled` (drops `href`); bundled-safe `external` rel; returns to accent-derived colour; getter `{text,href,external,disabled}`; **next — `status-indicator`** (di composite; led + label + link all in hand)
