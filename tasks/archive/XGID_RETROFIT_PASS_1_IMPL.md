...
> Version: 2.1  
> Date: May 2026  
> **Last updated**: 2026-05-26 (J-122 — Pass 1 milestone CLOSED. Seven atomic commits on `main`: Commit 1 `403ef3f` canonical-form module move; Commit 2 `8a94dee` convenience constructors on flavour wrappers; Commit 3 `75e81b4` xgen-common data-structure retypes (+5 invariance tests A–E); Commit 4 `774fe9d` xgen-core data-structure retypes (lib-clean per Path A Joe-lock); Commit 4a `4895446` xgen-core test-fixture projection sweep (split from Commit 4 per Joe-lock); Commit 5 `096162e` Appendix C + Appendix I retypes; this Commit 6 milestone-close commit. Plus J-121 hygiene atom (`1dd909e`) shipped immediately before Commit 6 closing a lib-level `unused import: NodeXgid` clippy regression surfaced at the Commit 6 verification gate. **Test count at close**: 489 (xgen-common 34 lib + 8 invariance + xgen-core 447). Negative delta vs the J-119 baseline of 627 is expected and honest per Path A — `xgen-node` + `xgen-client` consume retyped `xgen-core` types and don't build at workspace level until Pass 3 + Pass 4 (the missing ~140 tests live in those crates). Pass 5 close should restore the workspace test count to ≥ 627 plus the +5 Pass 1 invariance tests. **Two mid-implementation Joe-locks beyond the runbook's authored scope**: (i) `Borrow<str>` additive API on `Xgid` + flavour wrappers at Commit 4 implementation kickoff — enables `HashMap<NodeXgid, V>::get(&str)` and sibling Borrow-driven lookups without per-query wrapper allocation; soundness verified (derived `Hash` + `PartialEq` forward to inner `String`/`str`, hash-consistent with `&str` per std docs); locked over per-site explicit-wrap-with-comment churn at hundreds of lookup sites. (ii) Commit 4 → Commit 4 + Commit 4a split at Commit 4 implementation halfway — the runbook's "data structures only" framing did not anticipate the ~296 test-fixture errors that surfaced when xgen-core lib retypes completed; Joe-lock locked the split rather than absorbing into Commit 4, preserving D-074 atomic discipline and giving Commit 4a its own clean scope. Discipline implication for next sibling milestone runbook author: runbook estimates for "data structures only" scope should include an explicit test-fixture-sweep estimate alongside the lib retype estimate; test-fixture cost (~550 LOC at Commit 4a) was order-of-magnitude similar to the lib retype (~776 LOC at Commit 4). **Both additions flagged-not-promoted as candidate D-NNNs per D-069** at one instance each (three-instance threshold not met). **Layered-B3 audit answer per §6.7 DoD**: no layered surface emerged — Pass 1's scope is data-structure shape, not algorithm validation; validators consume the typed fields as `&str` projections through Borrow. **"Honest longer work over fast shortcuts" first recurrence within Pass 1 scope** (at J-121 hygiene atom; close-event-not-recurrence-event for Commit 6 itself, sibling to J-101/J-108 milestone-close framing). **Verification at close**: `cargo test -p xgen-common -p xgen-core` clean (489 tests); `cargo clippy -p xgen-common -p xgen-core --lib --all-features -- -D warnings` clean; `cargo clippy -p xgen-common -p xgen-core --tests -- -D warnings` clean; `cargo build --workspace` deliberately broken per Path A — xgen-node + xgen-client consume retyped xgen-core types, downstream Passes 2/3/4 own those. **J-NNN freeze guardrail (J-108 codification)**: `grep -rn 'J-NNN' . --include='*.rs' --include='docs/*.md' --include='tasks/*.md'` returns ZERO matches post-staging. Runbook flips Status ACTIVE → COMPLETED v2.0 → v2.1 in this same atom per D-074 + Lock #3.)  
> Language: English  
> Author: JozefN  
