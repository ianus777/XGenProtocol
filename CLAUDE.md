...
> Date: May 2026  
> **Status:** ACTIVE  
> **Last updated:** 2026-05-20 (Phase 7.5 Federation Cold-Start Bootstrap milestone CLOSED across Commits 1-3.5 + integration tests + close-commit (J-094). Failure-mode catalogue M5 structurally closed. Phase 7's B3 amendment (`tasks/FEDERATION_PROPAGATION_PHASE_7_B3_AMENDMENT.md`, COMPLETED v1.0) landed mid-implementation as Commit 3.5 (`ecbbf19`) when Commit 4 integration tests surfaced two latent Phase-7 gaps: predecessor-chain deadlock + step-11 sender-membership rejection for federation_add via federation channel. B3 walked through the same Joe-lock walkthrough discipline as Phase 7.5 itself. Test count 519 → 556 across Phase 7.5 implementation; final `cargo test --workspace` re-verified at milestone close. XGID Adoption is next-active per ROADMAP.md Near future; Phase 9 stays PAUSED at Commit 3 boundary with XGID now the active blocker.)  
> Author: JozefN  
> Credits: Concept, philosophy, requirements, project direction: Jozef Nižnanský. Technical assistance and implementation support: AI-assisted development tools.  

---

## 🟢 PLAY — XGID Adoption design phase (Phase 7.5 ✅ shipped; Phase 9 PAUSED behind XGID)

**Phase 7.5 Federation Cold-Start Bootstrap CLOSED** (Commits 1–3.5 + integration tests + close-commit, shipped across J-094 implementation session). Failure-mode catalogue M5 ("F-3 brand-new federation bootstrap dead-locked") structurally closed. Phase 7's B3 amendment (`tasks/FEDERATION_PROPAGATION_PHASE_7_B3_AMENDMENT.md`, COMPLETED v1.0) landed mid-implementation when Commit 4 integration tests surfaced two latent Phase-7 gaps: predecessor-chain deadlock + step-11 sender-membership rejection for federation_add via federation channel. Both closed in Commit 3.5 (`ecbbf19`). Test count 519 → 556 across Phase 7.5 implementation; final `cargo test --workspace` re-verified at milestone close.

**XGID Adoption is next-active per ROADMAP.md Near future** (first-in-queue, sequenced AFTER Phase 7.5 closure and BEFORE Phase 9 Commit 3 resumption so test code, trace events, and observability fields use XGID from the start rather than being retrofitted). XGID is a vocabulary + type-discipline decision, not a protocol-shape change. Design phase opens with one walkthrough session producing five deliverables: (1) **Appendix J — XGID: Vocabulary, Construction, Type Discipline** as the canonical expository home covering conceptual framing, construction taxonomy across XGID flavours (content-derived hashes vs. public-key URIs), immutability reasoning, wire-format-invariance promise, the field-name-vs-type discipline (Phase 7.5's `introducer_node_id` is the first instance), what-XGID-is-not boundaries; (2) Ch3 §3.X new terse normative section introducing XGID before any event/Space/Room references, forward-referencing Appendix J; (3) DECISIONS.md D-072 entry locking name + all-encompassing scope + medium-adoption discipline (no wire-format changes, ever); (4) implementation runbook for Clair as a separate task file, Phase-7.5-shaped; (5) the design task file itself (`tasks/XGID_ADOPTION_DESIGN.md`) recording the locks. Parent-type surface stays small (~3 touch points in `xgen-common`: `Primitive` + `SignedPrimitive` cover Event/Node/TrustAssertion; Ed25519-pubkey IDs get separate typing at their pubkey surface); wire-format field names (`event_id`, `space_id`, `sender`, `introducer_node_id`, etc.) unchanged.

**Sequence after XGID design closes:** Clair implements per the runbook → XGID Adoption milestone ships → Phase 9 resumes from Commit 3 boundary with the integration test code already using XGID types from the start. After Phase 9 ships, the Federation Event Propagation milestone flips PLAY → DONE and M6 (new) unblocks.

**Phase 9 status: PAUSED at Commit 3 boundary, XGID now the active blocker.** [tasks/FEDERATION_PROPAGATION_PHASE_9.md](tasks/FEDERATION_PROPAGATION_PHASE_9.md) remains Status: ACTIVE v1.0; scope intact, only dependency order changes (Phase 7.5 ✅ → XGID Adoption ships → Phase 9 resumes). J-092's Commits 1+2 (observability preconditions G1+G2+G3 + `#[serial_test::serial]` flake fixes) stay shipped — pure infrastructure, independent of XGID.

**Entry point for next session: XGID Adoption design walkthrough.** Open `tasks/XGID_ADOPTION_DESIGN.md` (Status: ACTIVE, v0.1) with `[JOE-LOCK: pending]` markers covering: vocabulary scope (which protocol identifiers are XGIDs, which aren't); type-system shape (newtype vs. type alias vs. trait); adoption discipline (new code only vs. opportunistic retrofitting); wire-format invariance promise framing; immutability framing for Ch3; the field-name-vs-type discipline as canonical principle. After walkthrough closes, the design phase produces Appendix J + Ch3 §3.X + DECISIONS.md D-072 + implementation runbook + design task file COMPLETED-flip, all in one coordinated commit shape (matching Phase 7.5's design-phase shape).
