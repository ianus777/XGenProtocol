...
> Version: 1.5  
> Date: May 2026  
> **Last updated**: 2026-05-24 (J-116 — §2.4.2 "Uniform assertion shape per test for variant row 2" Rust code block full rewrite to match production API shape; source-of-truth pointers gain `xgen-core/src/dag/pending.rs:491,498` (PendingBuffer accessors) + `xgen-core/src/dag/store.rs:48,52` (EventStore accessors); version bump v1.4 → v1.5. **Cause: D-078 second prospective-catch at Pre-Commit-3b-4 Joe-lock checkpoint #2** (second pass against J-115-amended findings v1.4 + runbook v1.1). Clair STOPPED per CLAUDE.md Rule 3 when D-078 production-code verification surfaced that the J-115 amended sample-code block at §2.4.2 referenced API shapes that don't exist in production: `DispatchOutcome::HeldPending(buffer_state)` (production is a UNIT variant at `xgen-core/src/node/runtime.rs:88-95`); the F-10 `missing_identity` info is stored in `PendingBuffer` via `add(event, missing_predecessors, missing_identity, ...)` at `xgen-core/src/dag/pending.rs:149+`, NOT carried on the DispatchOutcome itself. **Sibling-but-distinct from J-115's catch shape**: J-115 caught contract intent vs production behaviour mismatch (HeldPending vs Rejected outcome — what the test proves); this catch caught template assertion API vs production type shape mismatch (HeldPending unit variant; per-entry missing_identity inspection requires PendingBuffer accessor — how the test asserts). D-077-shape framing — forward-sustainability question (what does production do?) was asked at J-115 amendment authoring; backward-coherence question (does the proposed template's API-shape compile against production types?) was not. **Joe locked Reading α** (Track 1 sub-amendment of J-115; findings v1.4 → v1.5 + runbook v1.1 → v1.2; sibling-shape to J-099 / J-109 / J-113 / J-115 canonical-record-amendment-first precedent) over Reading β (Clair latitude + JOURNAL divergence) + Reading γ (hybrid bundle into Clair atom). **Open question ε resolution — Option ε.iii locked**: PendingBuffer does not expose `missing_identity` per-entry as a public accessor today. The §2.4.2 amended sample asserts F-10 buffering via `pending.contains(event_id)` + `pending.pending_identity_count() >= baseline + 1` — the available deployment-level observability surface. Stronger `missing_identity == Some(attacker_uri)` assertion would require a new public accessor (Option ε.ii); deferred per D-069 audit-vs-design boundary as its own audit-design-impl arc per D-071 if dependent work surfaces need. **§2.4.2 Rust code block fully rewritten** for both the 4-families HeldPending case + the state.federation_add Validated-then-ingested case, mirroring runbook v1.2 §4.5 amended templates exactly. **Source-of-truth pointers** gain the four production API anchors. **Two distinct candidate D-NNNs flagged-not-promoted per D-069** — D-NNN-α "prospective-catch count separation" (contract-surface; one instance at J-115) + D-NNN-β "template-compile-check at runbook authoring" (template-surface; one instance at J-116); both at one-instance below three-instance promotion threshold OR Joe-lock. **"Honest longer work over fast shortcuts" — tenth recurrence** within Federation Event Propagation milestone scope (Phase 7.5 first through J-115 ninth; this sub-amendment tenth). Framing α locked — each prospective catch that delays milestone closure is its own recurrence regardless of which structural layer (contract vs template-API) the catch surfaces at. **D-074 application count: sixteenth instance** (sub-amendment Track 1 atom per Lock #3 per-commit cadence; not a milestone-close). **Five-file atomic commit** per D-074 sixteenth instance (decision count six per Joe's enumeration, the sixth being the explicit no-amend on DECISIONS.md per D-077): findings v1.4 → v1.5 (this file); runbook v1.1 → v1.2; JOURNAL J-116; CLAUDE.md header chain; ROADMAP.md v1.30 → v1.31. **No state transitions**: Phase 9 milestone stays PLAY; Federation Event Propagation milestone stays PLAY; M6 (new) + XGID Retrofit Pass 1 stay PENDING. Clair's next-active stays Phase 9 Commit 3b-4 against further-further-amended runbook v1.2 + findings v1.5; pickup at Joe-lock checkpoint #2 per runbook §2.3 (re-runs D-078 verification against the corrected templates). Previous v1.4 J-115 content stands authoritative — see body §2.4.2 + header chain below. 2026-05-24 (J-115 — §2.4 §E variant row 2 (`forged_sender_with_resign_*`) substantive rewrite + new §2.4.2 sub-block documenting F-10 HeldPending contract per family + state.federation_add B3 asymmetry as documented design property; version bump v1.3 → v1.4. **Cause: D-078 first prospective-catch at Pre-Commit-3b-4 Joe-lock checkpoint #2.** Clair STOPPED per CLAUDE.md Rule 3 + runbook Lock 1 Trigger (a) when D-078 production-code verification surfaced that the v1.3 §2.4 §E variant row 2's `forged_sender_with_resign_*` row maps to `Rejected(UnknownSender)`, but `validate_event` at `xgen-core/src/message/exchange.rs:626-632` (Phase 6 F-10 amendment, J-087) returns `ValidationOutcome::HeldPending { missing_identity: Some(sender.clone()) }` for unknown-signer events, NOT `Rejected`. The `UnknownSender` ExchangeError variant exists in the enum (line 60) but is reachable only via `validate_steps_8_13` — the legacy accept_message path, test-only-reachable per `runtime.rs:334-340` doc-comment, NOT the F-4 path that `dispatch_event` calls. **D-078's first prospective-catch instance** — the principle was promoted at J-114 specifically to catch this class of gap before Clair writes code; the very next milestone (this one, Commit 3b-4) instantiated the prospective shape on its first checkpoint #2. **Three readings walked with Joe**: A (drop variant row entirely, 20 → 15 tests) — rejected on F-10 deployment-level coverage grounds; B (reframe variant: 4 families assert HeldPending with `missing_identity = Some(attacker_uri)`; state.federation_add asserts Validated-then-ingested per Phase 7 B3 design property) — sibling-shape to J-109 Reading B precedent; C (add hard-Rejected variant via spec change) — structurally infeasible without D-071 work. **Joe locked Reading B.i** (state.federation_add as Validated-then-ingested with security-property doc-comment over B.ii which would have dropped state.federation_add from the variant row). **Variant-uniformity at the strict level is broken** (4 HeldPending + 1 Validated for variant 2 row) but the structural property the matrix is designed to expose IS preserved: each variant either rejects-via-F-4 or defers-via-F-4 or ingests-via-F-4, and the asymmetries across families surface real protocol behaviour. The matrix's job is to expose drift; one asymmetric cell that documents a Joe-locked design property is the matrix doing its job. **D-077 bidirectional sustainability discipline applied at amendment authoring**: pre-amendment grep across the six federation canonical doc candidates surfaced per-surface lock decisions — three amended (findings v1.3→v1.4 substantive rewrite + new §2.4.2; runbook v1.0→v1.1 §4.2 + §4.5 + §4.6 + §4.7 amendments + clerical fixes; Phase 9 task §3 Commit 6 paragraph drift correction with no version bump per clarification-not-state-transition sibling-shape to J-109), three preserved unchanged with reasoning (design doc §13 already incorporates F-10 correctly post-J-087; completion runbook Phase 7 framing intact for its scope; survey doc supersession pointer per J-109 two-document framing). **Sibling-shape to J-099 + J-109 + J-113** — fourth instance of in-place canonical-record amendment at dependent-milestone implementation time, FIRST instance to fire prospectively (pre-implementation) rather than retroactively (mid-implementation). **D-074 application count: fifteenth instance.** **"Honest longer work over fast shortcuts" — ninth recurrence within Federation Event Propagation milestone scope** (Phase 7.5 first; bidirectional second; topo-sort design close third + runbook landing fourth + re-walk Step 2 fifth + re-walk Step 3 sixth + topo-sort implementation seventh; persistence-amendment sub-amendment eighth; this J-115 ninth — first prospective-catch instance). Candidate D-NNN "prospective-catch count separation" flagged-not-promoted per D-069 (one instance; three-instance threshold not met). Previous v1.3 J-113 content stands authoritative — see body §2.4 + §2.4.1 entries below. 2026-05-24 (J-113 — §2.4 Scenario 4 §E table substantive rewrite + new §2.4.1 production-code-verification sub-block + new §4.6 Gap G6 entry (timestamp-bound validation as production gap); version bump v1.2 → v1.3. **Cause: canonical-document staleness surfaced at Clair's mid-Commit-3b-4 implementation read.** Clair STOPPED per CLAUDE.md Rule 3 when constructing Scenario 4's forgery helpers — the survey §2.4 §E table specified 6 forgery variants including `future-timestamp` and `past-timestamp`, but `validate_event` at `xgen-core/src/message/exchange.rs:395+` does NOT include timestamp-bound validation. The doc-comment at `:371` references "timestamp check" historically as something the pre-F-4 asymmetric Path A did, but F-4's unified pipeline at line 376's sub-check coverage table (Step 8 hash / Step 9 predecessors / Step 10 DAG / Step 11 sender+membership / Step 12 signature / Step 13 permission) does NOT include a timestamp step. Timestamp validation was dropped in the F-4 unification, not preserved — production code has no `created_at` upper/lower bound checks. Second gap also surfaced: `mutated-sender` variant in the survey was under-specified — mutation-without-resign converges on `SignatureFailure` (duplicate of `bad-signature`); mutation-with-resign surfaces `UnknownSender` (step 11). The 6-variant survey enumeration had two structural issues, not one. **Reading B locked over Reading A + Reading C** per J-113 framing — amend canonical source FIRST in Chat Claude Track 1 atom (sibling-shape to J-099 + J-109 Track 1 precedents), then Clair picks up Commit 3b-4 against amended contract. Reading A (drop variants in JOURNAL only, don't amend survey) rejected as inconsistent with J-109 precedent. Reading C (add timestamp validation as production change in Commit 3b-4) rejected per Clair's own framing as D-071 violation (audits precede dependent work). **Enumeration revised from 6 variants × 5 families = 30 tests to 4 variants × 5 families = 20 tests.** The 4 retained variants each map 1:1 to a distinct ExchangeError variant in the production enum: `bad_signature` → `SignatureFailure`; `forged_sender_with_resign` → `UnknownSender` (forged sender re-signed with attacker's key not in IdentityRegistry); `mutated_event_id` → `EventIdMismatch`; `malformed_prev_events` → `DagError(String)`. Clean four-to-four mapping; no test asserts a contract that doesn't exist. **Timestamp-bound validation flagged as Gap G6** (production gap; severity medium — federation clocks can drift, malicious clients could spam future-dated events polluting downstream observers' timelines; resolution scope: own audit-design-impl arc per D-071, sibling to Phase 7.5 + bidirectional + topo-sort + persistence-amendment). Not promoted in this atom — surface-driven application per D-071 means the gap stays flagged-not-promoted until dependent work surfaces concrete drift or Joe locks a milestone for it. **Canonical-document-staleness-surfaces-at-dependent-milestone-implementation-time pattern: THIRD project instance** (first J-099 audit-doc + design-doc §11 amendments at re-walk Step 2; second J-109 survey §2.6 amendment; this third J-113 survey §2.4 amendment). Three instances make the pattern durable. **Discipline implication**: survey-shape documents should anticipate this pattern by including an explicit "production-code-verification check before any test enumeration is Joe-locked" precondition at survey close. Sibling-shape to D-077's framing — meta-level discipline surfacing from a third recurrence. Worth flagging at next sibling milestone's runbook-authoring as a runbook §1.1 lock candidate ("before enumerating test cases by name, grep the production code's error enum/reject-path table to verify each test case maps to a real production reject path"). **D-077 bidirectional sustainability applied at amendment authoring**: pre-amendment grep across `xgen-core/src/message/exchange.rs` confirmed the ExchangeError enum's 8 variants (EventIdMismatch, HeldPending, DagError, UnknownSender, NotASpaceMember, NotARoomMember, SignatureFailure, PermissionDenied, plus AiCapabilityViolation + AiRoleViolation + MissingEventId for AI/structural cases) and verified no timestamp variant exists; verified the validate_event sub-check coverage table at line 376 enumerates 6 active steps (8, 9, 10, 11, 12, 13) with no timestamp step. Backward-coherence audit on other survey-cited reject paths confirmed alignment. **Sibling-shape to J-099 audit-doc + design-doc §11 amendments + J-109 survey §2.6 amendment** — third instance of in-place canonical-record amendment at dependent-milestone implementation time. **D-074 application count: thirteenth instance.** **"Honest longer work over fast shortcuts" — count inherited at eighth from J-104, NOT incremented** at this within-milestone canonical-record amendment (close-event-not-recurrence-event sibling-shape to J-099 + J-109 framing). Previous v1.2 J-109 §2.6 + §2.6.1 content stands authoritative — see body §2.6 + §2.6.1 entries below. 2026-05-24 (J-109 — §2.6 Scenario 6 substantive rewrite + new §2.6.1 Phase 7.5 §6 contract-walkthrough sub-block; version bump v1.1 → v1.2. Cause: survey staleness surfaced at Clair's Pre-Commit-3b-2-equivalent verification read — §2.6 sub-item C step 3 claimed F-3 produces `DispatchOutcome::Rejected` with `reason = federation_relationship_missing`, but Phase 7.5 §6 "held-not-bypassed posture" (J-093 / J-094, locked 2026-05-19, shipped 2026-05-20) amended F-3's outcome shape to `DispatchOutcome::HeldPending` with new `disposition = "held_pending"` field on the existing `f3_reject` G2 trace event, recovery path via `drain_pending_by_federation_relationship` arrival hook, and 4007 federation_relationship_timeout sweep (default 180s via `[sync].federation_relationship_timeout_seconds`). Phase 7.5 §5 also extended the F-3 skip set from Lock B1's single `StateFederationAdd` type to three types (adds `StateSpaceCreate` + `StateDmSpaceCreate`). Survey was authored before Phase 7.5 shipped; survey findings v1.1 froze the pre-Phase-7.5 contract; the v1.1 §2.6 line ref `[federation_relationship_integration.rs:166]` was renamed at Phase 7.5 to `peer_without_relationship_held_pending_on_federation_relationship` at `:174`. **Sibling-shape to J-099 audit-doc + design-doc §11 in-place amendments at re-walk Step 2** (canonical-record drift surfaced at dependent-milestone implementation time; second project instance of "canonical document staleness surfaces at dependent-milestone implementation time" pattern — two instances not yet durable pattern, three would be). **D-077 bidirectional sustainability discipline applied at amendment authoring**: pre-amendment grep across federation canonical docs surfaced six file candidates with F-3-contract references; per-surface lock decisions recorded inline in J-109 sub-section 4 — three amended (findings v1.2, Phase 9 task drift correction, survey doc supersession pointer), three preserved unchanged with reasoning (completion runbook records Phase 7's locks correctly for their scope; design doc §6.4.1 already incorporates Phase 7.5; Compound C3 paragraphs acknowledged-stale in dropped-scenario context). **Reading B locked over Reading A + Reading C** per J-109 framing — amend canonical source FIRST in Chat Claude Track 1 atom (sibling-shape to J-107 Track 1 amending canonical records before Clair Track 2 pickup), then Clair picks up against the amended contract. Reading A (write-against-current-behaviour + note-staleness) rejected as no-drift-surface anti-pattern; Reading C (split Scenario 6 into HeldPending + recovery + 4007 timeout tests) rejected as scope-creep over Joe's GO for the original framing (if the amended survey surfaces a natural two-test framing, the call belongs in the survey amendment, not Clair's test-writing decision-space). M15 catalogue row added at topological-sort milestone-close commit per J-101 + runbook §6.3 exact phrasing. M15 covers wire-order non-determinism + causal-DAG-construction lie as one bug class with two layers (determinism layer + causality layer under amended D-076 v1.1); closed by topological-sort milestone J-101 across Commit 2 (determinism layer sort fix) + Commit 2a (causality layer Path B fix). Catches count now 12 of 15 (was 11 of 14 — Scenario 1 second `#[ignore]` lift at J-101 catches M15 in addition to its existing D-075 + D-076 v1.1 determinism locks; "Not caught" set unchanged at M6 + M8 + M13). Previous 2026-05-19: Joe-locked all four §8 open questions: Q1 → 12 scenarios; Q2 → defer G4 to M6; Q3 → option (i) serial-test first, escalate on Phase 9 signal only; Q4 → multi-commit Phase 9. See §8 for locked decisions inline.)  
> Language: English  
> Author: JozefN  
...
| Axis | Weak | Strong (recommended; J-113 amendment) | Rationale |
|---|---|---|---|
| Forgery variants | bad-signature | **{bad_signature, forged_sender_with_resign, mutated_event_id, malformed_prev_events}** | Four variants mapping to F-4 production outcomes at `xgen-core/src/message/exchange.rs`: `bad_signature` → `Rejected(SignatureFailure)`; `forged_sender_with_resign` → `HeldPending { missing_identity: Some(attacker_uri) }` for 4 families + `Validated`-then-ingested for state.federation_add per Phase 7 B3 design property (asymmetric cell, documented at §2.4.2 below per J-115 amendment); `mutated_event_id` → `Rejected(EventIdMismatch)`; `malformed_prev_events` → `Rejected(DagError)`. Original 6-variant set (which included `mutated-sender` ambiguous + `future-timestamp` + `past-timestamp`) revised at J-113; variant row 2 outcome mapping revised at J-115 against post-Phase-6 F-10 reality. See §2.4.2 (J-115) + §2.4.1 (J-113) for full walkthrough |
| Reject paths | step 12 only | **steps 8, 10, 11, 12 enumerated** | Coverage of the four active reject paths; bugs hide in the rare reject paths. Step 9 (HeldPending on unknown prev_events) is not a reject in the F-4 sense — it defers, not rejects — so it's outside Scenario 4's scope. Step 13 (permission) is event-family-specific and tested separately at the permission-policy layer. Steps 1-7 are wire-decoding layers tested at the transport scope |
| Event families | message.text | **5 families: message.text, membership.join, membership.kick, state.federation_add (skip-F-3 path per Phase 7.5 §5), state.room_create** | F-4's whole point is family-uniformity (unchanged from v1.1) |
| Volume | 1 | **20 mixed forged + 100 valid mixed in random order** | Catches catalogue bug M10 (asymmetry leak under load — overlaps C5; C5 below is the strong version per v1.1's final recommendation) (unchanged from v1.1) |

**Recommendation (J-113 + J-115 amendments):** 4 forgery variants × 5 event families = **20 NodeRuntime-level forgery tests**. Plus 1 end-to-end stress-complete-shape test with 20 forged + 100 valid mixed. **C5 below is the strong version** — Phase 9 should land C5 in place of an independent Scenario 4 stress-only test. The variant compression from 6 to 4 is recorded explicitly to make future readers aware that timestamp-bound validation (Gap G6 per §4.6 below) is a flagged production gap, NOT a forgotten test case. Variant row 2's outcome mapping was further revised at J-115 against post-Phase-6 F-10 reality — 4 families assert HeldPending, state.federation_add asserts Validated-then-ingested per Phase 7 B3 design property; see §2.4.2 below.

---

#### §2.4.1 Production-code-verification walkthrough (J-113 amendment)

> **Forward-pointer (J-115 amendment, 2026-05-24):** the production reject path inventory table below for the `UnknownSender` row, and the "Uniform assertion shape per test" block's step 3 (`Assert DispatchOutcome::Rejected`), are SUPERSEDED for variant row 2 (`forged_sender_with_resign_*`) by §2.4.2 below. The four other variants (`bad_signature_*`, `mutated_event_id_*`, `malformed_prev_events_*`) retain the J-113 shape. §2.4.1 below preserves the J-113 snapshot for historical record.

**Why this sub-block exists.** Sibling-shape to §2.6.1 (J-109 amendment) and to J-099's audit-doc §11 amendment. At J-113 implementation time Clair surfaced two structural issues with the v1.1/v1.2 §2.4 §E table's 6-variant enumeration: timestamp-bound variants assert against a production contract that doesn't exist, and the `mutated-sender` variant was under-specified about which attack shape it tests. This sub-block consolidates the production-code reality so future Scenario 4 implementers can map test cases directly to the production reject paths without re-discovering the gap.

**The ultimate source of truth is the code at `xgen-core/src/message/exchange.rs:46-87` (ExchangeError enum) plus `validate_event` at `:395+` (the dispatcher); this sub-block is a snapshot summary, not the canonical contract.**

**Production reject path inventory** (per ExchangeError enum at exchange.rs:46-87):

| Production variant | Step | Error string | Phase 9 Scenario 4 test variant |
|---|---|---|---|
| `EventIdMismatch` | Step 8 | "step 8: event_id does not match canonical content hash" | `mutated_event_id_*` (5 family tests) |
| `HeldPending(Vec<String>)` | Step 9 | "step 9: unknown prev_events — event held pending" | NOT a Scenario 4 case — step 9 defers, not rejects |
| `DagError(String)` | Step 10 | "step 10: DAG structural violation — {0}" | `malformed_prev_events_*` (5 family tests) |
| `UnknownSender` | Step 11 | "step 11: sender is not a registered Identity" | **NOT reachable via F-4 path** — only fires from `validate_steps_8_13` (legacy accept_message path, test-only-reachable per `runtime.rs:334-340`). F-4's `validate_event` returns `HeldPending { missing_identity: ... }` at unknown-signer per Phase 6 F-10 amendment (J-087); see §2.4.2 below per J-115 amendment |
| `NotASpaceMember` | Step 11 | "step 11: sender is not a Space member" | NOT a Scenario 4 case — covered by C5 + permission-policy tests |
| `NotARoomMember(String)` | Step 11 | "step 11: sender is not a member of room '{0}'" | NOT a Scenario 4 case — covered by C5 + permission-policy tests |
| `SignatureFailure` | Step 12 | "step 12: signature verification failed" | `bad_signature_*` (5 family tests) |
| `PermissionDenied(String)` | Step 13 | "step 13: permission denied for {0}" | NOT a Scenario 4 case — family-specific, separate test scope |

**No timestamp variant.** `validate_event` does not check `event.created_at` against any bounds. The pre-F-4 asymmetric Path A (messages) referenced "timestamp check" in legacy code; F-4's unification dropped it (intentional or not is open question at audit time per Gap G6 below). **Future-timestamp and past-timestamp tests would assert against a contract that does not exist** — dropped from Scenario 4 enumeration; production gap recorded at §4.6 Gap G6.

**`forged_sender_with_resign` vs `mutated_sender_no_resign`.** The v1.1/v1.2 survey's `mutated-sender` variant did not distinguish two distinct attack shapes:
- **Mutation without re-sign**: change `sender_node_id` after signing but keep the original signature — the canonical bytes change, so signature verification fails first (Step 12). Surfaces `SignatureFailure`, same as `bad_signature_*`. Redundant test shape.
- **Mutation with re-sign**: change `sender_node_id` to a forged identity AND re-sign with the attacker's key (which is NOT in IdentityRegistry) — signature verification passes (Step 12), but Step 11 (sender registration check) fails. Surfaces `UnknownSender`. Distinct test shape; this is the canonical "forged sender" attack.

J-113 keeps only the with-resign variant, renamed to `forged_sender_with_resign_*` for clarity. The no-resign variant is dropped as redundant with `bad_signature_*` — same reject path, same assertion shape; explicit duplicate adds nothing to regression coverage.

**The 20 enumerated cases** (variant-major / family-minor ordering; uniform assertion shape per §C):
...
**Uniform assertion shape per test** (per §C):
1. Construct event of family Y with forgery variant X applied (forge with key not in IdentityRegistry for `forged_sender_with_resign_*`)
2. Call `runtime.dispatch_event(forged, EventOrigin::ReceivedViaFederation, Some(peer_id))`
3. Assert `DispatchOutcome::Rejected`
4. Assert rejection `reason` matches the specific ExchangeError string for the variant
5. Assert event did NOT land in `runtime.spaces[space_id].event_count`

**Two load-bearing structural properties** (unchanged from J-109 Pre-Commit-3b-4 framing):
- **Family-uniformity**: same validation pipeline regardless of event family. If one family rejects with a different reason than the others for the same variant, that's a bug.
- **Variant-uniformity within a family**: all 4 variants reject via F-4's same dispatch shape; same `DispatchOutcome::Rejected` outcome, same code path through `validate_event`.

**`state.federation_add` row** specifically verifies Phase 7.5 §5. The extended skip set means F-3 doesn't reject this event type for missing relationship — F-4 is the rejection layer. So `bad_signature_state_federation_add` verifies "even though F-3 skips this event type, F-4 still catches forgery." Load-bearing sibling to C2's narrowness-regression assertion at the F-3 layer.

**Source-of-truth pointers.** Code: `xgen-core/src/message/exchange.rs:46-87` (ExchangeError enum) + `:395+` (validate_event dispatcher) + `:376` (sub-check coverage table doc-comment) + `xgen-core/src/node/runtime.rs:317` (dispatch_event entry). JOURNAL: J-113 entry for this amendment. Adjacent canonical record: `tasks/FEDERATION_PROPAGATION_PHASE_9.md` §3 Commit 4 paragraph for Phase 9-side enumeration.

---

#### §2.4.2 F-10 HeldPending contract for variant row 2 + state.federation_add B3 asymmetry (J-115 amendment)

**Why this sub-block exists.** Sibling-shape to §2.4.1 (J-113) and to §2.6.1 (J-109). At J-115 Pre-Commit-3b-4 Joe-lock checkpoint #2, D-078 production-code verification surfaced that the v1.3 §2.4 §E variant row 2 (`forged_sender_with_resign_*`) mapped to `Rejected(UnknownSender)`, but `validate_event` at `xgen-core/src/message/exchange.rs:626-632` (Phase 6 F-10 amendment, J-087) returns `ValidationOutcome::HeldPending { missing_identity: Some(sender.clone()) }` for unknown-signer events, NOT `Rejected`. The `UnknownSender` ExchangeError variant exists in the enum but is reachable only via `validate_steps_8_13` — the legacy accept_message path, test-only-reachable per `runtime.rs:334-340` doc-comment, NOT the F-4 path that `dispatch_event` calls. This sub-block supersedes §2.4.1's `UnknownSender` row + step-3 assertion shape FOR VARIANT ROW 2 ONLY; the four other variants retain the J-113 shape unchanged.

**D-078's first prospective-catch instance.** D-078 was promoted at J-114 specifically to catch this class of gap before Clair writes code. This is the principle's first prospective application — checkpoint #2 surfaced the gap; no test code had been written; Track 1 amendment landed cleanly. Sibling-shape to J-099 / J-109 / J-113 but procedurally distinct: those three were retroactive catches (Clair halts mid-implementation); this is the first prospective catch (Clair halts pre-implementation at D-078 verification). See J-115 entry for the prospective-vs-retroactive distinction recorded as discipline-notes data point.

**Production contract for variant row 2 per family** (per `validate_event` at exchange.rs:545+ and Phase 7 B3 amendment per J-088):

| Family | Outcome | Production reason | Assertion shape |
|---|---|---|---|
| `forged_sender_with_resign_message_text` | `ValidationOutcome::HeldPending { missing_identity: Some(attacker_uri), missing_predecessors: vec![] }` | F-4 step 11 unknown-sender per Phase 6 F-10 generalisation (J-087) | Destructure HeldPending; assert `missing_identity == Some(attacker_uri)` |
| `forged_sender_with_resign_membership_join` | `HeldPending { missing_identity: Some(attacker_uri), missing_predecessors: vec![] }` | Same as above | Same as above |
| `forged_sender_with_resign_membership_kick` | `HeldPending { missing_identity: Some(attacker_uri), missing_predecessors: vec![] }` | Same as above | Same as above |
| `forged_sender_with_resign_state_room_create` | `HeldPending { missing_identity: Some(attacker_uri), missing_predecessors: vec![] }` | Same as above | Same as above |
| `forged_sender_with_resign_state_federation_add` | **`ValidationOutcome::Validated` then ingests into B's DAG** | Phase 7 B3 amendment (J-088, locked 2026-05-20 at exchange.rs:455-509) — federation_add via federation channel SKIPS step 9 (predecessor presence), step 11 (sender registration AND sender membership), and step 13 (sender permission); only steps 8 (event_id hash), 10 (DAG structure), and 12 (signature) fire. Step 12 passes because the attacker re-signed with their own embedded key (B3 authority chain: session handshake + signature self-verification via `verify_event_signature`, no IdentityRegistry lookup) | Assert `DispatchOutcome::Accepted`; assert event lands in `runtime.spaces[space_id].event_count` (asymmetric cell — see security-property note below) |

**state.federation_add asymmetric cell — security-property documentation.** Phase 7 B3 amendment (J-088) is the protocol-level reason this cell is asymmetric. The skip of step 11 is intentional and necessary: federation_add IS the relationship-establishing event; the predecessor check would deadlock (predecessors are themselves held on Phase 7.5 §6 federation-relationship trigger); the sender-registration check fails because federation_add is Node-authored under Phase 4 §3.4.1 Q3 overload (sender = Node URI) and Node URIs are not inserted into IdentityRegistry by either of its populating paths. The authority chain is session-level handshake + step-12 signature verification (decodes pubkey from sender URI directly via `verify_event_signature` — pure crypto, no registry lookup, Q3-overload transparent at this layer). A forged state.federation_add re-signed with the attacker's own key passes step 12 (the signature matches the embedded pubkey) and ingests. This is the security property: **the protocol does NOT prevent an attacker who already has a federation channel from ingesting forged federation_add events via that channel.** Phase 7 B3 §4.1 + §4.3 + the F-1a tip-exchange shape lock this property; the test variant is the regression lock that proves the property persists. See `tasks/FEDERATION_PROPAGATION_PHASE_7_B3_AMENDMENT.md` (if exists) or design doc §6.4 for the authority-chain reasoning.

**Variant-uniformity at the matrix level.** Variant row 2 (`forged_sender_with_resign_*`) is structurally asymmetric (4 HeldPending + 1 Validated). The other three variants retain the J-113 family-uniformity (5 × `Rejected(specific_ExchangeError_variant)` per family). The matrix's job is to expose drift; the asymmetric cell IS the matrix exposing a Joe-locked design property (Phase 7 B3). Re-stating Lock 4's family-uniformity + variant-uniformity from runbook §1.1: family-uniformity = "same F-4 dispatch shape"; variant-uniformity = "same outcome class within a variant." Variant 2 row's outcome class is "defer-or-ingest at unknown-signer per F-4 + Phase 7 B3 layering" — the asymmetry IS the outcome class.

**Uniform assertion shape per test for variant row 2** (supersedes §2.4.1's step 3 for this variant; J-116 amendment rewrites the Rust block against production API shape — `DispatchOutcome::HeldPending` is a UNIT variant per `xgen-core/src/node/runtime.rs:88-95`; F-10 `missing_identity` info lives in `PendingBuffer`, NOT on `DispatchOutcome`):

```rust
// For 4 families (message_text, membership_join, membership_kick, state_room_create):
//
// (a) Outcome signal — DispatchOutcome::HeldPending is a UNIT variant.
//     Per J-116 amendment: J-115's HeldPending(buffer_state) destructure
//     won't compile against production type at runtime.rs:88-95.
assert!(
...
//     runtime.rs:126-137.
let buf = runtime.pending.get(&space_id)
...
assert!(
    ...
    "pending_identity_count must have incremented by at least 1"
);

// (c) Event did NOT land in DAG — still buffered, not ingested.
//     APIs: EventStore::contains(&str) -> bool at xgen-core/src/dag/store.rs:48;
//           EventStore::len() -> usize at :52.
//     NodeRuntime exposes `pub stores: HashMap<String, EventStore>` at runtime.rs:126-137.
assert!(
    !runtime.stores[&space_id].contains(&forged_event_id),
    "forged event {} must not be in EventStore (HeldPending != Accepted)",
    forged_event_id
);
assert_eq!(
    runtime.stores[&space_id].len(),
    baseline_store_len,
    "event store length must not increment on HeldPending"
);

// Note: stronger missing_identity == Some(attacker_uri) assertion deferred
// per Option ε.iii lock at J-116 — PendingBuffer does not expose per-entry
// missing_identity as a public accessor today; promoting that accessor
// would be its own audit-design-impl arc per D-071. The two-part (a)+(b)
// assertion above is the available deployment-level F-10 regression lock.

// For state_federation_add (asymmetric cell per Phase 7 B3 J-088):
//
...
// Phase 7 B3 asymmetry: forged federation_add via federation channel ingests
// because step 11 is skipped, step 12 passes on attacker's self-signature.
// This is the security property documented at the table above; the test
// pins it as the regression lock for the Joe-locked B3 design property.
// Explicit doc-comment in the test file should name Phase 7 B3 + J-088.
```

**Source-of-truth pointers.** Code: `xgen-core/src/message/exchange.rs:545+` (validate_event F-4 unified core) + `:626-632` (Phase 6 F-10 unknown-sender HeldPending return) + `:455-509` (Phase 7 B3 federation_add skip block); `xgen-core/src/node/runtime.rs:317` (dispatch_event entry) + `:334-340` (test-only-reachable doc-comment for validate_steps_8_13 path) + `:88-95` (DispatchOutcome enum — HeldPending unit, Rejected(String) tuple, Accepted struct; J-116 anchor) + `:126-137` (NodeRuntime fields — `pub stores` + `pub pending`; J-116 anchor). PendingBuffer + EventStore accessors: `xgen-core/src/dag/pending.rs:491` (PendingBuffer::contains; J-116 anchor) + `:498` (pending_identity_count; J-116 anchor); `xgen-core/src/dag/store.rs:48` (EventStore::contains; J-116 anchor) + `:52` (len; J-116 anchor). Event field: `xgen-common/src/wire.rs:332` (event_id: Option<String>; J-116 anchor). JOURNAL: J-116 entry for template-API amendment; J-115 for contract amendment; J-087 for Phase 6 F-10 origin; J-088 for Phase 7 B3 origin. Adjacent canonical record: `tasks/FEDERATION_PROPAGATION_PHASE_9_COMMIT_3B_4_IMPL.md` v1.2 §4.5 amended templates for runbook-side enumeration; `tasks/FEDERATION_PROPAGATION_PHASE_9.md` §3 Commit 6 paragraph for Phase 9-side enumeration.
...
### §4.6 Gap G6 — No timestamp-bound validation in `validate_event` (J-113 amendment)

**What gap.** `validate_event` at `xgen-core/src/message/exchange.rs:395+` does NOT check `event.created_at` against any clock-skew bounds. The sub-check coverage doc-comment table at `:376` enumerates 6 active validation steps (8 hash / 9 predecessors / 10 DAG / 11 sender+membership / 12 signature / 13 permission) and includes NO timestamp step. The pre-F-4 asymmetric Path A (messages-only) referenced a "timestamp check" in its legacy code path; F-4's unification of the pipeline across event families **dropped** the timestamp check, did not preserve it. Whether the drop was intentional (a deliberate scope reduction in F-4's design) or accidental (a silent loss during the unification refactor) is an open question for the audit phase if/when this gap is promoted.

This gap was surfaced at J-113 by Clair during Commit 3b-4 implementation when constructing Scenario 4 forgery helpers — the v1.1/v1.2 survey §2.4 §E enumeration included `future-timestamp` and `past-timestamp` forgery variants that would have asserted against a contract that doesn't exist in production. Variants dropped from the Phase 9 Scenario 4 enumeration per §2.4 + §2.4.1 amendment; gap recorded here as a production observability surface for future work.

**Severity.** **Medium.** Federation participants can have meaningfully-skewed clocks (no NTP requirement in the protocol); without bounds, malicious or buggy clients can spam future-dated events that pollute downstream observers' timelines (UI sort orders, audit trails, retention windows). Sibling-shape concerns to other distributed-systems clock-skew problems (e.g. CRDT timestamp manipulation attacks, log-append integrity). Not HIGH because the protocol's DAG-causality layer (steps 9 + 10) provides a structural ordering independent of `created_at`, so the F-3/F-4 federation security perimeter is unaffected; the impact is on consumer-facing observability layers downstream.

**Which scenarios are affected** (under the amended Phase 9 set). Scenario 4 (Phase 9 enumeration revised to drop timestamp variants); future Compound Cnn (clock-skew attack scenario, if Joe locks a follow-on milestone for it).

**Resolution scope.** **Own audit-design-impl arc per D-071.** Sibling to Phase 7.5 + bidirectional + topo-sort + persistence-amendment in shape: surface (here, J-113) → surface-driven design discussion when concrete dependent work surfaces (not now) → design lock → impl + tests → close. The shape ensures the timestamp window (±5 minutes? ±1 hour? ±1 day? clock-skew handling per federation? per identity?) is Joe-locked through proper walkthrough rather than guessed at by an implementer reading half-context.

Not promoted in this atom. The J-113 amendment is recording-only — a flag for future work, not a scheduled milestone. Per D-071 surface-driven application discipline, the gap stays flagged-not-promoted until either (a) dependent work surfaces concrete drift (a real federation deployment exhibits timestamp-manipulation impact and Joe escalates), or (b) Joe locks a milestone for it on its own merits.

**Options for future resolution** (when the audit-design-impl arc opens):
- (a) Add upper/lower bound checks to `validate_event` with operator-configurable bounds per the federation-relationship-timeout precedent (`[sync].federation_relationship_timeout_seconds`). Default would be Joe-locked at design time.
- (b) Defer at the protocol layer; surface timestamp anomalies to operator-side audit logging instead (sibling to Gap G4's M6 audit-log shape).
- (c) Per-Space policy (some Spaces require strict bounds, others accept skew). Larger scope but more flexible.

**Source-of-truth pointers.** Code: `xgen-core/src/message/exchange.rs:395+` (validate_event dispatcher) + `:376` (sub-check coverage table doc-comment) + `:371` (the historical "timestamp check" doc-comment reference to pre-F-4 Path A). JOURNAL: J-113 entry. Phase 9 §2.4 §2.4.1 amendment for the originating context.
