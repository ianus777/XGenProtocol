...
🟢 **(none currently active)** — the project is between milestones at this moment. The Propagation Reliability Audit closed on 2026-05-18; the next active work has not yet been picked up. Several pending tracks are queued (see below); which one becomes active next is a pacing call.
...
🟡 **Documentation correction pass — small, low-risk.** Post-audit edits to `docs/xgen_node_admin_ops_design.md` (§3.1 `TransportMessage::Error` shape correction, §3.2–§3.4 reference the envelope-level `event_id` rather than per-variant field, new short §3.6 describing rejection path, §9 D-070 framing aligned with audit findings) and `docs/xgen_ch4_implementation.md` (lines 779 and 825–827 describe a Node-to-Node `transport.sync_request` flow that doesn't exist; correct or remove). Chat Claude + Joe work. Estimated half a session. Independent of other tracks; can land any time.

🟡 **D-070 promotion to DECISIONS.md — small, recording-only.** The D-070 draft currently lives in §9 of the design doc. Promotion to a proper numbered DECISIONS.md entry happens with the corrected framing from §5 of the audit (the principle now requires both `EventAccepted` AND envelope-level `event_id`-correlated rejection signal in M6 Phase 2 — not just acceptance as Pass 3 originally framed it). Chat Claude + Joe work. Quick.
...
🟡 **Federation Event Propagation completion milestone — design phase needed first.** The Propagation Reliability Audit surfaced that Node-to-Node federation propagation does not exist in production code (only one-time history sync during handshake). This milestone closes the gap: design phase (Pass 1 / Pass 2 / Pass 3 like M6 just did) followed by implementation. **Includes the `process_inbound` validation asymmetry from audit §3 as an explicit precondition** — closing federation push without first closing the validation asymmetry would land a vulnerability (Paths B/C skip signature verification, becoming exploitable the moment federation push lands). Probably 3–4 sessions of design work plus several phases of implementation. Blocks M6 (new) going PLAY.
...
🟢 **Honest longer work over fast shortcuts.** Project-management principle. When project work surfaces a real gap, the default response is to close the gap properly, even if that delays downstream work. Locked during the audit's federation finding discussion; informs all milestone-sequencing calls. Pairs with the audit-precedes-dependency discipline above.
