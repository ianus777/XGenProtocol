...
│   ├── 🟢 **M-RP-MEMBER-ACT** — the members panel acts: LMC opens the DM, RMC opens the menu · Phase-0 `tasks/M_RP_MEMBER_ACT_PHASE0.md` v1.11 · Leg C Phase-0 `tasks/M_RP_MEMBER_ACT_LEG_C_PHASE0.md` v1.4 · Leg C runbook `tasks/RUNBOOK_MEMBER_ACT_LEG_C.md` **v1.1 ACTIVE, all nine points LOCKED** · runbook `tasks/RUNBOOK_MEMBER_ACT_LEG_AB.md` v1.3 COMPLETED · `ce82ebe` `8c70d14` `132ce85` `41bf61a` `7305df2` · `DECISIONS.md` D-143 + D-144 + D-145 · J-699 (J-680, J-681, J-682, J-683, J-685, J-686, J-687, J-688, J-689, J-690, J-691, J-692)
│   │   ↳ Owes: **`OQ5` re-sited J-690, not answered** — the partial first send and DM-creation-to-an-erased-identity are **Leg C-bis's** · **what a never-DM'd click does is OPEN (J-692), and Leg C ships that click as a NO-OP** — a dead control, named and owned, whose removal enters Leg C-bis's DoD · **Leg E owes the Space-never-clicked case — a DM entered leaves R2 listing the previous Space's rooms (J-694)** · the erased row's retention behaviour is owed by the milestone that builds history-expiry and auth tiers · cross-node invite discovery is a measurement of Chat's · clippy `-D warnings` has four pre-existing errors, not a tracked floor, left untouched
│   │   ↳ trigger: **RUNBOOK LOCKED v1.1 ACTIVE (Joe, all nine points L-1–L-9, on one word)** — Clair implements C-1 the `ui/core` flag alone, C-2 `roomLatch.latch()` + the header annotation, C-3 R7 acts, C-4 live verify; Chat re-drives every gate
