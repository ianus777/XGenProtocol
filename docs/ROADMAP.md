...
> Version: 1.0  
> Date: May 2026  
> **Last updated**: 2026-05-19 (J-085 — Federation Event Propagation implementation Phase 4 SHIPPED: F-1 federation event push at `apply_fanout` sibling position; F-1b drop-on-peer-down with no outbound queue; F-5 origin gating via in-memory `EventOrigin` enum threaded through `dispatch_event`/`process_inbound`/`apply_federation_push` per Q1 lock; new `FederationPeerSenders` registry mirroring `ClientSenders` per Q2 lock; Q3 reuse of `process_inbound` with two-comment overload documentation; Phase 3's R1 plug-in point in `handle_federation_incoming` now operational; the "missing mechanism" verdict from audit J-081 §2 closed — Stage 6 federation propagation exists as a production mechanism. 491 tests (+3). Present summary updated; Past gains a Phase-4 close entry under Federation Event Propagation implementation.)  
> Language: English  
> Author: JozefN  
...
🟢 **Federation Event Propagation implementation — Phases 1-4 ✅ SHIPPED; Phase 5 next-active.** The nine-phase runbook at `tasks/FEDERATION_PROPAGATION_COMPLETION.md` (Status: ACTIVE, v1.0) drives this milestone. **Phase 1** closed in J-082 on 2026-05-18 (F-6 + F-7). **Phase 2** closed in J-083 on 2026-05-18 (F-4 `process_inbound` validation pipeline unification). **Phase 3** closed in J-084 on 2026-05-19 (F-1a federation handshake reshape to bilateral tip exchange with `tips: BTreeMap<String, String>` on `Hello` + `Capabilities`; new `stream_federation_delta` orchestrator). **Phase 4** closed in J-085 on 2026-05-19 (F-1 federation event push + F-1b drop-on-peer-down + F-5 origin gating; new `EventOrigin` enum in `xgen-core::node::runtime`; new `FederationPeerSenders` registry mirroring `ClientSenders`; new `apply_federation_push` as sibling of `apply_fanout`; Phase 3's R1 plug-in point now operational; the "missing mechanism" verdict from audit J-081 §2 closed). 491 tests passing (+3 integration tests in new `federation_push_integration.rs`). **Next: Phase 5 — F-1c per-peer record + reconnect scheduling.** Persistent per-peer record at the Node-implementation layer (lost_connection flag, last_seen, last_successful_session, next_reconnect_attempt, operator_notes, priority) per design doc §4.6. Reconnect scheduler task reads F-1c records flagged lost_connection, computes next attempt via global backoff schedule (15/30/60/120 min capped), fires `run_initiating` for due peers — which gains its FIRST production caller in `xgen-node/src/` (audit J-081 §2.2 found zero today). Schema decision (extend `peer_announcements` vs sibling table) is Clair-latitude per runbook §3.5. Phase 3's bilateral `stream_federation_delta` is the initiator-side caller from Phase 5's reconnect path (per Phase 3 §3.3.1 Lock 7 R5). Remaining work after Phase 5: Phase 6 (HeldPending generalisation for unknown signer Identity, F-10) → Phase 7 (federation-relationship verification gate, F-3 second check) → Phase 8 (documentation pass) → Phase 9 (integration tests for full federation push path). Blocks M6 (new) going ACTIVE.
