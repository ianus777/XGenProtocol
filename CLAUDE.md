...
> Date: May 2026  
> **Status:** ACTIVE  
> **Last updated:** 2026-05-22 (Topological-sort wire-order non-determinism audit doc SHIPPED at `tasks/FEDERATION_TOPOSORT_AUDIT.md` Status: ACTIVE v1.0. Ten sections per the sibling-in-shape template at `tasks/FEDERATION_BIDIRECTIONAL_NODES_AUDIT.md`. Frames Q1 (tie-break choice), Q2 (D-067 audit scope — narrow/middle/wide readings), Q3 (wire-format normative question — LOAD-BEARING) and four candidate fix shapes (A `event_id` sort at topo primitive / B timestamp sort / C canonical-event-bytes sort / D EventStore container change with D.1 BTreeMap + D.2 IndexMap sub-options) with §7.5 summary table. Phase 9 Scenario 1 stays `#[ignore]`-annotated pending fix. Phase 9 Commit 3b stays paused. Topological-sort phase moves from 🟢 Audit ←── HERE to 🟢 Design ←── HERE in the four-step arc; Chat Claude + Joe author `tasks/FEDERATION_TOPOSORT_DESIGN.md` next, may promote a new D-NNN to DECISIONS.md at design close. Previous J-096 milestone-close content stands authoritative — see DONE-IN-FLIGHT block.) Previous 2026-05-21 update: Bidirectional `federation_nodes` implementation milestone CLOSED in four-commit sequence per JOURNAL J-096 — Commit 1 doc-pass (`e975162`), Commit 2 origin-aware applier + plumbing + six unit tests (`a730eda`), Commit 2.5 sibling vantage-aware drain hook (`cbceb41`, in-flight gap closure), Commit 3 Phase 9 Scenario 1 resurrection (`f051039`), Commit 4 milestone close (this commit). Test count 571 → 577 across the milestone (+6 unit tests in Commit 2; Scenario 1 originally lifted in Commit 3 then re-stood-down in this commit on a separate topological-sort finding, net wash on ignored count). Bidirectional fix is verified-correct — six unit tests in `xgen-core/src/space/state.rs::mod tests` (notably `apply_federation_add_two_vantages_mirror`) remain green and stand as the live D-075 regression lock. SEPARATE pre-existing finding surfaced during Commit 4 verification, NOT implicated by the bidirectional fix: wire-order non-determinism in `topological_sort_events` at `xgen-node/src/fanout.rs:193` fed by non-deterministic `HashMap.values()` iteration in `compute_federation_delta_for_space` at the same file ~:321. Both `state.space_create` and `state.room_create` are DAG roots with empty `prev_events`; they tie at the top of the topo sort and their wire order tracks HashMap iteration. Evidence: 105 `dispatch_event` calls on B in both pass and fail runs (divergence is upstream wire order); in a failing run `room_create` reaches B's dispatcher before `space_create` and is rejected with "space not found", cascading through the bootstrap chain. Per Joe-direction this opens as its own phase per D-071, sibling-shape to the bidirectional `federation_nodes` audit → design → impl arc just closed. Phase 9 Scenario 1 re-stood-down with updated stand-down comment naming the topological-sort finding and forward-referencing placeholder `tasks/FEDERATION_TOPOSORT_AUDIT.md` (to be authored by Chat Claude as the new active phase). Phase 9 Commit 3b stays paused inside milestone scope. ROADMAP.md v1.10 → v1.11 in same commit per same-commit discipline; `tasks/FEDERATION_BIDIRECTIONAL_NODES_IMPL.md` Status flipped ACTIVE → COMPLETED v1.1. Per D-069 canonical-document + D-071 audit-precedes-dependent-design + D-074 milestone-close-includes-JOURNAL + D-075 + "honest longer work over fast shortcut" discipline. M6 (new) blocking chain extended by one more node, unchanged in shape. Pass 1 of XGID Retrofit stays at Status: ACTIVE v2.0 unchanged.)  
> Author: JozefN  
> Credits: Concept, philosophy, requirements, project direction: Jozef Nižnanský. Technical assistance and implementation support: AI-assisted development tools.  

---

## 🟢 PLAY — Topological-sort wire-order non-determinism phase: Audit ✅ SHIPPED 2026-05-22, Design ←── HERE; Chat Claude + Joe walk Q1+Q2+Q3 and pick from Shapes A/B/C/D (Phase 9 PAUSED at Commit 3a, stand-down comment forward-references the now-shipped audit doc)

**Audit doc shipped 2026-05-22** at `tasks/FEDERATION_TOPOSORT_AUDIT.md` (Status: ACTIVE v1.0, ten sections, sibling-in-shape to `tasks/FEDERATION_BIDIRECTIONAL_NODES_AUDIT.md`). Code-grounded mechanism at file:line (Site 1 `compute_federation_delta_for_space:321` HashMap.values() feed + Site 2 `topological_sort_events:193` input-order-preserving tie-break compound into wire-order non-determinism). Sibling canonical sort at `xgen-core/src/node/runtime.rs:859-912` pinned as reference. Verbatim J-096 evidence in §2 (105 dispatch_event calls in pass and fail, 102/3/0 vs 2/101/2 cascade).

**Three audit-phase questions framed for design walkthrough:**
- **Q1 — Tie-break choice.** Four sub-options: Q1.a event_id (lex), Q1.b timestamp (millisecond collisions; wall-clock non-canonical across senders), Q1.c canonical-event-bytes (strongest; cross-crate dep), Q1.d container change at EventStore. Downstream of Q3.
- **Q2 — D-067 audit scope.** Three readings: narrow (primitive + optional sibling §3.1 only, other leaky sites deferred indefinitely), middle (fix the primitive's contract once; per-site surfacing becomes its own design question — known-leaky sites `collect_sync_history` + `apply_fanout` NOT ignored, scheduling decided separately), wide (every wire-visible-ordering site closed wholesale).
- **Q3 — Wire-format normative question (LOAD-BEARING).** Q3.i per-receiver-deterministic-tie-break sufficient (any shape closes Scenario 1) vs Q3.ii canonical wire ordering required (two senders with identical state MUST produce byte-identical deltas; Shape A requires sibling §3.1 fix, OR Shape D, OR hybrid). Both readings legitimate; Joe-lock-threshold.

**Four candidate fix shapes presented (audit does NOT pre-decide):**
- **A — event_id sort at topo primitive.** One line. Pass-1 coupling has v-options: v1 `&str` sort + comment-block flagging Pass 3 retype (Pass-1-neutral) vs v2 typed EventXgid from outset coupled to Pass 1 OR `Xgid::new(...)` wrap matching bidirectional Commit 2's `SpaceLocalMetadata.introducer_node_id` precedent. Admissible under both Q3 readings (Q3.ii requires sibling §3.1 fix).
- **B — Timestamp sort.** Disqualified under Q3.ii (wall-clock non-canonical across senders).
- **C — Canonical-event-bytes sort.** Strongest Q3.ii guarantee. Structural note in audit: canonical bytes include event_id by construction, so ordering ≈ Shape A for distinct-event_id events; additional benefit concentrated in duplicate-event_id edge case (not currently emitted) and explicit-canonicality contract-layer property.
- **D — EventStore container change.** D.1 BTreeMap by event_id (admissible both readings, widest D-067 reach); D.2 IndexMap (disqualified under Q3.ii).

**Phase 9 Scenario 1 stays `#[ignore]`-annotated** in `xgen-node/src/tests/phase9_two_node_smoke.rs` with stand-down comment forward-referencing the now-shipped audit. Lifts when fix lands. Phase 9 Commit 3b (Scenarios 2 + 3 + compounds C2/C3/C5/C7/C9/C10) stays paused inside milestone scope.

**Track 1 (Chat Claude + Joe, this session): design walkthrough.** Walk Q3 first (load-bearing — gates which shapes are admissible), then Q2 (narrow/middle/wide — determines blast radius), then Q1 (tie-break + Pass-1 v-option) against the Q3+Q2 frame. Author `tasks/FEDERATION_TOPOSORT_DESIGN.md` (sibling-in-shape to `tasks/FEDERATION_BIDIRECTIONAL_NODES_DESIGN.md`). May promote new D-NNN to DECISIONS.md at close. Implementation runbook (Clair-facing) authored same-day or next session per the bidirectional precedent.
...
**Entry point for Chat Claude's next session: design walkthrough opens (or `tasks/FEDERATION_TOPOSORT_DESIGN.md` authoring continues if design walked this session).** Bidirectional design at `tasks/FEDERATION_BIDIRECTIONAL_NODES_DESIGN.md` is the structural template.
