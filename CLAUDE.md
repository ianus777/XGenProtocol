...
> Date: May 2026  
> **Status:** ACTIVE  
> **Last updated:** 2026-05-19 (J-093 — Phase 9 Commit 3 STOOD DOWN at harness setup after Scenario 1 surfaced failure-mode catalogue M5 ("F-3 brand-new federation bootstrap dead-locked"): F-3 rejects `state.space_create` arriving via federation because target Space doesn't exist on receiver yet, and Phase 7's B1 skip rule only covers `state.federation_add`, not Space-create. Phase 9 paused at Commit 3 boundary; J-092 Commits 1 + 2 (observability preconditions G1+G2+G3 and `#[serial_test::serial]` flake fixes) stay shipped because both are pure infrastructure. New milestone phase: **Phase 7.5 Federation Cold-Start Bootstrap**. Design task file `tasks/FEDERATION_PROPAGATION_PHASE_7_5_DESIGN.md` authored (ACTIVE v0.1) with four `[JOE-LOCK: pending]` framework decisions: P7.5-A narrow F-3 + F-4-step1 skip for Space-create EventTypes (sibling to Phase 7 B1); P7.5-B third HeldPending trigger for missing federation relationship resolving on `state.federation_add` ingestion; P7.5-C per-trigger timeout (predecessor + Identity remain 30s, federation-relationship defaults 120s); P7.5-D observability counter `pending_federation_relationship: usize` in state.json. WIP harness + Scenario 1 stub reverted from working tree. Test count unchanged at 519. **Next session: Joe walks through the four Phase 7.5 design locks; after locking, implementation runbook authored, Clair implements, then Phase 9 resumes from Commit 3.**)  
> Author: JozefN  
> Credits: Concept, philosophy, requirements, project direction: Jozef Nižnanský. Technical assistance and implementation support: AI-assisted development tools.  

---

## 🟢 PLAY — Phase 7.5 Federation Cold-Start Bootstrap design phase (Phase 9 paused at Commit 3 boundary)

**Phase 7.5 design phase OPEN as of J-093 (2026-05-19), awaiting Joe-lock walkthrough.** Phase 9 Commit 3 surfaced failure-mode catalogue M5 ("F-3 brand-new federation bootstrap dead-locked") at harness setup — the survey predicted this and named it; Scenario 1's first runtime assertion (a brand-new in-process Node B receiving Space S from Node A via federation handshake) walked into it cleanly. Quoted rejection stream in [JOURNAL J-093](JOURNAL.md). Root cause: F-3 (Phase 7) rejects `state.space_create` arriving via federation because `SpaceState.federation_nodes[S]` lookup returns None when Space S doesn't exist locally yet; Phase 7's B1 skip rule only covers `state.federation_add`, not Space-create. Even `state.federation_add` itself then fails F-4 step 1 ("space not found") because the Space was never created upstream. Net: brand-new federation peers cannot bootstrap a Space — the cold-start path that Phase 5 reconnect + Phase 3 bilateral tip exchange + Phase 4 push + Phase 7 F-3 all assumed works does not actually work end-to-end.

**Phase 7.5 design task file at [tasks/FEDERATION_PROPAGATION_PHASE_7_5_DESIGN.md](tasks/FEDERATION_PROPAGATION_PHASE_7_5_DESIGN.md) (ACTIVE v0.1).** Four framework decisions enumerated, all currently `[JOE-LOCK: pending — awaiting walkthrough]`:
- **P7.5-A** — Narrow F-3 + F-4-step-1 skip rule for `state.space_create` and `state.dm_space_create` EventTypes (sibling to Phase 7 B1's skip for `state.federation_add`). Authority preserved via signature verification (still checked; F-10 HeldPending covers unknown-signer case during cold-start). DAG topology unchanged.
- **P7.5-B** — Third HeldPending trigger condition: "missing federation relationship for (peer, space)". Resolves on `state.federation_add` arrival hook. New error code `4007 federation_relationship_timeout`. Combines cleanly with F-10 (Identity) and F-4a (predecessor) triggers via existing struct-variant Option fields. **Held-not-bypassed posture** — F-3 enforcement is deferred until its data source is populated, not weakened.
- **P7.5-C** — Per-trigger timeout: predecessor + Identity remain at 30s (F-4a / F-10a unchanged); federation-relationship defaults to 120s with new `[sync].federation_relationship_timeout_seconds` config field. Brings forward F-10a's anticipated v2 evolution path because bootstrap streams are normal-case slow (not pathological) and routinely exceed 30s.
- **P7.5-D** — Observability: new `pending_federation_relationship: usize` counter in `xgen-node_state.json` (sibling to Phase 6's `pending_identity_replication`). Phase 9 Commit 1's G1+G2+G3 observability already covers the rest; no new trace events needed beyond the existing `f3_reject` (whose semantic shifts from "rejected" to "held pending" without changing the trace point).

**Sequence after Phase 7.5 design locks:** doc-pass commit (canonical design doc updated with §6.4.1 Phase 7.5 locks; this task file flipped to COMPLETED) → implementation runbook authored as separate Clair-facing task file per D-069 → Clair implements → Phase 7.5 closes → Phase 9 resumes from Commit 3 with Scenario 1 working as originally designed.

**Phase 9 status: PAUSED at Commit 3 boundary.** Implementation task file at [tasks/FEDERATION_PROPAGATION_PHASE_9.md](tasks/FEDERATION_PROPAGATION_PHASE_9.md) remains Status: ACTIVE v1.0 — its scope is intact, only the dependency order changes (Phase 7.5 ships first; Phase 9 Commit 3 then exercises the fix as end-to-end verification). J-092's Commits 1 + 2 (observability preconditions G1+G2+G3 + `#[serial_test::serial]` flake fixes at the two known-flake sites) **stay shipped** — pure infrastructure, neither depends on the broken cold-start path.

**Why D-071 ("subsystem audits precede dependent milestones") still won.** The Phase 9 survey (J-090) catalogued M5 explicitly; the design phase locked it as a known gap; Phase 9 Commit 3's harness setup walked into it exactly where predicted. The discipline extends one level finer: "design gaps surface during dependent work and close before the dependent work proceeds." Phase 7.5 is the formal recognition of that finer-grained pattern. The cost of pausing Phase 9 to do this design pass is small; the cost of "shipping" Phase 9 with a Scenario 1 that pre-loads Space state on B (avoiding the bug rather than testing it) would have been a green milestone ratifying broken production code.

**Entry point for next session: Joe walks through the four `[JOE-LOCK: pending]` markers in [tasks/FEDERATION_PROPAGATION_PHASE_7_5_DESIGN.md](tasks/FEDERATION_PROPAGATION_PHASE_7_5_DESIGN.md) §5 (P7.5-A), §6 (P7.5-B), §7 (P7.5-C), §8 (P7.5-D).** After locking, the implementation runbook gets authored.
