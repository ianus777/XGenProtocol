...
- **M-RP6.1e** — **client frame consolidation, split A/B/C (J-493).** See §10.
  ...
  - **M-RP6.1e-C** — `dialog`/`modal` `core` + **Help→About**. **Split three ways (Joe-locked, J-496)** — the milestone is `dialog` **+ real Rust**, so it does not close in one commit pair:
    ...
    - **M-RP6.1e-C3** 🟡 **(next-active)** — **Help→About assembly**: the Help menu (`help.about`, **no accelerator** — F1 conventionally means Help *contents*) + the `dialog` (C1) mounted and fed by `get_about_info` (C2) + the logo assets. About = name · version · link · hi-res logo · Built · Rust/Tauri/Svelte · Platform · app dir · data/config paths · **Close** (not "OK"). **Render `Built` + `commit` together** — `built` is the last compile, `commit` is what exactly identifies the build. **Open for Joe:** the shipped `name`/`link` literals (`"XGen Client"` / `alchemydump.com`) were Clair's grounded choice, not a Joe-lock — tune here. Verify **real client 9222**; the client registry grows from **7** — measure it.
