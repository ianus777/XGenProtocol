...
## 🟢 UI component-library / substrate (RP track) — M-RP2.15 ✅ CLOSED (J-420): `range` — the tenth `core` component, a stand-alone atomic `<input type="range">` (bounded numeric slider, numeric `bind:value`); shares root AND value-type with `number` yet stays separate — the case that sharpens the D-096 fold criterion to root + value-type + shared skin/surface (→ D-096 amendment, N-042); always-valued (never `null`); first pseudo-element-heavy skin (track + thumb) — CDP-verified BOTH apps
