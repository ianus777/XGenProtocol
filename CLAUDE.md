...
## 🟢 PLAY — M6 (new): **federation-admin-control 2a — Commit 4 (`accept`/`reject`/`initiate` verbs) SHIPPED at J-177**; **next-active = Clair Commit 5 (close — §6.A1 marks the 3 verbs SHIPPED, backing-audit row, runbook → COMPLETED, PLAY → 2b)** per runbook §7 (no checkpoint); then 2b (policy) → auth-module-registry → bootstrap-client; then M7 `--aicontrol`
