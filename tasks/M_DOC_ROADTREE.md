...
🛑 **THAT FIGURE IS WRONG AT THE ROOT AND IS ANNOTATED, NOT ERASED (measured 2026-07-31, J-635).** The enumeration used **`## Entry J-nnn`** — but `JOURNAL_ARCHIVE.md` carries **16 headings in an older form, `## J-nnn — date — title`** (against 353 in the `Entry` form), and **the predicate cannot see them.** ✅ **Measured:** `## J-nnn — …` = **16** · `## Entry J-nnn …` = **353**. 🔑 **EIGHTH INSTANCE OF THIS ARC'S DEFECT CLASS — A PREDICATE BUILT FROM ONE NOTATION REPORTING THE ABSENCE OF THE OTHERS AS THE ABSENCE OF THE THING — AND THIS ONE IS THE PREMISE OF AN ENTIRE LEG.** ⇒ **THE CENSUS MUST BE RE-RUN UNDER `D-135` §1① BEFORE ANY FIGURE FROM IT IS USED AGAIN.**

| Set | Numbers | Reading |
|---|---|---|
| **A — one contiguous block of 13** | `J-021 … J-033` | ⚠️ **Smells like a numbering artifact, not thirteen lost entries** — a reset, a renumber, or a batch never allocated. **Not claimed either way.** 🛑 **ALL THIRTEEN EXIST (J-635).** `J-021`–`J-033` are every one present in `JOURNAL_ARCHIVE.md` (L15843–L16422 as measured that day), in the **old `## J-nnn — date — title` form**. ✅ **They were never missing.** 📌 **The hedge was right and its reason was wrong.** |
...
✅ **`J-029`'s *“disposition genuinely open”* DISSOLVES (J-635): it has an entry, `JOURNAL_ARCHIVE.md` `## J-029 — 2026-04-30 — Fix 17 applied; Phase 1 smoke test with logging verified`.** 📌 **It was only ever open because the predicate was blind to its heading form.** ⚠️ **`J-171` (set C) is the ONE number confirmed to have no entry** — no `#`-prefixed heading in either journal names it. **A non-heading record form was NOT tested.**
...
✅ **SET B IS NOW CLOSED (J-603, proven J-634): six bodies migrated byte-identical, four recorded as never-written.** ⇒ **this stated delete condition is MET.** 🛑 **THE ARCHIVE IS NEVERTHELESS NOT DELETABLE YET — for a reason this line never named:** J-603's **second recovery pass, twelve `Past` entries whose record exists in no journal at all**, is **UNVERIFIED**. 🔑 **The delete condition was written narrower than the thing it protects.**
...
✅ **LEG F CLOSED 2026-07-31 (J-637). THE SWEEP RAN; THE AUTHORING DID NOT, ON JOE'S NARROWING.**

🛑 **§8's DEFINITION DISAGREES WITH ITSELF AND §9 CERTIFIED THE NARROWER HALF.** The first sentence names an **unbounded** sweep; the *Surface* line names **nine threads** and calls them a **test case**; §9's DoD then requires only the nine. ⇒ **a DoD narrower than the leg it closes** — the arc's recurring defect class, in the definition of the leg that exists to catch it. 📌 **Recorded, not repaired: §9's item is ticked because the nine ARE disposed, and the wider half is what spawned `M-DOC-BACKFILL`.**

✅ **THE NINE, DISPOSED:** `M-RP-LIVEFEED-REFRESH` **present** (node + six legs, J-616) · `NegotiatedCapabilities` **explicitly declined** (*"PROPOSAL WITHDRAWN, NOT WEAKENED"*, J-601) · the address-book eviction question **answered, not a work item** (J-601: zero production callers) · **H1 · H2 · `D-130` declined by kind** (Joe's decisions and a `DECISIONS.md` entry, not roadmap nodes) · the Ch0–Ch2 thesis read **carried** (already on §11's *Chat owes*) · 🛑 **the resync sibling and the outbox genuinely ABSENT.** **1 + 2 + 3 + 1 + 2 = 9 ✔** 🔑 **SIX OF THE NINE WERE NEVER ROADMAP-SHAPED** — a test case drawn from one session's chat threads tests the sweep against the category least likely to be a node.

🛑 **THE DESIGNATION CENSUS IS WHAT PAID.** Per `D-135`: wide predicate **258** tokens read individually (most lowercase prose matching case-insensitively) → bounded case-sensitive **147** → false positives read and struck (`M-RP6`, `M-RP-7` regex splits; **`M-TO-TOP` is `BOTTOM-TO-TOP`**) ⇒ **144 real designations.** **15 appear in `docs/ROADMAP.md` in any form; 🛑 TWO carry a node of their own** — `M-DOC-ROADTREE` and `M-RP-LIVEFEED-REFRESH`. Of the other 13: 4 roll-up names · 1 mention-inside-another-node (`M-RP-MEMBERS`) · **8 inside two `>` blockquotes parked at the end of `## Cross-cutting`**, a section this document defines as *"continuous rather than milestone-shaped"* — **`M-RP-SHELF-FRAME` and the whole closed `M-RP-SETTINGS` arc live there, off the board.**

| Absent bucket | Count | Verdict |
|---|---|---|
| collapsed into the five M-RP2–5 roll-ups | **50** | ✅ **not a defect** — the tree's preamble sanctions it (*"Past entries are collapsed to milestone-level depth"*). **This is the measurement that sized the leg: representation, not absence** |
| 🛑 M-RP6.x + M-RP7.x + M-RP8 | **32** | **no roll-up, no node** — the entire shipped client UI |
| named `M-RP-*` | **39** | `SKIN` · `PLATE` · `ROVING` · `FOCUS` · `RESTART` · `ROOM-HEADER` · `SELF-SURFACE` · `RESYNC` · … |
| non-RP | **6** | `M-SEC-TLS` · `M-SEC-AUTHSESS` · `M-BLOB-PLANE` · `M-AUTH-TIMEOUT-CONFIG` · `M-CONFIGURABLE-VALUES` · `M-TRANSPORT-TIMEOUT-CONFIG` |
| in the roll-up era but outside it | **2** | `M-RP2.23` · `M-RP4.9` |

**50 + 32 + 39 + 6 + 2 = 129 ✔** — classified, **not** taken as a remainder, so it could have failed.

🛑 **FOUR FINDINGS.** ① **`M-RP2.23` falls in the gap between the two roll-up ranges** (atomics `2.3–2.21, 2.30–2.31a`; composites `2.22, 2.24–2.29`) — 🔑 *a roll-up with a hole reads exactly like a roll-up without one*, which is the argument against extending roll-ups to the live arc. ② 🛑 **`M-RP6.1m` IS RECORDED ⏸️ POSTPONED AND IT SHIPPED** — J-513 filed it *"POSTPONED, NOT NEXT: four buttons, ZERO live feeders"*; `M-RP-SETTINGS` Leg B (J-537) shipped the action row saying ***"this IS M-RP6.1m."*** **The M-RP6.2 originating defect, second instance**, invisible to every check this milestone has run because all of them asked *is the entry true*, never *is the work on the board*. ③ **`M-RP6.1e-B1` has 11 mentions and no canonical record** — rolls into `-B` or gets no node; **no invented link** (§4c's precedent). ④ **The M-RP6/7 surface is 41 nodes** — 35 ✅ CLOSED · 6 open, every open one already carrying its trigger. 🛑 **`32` and `35` are DEAD as planning numbers** (`M-RP6.0d` invisible to the first source set; `M-RP6.1e`'s lettered children invisible to the predicate) — **annotated, not erased.**

⚠️ **TWO DEFECTS OF CHAT'S OWN.** ① *"the PLAY head already holds every close and J-ref"* is **FALSE** — `CLAUDE.md` **17** distinct M-RP6/7 designations, `CLAUDE_HISTORY.md` **33**, `JOURNAL.md` **35**; Legs D and E drained that head **by design**, and the claim was written in the session that read both closes. ② 🛑 **The 41-node proposal folded the back-fill INTO Leg F, against §11's own open item ②** — *"the back-fill milestone's name — belongs to Leg F's **sweep**"* ⇒ **Leg F is the sweep; the back-fill is its own milestone.** **Caught by Joe's question, not by re-reading** — the J-634 shape: agreeing with an answer is not knowing it exists.

🟡 **SPAWNED: `M-DOC-BACKFILL` — the milestones that were never on the board.** ⚠️ **ID PROVISIONAL under Rule 8; title and ID both Joe's.** Scope **~88 designations** + `M-RP6.1m`'s corrected state + `M-RP2.23`'s roll-up gap + `M-RP6.1e-B1`'s missing record + the two milestones parked in `## Cross-cutting`. ↳ **trigger: none — filed, not scheduled.** 🔑 **IT DOES NOT GATE LEG G — that is the whole point of the narrowing.**
...
✅ **LEG G CLOSED 2026-07-31 (J-638) — AND WITH IT THE MILESTONE.** 🛑 **ITS OWN DoD ITEM RAN FIRST AND WAS NOT SKIPPED BECAUSE THE MILESTONE WAS OVERDUE.** `Present — playing now` — **351,908 B, 47% of the archive, never examined by any leg** — audited in both directions per `D-135`: **12 lines carry no `J-` ref** (3 scaffolding; **9 verified against a real canonical record**: `docs/xgen-dock-engine-phase0.md` 70,912 B · `D-116` · `N-120` · `RUNBOOK_M_RP_MEMBERS_LEG_A_TER.md` 27,533 B · four `CLAUDE.md` blocks · the sequencing line in `CLAUDE_HISTORY.md`), and **186 / 186 cited J-refs RESOLVE** — where `Past` had **eleven that resolved nowhere.** 🔑 ***`Present` was in better shape than `Past`, which is the opposite of what the constraint feared — and the only way to know was to look.***

✅ **WHOLE-FILE CONTAINMENT, TWO INDEPENDENT PREDICATES.** Only **7** archive lines are absent from tag `roadtree-pre-legc` — **every one is the archive's own replacement header and preamble** (title · `Status: ARCHIVED` · version · date · its three notes). **ZERO body content was unique to the file.** Second, independent property: **389 distinct J-refs, all 389 present in the tag.** 🔑 **THE TAG HOLDS STRICTLY MORE** — `1c3f3d1:docs/ROADMAP.md` is **761,422 B against the archive's 749,428**, carrying all 100 M-RP designations including the **46 the live tree collapsed into roll-ups.**

🔑 **AND ONE OF THOSE 7 LINES WAS THE FILE TELLING US THIS:** *“Git holds this permanently at `0466cb2` regardless. A permanent in-tree copy would recreate exactly the duplication the roadtree migration exists to remove, and would cost 750 KB in every clone.”* Its own stated delete condition — **P1's 94 unlinked markers** — **cleared at J-599.** 📌 **The archive was disposable by its own written terms from the day it was taken; what was missing was the measurement, not the permission.**

⚠️ **THE BYTE SUM CAME OUT +1,023 OVER THE FILE — EXACTLY ONE PER LINE.** The probe added 2 bytes per line terminator; **the archive is LF.** A uniform offset across independent measurements is **definitional, not an error** — stated with the convention rather than hunted as a bug. Corrected partition: **`Past` 283,465 · `Present` 351,908 · rest 114,055 = 749,428 ✔.** 📌 **Leg H's `276,596 B` for `Past — settled` counts ENTRIES; this counts the SECTION — two conventions, not a contradiction.**

🛑 **TWO STALE LIVE CLAIMS FOUND AND CORRECTED IN THE SAME PASS — BOTH WOULD HAVE SURVIVED THE CLOSE.** ① **`Leg B` P2 still read *“NOT cleared: all 11 still have no journal entry”*** while J-603, J-635 and J-636 had disposed of every one ⇒ **under R-2 the milestone could not have gone ✅ with a 🟡 child, and the child was only 🟡 because nothing re-swept it.** ② **its `⇒ its retention is LOAD-BEARING` note** was the last live statement asserting the archive could not be deleted. 🔑 **`D-135` §5's rule earning itself a third time: a correction is not applied until the whole record set is searched.**

🛑 **THE TRIGGER THAT FIRED, AND IT IS THE MILESTONE'S OWN ORIGINATING DEFECT.** `M-RP-LIVEFEED-REFRESH` carried `↳ trigger: M-DOC-ROADTREE closes (Leg G)`. **Closing this milestone fires it**, and the tree's own maintenance rule says *a trigger that has fired is a defect: the node it guards is stale by definition.* ⇒ **un-postponed ⏸️ → 🟡 in this same commit**, and the closed node carries `→ M-RP-LIVEFEED-REFRESH · M-DOC-BACKFILL` per §3's arrow rule. 🔑 ***This is exactly the M-RP6.2 failure the milestone was founded on — a gate closing with nothing on the gate's side saying “things are waiting on me” — and §3's placement rule caught it by construction.*** **The UI track is unblocked; standing Clair up is Joe's.**

⚠️ **ONE NODE COULD NOT BE DERIVED HONESTLY AND WAS ANNOTATED RATHER THAN FLIPPED.** `UI component-library / substrate` has **five children, all ✅** ⇒ R-2 says ✅ — **but that would assert the RP track is COMPLETE, and `M-RP6.x`/`M-RP7.x` shipped the entire client UI with no nodes at all** (Leg F's finding). 🔑 ***A derived status is only as true as the child set it derives from.*** Held at 🟢 with the reason written on it; **it resolves when `M-DOC-BACKFILL` lands the missing children.**
...
- [x] M-RP-MEMBERS Leg C reads ⏸️ **with its trigger** in ROADMAP, CLAUDE.md and the board — **Leg A ✅ J-598**
- [x] All **94** P1 items **measured**: journal link found, or redundancy confirmed in writing — **Leg B ✅ J-599** (94 → 13 true orphans → **5 genuine**, all five resolved: two to journal entries, three to design documents)
- [x] All **11** P2 refs **measured**: entry located, or absence recorded — **Leg B ✅ CLEARED J-638** · six bodies migrated byte-identical (J-603, SHA256-proven J-634) · four recorded cited-but-never-written (J-603) · `J-029` **has an entry**, the predicate was blind (J-635) · `J-171` cited-but-never-written with a pointer (J-636). 🛑 **The node read *“NOT cleared: all 11 still have no journal entry”* until this leg — superseded three times over and never re-swept; annotated, not erased**
- [x] The **`ARCHIVED`-versus-repair question (§8b) ruled** by Joe, and the ruling recorded where a future reader of `JOURNAL_ARCHIVE.md` will find it — **✅ J-618, R2 with Joe's own boundary** (*no new records, but correctable each time it needs it*); the `Repaired:` line landed in the archive header at J-625
- [x] The **six surviving set-B bodies present in a journal file**, each carrying its provenance line, **and the four never-written numbers recorded as such** (§4a-i) — **✅ J-603, proven J-634** (SHA256 pair by pair: 6,670 · 4,678 · 6,664 · 4,905 · 1,064 · 1,518). ⚠️ **One textual shortfall recorded, not waived: the per-heading provenance form omits `(J-129 §8)`**
...
- [x] Every ✅ node in the new tree carries `· J-nnn` — **Leg C · re-measured J-638: 225 ✅ nodes, ZERO without a canonical-record link.** 📌 One predicate hit was a false positive — an `↳` annotation row, not a node; it inherits its parent's link (P1's own inherit rule)
- [x] Every 🟡 / ⏸️ node carries a trigger **or an explicit `trigger: none`** — **Leg C · re-measured J-638.** 13 flagged → **8 false positives** (inline `↳ trigger:` on the same line, or `↳` annotation rows) · **1 correct derived container** (Arc I, R-2a) · **3 genuine gaps — LIVEFEED Legs A/D/E, ALL CREATED AT J-616, AFTER LEG C CLOSED**, so Leg C's discharge stands. Structural triggers written at J-638; **none fabricated** (§3: a forced trigger is worse than none)
- [x] Every existing ⏸️ **POSTPONED** entry (**22** of them) **audited for a resume trigger** — **✅ SPENT BY LEG C's DELETION, NOT BY AN AUDIT (stated, not glossed).** The 22 lived in the `Past`/`Present` PROSE, which Leg C deleted (761,422 → 43,741 B); the surviving tree's ⏸️ nodes each carry a trigger and are covered by the item above. 📌 **The item was written against a document that no longer exists**
- [x] No `→` arrow points at a journal entry that does not record the loose end — **Legs B + C · J-638.** The tree's only `→` arrows are `M-DOC-ROADTREE → M-RP-LIVEFEED-REFRESH · M-DOC-BACKFILL`, `Leg F → M-DOC-BACKFILL` and two ⬛ DEPRECATED successors; **each target is a live node in this same tree, not a journal entry** ⇒ §3's precondition cannot fail for them. 📌 **The §3 failure that produced this item (the M-RP6.6 gate-versus-author collapse) is what `M-RP-LIVEFEED-REFRESH`'s un-postponement at J-638 discharges in practice**
- [x] ROADMAP re-**measured** under 100 KB — **Leg C ✅ 43,741 B at J-604; re-measured 69,428 B at J-638**, still under the original bar after four legs of annotation. 📌 *The §1a baseline was 755,033 B, not 749,717*
- [x] `CLAUDE.md`'s PLAY head **grounded and its format ruled** before any rewrite — **Leg D ✅ J-615** (§6b ruled S1, B2 only; §6a ruled E) **+ Leg E ✅ J-633** (B1's 124,299-char line moved byte-identical, SHA256-proven). **640,645 → 256,427 B; lines over 20,000: 1 → 0**
- [x] All nine of this session's threads present or explicitly declined — **Leg F ✅ J-637** (1 present · 2 declined on measurement · 3 declined by kind · 1 carried · **2 genuinely absent, routed to `M-DOC-BACKFILL`**) ⚠️ **this item is narrower than the leg it certifies — see §8's Leg F block**
- [x] Archive deleted, or its retention **stated with a reason** — **Leg G ✅ DELETED J-638** (749,428 B, SHA256 `4B306B6F…`). **Audited first, both directions:** `Present` **351,908 B never examined** → 12 unlinked lines (3 scaffolding, 9 verified against a real canonical record) and **186/186 cited refs resolve**; **whole-file containment against tag `roadtree-pre-legc` — only 7 lines absent, ALL of them its own replacement header, ZERO body content unique**, cross-checked on a second property (**389/389 J-refs present in the tag**). 🔑 **The tag holds STRICTLY MORE: 761,422 B vs 749,428**
...
✅ **Leg B-ter — the eleven that resolve nowhere. CLOSED AS DISCHARGED 2026-07-31 (Joe, J-635).** 🔒 **SPAWNED AND TITLED 2026-07-30 (Joe, J-626)** from B-bis's DoD gap: §6 demanded P2 cleared and the six set-B bodies migrated, **and no procedure section described either.** **Scope — the eleven P2 numbers, in three classes that are NOT the same kind of thing:** ① **six bodies to migrate** — `J-135 J-132 J-131 J-125 J-124 J-123` ② **four cited-only, no record anywhere** — `J-113 J-109 J-098` (**45 references and no record of its own**) `J-067` ③ **`J-029`, disposition genuinely open** — P2 counts it, §4a-i's table does not cover it, it falls inside set A. 🛑 **SOLE SOURCE: `docs/ROADMAP_ARCHIVE_2026-07-26.md`.** ⚠️ **Leg C DELETED all six bodies from `ROADMAP.md`** (refs fell 7→2 · 5→2 · 10→1 · 27→1 · 24→1 · 15→1); own bodies survive **only** at archive L512 · L524 · L528 · L548 · L552 · L556. 🔑 **§4a required that this prose NOT be deleted until the journal carried it. It was deleted first.** ⇒ 🛑 **HARD ORDERING CONSTRAINT: LEG G MUST NOT RUN BEFORE LEG B-TER.** Leg G deletes that archive iff Leg B clears — **had it run, six bodies would be gone permanently.** 📌 **No `Owes:` line is issued: `D-133` says leg runbooks never get one, and an ACTIVE parent whose own milestone is still in flight does not need one.** The debt is carried by this leg list and by Leg G's constraint. **Not runbooked yet; it is AUTHORING, not repair.**

🛑 **B-TER's SCOPE WAS ALREADY DISCHARGED AT J-603 (2026-07-28) — THREE DAYS BEFORE THIS LEG WAS SPAWNED AT J-626. MEASURED 2026-07-31 (J-634).** `JOURNAL.md` carries **`## Entry J-603 — six ROADMAP-only records recovered into the journal, and four numbers recorded as never written`**, written under **Leg C**, between stage 1 and stage 2.

⚠️ **J-634 CITED THAT ENTRY AND ITS SUB-SECTIONS BY LINE NUMBER — L842 · L856–878 · L882 · L895 — AND EVERY ONE WENT STALE BY +26 THE INSTANT J-634 WAS PREPENDED TO THE SAME FILE (corrected J-635).** 🔑 **A LINE CITATION INTO A FILE YOU ARE ABOUT TO PREPEND TO IS SELF-INVALIDATING: CORRECT WHEN MEASURED, FALSE WHEN WRITTEN.** ⇒ **All references below identify BY CONTENT**, per the standing rule the arc already carries for the closure log.
- ✅ **CLASS ① — THE SIX BODIES ARE IN `JOURNAL.md` AND ARE BYTE-IDENTICAL TO THE ARCHIVE.** They sit under J-603 as `### J-nnn — recovered body (migrated from ROADMAP…)`. SHA256 compared pair by pair: `J-123` 6,670 · `J-124` 4,678 · `J-125` 6,664 · `J-131` 4,905 · `J-132` 1,064 · `J-135` 1,518 — **all six identical.** J-603 states it as *“NOT RECONSTRUCTED, NOT SUMMARISED, NOT REFLOWED — the exact bytes that stood in ROADMAP.”* ⚠️ **ONE TEXTUAL SHORTFALL (J-635): §4b quotes the required provenance line as *“migrated from ROADMAP; originally a stripped header-chain entry **(J-129 §8)**”*, and the per-heading form OMITS `(J-129 §8)`.** The block cites J-129 once, elsewhere. **Substantively discharged; one token short of the specified text.**
- ✅ **CLASS ② — THE FOUR ARE ALREADY RECORDED**, under J-603's `### Cited but never written — four numbers with no body in any document`, and **explicitly recorded as never-written rather than reconstructed** because inferring them *“would be exactly the invent-a-link failure”*. 📌 **Chat independently recommended that same disposition at J-634 before finding this — the record had settled it on 2026-07-28, and a Phase-0 brief would have asked Joe to re-decide it.**
- ⚠️ **AND J-603 CARRIES A SECOND PASS THIS SCOPE STATEMENT DOES NOT MENTION:** `## Second recovery pass — twelve Past entries whose record exists in no journal at all`, measured as *`Past — settled` holds 103 entries / 276,596 bytes; 91 have a journal entry LARGER than the ROADMAP copy*. **Status of that pass is UNVERIFIED.**
- 🔒 **DISPOSITION TAKEN 2026-07-31 (Joe, J-635): B-TER IS CLOSED AS DISCHARGED, AND A SUCCESSOR IS SPAWNED FOR WHAT ACTUALLY REMAINS.** 📌 **Reduce-to-`J-029` was not available — that option dissolved when J-029 turned out to have an entry. Re-scoping B-ter onto the twelve was refused: the twelve are not *the eleven that resolve nowhere*, and putting two unlike things under one title is the fault the milestone-title rule exists to prevent.**
- 🔑 **FOUND ONLY BECAUSE `D-135` §1① IS LOCKED.** Chat's first predicate was `^##\s*(Entry\s*)?J-nnn`, built from the two heading forms already seen, and it **could not see `### J-nnn`** — it reported *“none of the ten has a journal entry”* with confidence. **The widened re-run found all six inside one turn. Seventh instance of this arc's defect class.**

✅ **Leg E — the two-way closure log. CLOSED 2026-07-31 (J-633). 📘 `tasks/RUNBOOK_ROADTREE_LEGE.md` v1.7 ✅ COMPLETED.** 🔒 Title locked (Joe, J-618). Trigger **FIRED (J-615)**. 🛑 **P1's CLEARANCE IS RETRACTED (J-629). THE HEAD SET IS `A 20 · B 51 · C 23 = 94 HEADS · 90 RECORDS`** — superseding *97 / 93* (J-628) and *95 / 91* (J-627), **annotated never erased.** 🛑 **THREE `**M-` MATCHES WERE BOLD EMPHASIS, NOT HEADS:** @115,900 `**M-RP5.5**` and @115,973 `**M-RP5.6**` inside **one clause** of the record at 115,464, and @119,871 `**M-RP5.6 CLOSED**` after an **em-dash inside J-485's own sentence**. 🔑 **EVERY §3 GATE PASSED WHILE THIS WAS TRUE** — §3.3 compares the predicate's count to a census figure **produced by that same predicate**, so it detects drift and never error; the same disease as §3.4's span-sum, **one line above it**, struck at J-628 while this survived. ⇒ 🔒 **`D-135` MINTED (delegated, reversible): a predicate is tested in BOTH directions, and an assertion built from the predicate it checks cannot fail.** ✅ **LOCKED BY JOE 2026-07-31 (J-631) — no longer reversible; the *delegated* wording above describes its state on 2026-07-30 and is kept, not erased.** ✅ **THE SECOND PREDICATE IS THE WHOLE DIFFERENCE:** A had one (20 `CLOSED` marks vs 1+19) → **0 false heads**; B had one (J-keys **strictly monotonic J-503→J-445**, which also forbids within-B twins) → **0 false heads**; **C had none → all three.** 🛑 **§4a's ELEVEN COLLISIONS ARE SIXTEEN** — **J-478 · J-479 · J-480 · J-483 · J-485** added. **The eleven were measured inside part two's two C sub-windows (110,516–111,445 and 122,026–123,912 = exactly 11); the 10,581 chars between them were never examined and all five sit in that gap.** ⚠️ **Part three positively asserted those J-numbers were NOT in the collision set — false.** 🛑 **AND THE PARSE ERROR WAS HIDING A REVERSED VERDICT:** C's J-485 read **93 chars** (a stub ⇒ *keep B*) while the false head stood; whole it is **1,813** and **richer than B's 906** — **the verdict inverts.** ✅ **§4c COMPLETE: zero collisions within-A, within-B, within-C, A↔B, A↔C**, structurally explained (A spans J-410–J-435, B spans J-445–J-503). ⚠️ **Key extraction is UNSOUND for C's 4 forward-looking heads** (first `(J-nnn)` there is a citation, not an identity) — no false collision resulted, all 16 were read on both sides, **but it must not be reused blind.** ⚠️ **One hand verdict flagged as a judgement:** @116,587 opens after a **semicolon**, admitted on head form. 🛑 **P2 CANNOT RUN.** 📌 *(True as written at J-629; **LIFTED by P0-bis + P1 at J-630**. Kept, not erased — the block was real and the reason it lifted is the record.)* ✅ **P0-BIS, P1 AND P2 ALL CLEARED 2026-07-31 (J-630).** **P0-bis** closed the one direction `D-135` still owed — a **closure-mark bijection** independent of every head predicate: **49 marks line-wide, 49 spans holding exactly one each, ZERO spans with two** ⇒ no missed head brings a closure with it; and the longest zero-mark spans read clean (their only interior `**M-` hits are bold emphasis **in B**, where the `) / ` anchor is structural — **direct evidence for `D-135`'s structural-vs-typographic claim**). **P1** re-ran under corrected §3: **1 line · 124,299 · A 1+19 · B 51 · C 22+1 = 94 · 4 stubs · 90 records**, open **and** close assertions both green, and **4(d) fired exactly once — on @116,587, the ALREADY-RECORDED hand verdict and nothing else.** 📌 **First time in this arc a gate fired on something already understood rather than on something missed.** 🛑 **P2 COMPLETE — ALL 16 PAIRS RE-READ ON BOTH SIDES, AND TWO INHERITED VERDICTS WERE WRONG.** ⇒ **11 B ⊃ C · 3 C-side · 2 DIVERGENT**, superseding *8 · 2 · 1 over eleven* (J-622). 🛑 **J-456 `M-RP4.5` WAS RECORDED `B ⊃ C` AND IS DIVERGENT:** C alone names **`transform.ts`**, **`toEditable`**, *“nothing on `input`”*, `string↔T` and *“3 of 4 processor kinds”*; B alone carries the *two-reps-cannot-ride-one-`bind:value`* rationale, `formatToParts` and `empty=revert`. **A *keep B* resolution would have deleted the only place `transform.ts` and `toEditable` are named.** ⚠️ **J-448 refined: C richer, but B alone carries `CDP-verified`.** ✅ **All FOUR of §4b's named risks confirmed by re-reading:** J-457's hex **`#ba7517`** · J-459's deferred filter/search rationale · J-460's `box-sizing` evidence — all **B-only**; J-454's `temperature-indicator` dd-block and the `ui/docs/`-at-open rule — **C-only**. ✅ **The eleven B figures reconcile to §4a by exactly +4 each** — the head-to-head vs payload convention (J-628); C figures identical. 🔑 **CONSEQUENCE: NOTHING IS A STRAIGHT DELETE, AND NO PAIR MAY BE RESOLVED BY LENGTH.** **Next: P3 — the move into `CLAUDE_HISTORY.md`.** ✅ **P3 EXECUTED AND VERIFIED 2026-07-31 (J-633) — THE LINE IS OUT OF `CLAUDE.md`.** 🛑 **§5 CONTRADICTED F1c AND COULD NOT BE RUN AS WRITTEN:** `CLAUDE_HISTORY.md` L17 defines the Leg D convention as ***blocks are byte-identical, this heading is the only thing written***, and L13's exception permits ***a batch inserted at the top and nothing else edited*** — yet §5 mandated annotation at the site, annotation of C's non-closure heads, **and** §4r's merges. **Three rewrites, all forbidden by the convention §5 itself cites.** 🔒 **JOE RULED OPTION ① — BYTE-IDENTICAL MOVE:** nothing merged, dropped or annotated in place; **the §4r verdicts are recorded and applied to NOTHING.** ⚠️ **Stated consequence: P2's sixteen verdicts are documentation, not transformation, and the archive keeps 16 near-duplicate pairs; deduplication is separable and was NOT spawned as a leg.** 🛑 **AND §11's OWN SURFACE WORDING WAS BROADER THAN ANYTHING LEG E MEASURED** — it would have swept in four sibling prose lines and **the two live `♻️ Recovery anchor` pointers**, while every measurement was inside the 124,299-char line alone ⇒ **P3 moved that line and nothing else.** ✅ **RESULT:** `CLAUDE.md` **382,712 → 256,427 B** (−126,285) · **lines over 20,000: 1 → 0** · `CLAUDE_HISTORY.md` **1,197,988 → 1,325,400 B** · **payload occurs exactly once and its SHA256 `0af97c2a…` MATCHES the pre-move hash — byte-identity PROVEN, not asserted** · line endings preserved per file (CLAUDE.md CRLF 719/0, history pure LF) · newest-archived-first held (Leg E L17, Leg D L21) · **both byte deltas reconcile from independently measured components, a check that could have failed.** ✅ **`CLAUDE.md` keeps a 324-byte D-094 forward pointer.**
✅ **Leg F — the bidirectional sweep. CLOSED 2026-07-31 (J-637).** The sweep ran; the authoring was narrowed OUT of it by Joe. **144 live designations · 15 in `ROADMAP.md` in any form · 🛑 TWO with a node of their own.** The nine threads disposed (1 present · 2 declined on measurement · 3 declined by kind · 1 carried · 2 absent). 🛑 **`M-RP6.1m` is recorded ⏸️ POSTPONED and it shipped under `M-RP-SETTINGS` Leg B (J-537) — the M-RP6.2 originating defect, second instance.** 🟡 **`M-DOC-BACKFILL` spawned, ID provisional (Rule 8), title Joe's, `trigger: none`, gating nothing.**

✅ **Leg G — records + close. CLOSED 2026-07-31 (J-638). 🏁 THE MILESTONE IS CLOSED.** The **465,963 B** Leg H did not cover was audited in both directions before anything was deleted; **`Present`'s 186/186 refs resolve and only 7 archive lines — all header — were absent from the tag, which holds 761,422 B against the archive's 749,428.** Archive **DELETED**. 🛑 **Two stale live claims corrected in the same pass** (`Leg B` P2's *“NOT cleared”* and its *retention is LOAD-BEARING* note) — **either would have survived the close.** ✅ **`M-RP-LIVEFEED-REFRESH` UN-POSTPONED, its trigger having fired — the UI track is unblocked and standing Clair up is Joe's.**

✅ **Leg H — what the census could not see. 🔒 TITLED 2026-07-31 (Joe, J-636). CLOSED 2026-07-31 (J-636).** Spawned J-635 as B-ter's successor. 📌 **Spawned rather than folded into B-ter because B-ter's title stopped describing its contents two findings ago.** 🛑 **RAN BEFORE LEG G, as required.** **Scope — three measurable items, none of them *the eleven*:**
- ① **RE-RUN THE J-001–J-598 CENSUS UNDER `D-135` §1①.** The standing *24 missing* rests on a `## Entry J-nnn` predicate blind to **16** old-form `## J-nnn — date — title` headings. **Every figure derived from it is suspect until re-measured.**
- ② **RESOLVE `J-171`** — the one number confirmed to have no `#`-prefixed heading in either journal. **A non-heading record form was never tested.** Set C, never cited by ROADMAP.
- ③ **VERIFY J-603's SECOND RECOVERY PASS — the twelve `Past` entries with no journal record.** 🔑 **THIS, NOT THE SIX, IS WHAT LEG G IS ACTUALLY WAITING ON.**

✅ **LEG H RESULT — ALL THREE RESOLVED BY MEASUREMENT; NOTHING WAS AUTHORED (J-636).**

✅ **① THE CENSUS IS 5, NOT 24.** Three heading forms declare a designation — `###` **6** · `##` **16** · `## Entry` **613** ⇒ **632 distinct designations.** Missing between J-001 and J-598: **`J-067` · `J-098` · `J-109` · `J-113` · `J-171`.** ✅ **The false-positive direction was tested too** (`D-135` §1②): **19** headings name a J-number without declaring one — *“Path B locked at J-098 session close”* and siblings — **all 19 read individually, none an entry head.** ⇒ **Set A (`J-021`–`J-033`) was never a gap, and `J-029`'s open disposition dissolves with it.**

✅ **② `J-171` — CITED × 9, NEVER WRITTEN, SUBSTANCE NOT LOST.** 23 raw references; the nine genuine ones sit in three archived M6 documents and all name it as the session that locked design decisions on **2026-05-30**: *“split into 2a/2b at design open (J-171)”*, *“Decisions locked J-171”*, and **FAC-D1 · FAC-D1a · FAC-D2** each marked **LOCKED (J-171)**. **No body anywhere** — the only body-shaped hit is a section heading, `## Decisions locked (J-171)`. 📌 **Its neighbours exist (J-173 runbook, J-174→J-178 commits), so it sits inside a live numbering run, not an unallocated gap.** 🔑 **DISPOSITION: recorded as CITED-BUT-NEVER-WRITTEN with a POINTER to `tasks/archive/M6_FEDERATION_ADMIN_CONTROL_DESIGN.md`, where the three decisions and their date survive in full. NOT RECONSTRUCTED** — the same treatment J-603 gave the four, and for the same reason.

✅ **③ THE TWELVE WERE NEVER AN OPEN TASK.** J-603's second recovery pass already holds them in `JOURNAL.md`, **verbatim** — *“Not reflowed, not summarised, not renumbered. Several carry no `J-` number at all; that absence is itself the record.”* **Arithmetic closes independently:** `Past — settled` = **103 entries / 276,596 B**; **91** already had a journal entry LARGER than the ROADMAP copy; **12** recovered ⇒ **103**. ⇒ ✅ **LEG G's STATED BLOCKER IS DISCHARGED.**

⚠️ **WHAT LEG H DOES NOT CLAIM.** `Past — settled` is covered; **the remaining ~470 KB of `docs/ROADMAP_ARCHIVE_2026-07-26.md` was NOT audited.** 🛑 **That check belongs to LEG G's own DoD and must not be skipped because the milestone is overdue.** **Nothing was authored: no record reconstructed, no number retired.**

🔑 **THE LEG'S WHOLE CONTENT IS THAT THE THING IT WAS SCOPED TO FIX MOSTLY WAS NOT THERE** — the third time in this milestone (after B-bis → B-ter, and B-ter itself) that a leg thinned on contact with its own grounding.

🛑 **LEG G's ORDERING CONSTRAINT IS RE-POINTED, NOT LIFTED (J-635): IT MUST NOT RUN BEFORE LEG H.** It deletes `docs/ROADMAP_ARCHIVE_2026-07-26.md`. ⚠️ **Its ORIGINAL justification — that the archive is the sole surviving source for the six set-B bodies — is FALSE and stays annotated below.** ✅ **The constraint survives on a different and still-live reason: the twelve are unverified.** 📌 **Re-pointed as a consequence of spawning Leg H, not as an independent ruling.**

✅ **THAT REASON IS NOW DISCHARGED TOO (J-636): the twelve were already in `JOURNAL.md` verbatim since J-603, and `91 + 12 = 103` accounts for all of `Past — settled`.** 🛑 **LEG G REMAINS BLOCKED ON A THIRD, NARROWER GROUND, STATED HERE SO IT IS NOT LOST:** Leg H covered `Past — settled` **only**; **the remaining ~470 KB of that archive has never been audited**, and Leg G deletes the whole file. ⇒ **The audit is LEG G's OWN DoD item.** 🔑 **Three justifications have now been offered for one constraint and two of them were wrong — the constraint itself has been right every time.**

🛑 **THE CONSTRAINT'S STATED JUSTIFICATION IS FALSE, AND THE CONSTRAINT IS NEVERTHELESS LEFT STANDING (J-634).** ✅ **The archive is NOT the sole source for the six** — they have been in `JOURNAL.md` **byte-identical** since J-603, proven by SHA256 pair by pair. **Leg G would not destroy them.** ⚠️ **BUT IT IS NOT LIFTED HERE.** Lifting it permits an **irreversible archive deletion**, and J-603's **second recovery pass — twelve `Past` entries whose record exists in no journal at all — is UNVERIFIED.** ⇒ 🔓 **JOE'S: re-point the constraint from the six to the twelve and keep Leg G blocked until that pass is verified.** 📌 **A safety constraint whose reason is wrong is not thereby unnecessary — it may be right for a reason nobody has written down yet.** ✅ **RESOLVED 2026-07-31 (J-635): re-pointed to LEG H, not lifted.**
...
🔓 **OPEN, JOE'S — NONE IS A GATE:** ① ✅ **sequencing SPENT — Leg F closed J-637; Leg G is next** ② **the back-fill milestone's name — STILL OPEN, and the milestone now EXISTS: `M-DOC-BACKFILL`, ID provisional under Rule 8 (J-637), scope ~88 designations.** 📌 **Two further names are owed with it and neither gates anything: the resync sibling (`M-RP-RESYNC` proposed, open since J-598) and the outbox sibling (no proposal — Chat has no grounded read on whether it is its own milestone or a leg of the resync one, and did not guess)** ③ 📌 **`D-134` §2's justification clause** — recommended as an **annotation**, not an amendment ④ 🔓 **`D-135` §5a — DRAFTED (J-632), delegated and reversible, AWAITING CONFIRM / AMEND / DISCARD.** ✅ **CLOSED: `D-135` locked (J-631) · Leg E closed (J-633) · Leg B-ter closed as discharged and Leg G's constraint re-pointed (J-635) · `Leg H` titled *what the census could not see* and CLOSED (J-636).**
