...
## 🟢 UI component-library / substrate (RP track) — M-RP2.18 ✅ CLOSED (J-427): `file` — the **thirteenth** `core` component, the native file picker; the **first non-`value` binding** (`bind:files` / FileList — the 4th binding shape), getter de-FileLists to `{count,files:[…]}`; a `file-field` composite logged; **next — `select multiple`** (the last atomic di)
