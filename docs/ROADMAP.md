...
> Version: 1.0  
> Date: May 2026  
> **Last updated**: 2026-05-19 (J-084 — Federation Event Propagation implementation Phase 3 SHIPPED: F-1a federation handshake reshape to bilateral tip exchange; Option 3 wire shape locked; Option A full migration scope; a-i symmetry rule for `state.federation_add` trigger; F-2 long-lived continuous session with R1 unused outbound mpsc as Phase-4 prep; new `xgen-node/src/federation_session.rs` module with `stream_federation_delta`. 488 tests (+8). Present summary updated; Past gains a Phase-3 close entry under Federation Event Propagation implementation.)  
> Language: English  
> Author: JozefN  
...
🟢 **Federation Event Propagation implementation — Phase 1 ✅ + Phase 2 ✅ + Phase 3 ✅ SHIPPED; Phase 4 next-active.** The nine-phase runbook at `tasks/FEDERATION_PROPAGATION_COMPLETION.md` (Status: ACTIVE, v1.0) drives this milestone. **Phase 1** closed in J-082 on 2026-05-18 (F-6 `transport.sync_complete` wire shape + F-7 response-size pagination + four-call-site migration). **Phase 2** closed in J-083 on 2026-05-18 (F-4 `process_inbound` validation pipeline unification; pre-F-4 three-path asymmetry collapsed; HeldPending uniform across all event families). **Phase 3** closed in J-084 on 2026-05-19 (F-1a federation handshake reshape to bilateral tip exchange: Option 3 wire shape with `tips: BTreeMap<String, String>` on `Hello` + `Capabilities`; new `compute_federation_delta_for_space` per-Space delta helper; new `xgen-node/src/federation_session.rs` module with `stream_federation_delta` orchestrator applying a-i symmetry rule for `state.federation_add`; `handle_federation_incoming` refactored to drop JoinRequest/dump/goodbye flow in favour of tip-exchange + delta + session-stays-open with R1 outbound mpsc as Phase-4 prep; Option A full migration scope — seven `xgen-client` call sites + smoke.rs step-9-through-11 block migrated to the new SyncComplete-terminated shape). 488 tests passing (+8 over Phase 2's 480: 4 wire-layer unit tests for tips round-trip / tampered-tips / pre-F-1a back-compat; 4 integration tests for bilateral tips propagation / brand-new / reconnect / pre-F-1a compat). **Next: Phase 4 — federation event push (F-1 + F-1b + F-5).** Implement the push mechanism: on Stage-4 accept of an event in `process_inbound`'s post-validation handler, the post-fan-out step pushes the event over the persistent F-2 session (established by Phase 3) to each federated peer that participates in the relevant Space. The R1 unused outbound mpsc receiver in `handle_federation_incoming` is where Phase 4's push sender plugs in. Origin gating per F-5 §8.5 is load-bearing — events that arrived via federation MUST NOT enter the federation-push code path. Remaining work after Phase 4: Phase 5 (per-peer record + reconnect scheduling, F-1c) → Phase 6 (HeldPending generalisation for unknown signer Identity, F-10) → Phase 7 (federation-relationship verification gate, F-3 second check) → Phase 8 (documentation pass) → Phase 9 (integration tests for full federation push path). Blocks M6 (new) going ACTIVE.
