...
⚠️ **`D-126` IS MISCITED IN THE SENTENCE ABOVE — CORRECTED 2026-08-05 (J-676), ANNOTATED NOT REPAIRED (`D-131`). THIS SENTENCE IS THE ORIGIN OF THE MISCITATION; EVERY OTHER SITE INHERITED IT FROM HERE.** Three claims in one clause, all false. **(1) `T3` IS A §6b-LOCAL OPTION LABEL** — the third item in this section's own `T1`/`T2`/`T3` menu, from which Joe ruled `T2` at J-658. It is **not a designation and never was**; no `D`-number owns the bounded per-row retry. **(2) `D-126` IS THE HUMANE PUBKEY LABEL** (`DECISIONS.md:4710`, J-579) and **the string `T3` does not appear anywhere in its span.** The weld is two senses of one token: §6b's *"no word"* means **no STATUS TERM for a timed-out row**, while D-126's *"word"* means **a WORD-RENDERING of a pubkey** (`amber-falcon`). **(3) *"deferred J-588"* IS FALSE IN THE OPPOSITE DIRECTION** — J-588 records *"D-121/D-123/**D-126**/D-127/D-129 all apply unextended"* (`JOURNAL.md:4436`); what it deferred was D-126's **word form for the members panel**, not any retry. 🔑 **THE REFUSAL OF T3 ITSELF STILL STANDS** on its own merits — a terminal state with no vocabulary, for a case not yet observed. **Only the citation was false, and the citation is what a later reader would have followed.**
...
🔒 **TWO DECISIONS LOCKED BY JOE (2026-08-04, phase-0 §§2–3):** **J1** — the joiner is a **headless CLI process** with its own `XGEN_DATA_DIR`, no GUI, no Vite, no CDP (✅ *and not a weaker stimulus:* `cmd_join`, `app.rs:3032`, is a thin shim onto `ops::join`, `ops.rs:1579`, which signs `EventType::MembershipJoin` — the same function the GUI reaches) · **E1** — the `not_found` state is produced by removing a row from `xgen-node_identities.db`, **recorded as FILE SURGERY and never as *"an erased identity"***.

🔑 **① AND ④ CANNOT BE THE SAME JOIN (Chat's ruling, mechanical, `D-123`).** `resolveMember` clears `unresolved` the moment the fetch returns, so on a healthy node `unasked` lives for one tick. ⇒ **instrument before stimulus** (a `MutationObserver` installed before the join), and **⑥ is also how ① is seen at rest** — a failed fetch parks the row indefinitely because nothing retries. ***Two obligations filed a week apart are one mechanism seen twice.*** ⇒ R-1 carries ①+④, R-2 carries ①-at-rest+⑥, R-3 carries ②③⑤, R-4 carries ⑦.

🛑 **THE ADVERSARIAL READ RAN BEFORE THE LOCK (J-642) AND WAS NOT CLEAN — FIVE FINDINGS, FOUR ABSENT FROM THE RUNBOOK'S OWN §11.** The headline is Chat's and it lands on **§5b of this document**: the runbook's first R-3 step resolved the erased-to-be joiner *"as a control"*, and **`partition_observed` (`ops.rs:2778-2786`, doc `:2767-2777`) touches held identities and NEVER re-fetches them** ⇒ ②③⑤ could not have been produced at all. 🔑 ***A control that destroys the state it is controlling for.*** 🛑 **And the species is not the usual narrow read — §5b is a locked finding in THIS document, which its author had read, and the phase-0 he wrote two hours earlier stated the correct timing.** *A source consulted, understood, recorded — and then not applied by the same author in the next document.* Repaired at runbook v1.1.

✅ **AND THE READ'S MOST VALUABLE RESULT WAS A CLEAN ONE: THE FEARED DEFECT DOES NOT EXIST.** `app_client.svelte:249-256` wraps the Tier-1 fetch in `try { … resolveMember(record ?? null) } catch { /* the row stays ④ */ }`, and `desktop.rs:763` maps a fetch `Err` to `Err(String)` so the invoke **rejects** ⇒ **a rejected invoke never reaches `resolveMember`; only `Ok(None)` routes to `_notFound`.** ***A node blip cannot show a live person as erased.*** 🔑 **§2b also changed status rather than being confirmed:** the marker clear and the badge both derive from the same `m.unresolved` flip ⇒ **badge-before-clear is UNREACHABLE, not merely unobserved**, so R-1 CORROBORATES a structural property rather than DISCOVERING an ordering.
...
- ⑥ **a joiner whose fetch FAILS or TIMES OUT** — the row stays ④ and **nothing retries it.** This is the T3 residue (`D-126`), and Leg D is what creates the state at all. ⚠️ **`D-126` MISCITED — see §6b's annotation (J-676, `D-131`).** `T3` is a §6b-local option label, not a designation; `D-126` is the humane pubkey label and contains no `T3`.
...
- [x] **The residue re-priced** — ✅ **J-675.** ⑥ measured: **108 s at rest, zero retries, zero mutations while connected**; the row then **healed on reconnect via a whole-list RE-FILL, not a retry**. ⇒ `D-126`'s **T3 stays a live option, re-priced on the ACCURATE residue**: the ROW carries no failure affordance, but the APP reports the outage globally (self panel `Reconnecting`) — *weaker than the "unreported" version Chat first wrote and retracted in-run.* ⚠️ **A bounded retry buys less than §6b assumed**, because the reconnect path already heals the row. ⚠️ **`D-126` MISCITED — see §6b's annotation (J-676, `D-131`).** The measurement and its conclusion are unaffected: what was re-priced is **§6b's option `T3`**, which no decision owns.
