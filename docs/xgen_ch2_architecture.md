...
**Architectural horizon (post-Phase 2, not scheduled):** long-term, Node administration via the XGen client itself — the operator connects to their Node as a privileged Identity and manages it through the standard client interface. This is the philosophically pure model: identity-first, no special admin binary, admin access works from anywhere over the protocol. It requires a stable client first and has a bootstrapping challenge (how to admin a Node before any client can connect). Noted in D-037b.
