...
> **SHIPPED status — ALL 7 A1 verbs now shipped (federation-admin-control 2a J-174→J-178 + 2b J-179→J-183).** `list` + `defederate` shipped in M6 (J-156). `accept` + `reject` + `initiate` shipped in the **2a** sub-arc (J-177). `set-policy` + `show-policy` shipped in the **2b** sub-arc (J-182) — see `tasks/M6_FEDERATION_POLICY_DESIGN.md` (FAC-D3/D4) + `tasks/M6_FEDERATION_POLICY_IMPL.md`. The federation-admin-control arc is **CLOSED**.
>
> **As-built deltas vs the Block-4 sketch below (honest, D-065). 2a:** added a `federation.require_approval` config flag (default false → today's auto-establish, byte-for-byte) gating an inbound approval queue (`PendingFederationQueue`) + a `FederationState` field (`Active`/`Pending`/`Rejected`/`Revoked`, FAC-D2). The pause-point is a node-side gate in `handle_federation_incoming` answering **`Reject` code 2003** (`approval_pending`). `accept` dequeues + upserts `Active` + schedules reconnect; `reject` writes a permanent `Rejected` tombstone that **suppresses re-enqueue** (a rejected peer is refused without re-queuing); `initiate` is **ungated** + targets a *known* peer (reuses `reconnect::attempt_reconnect`; fresh-URL-to-unknown-peer bootstrap deferred to the bootstrap-client arc). As-built error codes are **FED_3005** (no pending request — accept/reject), **FED_3006** (no known relationship — initiate), **FED_3007** (no stored endpoint — initiate); the `FED_3002`/`3003`/`3010`/`3011` codes and the `endpoint` accept arg in the per-verb sketch below were the Block-4 design guess, superseded by the 2a implementation. `federation list --state` now filters the real `FederationState`. **2b:** `set-policy`/`show-policy` back a per-peer `FederationPolicy { mode: Allow|Deny, allowed_spaces: Option<Vec<SpaceXgid>> }` **sibling store** (`xgen-node_federation_policy.json`, FAC-D4 — not a relationship field, so an operator can *pre-deny* a peer); `rate_limit` was **DEFERRED** (couples to unbuilt load-measurement). Enforcement (FAC-D3) is at **both** sites via one pure `policy_permits` helper, **default-permit** (no policy = today byte-for-byte): outbound in `apply_federation_push`, inbound node-side in `process_inbound`. As-built error code is **FED_3008** (invalid mode); the sketch's `FED_3004`/`FED_3020` + the `rate_limit` arg + the all-peers `show-policy` were Block-4 guesses, superseded — `set-policy` targets one peer (unknown peer allowed, pre-deny), `show-policy` shows one peer's policy or the default with an `is_default` marker. **Arc G (PG-04, J-250):** `FederationPolicy` gained `allowed_jurisdictions: Option<Vec<String>>` (additive serde, restrictive-only, default permit-all) + `set-policy` a repeatable `--allowed-jurisdiction` arg; a pure `jurisdiction_permits` is **AND-composed** with `policy_permits` at both enforcement sites (strict undeclared-denied). Dormant-but-correct — no-op until an operator sets the list (AG-D5/D6).
...
#### `federation set-policy` — WRITE — **SHIPPED ✅ (federation-admin-control 2b, J-182; jurisdiction Arc G, J-250)**
- **As-built** (`FederationSetPolicyArgs`): `peer_node_id: String`, `mode: String` (`allow | deny`; parsed → `PolicyMode`), `allowed_space: Vec<String>` (repeatable `--allowed-space`; empty → `None` = all shared Spaces; restrictive-only), `allowed_jurisdiction: Vec<String>` (repeatable `--allowed-jurisdiction`, **Arc G PG-04 / AG-D6**; empty → `None` = no jurisdiction restriction; restrictive-only — the peer federates only Spaces whose declared `jurisdiction` ∈ set, an undeclared Space denied under a non-empty set; open strings, verbatim). `rate_limit` **DEFERRED** (FAC-D4).
- **Result** `FederationSetPolicyResult { peer_node_id, mode: String, allowed_spaces: Option<Vec<String>>, allowed_jurisdictions: Option<Vec<String>>, set_at: String }`.
- **Error codes (as-built):** **FED_3008** invalid mode (at `validate`, pre-audit); `GENERIC_4000` (store save). The sketch's `FED_3004`/`FED_3020` were superseded — an unknown peer is *allowed* (pre-deny intent, FAC-D4).
- **Audit:** WRITE → entry written. `target` = peer_node_id.
- **Failure stages:** `validate` (mode) → `register` (set the live `FederationPolicyStore` + save) → `persist`.
- **Propagation:** local per-peer policy; mutates the live store the enforcement sites consult → effective immediately; no event.
- **Spec refs:** §3.15, §4.2; FAC-D3/D4.

#### `federation show-policy` — READ — **SHIPPED ✅ (federation-admin-control 2b, J-182)**
- **As-built** (`FederationShowPolicyArgs`): `peer_node_id: String` (one peer; the sketch's all-peers default was superseded).
- **Result** `FederationShowPolicyResult { peer_node_id, mode: String, allowed_spaces: Option<Vec<String>>, allowed_jurisdictions: Option<Vec<String>>, is_default: bool }` — `allowed_jurisdictions` is the Arc G PG-04 restriction set (None = no restriction); `is_default = true` when no policy is stored (the default permit-all shown explicitly, prime invariant).
