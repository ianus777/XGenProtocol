...
| **G-5** | ✅ **SHIPPED J-779 — AND IT CLOSES THE MILESTONE. `cargo` 1665 → 1667 / 0 / 64 × 57 SUITES**, re-driven independently by both seats on forced rebuilds, delta by `--skip` and libtest's own `filtered out = 2`, never arithmetic. Runbook `tasks/RUNBOOK_SPACE_ADMISSION_LEG_G5.md` **v1.3 COMPLETED**. 🔑 **ONE BEHAVIOUR CHANGE AND THREE COMMENTS THAT HAD GONE FALSE, WHICH IS WHAT A CLOSE LEG IS FOR.** `mutates_state_file` OMITTED `leave`, so the **MP-F7 leave anchor — the one record `G-4`'s fallback reads — had been written OUTSIDE `StateFileLock` for the whole of `G-1`…`G-4`**; the classifier's own comment had already said *revisit if `ops::*` grows a new state-file writer*, ***it grew one and nobody revisited***, and the pre-existing test asserted one positive and two negatives and **never the SET**, so a new writer could escape silently. 🛑 **`state.rs`'s `valid_until` doc stated the DM EXCEPTION AS THE GENERAL RULE** — absent means *never expires* only on a DM; on a regular Space both gates refuse. 🛑 **AND THE SYSTEM GATE'S OWN MODULE DOC SAID BOTH ITS SCENARIOS FAIL (they pass) WHILE ITS `V-9c` DISARM PROCEDURE NAMED A MATCH ARM THAT NEVER SHIPPED** ⇒ ***the negative control for the whole leg could not be executed***, and J-778's own finding — *a leg that edits a file invalidates its own citations into that file* — had recurred inside J-778's artefact. 🔒 **RULED (Joe, 2026-08-27): `1011` stays ONE WORD for every refusal** (differentiating it would make the refusal a **membership oracle**; the cost to a lapsed-invite member is carried, not traded away) · **`3048`'s operator message UNCHANGED** · **the fresh-install restore is `M-CLIENT-RESTORE`, FILED not built**, because the desktop client has no route to `ops::join` at all. 🛑 **FIVE DEVIATIONS, NONE ABSORBED, THREE OF THEM DEFECTS IN THIS ARC'S OWN RUNBOOK** — including an instruction that would have shipped a false comment *in the leg whose purpose is deleting false comments*, and a prescribed `file:line` citation staled by a sibling edit **inside the same commit**. | all |
...
📌 **"Commit pushed" is deliberately not a DoD item** — `Status: COMPLETED` in this header is the shipped signal, and this file stays `ACTIVE` until Leg G closes at `G-5`. 📌 **DISCHARGED 2026-08-27: `G-5` closed at J-779, and this file is now `COMPLETED`.**
