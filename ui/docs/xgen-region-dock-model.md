...
| **window** | ⚠️ **LOOSE UMBRELLA (D-122)** — a new area presenting information not displayed before it opened. **Names no mechanism.** The fixed terms are **modal area** (in-DOM overlay — About, Plugins, Settings; the default) and **separate window** (its own OS-level window; deferred, needs a named reason). **Display form is decided in situ when the surface is built, never inherited from a record.** | surfaces §3; Ch6 §6.8.3's **Window** UI form (which describes a *separate window*). |
