...
| Persistence amendment | [J-NNN] (2026-05-NN) | Drain-without-persist gap closure across `xgen-core::node::runtime`'s three drain helpers (`drain_pending_uniform`, `drain_pending_by_identity`, `drain_pending_by_federation_relationship`) and `xgen-node::app::process_inbound`'s persist site. `NodeRuntime::ingest_event` returns `Result<(), GraphError>` (Q1 a.iii.β, compiler-forced caller handling); `replay_spaces_from_dir` sorts events topologically before ingest (Q1 a.ii defensive layer). `DispatchOutcome::Accepted` gains `additional_persisted: Vec<Event>` (Q2 return-vector aggregation); all three drain helpers return `Vec<Event>` and `dispatch_event` aggregates at three call sites (Q3 same-family-same-atomic-close). Sentinel-tree (four files at `xgen-node/src/tests/`) ships atomic at milestone close as activating integration-level regression lock (Q4(a)); Phase 9 Scenario 3 transition FAIL → PASS verifies the fix end-to-end and effectively collapses Commit 3b-1 into this sub-amendment milestone close. Five-commit Clair-facing sequence per `tasks/PHASE_7_5_PERSISTENCE_AMENDMENT_IMPL.md`: doc-pass → Q1 ingest-path → Q2+Q3 dispatch+persist → sentinel-tree refinement + verify (5 isolated + 3 workspace = 8 green runs minimum) → milestone close. Candidate D-NNN "ingest path invariant encoding" flagged at design doc §8 (rungs above (a).iii.β — ValidatedEvent wrapper, sealed traits + visitor pattern, formal verification) stays flagged-not-promoted in this milestone. Wire-format-neutral, Pass-1-neutral. Test count baseline TBD at milestone close + N new unit tests + 1 resurrected Scenario 3 + sentinel-tree additions. |
