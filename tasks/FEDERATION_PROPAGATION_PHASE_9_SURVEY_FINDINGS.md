...
> Version: 1.1  
> Date: May 2026  
> **Last updated**: 2026-05-19 (Joe-locked all four §8 open questions: Q1 → 12 scenarios; Q2 → defer G4 to M6; Q3 → option (i) serial-test first, escalate on Phase 9 signal only; Q4 → multi-commit Phase 9. See §8 for locked decisions inline.)  
> Language: English  
> Author: JozefN  
...
**Failure-mode catalogue (§6).** 14 bugs catalogued. 11 HIGH-severity (federation-pipeline + protocol-integrity). 10 caught by recommended Phase 9 set (6 baseline + 4 compounds + 2 new compounds = 12 scenarios). 4 entries marked `not caught` and feed the Client-Side Consequences Audit per CLAUDE.md memory entry "Phase 9 close — milestone shipped" line in [`tasks/FEDERATION_PROPAGATION_COMPLETION.md`](tasks/FEDERATION_PROPAGATION_COMPLETION.md:949).
...
### §2.6 Scenario 6 — Federation-relationship rejection

**Owns F-item:** F-3 (Phase 7 verification gate); cross-surfaces `SpaceState.federation_nodes`, Lock B1 skip-rule.

#### A. Preconditions inventory

1. **Code surfaces.** F-3 check at [`runtime.rs:362-383`](xgen-core/src/node/runtime.rs:362). Lock B1 skip-rule at line 370. Rejection emit at line 378. Federation channel origin propagation in `process_inbound` at [`app.rs:1402-1406`](xgen-node/src/app.rs:1402). Rejection log line at [`app.rs:1441`](xgen-node/src/app.rs:1441).
2. **Configuration.** None scenario-specific.
3. **State-file surfaces.** `xgen-node_federation.json::peer_records` shows X as a peer if a session was established; `xgen-node_state.json::spaces[].event_count` should NOT increment by E's count.
4. **Operator controls.** Need a Node X that federates with B at the session level but is NOT a member of any of B's Spaces' `federation_nodes`. Achievable: spawn X, complete federation handshake with B, but never include X in any Space's `state.federation_add`. Then X attempts to push an event for Space S that B hosts but X is not federated for.

#### B. Observation strategy

Primary: **rejection log line** with reason `federation_relationship_missing`. The reason is uniquely identifying.

Secondary: **`peer_records`** shows X as `lost_connection: false` (session is fine), but X attempts have no effect on `event_count`.

Tertiary: **assertion on X's side** — if M6 Phase 2 ships the envelope-level `event_id` rejection signal, X observes the rejection wire-side. Not available in Phase 9 (M6 is blocked behind this milestone).

#### C. Honesty test framing

**Risk:** "Event from peer X is rejected" could pass for many wrong reasons — X has no events to send, X's connection drops, the event itself is malformed, any random reject path.

**Sharper assertion shape:**
1. Assert handshake A_session_X↔B completes (peer_records shows X active).
2. Assert event E from X to B is structurally valid (would pass F-4 in isolation).
3. Assert rejection log line includes exactly `federation_relationship_missing` AND names X AND names Space S — already covered by integration test `peer_without_relationship_rejects_with_federation_relationship_missing` at [`xgen-node/src/tests/federation_relationship_integration.rs:166`](xgen-node/src/tests/federation_relationship_integration.rs:166).
4. Assert event E is NOT in B's DAG.
5. Assert B's local fan-out is NOT invoked for E.

**Why this isolates F-3.** The `federation_relationship_missing` reason string is unique to the F-3 path. The Phase 7 integration test already proves the runtime-level guard fires; Phase 9 elevates the assertion to the deployment level over real TCP.

**Lock B1 honesty check.** A second test verifies the skip-rule: X sends a `state.federation_add` event (with X as the peer being added). This event MUST pass F-3 (Lock B1 skip) but will land in HeldPending if X's Identity isn't replicated. The Phase 7 unit test `state_federation_add_skips_f3_check` at [`federation_relationship_integration.rs:279`](xgen-node/src/tests/federation_relationship_integration.rs:279) is the precedent — assert "outcome is NOT `federation_relationship_missing`", per the **honesty-test precedent** the survey task file names.

#### D. Harness fit

**Reuse `stress-complete` shape** for the main rejection test. The Lock B1 skip-rule test fits better as a `NodeRuntime`-level integration test (already exists for Phase 7). Phase 9 adds one binary-level B1 test.

#### E. Stress dimensions

| Axis | Weak | Strong (recommended) | Rationale |
|---|---|---|---|
| Relationship state at the moment of push | none | **{never-existed, was-defederated-mid-session, asymmetric (A→B yes, B→A no)}** | F-3 surface — the three operationally-possible states |
| Event type | message.text | **state.* AND membership.* AND message.*** | All three event families that the F-3 path treats uniformly |
| Volume | 1 | **20 from X** | Surfaces guard contamination under load |
| Drain-time approximation | not tested | **HeldPending event whose peer X gets defederated before drain** | C9 below; catches bug catalogue M11 |

**Recommendation:** 3 relationship-state variants × 3 event families = 9 scenarios. Plus the C9 compound for drain-time hazard. The 20-volume axis can fold into C2 (anti-transitivity-at-load) since both involve queue-depth on B's outbound.
...
**Caught by Phase 9 set:** 11 of 14.
...
- **Recommended 12:** 6 baseline + C2, C3, C5, C7, C9, C10. Catches 11 of 14 catalogue bugs.
