...
**Owed to Joe (routed, not absorbed):** (1) a one-line D-095 footnote that dev-tooling dirs under `ui/` (e.g. `ui/sampler/`) are mirror-exempt; (2) the GPL-overview question — `ui/core/` = GPL (D-044) makes the Built-components registry the licensed-corpus catalogue of record; candidate `DECISIONS.md` touch tying that boundary to a catalogue-completeness duty.

**Next-active (UI/RP track):** sampler Phase-A (M-RP2.5) or continue data-independent component authoring — Joe's call. **Entry (Rule 0): this PLAY → JOURNAL J-405 → `ui/docs/xgen-ui-notes.md` (N-027 substrate proof, N-028 button + sampler) → `ui/docs/xgen-ui-components.md` (Built-components registry) → `docs/ROADMAP.md` (UI subtree + RP node).** UI sessions additionally read both `ui/docs/` files at open. Not pushed — Joe pushes.
