...
│   │   ↳ trigger: **C-bis-6, C-bis-7 and C-bis-8 are SPECIFIED and RULED (J-709); C-bis-5 moves LAST**: the member-activation path will move the Space context so R1 unlights while you are in a DM, R7 will read the counterpart from the Space record so a DM shows self and counterpart, the drafted-to row gets highlighted, and the failure surface becomes one line under the composer rendering the node own error string verbatim. Joe ruled option A for the orientation, and the D4 opt-2 lock was never ruling on this case because every selection it contemplated was inside a Space. OQ3 is ruled yes, DM Spaces leave the Spaces panel, while G13 semantics stays open with a proposed dissolution: counterpart for the render question, the dm_space_create root event for provenance. Filed and unscheduled: the startup destination, the client side replica fallback from D-049, and archival ownership between the auth module and the node. The startup milestone now carries its own ROADMAP node as M-RP-STARTUP, filed and unscheduled. The E to D gate was re-examined and RULED at J-710, option C: Leg E is re-pointed onto C-bis-6, which is what actually builds its render-only DM filter, and Leg D is POSTPONED with a checkable trigger because the RMC menu has no honest item today. A re-point, not a re-order
