...
## 🟢 PLAY — Federation Event Propagation implementation milestone (Phases 1–4 SHIPPED; Phase 5 next-active)

**Phase 4 closed in J-085 (2026-05-19).** F-1 federation event push + F-1b drop-on-peer-down + F-5 origin gating per design doc §4 + §4.5 + §8.5 + runbook §3.4 + §3.4.1. The "missing mechanism" verdict from audit J-081 §2 (Stage 6 federation propagation architecturally absent) is closed — federation push exists as a production mechanism. Three implementation locks: Q1 `EventOrigin` enum in `xgen-core::node::runtime` flowing through `dispatch_event`/`process_inbound`/`apply_federation_push` (rejected `#[serde(skip)]` on the wire-shape Event); Q2 `FederationPeerSenders` mirroring `ClientSenders` keyed by peer_node_id (rejected co-location with shared_spaces — single source of truth stays on `SpaceState.federation_nodes`); Q3 reuse of `process_inbound` with two-comment overload documentation (semantic overload of `identity_id` accepting Identity URI for clients OR Node URI for federation sessions). The Phase 3 R1 plug-in point in `handle_federation_incoming` is now operational. The F-5 §8.5 anti-transitivity guard at the top of `apply_federation_push` short-circuits on `ReceivedViaFederation` events. F-1b drop-on-peer-down: try_send drops on channel-full or peer-absent, R14 log line emitted, recovery falls to tip-exchange on next handshake.
...
- **Runbook:** `tasks/FEDERATION_PROPAGATION_COMPLETION.md`. Nine phases — **Phase 1 ✅, Phase 2 ✅, Phase 3 ✅, Phase 4 ✅**, Phases 5 through 9 PENDING.
- **Tests:** 468 (handoff) → 476 (Phase 1) → 480 (Phase 2) → 488 (Phase 3) → **491** (Phase 4 close, +3: three integration tests in new `federation_push_integration.rs` covering alice-propagates / F-5 anti-transitivity / F-1b drop-on-peer-down).
- **Blocks:** M6 (new) still blocked behind this milestone going DONE.
- **Next active phase:** Phase 5 — F-1c per-peer record + reconnect scheduling. Implement the persistent per-peer record at the Node-implementation layer (lost_connection flag, timestamps, operator notes, priority). Reconnect scheduler task periodically reads F-1c records, computes next attempt time via global backoff schedule (15/30/60/120 min capped), fires `run_initiating` for peers flagged lost_connection. `run_initiating` gains its first production caller in `xgen-node/src/` (audit J-081 §2.2 found zero today). Schema decision (extend `peer_announcements` vs sibling table) is Clair-latitude per runbook §3.5. The Phase 3 bilateral helper `stream_federation_delta` is the initiator-side caller from Phase 5's reconnect path (per Phase 3 §3.3.1 Lock 7 R5).

**Phase 4 shipped these structural changes:**
- `EventOrigin` enum (`LocallySubmitted` / `ReceivedViaFederation`) in [`xgen-core/src/node/runtime.rs`](xgen-core/src/node/runtime.rs) alongside `DispatchOutcome`. Forward-compatible with future variants (`ReceivedViaAdminInjection` M6, `ReceivedViaBackfill` hypothetical replay tooling). `dispatch_event` signature gains `origin` parameter — unused inside (validation is origin-uniform) but reserved per signature-transparency lock.
- `FederationPeerSenders` type alias in [`xgen-node/src/fanout.rs`](xgen-node/src/fanout.rs) — `Arc<Mutex<HashMap<peer_node_id, Sender<OutboundMsg>>>>` mirroring `ClientSenders`. R12 lifecycle: register on handshake-ACTIVE, deregister on session exit.
- `apply_federation_push` in [`xgen-node/src/federation_session.rs`](xgen-node/src/federation_session.rs) — sibling of `apply_fanout` (not a wrapper). F-5 guard at the top with §8.5 citation. R13 try_send (non-blocking). R14 log lines for both "peer absent from registry" and "channel full" cases.
- `process_inbound` in [`xgen-node/src/app.rs`](xgen-node/src/app.rs) gains `origin: EventOrigin` parameter with Q3 two-comment documentation at the function definition AND the federation call site.
- `handle_federation_incoming` Phase-3 plug-in points wired per R12: `out_tx` registered post-ACTIVE; outbound arm drains pushed events to the wire; inbound `Inbound::Event(_)` arm routes through `process_inbound`+`apply_fanout`+`apply_federation_push` (last short-circuits via F-5 guard); deregister on loop exit.
- Three new integration tests in [`xgen-node/src/tests/federation_push_integration.rs`](xgen-node/src/tests/federation_push_integration.rs) covering the full push path end-to-end, F-5 anti-transitivity guard verification, and F-1b drop-on-peer-down absence-of-panic.
...
- `collect_sync_history` signature `(runtime, requester_id, since, limit) -> (Vec<Event>, Option<String>)`.
...
**Federation Event Propagation implementation Phases 1-4 shipped (J-082 + J-083 + J-084 + J-085 across 2026-05-18 and 2026-05-19).** Phase 4 (J-085) closes the "missing mechanism" verdict from audit J-081 §2 — federation event push now exists as a production mechanism with F-5 anti-transitivity guard and F-1b drop-on-peer-down semantics. Tests: 468 (handoff) → 476 (Phase 1) → 480 (Phase 2) → 488 (Phase 3) → **491** (Phase 4, +3 integration tests in new `federation_push_integration.rs`). Next active phase: Phase 5 (F-1c per-peer record + reconnect scheduling — `run_initiating` gains its first production caller in xgen-node/src/). M6 (new) remains blocked behind this milestone going DONE. Roadmap: M5 ✅ → CLI Audit ✅ → J-080 ✅ → M6 Phase 0 Pass 3 ✅ → Propagation Reliability Audit ✅ → Federation design (Pass 2 + Pass 3) ✅ → **Federation implementation Phases 1-4 ✅, Phase 5 (F-1c reconnect) next** → M6 (new) → M7 → M8 → M9.
...
- **Federation Event Propagation implementation**: 🟢 PLAY. Nine-phase runbook; **Phases 1-4 ✅ SHIPPED** (J-082 + J-083 + J-084 + J-085). **Phase 4 ✅ (J-085, 2026-05-19)** — F-1 federation event push at `apply_fanout` sibling position; F-1b drop-on-peer-down with no outbound queue; F-5 origin gating via `EventOrigin` enum threaded through `dispatch_event`/`process_inbound`/`apply_federation_push`; new `FederationPeerSenders` registry mirroring `ClientSenders`; Phase 3's R1 plug-in point operational; uniform `apply_federation_push` call shape from both entry points with F-5 guard short-circuiting on `ReceivedViaFederation`. Phase 5 (F-1c per-peer record + reconnect scheduling) is next-active. 491 tests at Phase 4 close.
...
**Roadmap:** M5 ✅ → CLI Audit ✅ → J-080 ✅ → M6 Phase 0 Pass 3 ✅ → Propagation Reliability Audit ✅ → Federation design (Pass 2 + Pass 3) ✅ → **Federation implementation Phases 1-4 ✅, Phase 5 (F-1c per-peer record + reconnect scheduling) next** → ~~M6 multiparty~~ DEPRECATED → M6 (new) → M7 → M8 → M9. D3 (MLS) parallel.
...
*Read `DECISIONS.md` (current range D-000 through D-071) before making any decision that isn't explicitly covered by the spec. If you're unsure whether something needs a DECISIONS.md entry, it does.*
