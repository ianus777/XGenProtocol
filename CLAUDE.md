...
> Date: May 2026  
> **Status:** ACTIVE  
> **Last updated:** 2026-05-21 (Bidirectional `federation_nodes` audit shipped at `tasks/FEDERATION_BIDIRECTIONAL_NODES_AUDIT.md` Status: ACTIVE v1.0 ~44 KB. The audit is the canonical record of the finding surfaced during Phase 9 Commit 3a Scenario 1 diagnostic run; it presents the gap, the code-verified mechanism, the relationship to the locked design, the two structural questions for the design walkthrough (Q1: one event or two; Q2: signature carries enough info to derive receiver's value), and four candidate fix shapes (A: origin-aware applier with replay sub-options; B: reciprocal mint on ingest; C: two events at handshake; D: schema extension) with code-grounded cost/benefit and Pass-1 impact for each. Summary table at §7.5. Per D-069 + D-071 + D-074 + "honest longer work over fast shortcut" discipline. Sibling-in-shape to J-081 Propagation Reliability Audit and Phase 7.5's design task file precedents. Status: ACTIVE because the audit is the input to the next-active design phase walkthrough (Chat Claude + Joe). It flips COMPLETED when the design phase closes and locks a fix shape; the locked design then produces its own task file (sibling to `tasks/FEDERATION_PROPAGATION_PHASE_7_5_DESIGN.md`) which becomes the new active artefact. ROADMAP.md v1.8 → v1.9 in same commit per same-commit discipline: Visual structure tree's audit phase node flips 🟢 → ✅; design phase node flips 🟡 → 🟢 ACTIVE next-active; Present section adds the audit-shipped Past entry and reframes the design phase as next-active with the audit doc as input; new Past entry recording the audit doc shipment. Pass-1 impact summary: Shapes A/B/C are Pass-1-neutral; only Shape D adds one new field row to Appendix C + I, which would extend Pass 1's coverage table by one row but not invalidate any sub-question lock. Pass 1 runbook stays at Status: ACTIVE v2.0 with all four sub-questions Joe-locked; the Pass-1-impact question is resolved when the design phase locks a shape. Previous Last-updated note (Phase 9 stand-down at Commit 3a boundary + bidirectional finding) stands authoritative in the PLAY block body below; this header is one click closer to the next-active work (the audit doc is the new entry point).)  
> Author: JozefN  
> Credits: Concept, philosophy, requirements, project direction: Jozef Nižnanský. Technical assistance and implementation support: AI-assisted development tools.  

---

## 🟡 PLAY — Bidirectional `federation_nodes` audit SHIPPED 2026-05-21; design phase walkthrough next-active for Chat Claude + Joe (Phase 9 PAUSED at Commit 3a)

**Audit doc shipped 2026-05-21.** `tasks/FEDERATION_BIDIRECTIONAL_NODES_AUDIT.md` (Status: ACTIVE v1.0, ~44 KB) is the canonical record of the finding surfaced during Phase 9 Commit 3a Scenario 1 diagnostic run. Sibling-in-shape to J-081 Propagation Reliability Audit (pre-design subsystem audit) and to `tasks/FEDERATION_PROPAGATION_PHASE_7_5_DESIGN.md` (Phase 7.5 design task file pattern). Per D-069 (canonical document) + D-071 (audit precedes dependent design) + D-074 (forward-binding for the design + implementation phases that follow) + "honest longer work over fast shortcut" discipline.

**Audit doc contents (entry point for design walkthrough).** Ten sections: (1) what this document is + position in milestone; (2) provenance — how the gap surfaced including the test-surface structural gaps that hid the bug through eight Federation Event Propagation milestone phases plus Phase 7.5; (3) the mechanism, code-verified — sender side `stream_federation_delta` construction, builder `build_federation_add_event`, receiver side `apply_federation_add` application, combined outcome table, F-3 then-what; (4) the relationship to the design as locked — §6.4 Phase 7 Lock A1's anticipated-but-different failure mode, the unstated principle the bug reveals ("event semantic and event application semantic must be congruent across all ingestors"); (5) scope; (6) audit-phase questions — Q1 (one event or two logical events) + Q2 (signature carries enough info to derive receiver's value, code-verified yes), with conditional Q3-Q6 surfaced for each shape; (7) candidate fix shapes A/B/C/D with code-grounded cost/benefit + Pass-1 impact + summary table at §7.5; (8) Phase-9 + milestone implications; (9) discipline notes; (10) cross-references. The design walkthrough's first move is resolving Q1 (the option space below depends on the answer).

**Q1 framing (resolved by walkthrough).** The bug exists because the event has a single `node_id` field that means different things to different readers. Two readings of what the event fundamentally IS: **Reading (i)** — one event, asymmetric interpretation ("A approves B as a federation peer for Space S"); current schema records only the "other" party from sender's perspective; fix teaches receiver to derive the OTHER party from `event.sender`. **Reading (ii)** — two events, one per side; each side produces its own `state.federation_add` naming the other; both events live in respective DAGs. Reading (i) admits Shape A + Shape D; Reading (ii) admits Shape B + Shape C.

**Q2 result (already code-verified in the audit).** A's Node ID is reliably available to B via `event.sender` because `build_federation_add_event` calls `sender_id(key)` where `key = node_keypair` (A's Node keypair), and Phase 4 Q3 overload allows `event.sender` to be a Node URI. So Reading (i) is implementable via origin-aware apply.

**Four candidate fix shapes (from audit §7).** A — origin-aware applier (smallest code change; replay concern Q3 with three sub-options A.1/A.2/A.3; Pass-1-neutral). B — reciprocal mint on ingest (no wire change; breaks the "apply is pure function of event" principle differently than A; reciprocal-propagation-to-A concern; Pass-1-neutral). C — two events at handshake (re-opens Phase 3 §3.3.1 Lock 2 a-i symmetry rule; prev_events sub-options C.1/C.2; Pass-1-neutral). D — wire-format change extending content schema (cleanest semantics; two sub-options D.1 add `peer_node_id` field, D.2 symmetric `{a_node, b_node}`; ONLY shape with Pass-1 impact — adds one field row to Appendix C + I, would extend Pass 1's coverage but not invalidate any sub-question lock).

**Pass 1 impact resolution.** Shapes A/B/C are Pass-1-neutral. Shape D adds one new field row to Appendix C primitive schemas + Appendix I content-struct field tables. The `event_content_struct_xgid_roundtrip` invariance test (Pass 1 sub-question 4) would need to cover the new field. If Shape D is locked, Pass 1's runbook gets a one-row extension to its coverage table; sub-question locks themselves remain valid. If any other shape is locked, Pass 1 proceeds unmodified at Status: ACTIVE v2.0.

**Phase 9 still PAUSED at Commit 3a boundary.** Scenario 1 (`xgen-node/src/tests/phase9_two_node_smoke.rs`) `#[ignore]`-annotated, stays on disk as regression witness. When the fix lands, `#[ignore]` lifts and Scenario 1 becomes activating regression lock. Phase 9 Commit 3b (Scenarios 2 + 3 + compound scenarios C2/C3/C5/C7/C9/C10) stays paused inside the same milestone scope. Milestone scope unchanged (12 scenarios still in-scope; deferral is not descope).

**Track 1 (Clair): standby until design phase closes.** No active Rust implementation track. Pass 1 implementation blocked behind Federation Event Propagation milestone closure (which itself is now blocked behind the bidirectional fix landing AND Phase 9 Commit 3b completing). When the design phase closes and the implementation runbook ships, Clair picks up the implementation against the locked design.

**Track 2 (Chat Claude + Joe): design phase walkthrough next-active.** Entry point for next Chat Claude session is `tasks/FEDERATION_BIDIRECTIONAL_NODES_AUDIT.md`. The walkthrough resolves Q1 (§6.1 of the audit doc) and picks one of the four candidate fix shapes (§7) with Joe-lock. Expected size: similar to Phase 7.5 (multi-session design phase, multiple commits to ship the fix). The locked design phase produces its own task file (sibling to `tasks/FEDERATION_PROPAGATION_PHASE_7_5_DESIGN.md`); after the design phase closes, the implementation runbook is authored (sibling to `tasks/FEDERATION_PROPAGATION_PHASE_7_5_IMPL.md`) and handed off to Clair.

**M6 (new) Node admin write path** remains 🟡 PENDING behind Federation Event Propagation milestone closure (which itself waits behind the bidirectional fix landing AND Phase 9 Commit 3b completing). M6 Block 4 verb-by-verb walks remain parallel-eligible per the standing arrangement.

**Entry point for Chat Claude's next session: `tasks/FEDERATION_BIDIRECTIONAL_NODES_AUDIT.md`.** Read the audit doc (it's self-contained — §3 grounds the mechanism in code with file:line references, §6 frames Q1+Q2, §7 presents the four shapes with summary table at §7.5). Then walk the design phase with Joe. No re-reconnaissance needed; the audit doc replaces it. Sibling docs for shape precedent: `tasks/FEDERATION_PROPAGATION_PHASE_7_5_DESIGN.md` (design task file shape) and `tasks/FEDERATION_PROPAGATION_PHASE_7_5_IMPL.md` (implementation runbook shape that follows).

**Entry point for Clair's next session: standby.** Same as previous header note — no active Rust implementation track until the audit + design phase closes and an implementation runbook ships against the locked fix shape.
