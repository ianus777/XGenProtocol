...
- **Column reserved.** The grid keeps the avatar track pinned to `28px` (`.message` → `28px 1fr`, `.message[data-own]` → `1fr 28px`; `28px` = list-avatar width, `1fr` = content — 288px in an earlier draft was illustrative, never literal in code); only the avatar **cell content** is empty, so continuation bodies stay aligned under the head (no left-shift). Symmetric for `isOwn`.
