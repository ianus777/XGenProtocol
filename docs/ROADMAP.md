...
│   │   └── 🟡 **Leg E — the DM home + the R1 filter** — DM Spaces leave R1 and gain somewhere to live · **THE MILESTONE'S LAST LEG. Minted at J-717, which is late: it has been referenced by three records and owned a row in the on-screen table since J-713 while carrying NO NODE** — the dangling-pointer defect J-710 wrote the rule about, committed against the very milestone that wrote it · no Phase-0 of its own; content ruled by Joe at J-709 (**OQ3 = yes, DM Spaces leave R1**) · grounded in Phase-0 section 6 and `tasks/M_RP_MEMBER_ACT_PHASE0.md` `A3` + `F-D` + `K2`
│   │       ↳ Owes: **`G13`'s semantics are Joe's and UNRULED** — Chat's dissolution is a PROPOSAL: `counterpart != null` answers the render question, the `dm_space_create` root event answers provenance permanently · **`N-173`'s DM-row rename is Joe's** — R1's DM rows read `DM with xgen://pubkey/e...` today · **`K2`** — `KnownSpace` gains a real `counterpart` field, folded here from Leg B · the Space-never-clicked case (J-694) · **the filter lives in `spaces-panel`'s `$derived` ONLY — the store retains every DM so the latch, `canSend` and the DM home can still resolve them**
