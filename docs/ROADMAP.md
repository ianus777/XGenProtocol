...
│   │   ↳ 🔑 **SPAWNED BY `M-RP-LIVEFEED-REFRESH` Leg A (J-643)** — the live router created a state the panel had never been in: a roster row whose identity was **NEVER LOOKED UP**. Phase-0 `tasks/M_RP_IDENTITY_RESOLUTION.md` **v1.9 ACTIVE**
...
│   │   ├── 🟡 **Leg B the render rules** — ③ filtered from the rendered list (except the DM counterpart, §5a) · 🔒 **runbook `tasks/RUNBOOK_IDENTITY_RESOLUTION_LEG_B.md` v1.2 ACTIVE + LOCKED (Joe, J-652)**
│   │   │   ↳ 🛑 **LOCKED AND IDLE — the trigger is *"Joe locks the runbook AND stands Clair up"*, TWO ACTS, and he has performed ONE. NOBODY IS IMPLEMENTING IT.** ⚠️ ***"Locked" reads like "started" to anyone skimming*** (J-646) · ✅ *four lock-state countdowns inside the runbook were struck and annotated IN THE SAME EDIT THAT LOCKED IT — a countdown is discharged by the act it waited for, never left for the close*
...
│   │   │   ↳ ✅ **Leg B's OWED `Vec<IdentityXgid>` wire witness IS SCHEDULED AS COMMIT B-i (J-650)** — it ships **alone**, so the cargo move 1588 → 1589 stays attributable
│   │   │   ↳ trigger: **ONE ACT LEFT — Joe stands Clair up. The lock landed at J-652.** ✅ *An adversarial READ is neither act, and she has done that (J-651) — the distinction is the whole point of the two acts.* ⚠️ *v6.35–v6.37 read "NEXT-ACTIVE, and it has NO RUNBOOK" (false from J-650); v6.38–v6.39 read "TWO ACTS, and NEITHER has been performed" (false from J-652). Both superseded, kept not erased (`D-131`).*
│   │   ├── 🟡 **Leg C the skin** — the dimmed treatment in `ui/assets/skin.css` ⚠️ **JOE'S FILE** · 🔒 **LOCKED SEPARATE FROM LEG B — IT DOES NOT RIDE ALONG (Joe, J-650)** 🛑 **⇒ LEG B LANDS SILENTLY: ④'s dimming and E2's mark are BOTH skin values, so the only visible change after Leg B is that erased non-DM members disappear from the list** ↳ trigger: Leg B lands
