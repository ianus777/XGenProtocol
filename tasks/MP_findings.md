...
- **UPDATE (J-374) — RESOLVED at M10.5 (C1b, `7a2ff89`); fix shape (b) dispatch-level `banned` pre-check.** The M10.5 D-071 audit (`6947a78`) swept `dispatch_event`'s apply sites D-077-bidirectionally and **found the swallow load-bearing nowhere dangerous**: the `let _ = state.apply_event` silence (line-drift corrected — live HEAD `runtime.rs:748`, not `:691`; `:693` is now `let _ = store.append`) is load-bearing **backward for replay only** (`replay_spaces_from_dir → ingest_event` depends on tolerant apply), not for live dispatch; sibling sites benign. **Fix = a dispatch-level `banned` pre-check** (M10.5-D2) in the existing `MembershipJoin` block (`runtime.rs:1387`, before the PG-13 tier-gate), returning `DispatchOutcome::Rejected(RejectInfo)` for a banned joiner — chosen over fallible `ingest_event` (which would ripple into the replay caller; zero replay blast-radius this way). The apply-layer silence **stays** (it serves replay). **Reject code = reuse the PermissionDenied-class** (`to_wire_code(PermissionDenied)=None → 4000`-unmapped, sibling to MP-C-09's banned-*send* reject; M10.5-D3) — **no new wire code, no ch3 edit**; a precise `join_banned` code is routed to MP-F2-followon. Witness `dispatch_banned_join_rejected_not_accepted_but_inert` RED-on-revert genuine (neutered → `Accepted{new_joiner:Some(bob)}`, the exact accepted-but-inert symptom). `cargo test --workspace` 1399/0, clippy clean (default + all-features).
- **Status:** **RESOLVED** (M10.5 C1b, `7a2ff89`, J-374). Reply-honesty restored at dispatch; the apply-layer silence retained for replay-tolerance (D-077-confirmed).
...
- **Status:** **RESOLVED** (Layer-1 fix M10.4 `de53cf0`; observed full-oracle green M10.5 C1a, J-374). See the J-374 update below.
...
- **UPDATE (J-374) — RESOLVED on observed full-oracle green (M10.5 C1a).** The box-gated `mp_r2_fixed::mp_c_16_live_migration_space_rehomes` ran on a freed box and went **GREEN, stable 3/3** — the first real-binary end-to-end proof the M10.4 namespace fix flows up to the migration gate. Both halves of the matrix:150 oracle are now **asserted, neither inferred**: `home_node`-flip-on-both (post-cutover `hosted_by_node = [("a",[]),("b",[<space>])]` — A no longer homes it, B now does) **and** state-equality (B's resolved `member_count` == A's resolved member-set size; B carries Room state). Honest catch (D-065): J-373/D1's "verification re-run, nothing to build on the fix path" was **falsified** — the M10.4 fix cleared Site 1/Site 2, but a **distinct downstream Arc-F admission asymmetry** sat on the federated path (the destination rejected `already_hosting` because it replicates the Space via federation). Closed via Opt-2 scenario fix (migrate to a **bare** B — corrects the scenario to the admin-migration model; `93bbeab`) + the state-equality witness (`55a308e`). The admission asymmetry is recorded as **MP-F15** (routed to a future migration-depth arc), **not** a carve-out (MP-C-16 doesn't need it under the admin model). **Status: RESOLVED** (J-374).
...
- **UPDATE (J-353) — exec-step-1 FALSIFIED Fork A; mechanism RE-LOCKED (MP-F14-D7 fired). The finding above (federation_nodes-growth / pre-join backfill / gap-2 / MP-F11/MP-F1b family / J-333 hole-safety) is the falsified hypothesis; the real root is the MP-F4/J-331 causal-frontier family.** The box-gated re-trace (runbook §2) disproved the federation_nodes-growth premise: when n0 pushed p0, `federation_nodes[S]` = {n1,n2,n3} — the leaf **was** a push target (established ~2.2s before p0), `federation_push_sent` succeeded, n1 **received** p0. Federation_nodes were correct + delivery happened → **Fork A / Fork B off-target.** **Actual root (grounded to file:line across n0+n1 kept logs):** p0 arrives on n1 and is **HeldPending for a missing causal predecessor** (the F-4a buffer, `runtime.rs:1193`) that **never drains**. p0's `prev_events` = the `get_dag_tips` frontier (`xgen-client/src/batch.rs:96`, the **MP-F4/J-331** fix): 5 tips = 3 `membership.join` + `state.room_create` + one **`state.federation_add`**. That `state.federation_add` tip is **vantage-specific / directional (D-075)** — it never converges to n1 (independently confirmed: the oracle **excludes** `state.federation_add` via `INFRA_EVENT_KINDS`, MP-R1-D7 — *which is why the verdict flags only p0*). So a cooperative message **anchors to an infra predecessor that can never reach the peer** → un-applyable → permanent miss. The ~60% intermittency = the race on whether a `state.federation_add` is a `get_dag_tips` frontier tip at p0's `after_ms:40` send (concurrent with the G-6 establish); `compute_frontier` does **not** filter by event kind. MP-F4 widened `get_dag_tips` to the full frontier (to fix room-join anchoring); that same frontier now over-anchors cooperative content to non-converging infra. **Re-locked fix (MP-F14-D2 supersedes Fork A; Joe-confirmed):** exclude infra kinds (`state.federation_add`) from the cooperative frontier `get_dag_tips` returns for a new cooperative event's `prev_events`, so DAG-anchoring **agrees with** the convergence contract. Site: `xgen-client/src/batch.rs` collection loop (:145-161; `ev.event_type` available → client-side, one function, all cooperative callers inherit). **NOT** the federation_nodes outbound re-stream; the **J-333 hole-safety lens no longer applies** (not an F-3 path) — the relevant safety is the **MP-F4 no-regression** (membership/room tips stay; only infra excluded), proven RED-on-revert (spine #2). **Precision (design-relock):** the batch.rs exclusion set must bind to the **same source of truth** as the oracle's `INFRA_EVENT_KINDS` (a shared constant/predicate, not a hand-copied parallel list) so "what the DAG anchors to" = "what must converge" by construction — a tiny set-identity test guards drift. **Independently re-grounded (Chat, J-353):** `INFRA_EVENT_KINDS = ["state.federation_add"]` (oracle docs: "directional, not symmetric, **not** a lost event") + `get_dag_tips` (batch.rs:96) accumulates every space event with **no kind filter** — both pillars confirmed on `main`; the fix corrects a genuine anchoring↔convergence contract inconsistency, not the symptom. **Gate disposition unchanged (D7):** fix-in-round, terminal = GREEN-on-rerun. Arc-docs re-locked v1.1 (`9faab4f`; Fork-A body retained as the historical conditional the trace resolved). **Next = spine-first Commit 1** (box-free, `xgen-client`): cooperative-frontier infra-exclusion + 2 RED-on-revert spine tests (excludes-`federation_add` + keeps-membership/room) → D5 enrichment → box witness → R3 rerun.
- **Status:** the **sole MP-R3 fix-phase gate item** (J-351; mechanism **re-locked J-353** — Fork A falsified → `get_dag_tips` cooperative-frontier infra-exclusion, the MP-F4/J-331 family, NOT the MP-F11/federation_nodes family). Terminal = (a) GREEN-on-rerun (gates the MP-R3 close). MP-C-14 is its witness row. **exec-step-1 DONE; next = spine-first Commit 1.**

- **UPDATE (J-356) — RESOLVED, GREEN-on-rerun (MP-R3 close).** Commit 1 `ad6340b` (`get_dag_tips` cooperative-frontier infra-exclusion: exclude infra kinds — `state.federation_add` — from a new cooperative event's `prev_events` frontier so DAG-anchoring agrees with the convergence contract; the MP-F4/J-331 family root; Option A — typed `EventType::is_federation_infra()` predicate + derived `INFRA_EVENT_KINDS` in `xgen-common`, re-exported `xgen_core::INFRA_EVENT_KINDS`, set-identity no-drift guard; 2 RED-on-revert spine tests) + Commit 2 `20a6377` (D5 3-class coverage enrichment) + correction `339b770` (leaf room-join — the D5 framing under-diagnosed the leaf gap as a join-race; the real missing step was room membership, `message.text` requires `is_room_member`; a D-065 catch). **Box-gated witness:** MP-C-14 flipped **RED→GREEN, stable 5/5** (the ~60% intermittent stuck-`p0` gone); dep-witnesses MP-A-01(ii) + MP-A-08 GREEN; result **all-green-except-{MP-C-06, MP-C-16}** (both ⏸️ DEFERRED → M10). MP-F14 terminal (GREEN-on-rerun) → the {MP-F14} gate satisfied → **MP-R3 CLOSED = the Multiparty-tests milestone close** (consolidated R1+R2+R3 ledger: `tasks/HANDOFF_MP_R3.md` §3). **Status: RESOLVED.**

---

## MP-F15 — Arc-F migration admission rejects an already-federated destination as `already_hosting` (presence-vs-home-ownership asymmetry)

- **Surfaced:** MP-C-16 (live migration), the M10.5 C1a box-gated RUN (`mp_r2_fixed::mp_c_16_live_migration_space_rehomes`), pinned by Clair from kept node logs (J-374). Status: **ROUTED → a future migration-depth arc (Joe-LOCKED J-374). NOT a carve-out** (MP-C-16 doesn't need it under the admin-migration model).
- **Symptom (empirical, certain):** when the destination node **already replicates** the target Space (it is a federated peer that received the Space via federation before the migration proposes), `migration initiate` is rejected: source log `migration: destination rejected proposal reason=already_hosting`; destination log `Incoming migration connection → rejecting proposal reason=already_hosting`.
- **Mechanism (grounded, Clair):** `handle_migration_propose` (`xgen-node` state_machine.rs:289) rejects on **mere presence** — `existing_space_ids.contains(...)` over `rt.spaces.keys()`, which **includes federated replicas** — whereas the **source** side correctly distinguishes **home-ownership** (`require_hosted_home_space`: `home_node == self`). So the destination refuses to accept a migration of a Space it merely *replicates*, even though it does not *home* it. That asymmetry is the gap.
- **The realistic case it blocks:** migrate-to-an-already-federated-peer (a replica→home promotion — the common production deployment, where the destination is typically already a federation peer). This is exactly Opt-1's case from the M10.5 disposition: the honest in-frame fix for MP-C-16 was Opt-2 (migrate to a **bare** destination, matching the admin model), and Opt-1 (fix admission to home-ownership + make `verify_transfer` robust to a pre-existing replica) was deliberately **not** folded into M10.5 as out-of-scope feature work.
- **Why first surfaced now:** the in-process Arc-F migration tests set `home_node = node_id` directly and never federated source↔destination, so the propose-against-a-replica path was never exercised until the M10.5 real-binary RUN (M10.4-D3 — first end-to-end migration on real binaries).
- **Severity:** does not block MP-C-16 (closed green via Opt-2). Blocks the **already-federated-peer migration** deployment case. Sibling to MP-F13 (the home_node-identity family) — a migration-depth gap, not a correctness defect on the admin path.
