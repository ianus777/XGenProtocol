...
> Version: 1.1  
> Date: May 2026  
> **Last updated**: 2026-05-21 (Commit 4 of 4 SHIPPED — milestone close. All four commits landed: Commit 1 doc-pass (`e975162`), Commit 2 origin-aware applier + plumbing + six unit tests (`a730eda`), Commit 2.5 sibling vantage-aware drain hook (`cbceb41`, in-flight gap closure), Commit 3 Phase 9 Scenario 1 resurrection (`f051039`), Commit 4 milestone close (this commit, JOURNAL J-096 + CLAUDE.md PLAY block flip + ROADMAP.md state move + this Status flip + Phase 9 task file paragraph update + Scenario 1 re-stood-down on separate topological-sort finding). Test count 571 → 577 across the milestone (+6 unit tests in Commit 2; Scenario 1 originally lifted in Commit 3 then re-stood-down in Commit 4 on the topological-sort finding, net wash on ignored count). Bidirectional fix is verified-correct — six unit tests in `xgen-core/src/space/state.rs::mod tests` (notably `apply_federation_add_two_vantages_mirror`) remain green and stand as the live D-075 regression lock. Status flipped ACTIVE → COMPLETED; Version 1.0 → 1.1. Locked content below preserved as authoritative record of the four-commit sequence. Per D-069 (canonical document) + D-074 (milestone-close commits MUST include JOURNAL.md) + D-075 (the protocol-design principle the locks instantiate). See JOURNAL J-096 for the milestone close-out narrative including the separate topological-sort wire-order non-determinism finding that surfaced during Commit 4 verification (forward-referenced from Scenario 1's re-applied `#[ignore]` to placeholder `tasks/FEDERATION_TOPOSORT_AUDIT.md` as the new active phase, sibling-shape per D-071).)  
> Language: English  
> Author: JozefN  
