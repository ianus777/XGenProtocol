...
## 🟢 UI component-library / substrate (RP track) — M-RP2.20 ✅ CLOSED (J-431): `led` — the **fifteenth** `core` component, the **fourth simple display-di**; a caller-supplied colour map (the N-034 shape) + the **first data-coloured atomic** (colour rides an inline CSS var, not the accent); `#000000` unknown-sentinel; getter `{state,colour}`; **next — `link`** → `status-indicator` (di catalogue, locked N-049)
